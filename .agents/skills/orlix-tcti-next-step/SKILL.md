...
- Read `AGENTS.md`, `docs/index.md`, `docs/objects/epic/doing/orlix-tcti.md`, and current TCTI reports.
...
The loop regenerates status and next-task with `ORLIX_TCTI_HARNESS_QUIET=1`. Autonomous iterations emit only neutral progress counters and capture refresh and selected-command output under a unique run directory in `Build/AgentHarness/orlix-tcti/goal-loop/`; one final handoff carries the selected action policy. Set `ORLIX_TCTI_GOAL_STREAM_OUTPUT=1` only for an explicitly requested live diagnostic run. Direct status and next commands remain human-readable.
