...
description: Select the next owning TCTI test suite and emit a narrow scope envelope without reimplementing or interpreting test behavior.
---
...
```sh
rtk make agent-status AREA=orlix-tcti
rtk make agent-next AREA=orlix-tcti
rtk make agent-task-envelope-check AREA=orlix-tcti
```

The resulting `Build/AgentHarness/orlix-tcti/next-task.json` is a scope contract. It lists the owning test suites in ADR 0017 order, but it does not execute tests, parse their output, infer readiness, or replace their native result formats.
