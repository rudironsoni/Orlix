...
[STEP10-ACTIVATION 2026-09-18]: Step 10 activation consumed that exact signed proposal and Sigstore bundle with the trusted public key (fingerprint `7d2b6c7a…`, local `~/Library/Orlix/cosign/cosign.pub`, matching `trust-policy.json`), verified all bound evidence and every component signature, and applied the lock without regenerating or re-signing anything. `artifacts.lock.json` moved from schema 1 (`7e084cbef2b8…`) to schema 2 (`f29ff338f926…`) with buildset `15114da1…` and the seven exact OCI digests from Step 9. Promotion ownership stays separate: this only activates an already-signed buildset.
