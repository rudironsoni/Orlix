...
[PROMOTION-CHECKPOINT 2026-09-18]: the promotion path paid the full dual clean build every retry, so each late-stage defect cost another ~60 minutes (two such cycles so far). Added `bazel/promotion/checkpoint.py`: after A/B builds and staging complete, it records the verified per-component artifact-identity-v2 digests bound to the exact source SHA, toolchain digest, and component set. `__bazel-promote-buildset` now checks that checkpoint first and, on a hit, skips both clean builds and staging while `compare.py` still re-validates every reused tree against its recorded digest before emitting evidence. `bazel-promote.yml` caches `Build/Bazel/promote` under `orlix-promote-<source sha>-<lock hash>` and saves it immediately after the dual build, before signing/publication. A checkpoint from a different source SHA fails validation and forces a fresh build. Net effect: one expensive build per source revision, then cheap retries for proposal/sign/publish defects. Regression tests cover round trip, source/toolchain/component mismatch, tampered digest, side disagreement, and the Make/workflow ordering.

[STEP9-SUCCESS 2026-09-18]: run `35355912770` at `eec620629b9de24ffdfd11325d33f3abc1f79e98` completed the full seven-component promotion: clean build A and B, all seven A/B comparisons, seven artifact-identity-v2 identities, seven signatures, seven immutable GHCR publications, and signature/pull-back/product verification. It then produced the signed schema-2 buildset proposal in `bazel/promotion/lock_proposal.py` fixed by `eec6206` (top-level `source_sha`). Buildset `15114da1cbb9b97eb73669e7d3bd9a095c2a5f6aa25d94f2f04595732260cc51`. Evidence artifact `signed-buildset-eec620629b9de24ffdfd11325d33f3abc1f79e98` (artifact id 10555063204).

[STEP10-ACTIVATION 2026-09-18]: Step 10 activation consumed that exact signed proposal and Sigstore bundle with the trusted public key (fingerprint `7d2b6c7a…`, local `~/Library/Orlix/cosign/cosign.pub`, matching `trust-policy.json`), verified all bound evidence and every component signature, and applied the lock without regenerating or re-signing anything. [CORRECTION] This was NOT a schema-1 to schema-2 transition. The pre-activation lock at `eec6206` was already schema 2 with buildset `570bd6425e4f22b9818149b53c882e7782648a4714c53bdec67b7be71031d8a8` and seven components, but uapi/mlibc/rootfs still carried `legacy-marker-sha256` identities while the four kernel variants were already `artifact-identity-v2`. Activation replaced that provisional buildset with the fully promoted all-v2 buildset: `artifacts.lock.json` `7e084cbef2b8…` -> `f29ff338f926…`, buildset `15114da1cbb9b97eb73669e7d3bd9a095c2a5f6aa25d94f2f04595732260cc51`, all seven `artifact-identity-v2`, and the seven exact Step 9 OCI digests. Promotion ownership stays separate: this only activates an already-signed buildset.

[PROMOTION-CHECKPOINT 2026-09-18 CORRECTION]: the first checkpoint keyed reuse on `source_sha`, so a promotion-control-plane fix (proposal/signing/publication code) created a new commit, invalidated the checkpoint, and forced another full clean A/B build — the exact cycle it was meant to remove. Corrected: reuse is now decided by `component_input_identity`, the hash of the component-producing inputs (source files the component registry targets depend on, enumerated by `bazel query`, plus the registry and the effective build config), together with the toolchain digest and component set. `proof_source_sha` is recorded for provenance only and no longer gates reuse. The Actions cache key is the identity the build computes (`orlix-promote-v2-<identity>`), not `github.sha`; the restore key is deliberately broad and reuse is enforced by validation, not by the key. A control-plane-only change reuses; any component source, producer rule, registry, dependency-lock, build-config, or toolchain change forces clean A/B builds. Regression tests cover same-input reuse, control-plane-only reuse across differing proof SHAs, and rejection on kernel/mlibc/rule/registry/toolchain/build-config change, missing inputs, component-set change, and tampered staged digests.

[PROMOTED-PROOF 2026-09-18]: added an explicit manual diagnostic component mode to `bazel-ci.yml` (`workflow_dispatch.inputs.component_mode` = auto|source|promoted, default auto; normal PR/push behavior unchanged). Requesting `promoted` validates the lock first (schema 2, exactly seven components, every component `artifact-identity-v2` with an immutable `sha256:` OCI digest, non-empty buildset) and then runs a cold-then-warm promoted-consumer proof in one macos-26 job: cold uses an empty promoted store plus a fresh build root and Bazel output base, warm keeps only the populated store and uses a second fresh root and output base, so a warm hit can only come from the store, never the first pass's Bazel output tree. `reconstruct.py` now records machine-readable acquisition evidence (`Build/AgentHarness/bazel-promotion/acquisition.json`: per-component source `local-store`/`network`, signature verification, locked OCI digest, artifact identity; counters `network_downloads`, `local_store_hits`, `components_total`), and `component_actions.py` counts component-producing actions from the executed Bazel execution log, so zero component compilation is proven from execution evidence rather than inferred. [CORRECTION to the checkpoint mechanism]: the identity enumeration used `bazel query --output=location` and mis-parsed its output, and `deps(...)` alone misses BUILD and loaded `.bzl` files, so a real producer change could have produced an unchanged identity. The recipe now queries `kind('source file', deps(...)) union buildfiles(deps(...)) union loadfiles(deps(...))` with `--output=label`, converts main-repo labels to paths, keeps only existing files, and additionally binds `MODULE.bazel`/`MODULE.bazel.lock`; external repo contents are covered by the in-repo pins that `buildfiles`/`loadfiles` capture. `test_component_inputs_integration.py` proves against the real repository graph that the kernel/mlibc/rootfs producer sources, a producer `BUILD.bazel`, and loaded `.bzl` files are captured while `lock_proposal.py`/`sign.py`/`publish.py`/`checkpoint.py`/`compare.py` are excluded. Until that integration proof runs in CI, do not use a restored checkpoint to skip clean A/B promotion; no further promotion is dispatched for this change.

[PROMOTED-PROOF-OUTCOME 2026-09-18]: the manual promoted proof is implemented but has NOT yet passed. Run `35382931728` (`a005d34`) first failed because `MODULE.bazel.lock` binds the pybind11 input to the repository-relative output-base path, so moving the output base broke bootstrap; fixed by clearing Bazel state at the repository-relative path. Run `35385623081` (`4ee4f1c`) then failed in the cold pass with `ReconstructError: existing reconstructed contents differ from signed artifacts`: the proof stages components first (single reconstruct), then Apple CI runs reconstruct a second time, and the store round-trip (`shutil.copytree` in `publish_download` then `materialize_local`) does not reproduce the network-pulled tree byte-for-byte, so `tree_digest` differs. That second reconstruct is a property of running the proof as two make invocations; the product path reconstructs once. Two things remain: (1) make staging a prerequisite of the whole Apple CI operation so reconstruct runs exactly once, or fix the store round-trip so a second reconstruct is idempotent (this also matters for warm reuse correctness); then (2) re-run the cold/warm proof and require cold `network_downloads=7`/`local_store_hits=0`, warm `network_downloads=0`/`local_store_hits=7`, and `component_actions=0` in both. The checkpoint integration proof (`test_component_inputs_integration.py`, real Bazel graph) passes: producer sources, a producer `BUILD.bazel`, and loaded `.bzl` files are captured; `lock_proposal.py`/`sign.py`/`publish.py`/`checkpoint.py`/`compare.py` are excluded.

[CORRECTION 2026-09-18]: the promoted rootfs failure in run `35388340956` was not a staging defect and fix (a) (real directory instead of symlink) would not have made the promoted build correct. Inspecting the signed OCI products for buildset `15114da1cbb9b97eb73669e7d3bd9a095c2a5f6aa25d94f2f04595732260cc51` shows the root cause is a product-boundary mismatch: every schema-2 component is published as `product/`, but the promoted BUILD was written for the pre-schema-2 flat layout, and the v2 product identities for uapi, mlibc, and rootfs do not contain every semantic artifact their promoted providers require.

Measured against the reconstructed trees:
- uapi product is 963 files, all `include/**`. Missing `kbuild-archive.tar`, `manifest.json`, `uapi.sha256`.
- mlibc product is 333 files under `usr/**`. Missing `libcompiler_rt.a`, `abi.txt`, `manifest.json`, `sysroot.sha256` (headers and libraries are derivable from `usr/include` and `usr/lib`).
- rootfs product is 3 files (`initramfs.cpio.gz`, `base.ext4`, `state.ext4`). Missing `file-manifest.txt`, `payload-metadata.txt`, `source-input.sha256`.
- kernel products are structurally complete (`OrlixKernel.a` plus both DTBs).

Fixes in this change:
- `bazel/feasibility/kernel/kernel_uapi.bzl`, `bazel/feasibility/mlibc/mlibc_sysroot.bzl`: assemble one canonical `product/` tree per component (headers/sysroot plus the extra semantic outputs) and declare the artifact identity over it.
- `bazel/feasibility/rootfs/rootfs.bzl`: add the three missing semantic outputs to the rootfs identity.
- `bazel/promotion/components.json`: uapi and mlibc product layout is now `product` (rootfs stays `manifest-files`).
- `bazel/promotion/BUILD.bazel`, `bazel/promotion/promoted.bzl`: promoted consumers read `imported/<name>/product/...` and the `artifact-identity-v2.sha256` envelope; verification compares the envelope digest to the signed lock, and semantic digests (`uapi.sha256`, `sysroot.sha256`, `source-input.sha256`) stay separate provider fields.
- `bazel/promotion/substitute.py`: `stage_imported()` now projects each component as a real, package-visible directory (hard links when the store and worktree share a filesystem, copy fallback), never an opaque directory symlink, and removes stale entries first.
