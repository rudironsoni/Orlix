...
agent-rules-generated-status-check:
	@test -n "$(PR_NUMBER)" -a -n "$(HEAD_REVISION)" || { echo "PR_NUMBER=<number> and HEAD_REVISION=<sha> required" >&2; exit 2; }
	@PYTHONDONTWRITEBYTECODE=1 python3 -m Tools.AgentHarness.rulesync_reports generated-status-guard "$(PR_NUMBER)" "$(HEAD_REVISION)"
