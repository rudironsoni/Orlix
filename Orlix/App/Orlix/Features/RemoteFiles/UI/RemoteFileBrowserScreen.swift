...
struct RemoteFileBrowserScreen: View {
    ...
    @AppStorage(CloudKitSyncConstants.terminalThemeNameKey) var terminalThemeName = "Orlix Dark"
    @AppStorage(CloudKitSyncConstants.terminalThemeNameLightKey) var terminalThemeNameLight = "Orlix Light"
    ...
    var remoteRowDropTypeIdentifiers: [String] {
        [UTType.orlixRemoteFileEntry.identifier, UTType.fileURL.identifier]
    }
    ...
    func handleRemoteDrop(_ providers: [NSItemProvider], to destinationPath: String) -> Bool {
        let remoteProviders = providers.filter { provider in
            provider.hasItemConformingToTypeIdentifier(UTType.orlixRemoteFileEntry.identifier)
        }
    ...
    func temporaryDragExportDirectory(named folderName: String? = nil) throws -> URL {
        ...
        let rootDirectory = fileManager.temporaryDirectory
            .appendingPathComponent("OrlixDraggedItems", isDirectory: true)
    ...
    func loadDroppedRemotePayload(from provider: NSItemProvider) async throws -> RemoteFileDragPayload {
        try await withCheckedThrowingContinuation { continuation in
            provider.loadDataRepresentation(forTypeIdentifier: UTType.orlixRemoteFileEntry.identifier) { data, error in
                if let error {
                    continuation.resume(throwing: error)
                    return
                }
    ...
    func temporaryDownloadURL(for entry: RemoteFileEntry) throws -> URL {
        ...
        let downloadDirectory = fileManager.temporaryDirectory
            .appendingPathComponent("OrlixDownloads", isDirectory: true)
