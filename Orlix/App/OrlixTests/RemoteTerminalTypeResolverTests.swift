...
struct RemoteTerminalTypeResolverTests {
    ...
    func resolveUsesGhosttyWhenProbeSucceeds() async {
        let executor = FakeExecutor(outputs: [
            .success("__ORLIX_XTERM_GHOSTTY_OK__")
        ])
    ...
    func probeCommandChecksCompiledTerminfoLocationsWhenInfocmpIsUnavailable() {
        ...
        #expect(command.contains("orlix_has_xterm_ghostty_terminfo"))
    ...
    func resolveInstallsGhosttyTerminfoWhenProbeMisses() async {
        let executor = FakeExecutor(outputs: [
            .success("__ORLIX_XTERM_GHOSTTY_NO__"),
            .success("__ORLIX_XTERM_GHOSTTY_INSTALLED__")
        ])
    ...
    func resolveFallsBackWhenInstallationFails() async {
        let executor = FakeExecutor(outputs: [
            .success("__ORLIX_XTERM_GHOSTTY_NO__"),
            .success("__ORLIX_XTERM_GHOSTTY_INSTALL_FAILED__")
        ])
