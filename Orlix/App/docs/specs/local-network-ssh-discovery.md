...
- Add menu command in `OrlixCommands`:
...
- Presentation:
  ...
  - macOS: modal sheet reusing existing Orlix sheet container conventions.
- Sections:
...
### Data Model
Create `Orlix/Models/DiscoveredSSHHost.swift`:
...
Create `Orlix/Services/Discovery/LocalSSHDiscoveryService.swift`:
...
Create `Orlix/Managers/LocalSSHDiscoveryManager.swift` (`@MainActor`, `ObservableObject`):
...
- `Orlix/Views/Sidebar/ServerSidebarView.swift`
