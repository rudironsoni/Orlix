...
## Summary
Explore a future non-Apple expansion of Orlix that keeps each platform UI fully native while moving shared connection and session logic into a headless portable core exposed through a small C ABI / FFI bridge.
...
## Problem
Orlix is currently shaped around Apple platforms. The app lifecycle, UI shell, terminal embedding, secure storage, sync, purchases, biometrics, and several UX flows are tightly coupled to Apple frameworks and Apple runtime assumptions.
...
## Product Direction
The product name should remain `Orlix`.

This architecture assumes:
- `Orlix` remains the umbrella product brand
- Apple remains one platform edition of Orlix, not the entire product identity
...
- `OrlixBridge`
  - narrow C ABI wrapper around `OrlixCore`
...
- `orlix_core_create`
- `orlix_core_destroy`
- `orlix_core_set_event_callback`
- `orlix_core_set_host_callbacks`

### Server and workspace operations
- `orlix_core_load_state_json`
- `orlix_core_export_state_json`
- `orlix_core_validate_server_json`
- `orlix_core_validate_workspace_json`

### Session operations
- `orlix_core_connect_json`
- `orlix_core_disconnect`
- `orlix_core_send_input`
- `orlix_core_resize`
- `orlix_core_send_signal`

### Utility
- `orlix_string_free`
- `orlix_buffer_free`
- `orlix_last_error_message`

## Example C ABI Sketch
```c
typedef void* OrlixCoreHandle;
typedef void* OrlixSessionHandle;

typedef void (*orlix_event_callback)(
    const char* event_name,
    const char* event_json,
    void* context
);

OrlixCoreHandle orlix_core_create(void);
void orlix_core_destroy(OrlixCoreHandle core);

void orlix_core_set_event_callback(
    OrlixCoreHandle core,
    orlix_event_callback callback,
    void* context
);

char* orlix_core_list_servers_json(OrlixCoreHandle core);
OrlixSessionHandle orlix_core_connect_json(OrlixCoreHandle core, const char* request_json);
void orlix_core_send_input(OrlixSessionHandle session, const uint8_t* bytes, int length);
void orlix_core_resize(OrlixSessionHandle session, int cols, int rows);
void orlix_core_disconnect(OrlixSessionHandle session);
void orlix_string_free(char* value);
```
...
- Can libghostty or an equivalent renderer strategy be hosted cleanly on HarmonyOS and Windows without polluting `OrlixCore`?
- Which current managers contain real domain logic versus Apple-bound orchestration that should stay in `OrlixApple`?
...
- Define a first-pass `OrlixCore` boundary in terms of modules, DTOs, and capabilities
...
- create `OrlixBridge` with create/destroy/list/connect/send/resize/disconnect
