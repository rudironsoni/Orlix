...
## Summary
Add host-to-remote clipboard forwarding for non-text payloads, starting with images, so a user can press paste in Orlix and have the image become usable on the remote machine.
...
- Orlix uses a layered fallback strategy:
...
## Problem
Today Orlix forwards clipboard reads through Ghostty as text only.
...
- [Clipboard.swift](Orlix/Utilities/Clipboard.swift)
- [Ghostty.App.swift](Orlix/GhosttyTerminal/Ghostty.App.swift)
...
- As a user, when the remote environment supports it, Orlix also seeds the remote machine clipboard as a best-effort convenience.
...
1. Orlix inspects the host clipboard.
2. If clipboard has text and no richer supported payload, use existing `paste_from_clipboard`.
3. If clipboard has an image, Orlix first checks whether terminal-native rich clipboard transfer is available for this session.
4. If Kitty `OSC 5522` is supported and succeeds, Orlix completes a true rich paste without creating a remote temp file.
5. Otherwise Orlix uploads the image to a temporary file on the remote host.
6. Orlix optionally tries to copy that image into the remote machine clipboard.
7. Orlix inserts the temporary remote absolute path as text into the terminal.

Example pasted text:
```text
/tmp/orlix-clipboard-a1b2c3.png
```
...
- If Orlix had to fall back from Kitty `5522` to upload/path insertion, or if remote clipboard seeding succeeded or failed along the way, the UI should say so clearly.
...
Likely touchpoints:
- [GhosttyTerminalView+iOS.swift](Orlix/GhosttyTerminal/GhosttyTerminalView+iOS.swift)
- [GhosttyTerminalView+macOS.swift](Orlix/GhosttyTerminal/GhosttyTerminalView+macOS.swift)
- [SSHTerminalWrapper.swift](Orlix/Views/Terminal/SSHTerminalWrapper.swift)
...
- add an SSH upload primitive in [SSHClient.swift](Orlix/Services/SSH/SSHClient.swift)
...
```sh
tmp_base="${TMPDIR:-/tmp}"; mktemp "${tmp_base%/}/orlix-clipboard-XXXXXX.png"
```

Permissions:
- uploaded files should default to `0600`
- Orlix should track and clean up files it created
...
Ownership:
- Orlix should track temp files per `sessionId` for provenance and cleanup hints
...
- a best-effort background sweeper can remove stale Orlix temp files left behind after crashes

### 6) Use terminal-native rich clipboard when available; otherwise insert the uploaded path
For image payloads, Orlix has two distinct completion modes:
...
- Orlix should complete the paste in-app without requiring a second user gesture
...
- Orlix should continue with upload and path-based completion
...
- macOS GUI session:
```sh
osascript -e 'set the clipboard to (read (POSIX file "/tmp/orlix-clipboard-a1b2c3.png") as PNG picture)'
```
- Linux Wayland:
```sh
wl-copy < /tmp/orlix-clipboard-a1b2c3.png
```
- Linux X11:
```sh
xclip -selection clipboard -t image/png -i /tmp/orlix-clipboard-a1b2c3.png
```
...
- Orlix should model Kitty support as a capability on the session
...
- or use a separate `%USERPROFILE%\\.orlix\\clipboard` path design in V2
...
- Should Orlix offer a dedicated `Paste Image` action in the keyboard toolbar on iOS?
...
- How should Orlix detect future Ghostty `OSC 5522` support reliably from the embedded runtime?
