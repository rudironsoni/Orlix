...
extension NSToolbarItem.Identifier {
    static let orlixViewPicker = NSToolbarItem.Identifier("orlix.viewPicker")
    static let orlixTabStrip = NSToolbarItem.Identifier("orlix.tabStrip")
    static let orlixFilesMenu = NSToolbarItem.Identifier("orlix.filesMenu")
    static let orlixEnterZen = NSToolbarItem.Identifier("orlix.enterZen")
    static let orlixServerMenu = NSToolbarItem.Identifier("orlix.serverMenu")
    static let orlixZenControls = NSToolbarItem.Identifier("orlix.zenControls")
}
...
final class MacConnectionToolbarController: NSObject, NSToolbarDelegate, NSMenuDelegate {
    ...
    private static let filesMenuIdentifier = NSUserInterfaceItemIdentifier("orlix.filesMenu.menu")
    private static let serverMenuIdentifier = NSUserInterfaceItemIdentifier("orlix.serverMenu.menu")

    override init() {
        toolbar = NSToolbar(identifier: "orlix.connection.toolbar")
        super.init()
    ...
    private var desiredIdentifiers: [NSToolbarItem.Identifier] {
        ...
        if bridge.isZenMode {
            ...
            return [.flexibleSpace, .toggleSidebar, .sidebarTrackingSeparator, .flexibleSpace, .orlixZenControls]
        }
        if bridge.showsViewPicker { ids.append(.orlixViewPicker) }
        if bridge.showsTabStrip {
            ids.append(.orlixTabStrip)
        ...
        if bridge.showsFilesMenu { ids.append(.orlixFilesMenu) }
        ids.append(.orlixEnterZen)
        ids.append(.orlixServerMenu)
        return ids
    }
    ...
    func toolbarAllowedItemIdentifiers(_ toolbar: NSToolbar) -> [NSToolbarItem.Identifier] {
        [.toggleSidebar, .sidebarTrackingSeparator, .orlixViewPicker, .orlixTabStrip,
         .orlixFilesMenu, .orlixEnterZen, .orlixServerMenu, .orlixZenControls,
         .flexibleSpace, .space]
    }
    ...
    func toolbar(
        ...
        case .orlixServerMenu:
            return makeServerMenuItem()
        case .orlixZenControls:
    ...
    private func makeZenControlsItem() -> NSToolbarItem {
        ...
        let item = NSToolbarItem(itemIdentifier: .orlixZenControls)
    ...
    private func makeViewPickerItem() -> NSToolbarItem {
        let item = NSToolbarItem(itemIdentifier: .orlixViewPicker)
        item.label = "View"
    ...
    private func makeTabStripItem() -> NSToolbarItem {
        let item = NSToolbarItem(itemIdentifier: .orlixTabStrip)
        item.label = "Tabs"
    ...
    private func makeFilesMenuItem() -> NSMenuToolbarItem {
        let item = NSMenuToolbarItem(itemIdentifier: .orlixFilesMenu)
        item.label = "Files"
    ...
    private func makeServerMenuItem() -> NSMenuToolbarItem {
        let item = NSMenuToolbarItem(itemIdentifier: .orlixServerMenu)
        item.label = "Server"
    ...
    private func makeEnterZenItem() -> NSToolbarItem {
        let item = NSToolbarItem(itemIdentifier: .orlixEnterZen)
        item.label = "Zen"
