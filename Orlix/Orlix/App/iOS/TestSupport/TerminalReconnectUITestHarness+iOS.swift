...
struct TerminalReconnectUITestHarness: View {
    ...
    private static let fixtureDefaults = UserDefaults(suiteName: "com.rudironsoni.orlix.dev199-ui-test")!
            ...
            .overlay(alignment: .topLeading) {
                if exposesKeyboardLossControl {
                    HStack(spacing: 6) {
                        ...
                        .accessibilityIdentifier("orlix.reconnectTest.keyboard.unexpectedLoss")

                        if simulatesKeyboardFrames {
                            Button("Stale Find Frame") {
                                simulateVisibleFindKeyboardFrame()
                            }
                            .accessibilityIdentifier("orlix.reconnectTest.keyboard.staleFindFrame")
                        }
                    }
            ...
            .overlay(alignment: .topTrailing) {
                if usesNavigationHarness, activeServer != nil {
                    ...
                    .accessibilityIdentifier(
                        "orlix.navigationTest.toggleServerMetadata"
                    )
                    .buttonStyle(.borderedProminent)
                    .controlSize(.small)
                    .padding(.top, 140)
            ...
            .overlay(alignment: .bottomLeading) {
                if usesColdRelaunchHarness {
                    Text(coldRelaunchDiagnostics)
                        ...
                        .accessibilityIdentifier("orlix.coldRelaunchTest.diagnostics")
                        .padding(6)
                }
            }
            .task {
    ...
    private var exposesKeyboardLossControl: Bool {
        Foundation.ProcessInfo.processInfo.arguments.contains(
            "--orlix-ui-test-unexpected-keyboard-loss-control"
        )
    }

    private var simulatesKeyboardFrames: Bool {
        Foundation.ProcessInfo.processInfo.arguments.contains(
            "--orlix-ui-test-simulate-keyboard-frames"
        )
    }

    private var usesNavigationHarness: Bool {
        Foundation.ProcessInfo.processInfo.arguments.contains(
            "--orlix-ui-test-server-navigation"
        )
    }
    ...
    private var seedsColdRelaunchHarness: Bool {
        Foundation.ProcessInfo.processInfo.arguments.contains(
            "--orlix-ui-test-seed-cold-relaunch"
        )
    }
            ...
            if !usesColdRelaunchHarness || seedsColdRelaunchHarness {
                ...
                if usesColdRelaunchHarness {
                    guard tabManager.splitHorizontal(
                        ...
                        throw OrlixError.connectionFailed("Unable to seed split layout")
                    }
                ...
                throw OrlixError.connectionFailed("Cold relaunch snapshot was not restored")
            }
...
private struct TerminalReconnectDiagnosticsLabel: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UILabel {
        let label = UILabel()
        label.accessibilityIdentifier = "orlix.reconnectTest.diagnostics"
    ...
    final class Coordinator {
        ...
        private func refresh() {
            ...
            terminal.accessibilityIdentifier = "orlix.reconnectTest.terminalSurface"
