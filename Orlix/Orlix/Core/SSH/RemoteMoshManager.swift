...
actor RemoteMoshManager {
    ...
    private let logger = Logger(subsystem: "com.rudironsoni.orlix", category: "mosh-bootstrap")
    private static let installSuccessMarker = "__ORLIX_MOSH_INSTALLED__"
    ...
    func isMoshServerAvailable(using client: SSHClient) async -> Bool {
        let okMarker = "__ORLIX_MOSH_OK__"
        let body = "\(RemoteTerminalBootstrap.shellPathExport()); if command -v mosh-server >/dev/null 2>&1; then printf '\(okMarker)'; else printf '__ORLIX_MOSH_NO__'; fi"
    ...
    nonisolated func installScript() -> String {
        ...
        if command -v mosh-server >/dev/null 2>&1; then printf '\(Self.installSuccessMarker)'; else printf '__ORLIX_MOSH_INSTALL_FAILED__'; fi
        """
    }
    ...
    nonisolated func utf8LocaleExportScript() -> String {
        """
        orlix_validate_utf8_locale() {
          [ -n "$1" ] || return 1;
          ORLIX_TEST_CHARMAP="$(LANG="$1" LC_ALL="$1" LC_CTYPE="$1" locale charmap 2>/dev/null)" || return 1;
          case "$ORLIX_TEST_CHARMAP" in *[Uu][Tt][Ff]*8*) return 0 ;; *) return 1 ;; esac
        };
        ORLIX_CURRENT_CHARMAP="$(locale charmap 2>/dev/null || true)";
        case "$ORLIX_CURRENT_CHARMAP" in
          *[Uu][Tt][Ff]*8*) ORLIX_UTF8_LOCALE="" ;;
          *)
            ORLIX_UTF8_LOCALE="";
            for ORLIX_LOCALE_CANDIDATE in "${LC_ALL:-}" "${LC_CTYPE:-}" "${LANG:-}" C.UTF-8 C.utf8 en_US.UTF-8 en_US.utf8; do
              if orlix_validate_utf8_locale "$ORLIX_LOCALE_CANDIDATE"; then
                ORLIX_UTF8_LOCALE="$ORLIX_LOCALE_CANDIDATE";
                break;
              fi;
            done;
            if [ -z "$ORLIX_UTF8_LOCALE" ] && command -v locale >/dev/null 2>&1; then
              for ORLIX_LOCALE_CANDIDATE in $(locale -a 2>/dev/null || true); do
                case "$ORLIX_LOCALE_CANDIDATE" in
                  *[Uu][Tt][Ff]*8*)
                    if orlix_validate_utf8_locale "$ORLIX_LOCALE_CANDIDATE"; then
                      ORLIX_UTF8_LOCALE="$ORLIX_LOCALE_CANDIDATE";
                      break;
            ...
            if [ -n "$ORLIX_UTF8_LOCALE" ]; then
              export LANG="$ORLIX_UTF8_LOCALE";
              export LC_ALL="$ORLIX_UTF8_LOCALE";
              export LC_CTYPE="$ORLIX_UTF8_LOCALE";
            fi
            ;;
        esac;
        unset ORLIX_CURRENT_CHARMAP ORLIX_LOCALE_CANDIDATE ORLIX_TEST_CHARMAP;
        unset -f orlix_validate_utf8_locale 2>/dev/null || true
        """
    }
