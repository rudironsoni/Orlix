...
enum RemoteTerminalTypeResolver {
    ...
    private static let probeMarker = "__ORLIX_XTERM_GHOSTTY_OK__"
    private static let probeMissMarker = "__ORLIX_XTERM_GHOSTTY_NO__"
    private static let installMarker = "__ORLIX_XTERM_GHOSTTY_INSTALLED__"
    private static let missingTicMarker = "__ORLIX_XTERM_GHOSTTY_NO_TIC__"
    private static let installFailedMarker = "__ORLIX_XTERM_GHOSTTY_INSTALL_FAILED__"
    private static let hereDocMarker = "__ORLIX_XTERM_GHOSTTY_TERMINFO__"
    ...
    static func probeCommand(okMarker: String = probeMarker) -> String {
        ...
        if { command -v infocmp >/dev/null 2>&1 && infocmp -x xterm-ghostty >/dev/null 2>&1; } || orlix_has_xterm_ghostty_terminfo; then
          printf '\(okMarker)';
        else
    ...
    private static func hasGhosttyTerminfoFunction() -> String {
        """
        orlix_has_xterm_ghostty_terminfo() {
          ORLIX_OLD_IFS=$IFS;
          IFS=:;
