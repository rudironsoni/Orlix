...
actor RemoteTmuxManager {
    ...
    private let configDirectory = "~/.orlix"
    private let configPath = "~/.orlix/tmux.conf"
    ...
    func tmuxBackend(using client: SSHClient) async -> RemoteTmuxBackend? {
        ...
        let okMarker = "__ORLIX_TMUX_OK__"
    ...
    func cleanupLegacySessions(using client: SSHClient) async {
        ...
        if command -v tmux >/dev/null 2>&1; then
          tmux list-sessions -F '#{session_name} #{session_attached}' 2>/dev/null | awk '$1 ~ /^orlix_[0-9a-fA-F-]+$/ && $2 == 0 { print $1 }' | while IFS= read -r name; do
    ...
    nonisolated func tmuxAvailabilityProbeCommand(okMarker: String) -> String {
        ...
        ORLIX_TMUX_BIN="";
        if command -v tmux >/dev/null 2>&1; then
          ORLIX_TMUX_BIN="$(command -v tmux 2>/dev/null)";
        fi;
        if [ -z "$ORLIX_TMUX_BIN" ]; then
          for candidate in /usr/bin/tmux /bin/tmux /usr/local/bin/tmux /opt/local/bin/tmux /snap/bin/tmux; do
            if [ -x "$candidate" ]; then
              ORLIX_TMUX_BIN="$candidate";
              break;
            fi;
          done;
        fi;
        if [ -n "$ORLIX_TMUX_BIN" ] && "$ORLIX_TMUX_BIN" -V >/dev/null 2>&1; then
          printf '\(okMarker)';
        else
          printf '__ORLIX_TMUX_NO__';
        fi
        """
    ...
    private func windowsPsmuxBackend(
        ...
        for commandName in ["psmux", "pmux"] {
            ...
            if output?.contains("__ORLIX_TMUX_OK__:\(commandName)") == true {
                return backend
            }
        }
        ...
        if output?.contains("__ORLIX_TMUX_OK__:tmux") == true {
            return tmuxBackend
        }
    ...
    nonisolated func windowsPsmuxAvailabilityProbeCommand(
        ...
        let marker = "__ORLIX_TMUX_OK__:\(commandName)"
        let script = """
    ...
    nonisolated private func windowsAttachExistingPowerShell(
        ...
        if ($LASTEXITCODE -eq 0) {
          & $orlixPsmux -f $orlixConfig source-file $orlixConfig 2>$null
          & $orlixPsmux -u -f $orlixConfig attach-session -d -t $orlixSession
        } else {
    ...
    nonisolated private func windowsCreateSessionPowerShell(
        ...
        & $orlixPsmux -u -f $orlixConfig new-session -A -s $orlixSession -c $orlixWorkingDirectory
        """
    }
    ...
    nonisolated private func configLines(
        ...
        let themeName = UserDefaults.standard.string(forKey: CloudKitSyncConstants.terminalThemeNameKey) ?? "Orlix Dark"
    ...
    nonisolated private func windowsConfigWritePowerShell(
        ...
        @'
        \(content)'@ | Set-Content -Encoding UTF8 -NoNewline -Path $orlixConfigPath
        """
    }
