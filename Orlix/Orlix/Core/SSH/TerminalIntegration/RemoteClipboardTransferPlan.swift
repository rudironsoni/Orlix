...
enum RemoteClipboardTransferPlan: Sendable {
    ...
    nonisolated func temporaryPathCommand(fileExtension: String) -> String {
        ...
        case .posix:
            return RemoteTerminalBootstrap.wrapPOSIXShellCommand(
                """
                tmp_base="${TMPDIR:-/tmp}";
                tmp_path="$(mktemp "${tmp_base%/}/orlix-clipboard-XXXXXX")" || exit 1;
        ...
        case .windows(let powerShellExecutable, _):
            let script = """
            $name = 'orlix-clipboard-' + [Guid]::NewGuid().ToString('N') + '.\(sanitizedExtension)'; $path = [IO.Path]::Combine([IO.Path]::GetTempPath(), $name); [Console]::Out.Write($path)
            """
    ...
    nonisolated var staleSweepCommand: String {
        ...
        case .posix:
            return RemoteTerminalBootstrap.wrapPOSIXShellCommand(
                """
                tmp_base="${TMPDIR:-/tmp}";
                for path in "${tmp_base%/}"/orlix-clipboard-*; do
                    [ -f "$path" ] || continue
        ...
        case .windows(let powerShellExecutable, _):
            let script = """
            $cutoff = [DateTime]::UtcNow.AddDays(-1); Get-ChildItem -LiteralPath ([IO.Path]::GetTempPath()) -Filter 'orlix-clipboard-*' -File -ErrorAction SilentlyContinue | Where-Object { $_.LastWriteTimeUtc -lt $cutoff } | ForEach-Object { try { [IO.File]::Delete($_.FullName) } catch {} }
            """
            return RemoteTerminalBootstrap.wrapPowerShellCommand(
                script,
                executableName: powerShellExecutable
    ...
    nonisolated private static func isOwnedTemporaryPath(_ path: String) -> Bool {
        let fileName = path
            ...
            .split(separator: "/")
            .last
        return fileName?.hasPrefix("orlix-clipboard-") == true
    }
