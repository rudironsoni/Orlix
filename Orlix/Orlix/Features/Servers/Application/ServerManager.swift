...
final class ServerManager: ObservableObject {
    ...
    func addServer(_ server: Server, credentials: ServerCredentials) async throws {
        guard canAddServer else {
            throw OrlixError.proRequired(String(localized: "Upgrade to Pro for unlimited servers"))
        }
    ...
    func addWorkspace(_ workspace: Workspace) async throws {
        guard canAddWorkspace else {
            throw OrlixError.proRequired(String(localized: "Upgrade to Pro for unlimited workspaces"))
        }
    ...
    func moveServer(
        ...
        guard let refreshedDestination = workspace(withId: destination.id) else {
            throw OrlixError.moveNotAllowed(String(localized: "The destination workspace is no longer available."))
        }
    ...
    private func moveRestriction(for server: Server, destination: Workspace) -> OrlixError? {
        ...
        if !StoreManager.shared.isPro && isWorkspaceLocked(destination) {
            return OrlixError.proRequired(String(localized: "Upgrade to Pro to move servers into locked workspaces"))
        }

        return OrlixError.moveNotAllowed(String(localized: "This server can't be moved to that workspace right now."))
    }
    ...
    func createCustomEnvironment(name: String, color: String) throws -> ServerEnvironment {
        guard canCreateCustomEnvironment else {
            throw OrlixError.proRequired(String(localized: "Upgrade to Pro for custom environments"))
        }
...
enum OrlixError: LocalizedError {
