...
struct TerminalSettingsView: View {
    ...
    @AppStorage(CloudKitSyncConstants.terminalThemeNameKey) private var themeName = "Orlix Dark"
    @AppStorage(CloudKitSyncConstants.terminalThemeNameLightKey) private var themeNameLight = "Orlix Light"
    ...
    private var knownHostsFooterText: String {
        ...
        if count == 1 {
            return String(localized: "Orlix has 1 trusted SSH host on this device. Resetting trusted hosts makes Orlix trust the host key presented on the next connection.")
        }
        return String(format: String(localized: "Orlix has %lld trusted SSH hosts on this device. Resetting trusted hosts makes Orlix trust the host key presented on the next connection."), count)
    }
    ...
    var body: some View {
        ...
        .alert("Reset Trusted SSH Hosts", isPresented: $showingResetKnownHostsConfirmation) {
            ...
            Text("Orlix will forget all saved SSH host fingerprints on this device. The next connection to each host will trust the key it presents.")
        }
    ...
    private func ensureThemeSelectionIsValid() {
        ...
        if !available.contains(themeName) {
            themeName = "Orlix Dark"
        }
        if !available.contains(themeNameLight) {
            themeNameLight = "Orlix Light"
        }
    }
