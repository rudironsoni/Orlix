...
struct TrustedHostsSettingsView: View {
    ...
    var body: some View {
        ...
        .accessibilityIdentifier("orlix.settings.page.trustedHosts")
        .alert(
            resetTarget?.title ?? "",
    ...
    func resetAction(for knownHost: KnownHostSettingsItem) -> some View {
        ...
        .accessibilityIdentifier(
            "orlix.settings.trustedHosts.reset.\(knownHost.id)"
        )
    }
...
struct TrustedHostSettingsRow: View {
    ...
    var body: some View {
        ...
        .accessibilityIdentifier("orlix.settings.trustedHosts.entry.\(knownHost.id)")
    }
}
...
private enum TrustedHostResetTarget: Identifiable {
    ...
    var message: String {
        ...
        case .host(let knownHost):
            String(
                format: String(localized: "Orlix will forget the saved SSH fingerprint for %@. Verify it again when you reconnect."),
                knownHost.endpoint
            )
        case .all:
            String(localized: "Orlix will forget all saved SSH host fingerprints on this device. The next connection to each host will trust the key it presents.")
        }
    }
}
