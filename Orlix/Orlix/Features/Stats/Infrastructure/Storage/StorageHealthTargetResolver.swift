...
nonisolated enum StorageHealthTargetResolver {
    ...
    static func linuxResolutionCommand(source: String, mountPoint: String) -> String? {
        ...
        if command -v findmnt >/dev/null 2>&1; then
            orlix_candidate=$(findmnt -n -o SOURCE -T \(mountPoint) 2>/dev/null | sed -n '1p')
            case "$orlix_candidate" in
                /dev/*) orlix_source=${orlix_candidate%%\\[*} ;;
            esac
        fi
        if command -v lsblk >/dev/null 2>&1; then
            printf '%s\n' '\(resolutionBeginMarker)'
            lsblk -s -J -p -o PATH,TYPE -- "$orlix_source" 2>/dev/null || true
            printf '%s\n' '\(resolutionEndMarker)'
        else
    ...
    static func zfsDiscoveryCommand(source: String, mountPoint: String) -> String? {
        ...
        let script = """
        export LC_ALL=C LANG=C
        orlix_dataset=\(source)
        if command -v findmnt >/dev/null 2>&1; then
            orlix_candidate=$(findmnt -n -o SOURCE -T \(mountPoint) 2>/dev/null | sed -n '1p')
            case "$orlix_candidate" in
                '') ;;
                *) orlix_dataset=$orlix_candidate ;;
            esac
        fi
        orlix_pool=${orlix_dataset%%/*}
        if command -v zpool >/dev/null 2>&1; then
            printf '%s\n' '\(resolutionBeginMarker)'
            zpool status -P "$orlix_pool" 2>&1 || true
            printf '%s\n' '\(resolutionEndMarker)'
        else
            printf '%s\n' '\(resolutionToolMissingMarker)'
        fi
    ...
    static func windowsResolutionScript(source: String, mountPoint: String) -> String? {
        ...
        if let driveLetter {
            ...
            volumeLookup = """
            $orlixPath = '\(source)'
            $volume = @(Get-Volume -ErrorAction Stop | Where-Object { [string]$_.Path -eq $orlixPath })
            """
        } else {
            return nil
        }
