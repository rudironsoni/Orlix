...
final class StoreManager: ObservableObject {
    ...
    var monthlyProduct: StoreProduct? {
        products.first { $0.id == OrlixProducts.proMonthly }
    }

    var yearlyProduct: StoreProduct? {
        products.first { $0.id == OrlixProducts.proYearly }
    }

    var lifetimeProduct: StoreProduct? {
        products.first { $0.id == OrlixProducts.proLifetime }
    }
    ...
    private static func loadProducts(
        ...
        for attempt in 0..<maxRetries {
            do {
                let products = try await client.products(for: OrlixProducts.allProducts)
    ...
    private func applyEntitlementResult(_ result: StoreEntitlementResult) {
        let hasLifetime = result.verifiedProductIds.contains(OrlixProducts.proLifetime)
        let hasVerifiedSubscription = result.verifiedProductIds.contains(OrlixProducts.proMonthly)
            || result.verifiedProductIds.contains(OrlixProducts.proYearly)
    ...
    private static let subscriptionProductIds = [
        OrlixProducts.proMonthly,
        OrlixProducts.proYearly
    ]
}
