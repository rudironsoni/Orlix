...
enum MoshResumeStoreError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .secureStorageUnavailable:
            String(localized: "Orlix could not access the saved Mosh session key securely. Check Keychain access and try again.")
        case .checkpointStorageUnavailable:
            String(localized: "Orlix could not save the Mosh recovery data. Check available device storage and try again.")
        }
    }
...
final class DeviceOnlyMoshResumeSecretStore: MoshResumeSecretStoring {
    ...
    init(
        store: KeychainStore = KeychainStore(
            service: "com.rudironsoni.orlix.mosh.resume"
        )
    ) {
        self.store = store
    }
