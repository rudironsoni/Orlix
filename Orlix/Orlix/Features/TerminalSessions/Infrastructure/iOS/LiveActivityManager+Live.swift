...
private final class ActivityKitTerminalLiveActivityController: TerminalLiveActivityControlling {
    ...
    func reconcile(toward target: TerminalLiveActivityTarget) async {
        let activities = Activity<OrlixActivityAttributes>.activities
        ...
        let contentState = OrlixActivityAttributes.ContentState(
        ...
        do {
            let attributes = OrlixActivityAttributes(appName: "Orlix")
    ...
    func endForApplicationTermination() -> Bool {
        ...
        Task.detached(priority: .high) {
            ...
            for activity in Activity<OrlixActivityAttributes>.activities {
                await activity.end(dismissalPolicy: .immediate)
            }
        }
