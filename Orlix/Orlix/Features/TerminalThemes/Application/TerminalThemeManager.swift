...
final class TerminalThemeManager: ObservableObject {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.rudironsoni.orlix", category: "TerminalThemeManager")
    ...
    private init(defaults: UserDefaults = .standard, cloudKit: CloudKitManager? = nil) {
        ...
        self.lastKnownPreferenceSnapshot = PreferenceSnapshot(
            darkThemeName: defaults.string(forKey: darkThemeKey) ?? "Orlix Dark",
            lightThemeName: defaults.string(forKey: lightThemeKey) ?? "Orlix Light",
    ...
    private func ensureThemeSelectionIsValid() {
        ...
        let fallbackDark = "Orlix Dark"
        let fallbackLight = "Orlix Light"
