...
private final class TerminalIMEProxyTextView: UIView, UITextInput {
    ...
    static let dictationLogger: Logger = Foundation.ProcessInfo.processInfo.environment["ORLIX_DICTATION_LOG"] != nil
...
class GhosttyTerminalView: UIView {
    private static let textInputContextID = "com.rudironsoni.orlix.GhosttyTerminalView"
    ...
    private static let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.rudironsoni.orlix", category: "GhosttyTerminal")
    ...
    private var isKeyboardTextInputActive: Bool {
...
private class TerminalInputAccessoryView: UIInputView {
    ...
    init(
        ...
        accessibilityIdentifier = "orlix.keyboard.accessory"
        setupView()
        observeThemeChanges()
    ...
    private func setupView() {
        ...
        voice.accessibilityIdentifier = "orlix.keyboard.accessory.voice"
        voiceButton = voice
        ...
        dismissKeyboard.accessibilityIdentifier = "orlix.keyboard.accessory.hide"
        ...
        ctrl.accessibilityIdentifier = "orlix.keyboard.accessory.modifier.ctrl"
        let alt = makeModifierButton(title: String(localized: "Alt")) { [weak self] in
            self?.altActive.toggle()
            self?.updateModifierState()
        }
        alt.accessibilityIdentifier = "orlix.keyboard.accessory.modifier.alt"
        let shift = makeModifierButton(title: String(localized: "Shift")) { [weak self] in
            self?.shiftActive.toggle()
            self?.updateModifierState()
        }
        shift.accessibilityIdentifier = "orlix.keyboard.accessory.modifier.shift"
        ctrlButton = ctrl
        altButton = alt
        shiftButton = shift
    ...
    private func makeCustomActionButton(for action: TerminalAccessoryCustomAction) -> UIButton {
        ...
        button.accessibilityIdentifier = "orlix.keyboard.accessory.custom.\(action.id.uuidString)"
        return button
    }
    ...
    private func resolveThemeBackgroundColor() -> UIColor {
        ...
        let darkTheme = defaults.string(forKey: CloudKitSyncConstants.terminalThemeNameKey) ?? "Orlix Dark"
        let lightTheme = defaults.string(forKey: CloudKitSyncConstants.terminalThemeNameLightKey) ?? "Orlix Light"
