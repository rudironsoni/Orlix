...
final class StoreManagerLifecycleTests: XCTestCase {
    ...
    func testVerifiedPurchaseRefreshesEntitlementsAndCompletesPurchase() async {
        ...
        client.purchaseResult = .verified(productId: OrlixProducts.proMonthly)
        client.entitlementResult = entitlementResult(productIds: [OrlixProducts.proMonthly])
        ...
        XCTAssertEqual(manager.lastPurchasedProductId, OrlixProducts.proMonthly)
        XCTAssertTrue(manager.isPro)
        XCTAssertEqual(client.purchasedProductIds, [OrlixProducts.proMonthly])
        ...
        XCTAssertEqual(
            ...
            [
                .purchaseStarted(
                    source: .general,
                    productID: OrlixProducts.proMonthly
                ),
    ...
    func testLatePurchaseResultCannotOverwriteANewerPurchase() async {
        ...
        firstGate.resume(with: .verified(productId: OrlixProducts.proMonthly))
        ...
        XCTAssertEqual(
            effects,
            [
                .purchaseStarted(source: .general, productID: OrlixProducts.proMonthly),
                .purchaseStarted(source: .general, productID: OrlixProducts.proMonthly),
                .purchasePending(source: .general, productID: OrlixProducts.proMonthly)
            ]
        )
    }
    ...
    func testUnverifiedPurchaseFailsWithoutGrantingAccess() async {
        let client = StoreClientFake()
        client.purchaseResult = .unverified(productId: OrlixProducts.proMonthly)
    ...
    func testRestoreSyncsAndAppliesLifetimeEntitlement() async {
        let client = StoreClientFake()
        client.entitlementResult = entitlementResult(productIds: [OrlixProducts.proLifetime])
        let manager = makeManager(client)
    ...
    func testLateEntitlementResultCannotOverwriteANewerCheck() async {
        ...
        firstGate.resume(with: entitlementResult(productIds: [OrlixProducts.proLifetime]))
    ...
    func testPaywallAndNonSuccessPurchaseEffectsAreExact() async {
        let scenarios: [(StorePurchaseResult, StoreManagerEffect)] = [
            ...
            (
                .unverified(productId: OrlixProducts.proMonthly),
                .purchaseFailed(
                    source: .serverLimit,
                    productID: OrlixProducts.proMonthly,
                    reason: "StoreError"
    ...
    func testProductLoadingFailureStillGrantsLifetimeAccess() async {
        await assertProductLoadingFailureStillGrantsAccess(
            productID: OrlixProducts.proLifetime,
    ...
    func testUnverifiedUpdateIsIgnoredBeforeVerifiedUpdateRefreshesEntitlements() async {
        ...
        client.entitlementResults = [
            .free,
            entitlementResult(productIds: [OrlixProducts.proMonthly])
        ]
        ...
        client.emit(.unverified(productId: OrlixProducts.proMonthly))
        client.emit(.verified(productId: OrlixProducts.proMonthly))
    ...
    func testStopRejectsLateEntitlementResultFromVerifiedUpdate() async {
        ...
        manager.stop()
        gate.resume(with: entitlementResult(productIds: [OrlixProducts.proMonthly]))
    ...
    func testOwnerReleaseAfterStartupCompletesTerminatesTransactionStream() async {
        ...
        client.entitlementResult = entitlementResult(
            productIds: [OrlixProducts.proMonthly]
        )
        let startupCompleted = expectation(description: "Startup completed")
        let streamTerminated = expectation(description: "Transaction stream terminated")
        client.onEntitlementRequest = { startupCompleted.fulfill() }
