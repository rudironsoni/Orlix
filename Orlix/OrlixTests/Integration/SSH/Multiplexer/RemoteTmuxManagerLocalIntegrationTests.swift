...
struct RemoteTmuxManagerLocalIntegrationTests {
    ...
    func externalAttachPreservesRealTmuxGlobalOptions() throws {
        ...
        let root = temporaryDirectory
            .appendingPathComponent("orlix-dev218-\(UUID().uuidString)", isDirectory: true)
        let home = root.appendingPathComponent("home", isDirectory: true)
        let managedConfigDirectory = home.appendingPathComponent(".orlix", isDirectory: true)
    ...
    func managedCreationAndReattachScopeConfigurationToManagedSession() throws {
        ...
        let temporaryDirectory = FileManager.default.temporaryDirectory
        let root = temporaryDirectory
            .appendingPathComponent("orlix-dev220-\(UUID().uuidString)", isDirectory: true)
        let home = root.appendingPathComponent("home", isDirectory: true)
        let configDirectory = home.appendingPathComponent(".orlix", isDirectory: true)
        ...
        try Data(
            ...
            set-environment -g TERM_PROGRAM "orlix"
        ...
        let managedSession = "orlix_managed"
        defer {
            _ = try? runTmux(
                installedTmux,
                socket: socket,
        ...
        tmux() {
          for orlixArgument in "$@"; do
            if [ "$orlixArgument" = "-e" ] || [ "$orlixArgument" = "-T" ]; then
              return 1
            fi
          done
          \(installedTmux) -S '\(socket.path)' "$@"
        ...
        #expect(!managedWindows.output.contains("__orlix_bootstrap__"))
