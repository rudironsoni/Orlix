...
struct RemoteTmuxManagerParserTests {
    ...
    func parseWhitespaceFormatFromRealTmuxOutput() {
        let output = """
        orlix-00F43729-7E11-4731-ADFE-603A766AFCF6 1 1
        orlix-7922A0D1-DD37-4530-866F-30C60B0E9C26 0 1
        """

        let sessions = RemoteTmuxManager.shared.parseSessionListOutput(output, allowLegacy: false)
        #expect(sessions.count == 2)
        #expect(sessions[0].name == "orlix-00F43729-7E11-4731-ADFE-603A766AFCF6")
        #expect(sessions[0].attachedClients == 1)
        #expect(sessions[0].windowCount == 1)
        #expect(!sessions[0].name.hasSuffix(" 1 1"))
        #expect(sessions[1].name == "orlix-7922A0D1-DD37-4530-866F-30C60B0E9C26")
    ...
    func unixConfigWriteExecutesThroughSh() {
        ...
        #expect(command.contains("mkdir -p ~/.orlix"))
    ...
    func availabilityProbeUsesFallbackPathsAndNonLoginShell() {
        let probe = RemoteTmuxManager.shared.tmuxAvailabilityProbeCommand(okMarker: "__ORLIX_TMUX_OK__")
        ...
        #expect(probe.contains("__ORLIX_TMUX_OK__"))
    }
    ...
    func windowsPsmuxAttachCommandUsesPowerShellAndPsmux() {
        ...
        #expect(command.contains("new-session -A -s $orlixSession -c $orlixWorkingDirectory"))
    ...
    func windowsPsmuxAvailabilityProbeConfirmsTmuxAliasWithPsmuxExtension() {
        ...
        #expect(probe.contains("__ORLIX_TMUX_OK__:tmux"))
    }
