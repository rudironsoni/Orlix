...
final class NoticePresentationUITests: XCTestCase {
    ...
    func testConnectionFailureUsesBottomSheetWithLargePrimaryAction() throws {
        ...
        let close = app.buttons["orlix.connectionStatus.close"]
    ...
    func testConnectionFailureCloseExposesNavigationWithoutRetrying() throws {
        ...
        let back = app.buttons["orlix.noticeTest.back"]
        XCTAssertTrue(back.waitForExistence(timeout: 5))
        back.tap()
        XCTAssertTrue(
            app.descendants(matching: .any)["orlix.noticeTest.serverList"]
    ...
    func testDisconnectedSheetSupportsSwipeDismissalAndKeepsReconnect() throws {
        let app = launchNoticeHarness(
            additionalArguments: ["--orlix-ui-test-notice-disconnected"]
        )
        ...
        XCTAssertTrue(close.waitForNonExistence(timeout: 5))
        XCTAssertTrue(app.buttons["Reconnect"].waitForExistence(timeout: 5))
        XCTAssertTrue(app.buttons["orlix.noticeTest.back"].exists)
    }

    @MainActor
    func testHostKeyFailureRetainsCloseAndTrustActions() throws {
        let app = launchNoticeHarness(
            additionalArguments: ["--orlix-ui-test-notice-host-key"]
        )

        XCTAssertTrue(app.staticTexts["Connection Failed"].waitForExistence(timeout: 10))
        XCTAssertTrue(app.buttons["orlix.connectionStatus.close"].waitForExistence(timeout: 5))
    ...
    func testInitialConnectionUsesNonBlockingTopBanner() throws {
        let app = launchNoticeHarness(additionalArguments: ["--orlix-ui-test-notice-connecting"])
        ...
        let banner = app.descendants(matching: .any)
            .matching(identifier: "orlix.notice.banner")
            .firstMatch
    ...
    func testInitialConnectionBannerYieldsToTmuxSelectionSheet() throws {
        let app = launchNoticeHarness(
            additionalArguments: ["--orlix-ui-test-connection-banner-handoff"]
        )
    ...
    func testInactiveSplitPaneCannotPresentConnectionBanner() throws {
        let app = launchNoticeHarness(
            additionalArguments: ["--orlix-ui-test-inactive-connection-banner"]
        )
        let terminal = app.staticTexts["$ ssh production"]
    ...
    func testFilesOperationNoticeRemainsVisibleOnPushedPreview() throws {
        let app = launchNoticeHarness(additionalArguments: ["--orlix-ui-test-notice-files-preview"])
    ...
    func testFilesEntryCanReopenPreviewAfterBackNavigation() throws {
        ...
        let entry = app.buttons["orlix.noticeTest.filesEntry"]
    ...
    func testConcurrentOperationsStackAboveBottomToolbar() throws {
        let app = launchNoticeHarness(additionalArguments: ["--orlix-ui-test-notice-operation-stack"])
        let first = app.staticTexts["Upload 1"]
        let second = app.staticTexts["Upload 2"]
        let third = app.staticTexts["Upload 3"]
        let stackCount = app.otherElements["orlix.notice.operationStackCount"]
        let toolbarButton = app.buttons["orlix.noticeTest.bottomToolbar"]
    ...
    func testDiagnosticBannerExpandsCopiesScrollsAndDismisses() throws {
        let app = launchNoticeHarness(
            additionalArguments: ["--orlix-ui-test-notice-diagnostics"]
        )
        let details = app.descendants(matching: .any)
            .matching(identifier: "orlix.notice.details")
            .firstMatch
        XCTAssertTrue(details.waitForExistence(timeout: 10))
        details.tap()

        let detailText = app.descendants(matching: .any)
            .matching(identifier: "orlix.notice.detailText")
            .firstMatch
        let copy = app.descendants(matching: .any)
            .matching(identifier: "orlix.notice.copyDiagnostics")
            .firstMatch
        ...
        let close = app.descendants(matching: .any)
            .matching(identifier: "orlix.notice.detailClose")
            .firstMatch
        XCTAssertTrue(close.waitForExistence(timeout: 5))
        close.tap()

        let dismiss = app.descendants(matching: .any)
            .matching(identifier: "orlix.notice.dismiss")
            .firstMatch
        let banner = app.descendants(matching: .any)
            .matching(identifier: "orlix.notice.banner")
            .firstMatch
