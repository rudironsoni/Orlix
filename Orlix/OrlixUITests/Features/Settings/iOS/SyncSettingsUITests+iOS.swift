...
final class SyncSettingsUITests: XCTestCase {
    ...
    @MainActor
    func testSyncPageShowsHeroActionAndInlineDetails() {
        let app = launchHarness(syncEnabled: true)
        defer { app.terminate() }

        XCTAssertTrue(app.staticTexts["iCloud Sync"].waitForExistence(timeout: 10))
        let hero = app.descendants(matching: .any)["orlix.settings.sync.statusHero"]
        ...
        let toggle = app.switches["orlix.settings.sync.toggle"]
        ...
        XCTAssertFalse(app.buttons["orlix.settings.sync.detailsButton"].exists)
        ...
        let syncNow = app.buttons["orlix.settings.sync.action.primary"]
    ...
    func testCredentialRemovalUsesNativeConfirmation() {
        ...
        XCTAssertFalse(app.buttons["orlix.settings.sync.action.primary"].exists)

        XCTAssertFalse(app.buttons["orlix.settings.sync.detailsButton"].exists)
        XCTAssertFalse(app.staticTexts["iCloud Sync Details"].exists)

        let remove = app.buttons["orlix.settings.sync.removeCredentials"]
    ...
    private func launchHarness(syncEnabled: Bool) -> XCUIApplication {
        ...
        if !syncEnabled {
            app.launchArguments.append("--orlix-ui-test-sync-settings-disabled")
        }
        app.launch()
        return app
    }
