...
final class TrustedHostsSettingsUITests: XCTestCase {
    ...
    func testRowsShowExactLastUsedDateAndUseSwipeAction() {
        ...
        let row = app.descendants(matching: .any)[
            "orlix.settings.trustedHosts.entry.\(rowID)"
        ]
        ...
        XCTAssertFalse(
            app.buttons["orlix.settings.trustedHosts.actions.\(rowID)"].exists
        )

        row.swipeLeft()

        XCTAssertTrue(
            app.buttons["orlix.settings.trustedHosts.reset.\(rowID)"]
