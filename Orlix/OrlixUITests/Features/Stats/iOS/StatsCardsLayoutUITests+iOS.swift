...
final class StatsCardsLayoutUITests: XCTestCase {
    ...
    func testCompactCardsRemainContainedAcrossWideNarrowAndWideTransitions() throws {
        ...
        launch(extraArguments: ["--orlix-ui-test-stats-cards-compact"])
        try assertExpectedColumnsAndContainment()
    ...
    func testLockedDockerCardRemainsContainedAfterRepeatedRotation() throws {
        ...
        launch(extraArguments: ["--orlix-ui-test-stats-cards-locked-docker"])
        try assertExpectedColumnsAndContainment()
    ...
    private func launch(extraArguments: [String] = []) {
        app = XCUIApplication()
        app.launchArguments = [
            "--orlix-ui-test-stats-cards-layout-harness",
            "-AppleLanguages", "(en)",
            "-AppleLocale", "en_US",
            "-hasSeenWelcome", "YES",
    ...
    private var container: XCUIElement {
        app.descendants(matching: .any)["orlix.stats.layout.container"]
    }

    private func card(_ identifier: String) -> XCUIElement {
        app.descendants(matching: .any)["orlix.stats.card.\(identifier)"]
    }
