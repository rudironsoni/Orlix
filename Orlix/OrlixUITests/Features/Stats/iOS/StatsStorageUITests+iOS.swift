...
final class StatsStorageUITests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        app = XCUIApplication()
        app.launchArguments = [
            "--orlix-ui-test-stats-storage-harness",
            "-AppleLanguages", "(en)",
            "-AppleLocale", "en_US",
            "-hasSeenWelcome", "YES",
        ...
        app.launch()
        XCTAssertTrue(app.otherElements["orlix.stats.storage.details"].waitForExistence(timeout: 8))
    }
    ...
    func testBrowsingSwipeEditVisibilityAndHealthStates() throws {
        ...
        let editButton = app.buttons["orlix.stats.storage.editMode"]
        ...
        let showContainers = app.buttons["orlix.stats.storage.showContainers"]
        XCTAssertTrue(showContainers.waitForExistence(timeout: 3))
        showContainers.tap()
        XCTAssertEqual(containerVisibility.value as? String, "Visible")

        let hideContainers = app.buttons["orlix.stats.storage.hideContainers"]
        ...
        XCTAssertTrue(app.buttons["orlix.stats.storage.health.member.1"].exists)
        if !app.buttons["orlix.stats.storage.health.member.2"].exists {
            app.swipeUp()
        }
        XCTAssertTrue(app.buttons["orlix.stats.storage.health.member.2"].waitForExistence(timeout: 3))
        closePresentedHealth()
    ...
    private func closePresentedHealth() {
        ...
        close.tap()
        XCTAssertTrue(app.otherElements["orlix.stats.storage.details"].waitForExistence(timeout: 3))
    }
    ...
    private func volumeIdentifier(_ identity: String, suffix: String) -> String {
        let token = SHA256.hash(data: Data(identity.utf8))
            ...
            .joined()
        return "orlix.stats.storage.volume.\(token).\(suffix)"
    }
