...
class TerminalKeyboardUITestCase: XCTestCase {
    ...
    func launchKeyboardHarness(
        ...
        app.launchArguments = [
            "--orlix-ui-test-terminal-keyboard-harness",
        ...
        if splitPaneFocus {
            app.launchArguments.append("--orlix-ui-test-terminal-split-keyboard-harness")
        }
        if testsAppShortcutInputs {
            app.launchArguments.append("--orlix-ui-test-terminal-app-shortcut-inputs")
        }
        if simulatesTerminalMetadataChurn {
            app.launchArguments.append("--orlix-ui-test-terminal-metadata-churn")
        }
        if preservesTerminalSize {
            app.launchArguments.append("--orlix-ui-test-preserve-terminal-size")
        }
        if simulatesKeyboardFrames {
            app.launchArguments.append("--orlix-ui-test-simulate-keyboard-frames")
        }
        if simulatesCodexTUIResponse {
            app.launchArguments.append("--orlix-ui-test-codex-tui-response")
        }
        if simulatesTerminalMouseCapture {
            app.launchArguments.append("--orlix-ui-test-terminal-mouse-capture")
        }
        if seedsTerminalSelectionFixture {
            app.launchArguments.append("--orlix-ui-test-terminal-selection-fixture")
        }
        if seedsTerminalPasteboard {
            app.launchArguments.append("--orlix-ui-test-terminal-pasteboard")
        }
        if simulatesDetachedLightAccessoryHost {
            app.launchArguments += [
                "--orlix-ui-test-detached-light-accessory-host",
                "--orlix-ui-test-clear-terminal-background-cache",
                "-appearanceMode", "dark",
                "-terminalUsePerAppearanceTheme", "YES",
                "-terminalThemeName", "Orlix Dark",
                "-terminalThemeNameLight", "Orlix Light"
            ]
        }
        ...
        let ready = app.staticTexts["orlix.keyboardTest.ready"]
    ...
    func waitForTerminal(in app: XCUIApplication) -> XCUIElement {
        let terminal = app.descendants(matching: .any)
            .matching(identifier: "orlix.keyboardTest.terminalSurface")
            .firstMatch
    ...
    func openSettingsSheet(in app: XCUIApplication) -> XCUIElement {
        let menu = app.buttons["orlix.keyboardTest.menu"]
        XCTAssertTrue(menu.waitForExistence(timeout: 5), diagnosticsText(in: app))
        menu.tap()

        let settingsItem = app.descendants(matching: .any)["orlix.keyboardTest.menu.settings"]
        XCTAssertTrue(settingsItem.waitForExistence(timeout: 5), diagnosticsText(in: app))
        settingsItem.tap()

        let settingsSheet = app.descendants(matching: .any)["orlix.keyboardTest.settings.sheet"]
    ...
    func closeSettingsSheet(
        ...
        let closeButton = app.buttons["orlix.keyboardTest.settings.close"]
    ...
    func requestKeyboardFromMenu(in app: XCUIApplication) {
        ...
        let keyboardItem = app.descendants(matching: .any)["orlix.keyboardTest.menu.showKeyboard"]
    ...
    func induceUnexpectedKeyboardLoss(
        ...
        let lossButton = app.buttons["orlix.keyboardTest.keyboard.unexpectedLoss"]
    ...
    func diagnosticMetrics(in app: XCUIApplication) -> [String: Double] {
        let label = app.staticTexts["orlix.keyboardTest.diagnostics"].label
