...
class TerminalReconnectUITestCase: XCTestCase {
    ...
    func openProductionTerminalMenu(in app: XCUIApplication) {
        let menu = app.navigationBars.firstMatch.buttons["orlix.terminal.moreMenu"]
    ...
    func assertKeyboardAndAccessoryVisible(
        ...
        XCTAssertTrue(
            app.descendants(matching: .any)["orlix.keyboard.accessory.hide"]
                .waitForExistence(timeout: 5),
            diagnosticText(in: app)
        )
        XCTAssertTrue(
            app.descendants(matching: .any)["orlix.keyboard.accessory.voice"]
                .waitForExistence(timeout: 5),
