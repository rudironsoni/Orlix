...
final class TerminalKeyboardLifecycleUITests: TerminalKeyboardUITestCase {
    ...
    func testBackgroundRoundTripPreservesTerminalTyping() throws {
        ...
        app.buttons["orlix.keyboardTest.geometry.docked"].tap()
        wait(
            for: diagnostics,
        ...
        for _ in 0..<3 {
            ...
            XCTAssertTrue(
                ...
                "Orlix did not enter the background. \(diagnosticsText(in: app))"
            )

            app.activate()
            XCTAssertTrue(
                app.wait(for: .runningForeground, timeout: 8),
                "Orlix did not return to the foreground. \(diagnosticsText(in: app))"
            )
    ...
    func testKeyboardMenuDismissesFindAndTransfersInputToTerminal() throws {
        ...
        let findItem = app.descendants(matching: .any)["orlix.keyboardTest.menu.find"]
    ...
    func testPrivacyShieldHidesAccessoryAndRestoresResponsiveTerminal() throws {
        ...
        app.buttons["orlix.keyboardTest.privacy.shield"].tap()
        XCTAssertTrue(
            app.descendants(matching: .any)["orlix.keyboardTest.privacyShield"]
                .waitForExistence(timeout: 5),
            diagnosticsText(in: app)
        )
        assertKeyboardAndAccessoryHidden(in: app)

        app.buttons["orlix.keyboardTest.privacy.resume"].tap()
        XCTAssertTrue(
            app.descendants(matching: .any)["orlix.keyboardTest.privacyShield"]
        ...
        wait(
            for: app.staticTexts["orlix.keyboardTest.diagnostics"],
            labelContaining: "renderingPaused=false",
            timeout: 5,
            diagnostics: diagnosticsText(in: app)
        )
    ...
    func testTemporarySystemOverlayDetachesAndRestoresAccessoryWithoutLosingTyping() throws {
        ...
        app.buttons["orlix.keyboardTest.scene.inactive"].tap()
        wait(
            for: diagnostics,
        ...
        app.buttons["orlix.keyboardTest.scene.active"].tap()
        wait(
            for: diagnostics,
            labelContaining: "reconnect=connected",
            timeout: 5,
        ...
        app.buttons["orlix.keyboardTest.geometry.floating"].tap()
        wait(
            for: diagnostics,
            labelContaining: "accessorySuppressed=false",
            timeout: 5,
    ...
    func testTemporarySystemOverlayPreservesUserHiddenKeyboardIntent() throws {
        ...
        app.buttons["orlix.keyboardTest.hideViaToolbar"].tap()
        wait(
            for: diagnostics,
            labelContaining: "softwareInputActive=true",
            timeout: 5,
    ...
    func testCrossAppFocusTransferReleasesResponderWithoutRebuild() throws {
        ...
        let dockedButton = app.buttons["orlix.keyboardTest.geometry.docked"]
        ...
        let transferButton = app.buttons["orlix.keyboardTest.window.notKey"]
        ...
        let returnButton = app.buttons["orlix.keyboardTest.window.key"]
    ...
    func testSameScreenForeignKeyboardDoesNotReclaimTerminalAccessory() {
        ...
        app.buttons["orlix.keyboardTest.geometry.foreignDocked"].tap()
        wait(
            for: diagnostics,
        ...
        app.buttons["orlix.keyboardTest.window.key"].tap()
        wait(
            for: diagnostics,
            labelContaining: "softwareInputActive=true",
            timeout: 5,
    ...
    func testRepeatedTerminalReconstructionKeepsRenderingAndInputResponsive() throws {
        ...
        for _ in 0..<12 {
            app.buttons["orlix.keyboardTest.mode.other"].tap()
            XCTAssertTrue(
                app.buttons["orlix.keyboardTest.nonTerminalSurface"].waitForExistence(timeout: 3),
                diagnosticsText(in: app)
            )

            app.buttons["orlix.keyboardTest.mode.terminal"].tap()
