...
final class TerminalKeyboardOwnershipUITests: TerminalKeyboardUITestCase {
    ...
    func testRepeatedSplitPaneFocusKeepsOneInputUISessionWithoutReloadLoop() throws {
        ...
        app.buttons["orlix.keyboardTest.focus.first"].tap()
        firstTerminal.tap()
        wait(
            for: diagnostics,
        ...
        for index in 0..<20 {
            ...
            app.buttons[
                focusesSecond
                    ? "orlix.keyboardTest.focus.second"
                    : "orlix.keyboardTest.focus.first"
            ].tap()
            wait(
                for: diagnostics,
        ...
        app.buttons["orlix.keyboardTest.hardware.attach"].tap()
        wait(
            for: diagnostics,
        ...
        let commands: [(button: String, action: String)] = [
            ("orlix.keyboardTest.command.cmdD", "splitRight"),
            ("orlix.keyboardTest.command.cmdAltLeft", "selectLeft"),
            ("orlix.keyboardTest.command.cmdCtrlRight", "moveDividerRight"),
        ]
    ...
    func testKeyboardButtonRestoresAfterUserHideButScrollingDoesNot() throws {
        ...
        let harnessHideButton = app.buttons["orlix.keyboardTest.hideViaToolbar"]
        XCTAssertTrue(
        ...
        let floatingKeyboardButton = app.buttons["orlix.terminal.floating.keyboard"]
        let floatingVoiceButton = app.buttons["orlix.terminal.floating.voiceInput"]
        XCTAssertTrue(
        ...
        app.buttons["orlix.keyboardTest.voice.transcriptionSent"].tap()
        let floatingReturnButton = app.buttons["orlix.terminal.floating.return"]
        XCTAssertTrue(
        ...
        app.buttons["orlix.keyboardTest.hardware.detach"].tap()
    ...
    func testKeyboardIsScopedToTerminalSurface() throws {
        ...
        XCTAssertTrue(
            app.buttons["orlix.keyboardTest.nonTerminalSurface"].waitForExistence(timeout: 5),
            diagnosticsText(in: app)
        )
