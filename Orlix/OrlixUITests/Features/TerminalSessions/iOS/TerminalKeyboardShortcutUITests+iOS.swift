...
final class TerminalKeyboardShortcutUITests: TerminalKeyboardUITestCase {
    ...
    func testSoftwareToolbarAndCustomShortcutCombinationsUseAppRouting() throws {
        ...
        let localActions: [(button: String, action: String)] = [
            ("orlix.keyboardTest.shortcut.software.cmdD", "splitRight"),
            ("orlix.keyboardTest.shortcut.software.cmdShiftD", "splitDown"),
            ("orlix.keyboardTest.shortcut.toolbar.cmdAltLeft", "selectLeft"),
            ("orlix.keyboardTest.shortcut.custom.cmdCtrlRight", "moveDividerRight"),
        ]
        ...
        app.buttons["orlix.keyboardTest.shortcut.custom.ctrlX"].tap()
        wait(
            for: diagnostics,
        ...
        app.buttons["orlix.keyboardTest.shortcut.software.cmdW"].tap()
        wait(
            for: diagnostics,
    ...
    func testPaneCloseAlertRestoresTerminalFocusAfterCancel() throws {
        ...
        let terminal = waitForTerminal(in: app)
        let diagnostics = app.staticTexts["orlix.keyboardTest.diagnostics"]
        let closeAlertButton = app.buttons["orlix.keyboardTest.closeAlert"]
        let confirmation = app.alerts["Close this terminal?"]
