...
#include "OrlixHostAdapter/execution/trap.h"
                                               ...
                                               unsigned long fault_address)
{
    ...
    struct orlix_host_user_tls_repair_request request = {0};
    struct orlix_host_user_tls_repair_decision decision = {0};
    ...
    request.user_base = OrlixHostUserTrap.user_base;
    request.user_limit = OrlixHostUserTrap.user_limit;
    request.fault_address = fault_address;
    request.installed_host_tls = OrlixHostUserTrap.host_tls;
    request.live_host_tls = OrlixHostReadTls();
    request.active_user_tls =
        __atomic_load_n(OrlixHostUserTrap.active_user_tls, __ATOMIC_ACQUIRE);
    request.faulting_instruction = *(const uint32_t *)user_pc;

    for (unsigned int reg = 0; reg < 31U; reg++) {
    ...
    if (!OrlixHostUserTrap.tls_repair_handler(&request, &decision) ||
        decision.register_index >= 31U ||
        !OrlixHostUserTrapValidUserTls(decision.user_tls) ||
    ...
    OrlixHostWriteTls(decision.user_tls);
    return true;
}
    ...
    orlix_host_user_trap_entry_t entry)
{
    ...
    if (!kernel_sp || !active_user_tls || !user_active ||
        !tls_repair_handler || !entry ||
    ...
    OrlixHostUserTrap.tls_repair_handler = tls_repair_handler;
