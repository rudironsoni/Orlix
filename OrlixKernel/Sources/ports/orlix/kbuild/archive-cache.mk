...
orlix_depfile_dependencies_current() { \
	perl -e 'use strict; use warnings; my ($$artifact, $$depfile) = @ARGV; my $$artifact_mtime = (stat $$artifact)[9]; defined $$artifact_mtime or exit 1; open my $$fh, q{<}, $$depfile or exit 1; local $$/; my $$text = <$$fh>; $$text =~ s/\\\n/ /g; $$text =~ s/^[^:]*:\s*//s or exit 1; my $$count = 0; while (length $$text) { $$text =~ s/^\s+//; last unless length $$text; $$text =~ s/^((?:\\.|[^\s\\])+)// or exit 1; my $$path = $$1; $$path =~ s/\\(.)/$$1/gs; my $$dependency_mtime = (stat $$path)[9]; defined $$dependency_mtime && $$artifact_mtime > $$dependency_mtime or exit 1; $$count++; } exit($$count ? 0 : 1);' "$$1" "$$2"; \
}; \
orlix_archive_cache_current() { \
	...
	for index in "$${!sources[@]}"; do \
		...
		orlix_depfile_dependencies_current "$$archive" "$$depfile" || return 1; \
	done; \
	};
endef
...
__archive-cache-tests:
	...
	archive="$$tmp/OrlixKernel.a"; symbols="$$tmp/symbols.txt"; source="$$tmp/source.c"; object="$$tmp/source.o"; depfile="$$tmp/source.d"; header="$$tmp/source.h"; symlink_header="$$tmp/header symlink.h"; spaced_header="$$tmp/header with space.h"; reconciliation="$$tmp/target_system_accessor_reconciliation.def"; test_kbuild="$$tmp/orlix_tcti.Makefile"; test_declaration="$$tmp/target_refresh_artifacts.def"; \
	printf 'source\n' > "$$source"; printf 'header\n' > "$$header"; ln -s "$$header" "$$symlink_header"; printf 'spaced header\n' > "$$spaced_header"; printf 'reconciliation\n' > "$$reconciliation"; printf 'OrlixTCTI kbuild\n' > "$$test_kbuild"; printf 'publisher declaration\n' > "$$test_declaration"; printf 'object\n' > "$$object"; \
	printf '%s: %s %s %s %s\n' "$$object" "$$source" "$$header" "$${symlink_header// /\\ }" "$${spaced_header// /\\ }" > "$$depfile"; \
	printf '_arch_boot_entry\n_arch_boot_params\n' > "$$symbols"; printf 'archive\n' > "$$archive"; \
	touch -t 202607251759 "$$source" "$$object" "$$header" "$$spaced_header" "$$depfile" "$$symbols" "$$reconciliation" "$$test_kbuild" "$$test_declaration"; touch_newer "$$archive"; \
	required_symbols=(_arch_boot_entry _arch_boot_params); cache_deps=("$$reconciliation" "$$test_kbuild" "$$test_declaration"); sources=("$$source"); objects=("$$object"); depfiles=("$$depfile"); \
	...
	touch_newer "$$symlink_header"; assert_stale "symlink depfile dependency"; touch_newer "$$archive"; assert_current; \
	touch_newer "$$spaced_header"; assert_stale "escaped-space depfile dependency"; touch_newer "$$archive"; assert_current; \
	printf '%s:\n' "$$object" > "$$depfile"; assert_stale "empty depfile dependency list"; \
	printf '%s: %s \\' "$$object" "$$source" > "$$depfile"; assert_stale "unterminated dependency escape"; printf '%s: %s %s %s %s\n' "$$object" "$$source" "$$header" "$${symlink_header// /\\ }" "$${spaced_header// /\\ }" > "$$depfile"; touch_newer "$$archive"; assert_current; \
