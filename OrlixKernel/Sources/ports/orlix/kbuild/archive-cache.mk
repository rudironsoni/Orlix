...
# Recipe-local dependency and archive reuse predicates. Callers provide archive,
# symbols, required_symbols, cache_deps, sources, objects, and depfiles shell
# variables.
define orlix_object_dependency_predicate
orlix_object_dependencies_current() { \
	object="$$1"; depfile="$$2"; \
	[ -s "$$object" ] && [ -s "$$depfile" ] || return 1; \
	perl -0pe 's/\\\n/ /g; s/^[^:]*:\s*//' "$$depfile" | tr ' ' '\n' | \
		while IFS= read -r dependency; do \
			[ -n "$$dependency" ] || continue; \
			[ -e "$$dependency" ] && [ "$$object" -nt "$$dependency" ] || exit 1; \
		done; \
};
endef
...
__archive-cache-tests:
	...
	grep -Fq '$$(orlix_object_dependency_predicate)' "$$kernel_rules" || fail "product archive does not use the shared object dependency predicate"; \
	...
	archive="$$tmp/OrlixKernel.a"; symbols="$$tmp/symbols.txt"; source="$$tmp/source.c"; object="$$tmp/source.o"; depfile="$$tmp/source.d"; object_depfile="$$tmp/object.d"; header="$$tmp/source.h"; spaced_header="$$tmp/header with space.h"; reconciliation="$$tmp/target_system_accessor_reconciliation.def"; test_kbuild="$$tmp/orlix_tcti.Makefile"; test_declaration="$$tmp/target_refresh_artifacts.def"; test_proof_provenance="$$tmp/proof-provenance.mk"; \
	...
	printf '%s: %s %s\n' "$$object" "$$source" "$$header" > "$$object_depfile"; \
	...
	touch -t 202607251759 "$$source" "$$object" "$$header" "$$spaced_header" "$$depfile" "$$object_depfile" "$$symbols" "$$reconciliation" "$$test_kbuild" "$$test_declaration" "$$test_proof_provenance"; touch_newer "$$archive"; \
	...
	fd_count() { lsof -p "$$$$" -Fn 2>/dev/null | awk '/^f/ { count += 1 } END { print count + 0 }'; }; \
	assert_object_current() { orlix_object_dependencies_current "$$object" "$$object_depfile"; }; \
	assert_object_stale() { if assert_object_current; then fail "$$1 remained current"; fi; }; \
	assert_object_current; \
	touch_newer "$$header"; assert_object_stale "newer object dependency"; touch_newer "$$object"; touch_newer "$$archive"; assert_object_current; \
	fd_before="$$(fd_count)"; \
	for ((iteration = 0; iteration < 128; iteration += 1)); do assert_object_current; done; \
	fd_after="$$(fd_count)"; \
	[ "$$fd_after" -le "$$fd_before" ] || fail "object dependency predicate leaked file descriptors ($$fd_before to $$fd_after)"; \
