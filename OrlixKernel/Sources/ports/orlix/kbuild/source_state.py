...
def _build_content(root: Path) -> dict:
    result = {}
    for directory in ("build", "release", "development"):
        tree = root / "OrlixKernel" / directory
        if tree.is_symlink():
            raise ValueError(f"build tree is a symlink: {tree}")
        for name, path in _files(tree).items():
            mode, data = _entry(path)
            result[directory + "/" + name] = [mode, hashlib.sha256(data).hexdigest()]
    return result
...
def resume(root: Path, identity_inputs: list[Path]) -> None:
    ...
    try:
        ...
        valid = isinstance(previous, dict) and previous["identity"] == expected and previous["files"] == _build_content(root)
    except (OSError, ValueError, KeyError, TypeError):
        valid = False
    if not valid:
        for name in ("build", "release", "development"):
            _remove(root / "OrlixKernel" / name)
...
def record(root: Path) -> None:
    payload = {"identity": (root / "build-identity").read_text(), "files": _build_content(root)}
