...
# Keep the active execution profile generated artifact as one canonical
# build-time contributor. The publisher declaration owns this path, so this
# check must not add a second materialization or cache input.
ORLIX_TCTI_ISA_ACTIVE_EXECUTION_PROFILE_ARTIFACT_INPUTS := \
    $(filter %/generations/current/target_active_execution_profile_artifact.def,$(ORLIX_TCTI_INSTRUCTION_ARTIFACT_INPUTS))
ifneq ($(words $(ORLIX_TCTI_ISA_ACTIVE_EXECUTION_PROFILE_ARTIFACT_INPUTS)),1)
$(error canonical active execution profile artifact must occur exactly once in ORLIX_TCTI_INSTRUCTION_ARTIFACT_INPUTS)
endif
