	...
	struct registry_row *rows, size_t *count)
{
	...
	*count = 0;
	while ((cursor = next_line(cursor, end, &line, &line_length))) {
		...
		if (!row_prefix(line, line_length,
			"ORLIX_TCTI_A64_PROOF_REGISTRY_BINDING(", &body))
			continue;
		if (*count == ORLIX_TCTI_ACTIVE_EXECUTION_MAX_ROWS ||
			sscanf(body,
...
static int tcnd_hex_decode(const char *text, size_t hex_length,
	unsigned char **bytes_out, size_t *byte_length_out)
{
	unsigned char *bytes;
	size_t byte_length;
	size_t index;

	if (!text || !bytes_out || !byte_length_out)
		return 0;
...
static int tcnd_hex_valid(const char *text)
{
	...
	if (!text || !tcnd_hex_decode(text, strlen(text), &bytes, &byte_length))
		return 0;
	offset = 5U;
...
enum tcnd_profile_relation {
	TCND_PROFILE_INVALID = -1,
	TCND_PROFILE_FALSE,
	TCND_PROFILE_TRUE,
	TCND_PROFILE_MATCH,
	TCND_PROFILE_UNKNOWN,
};

static enum tcnd_profile_relation tcnd_profile_and(
	enum tcnd_profile_relation left, enum tcnd_profile_relation right)
{
	if (left == TCND_PROFILE_FALSE || right == TCND_PROFILE_FALSE)
		return TCND_PROFILE_FALSE;
	if (left == TCND_PROFILE_TRUE)
		return right;
	if (right == TCND_PROFILE_TRUE)
		return left;
	if (left == TCND_PROFILE_MATCH || right == TCND_PROFILE_MATCH)
		return TCND_PROFILE_MATCH;
	return TCND_PROFILE_UNKNOWN;
}

static enum tcnd_profile_relation tcnd_profile_or(
	enum tcnd_profile_relation left, enum tcnd_profile_relation right)
{
	if (left == TCND_PROFILE_TRUE || right == TCND_PROFILE_TRUE)
		return TCND_PROFILE_TRUE;
	if (left == TCND_PROFILE_FALSE)
		return right;
	if (right == TCND_PROFILE_FALSE)
		return left;
	if (left == TCND_PROFILE_MATCH || right == TCND_PROFILE_MATCH)
		return TCND_PROFILE_MATCH;
	return TCND_PROFILE_UNKNOWN;
...
/*
 * Resolve only the Boolean fragment needed by active-profile projection.
 * Feature atoms can establish a positive profile join.  Operand, value, set,
 * comparison, and negation semantics require a typed environment that this
 * producer does not own, so those forms fail closed.
 */
static int tcnd_profile_record(const unsigned char *bytes, size_t length,
	size_t *offset, size_t depth, const char *feature,
	enum tcnd_profile_relation *relation)
{
	unsigned char tag;
	uint32_t payload_length;
	size_t payload_start;
	size_t payload_end;
	enum tcnd_profile_relation left;
	enum tcnd_profile_relation right;
	...
	*offset = payload_start;
	if (tag == 1U) {
		if (payload_length != 1U || bytes[payload_start] > 1U)
			return -1;
		*relation = bytes[payload_start] ? TCND_PROFILE_TRUE :
			TCND_PROFILE_FALSE;
		*offset = payload_end;
		return 0;
	}
	if (tag == 2U) {
		...
		if (payload_length < 4U ||
			big_endian_u32(bytes + payload_start) != payload_length - 4U)
			return -1;
		text_length = payload_length - 4U;
		feature_length = strlen(feature);
		if (!text_length || feature_length != (size_t)text_length)
			*relation = TCND_PROFILE_UNKNOWN;
		else if (!memcmp(bytes + payload_start + 4U, feature, feature_length))
			*relation = TCND_PROFILE_MATCH;
		else
			*relation = TCND_PROFILE_UNKNOWN;
		*offset = payload_end;
		return 0;
	...
	if (tag == 7U || tag == 8U) {
		if (tcnd_profile_record(bytes, payload_end, offset, depth + 1U,
			feature, &left) ||
			tcnd_profile_record(bytes, payload_end, offset, depth + 1U,
				feature, &right) || *offset != payload_end)
			return -1;
		*relation = tag == 7U ? tcnd_profile_and(left, right) :
			tcnd_profile_or(left, right);
		return 0;
	}
	/* NOT and all value/set operators are unresolved in this producer. */
	return -1;
}

static int tcnd_profile_mentions_feature(const char *text, size_t length,
	const char *feature)
{
	unsigned char *bytes = NULL;
	size_t byte_length;
	...
	if (!tcnd_hex_decode(text, length, &bytes, &byte_length))
		return 0;
	offset = 5U;
	if (tcnd_profile_record(bytes, byte_length, &offset, 0U, feature,
		&relation) || offset != byte_length) {
		free(bytes);
		return 0;
	}
	free(bytes);
	return relation == TCND_PROFILE_MATCH;
}
...
static int applicability_mentions_feature(const char *text,
	const struct applicability_row *row, const char *feature)
{
	if (row->condition_offset > SIZE_MAX - row->condition_text_length)
		return 0;
	return tcnd_profile_mentions_feature(text + row->condition_offset,
		row->condition_text_length, feature);
}
	...
	size_t count, const char *name, const char *proof)
{
	...
	for (index = 0; index < count; index++)
		if ((!strcmp(rows[index].state, "REQUIRED") ||
			 !strcmp(rows[index].state, "NON_EL0")) &&
	...
	const char *source_manifest, size_t source_manifest_length, FILE *output)
{
	...
	if (parse_registry_rows(proof_registry, proof_registry_length, registry,
		&registry_count)) {
		result = ORLIX_TCTI_ACTIVE_EXECUTION_PROFILE_ARTIFACT_GENERATOR_SOURCE_MISMATCH;
		goto out;
	}
		...
		for (promotion_index = 0; promotion_index < promotion_count;
			...
			if (profiles[index].state != ENABLED ||
				...
				strcmp(applicability[promotion->ordinal].name,
					expected_source_name) ||
				!applicability_mentions_feature(feature_applicability,
