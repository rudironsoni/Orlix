...
static int build_source_manifest(struct text *source)
{
	static const char *const condition =
			"54434e44010700000017010000000101020000000c00000008464541545f535645";
	...
	for (ordinal = 0; ordinal < 4350U; ordinal++)
		if (appendf(source,
			...
			ordinal, ordinal == 2166U ? "leaf_x" : "source_leaf", condition,
			ordinal + 1U))
			return -1;
	return 0;
}
...
static int build_applicability(struct text *applicability)
{
	...
	for (ordinal = 0; ordinal < 4350U; ordinal++)
		if (appendf(applicability,
			"ORLIX_TCTI_A64_FEATURE_APPLICABILITY_ROW(%uU, \"%s\", \"ADD\", \"add\", 0U, 33U, 1U, \"%s\", \"\", 0U, 0U, 0U, 0x1ULL)\n",
...
static int boolean_condition_semantics(void)
{
	static const char *const positive =
		"54434e44010700000017010000000101020000000c00000008464541545f535645";
	static const char *const false_conjunct =
	...
	static const char *const alternative =
		"54434e44010800000022020000000c00000008464541545f535645020000000c00000008464541545f534d45";
	struct applicability_row row = { 0 };

	row.condition_text_length = strlen(positive);
	EXPECT(applicability_requires_feature(positive, &row, "FEAT_SVE"));
	row.condition_text_length = strlen(false_conjunct);
	EXPECT(!applicability_requires_feature(false_conjunct, &row, "FEAT_SVE"));
	row.condition_text_length = strlen(negative);
	EXPECT(!applicability_requires_feature(negative, &row, "FEAT_SVE"));
	row.condition_text_length = strlen(alternative);
	EXPECT(!applicability_requires_feature(alternative, &row, "FEAT_SVE"));
	return 0;
}
...
static int valid_enabled_and_mutations(void)
{
	...
	EXPECT(replace_once(&mutation, "source_bound_proof.def", "stale-proof.def") == 0);
	EXPECT(emit_fixture(&fixture.source_manifest, &fixture.applicability,
		&fixture.cohort,
		&fixture.profile, &fixture.promotion, &fixture.proof,
	...
	EXPECT(text_copy(&mutation, &fixture.registry) == 0);
	EXPECT(appendf(&mutation,
		"ORLIX_TCTI_A64_PROOF_REGISTRY_BINDING(2166U, \"stale-proof\")\n") == 0);
	EXPECT(emit_fixture(&fixture.source_manifest, &fixture.applicability,
		&fixture.cohort, &fixture.profile, &fixture.promotion, &fixture.proof,
		&fixture.classification, &mutation, &second) !=
		ORLIX_TCTI_ACTIVE_EXECUTION_PROFILE_ARTIFACT_GENERATOR_OK);
	text_destroy(&mutation);
	free(second);
	...
	EXPECT(text_copy(&mutation, &fixture.classification) == 0);
	EXPECT(replace_once(&mutation, "REQUIRED", "NON_EL0") == 0);
	EXPECT(emit_fixture(&fixture.source_manifest, &fixture.applicability,
		&fixture.cohort, &fixture.profile, &fixture.promotion, &fixture.proof,
		&mutation, &fixture.registry, &second) !=
		ORLIX_TCTI_ACTIVE_EXECUTION_PROFILE_ARTIFACT_GENERATOR_OK);
	text_destroy(&mutation);
	free(second);
	second = NULL;
	...
	EXPECT(replace_once(&mutation,
		"54434e4401070000001701000000010102",
		"bad") == 0);
...
int main(void)
{
	return boolean_condition_semantics() || valid_enabled_and_mutations() ? 1 : 0;
}
