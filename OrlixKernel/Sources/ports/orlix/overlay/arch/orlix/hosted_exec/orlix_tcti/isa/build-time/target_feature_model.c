...
struct parser { const char *s; size_t n, at, depth, count, capacity; struct token *tokens; struct orlix_tcti_feature_error *error; };
struct importer { const char *s; struct token *tokens; size_t count, depth; struct orlix_tcti_feature_model *model; struct orlix_tcti_feature_error *error; };

static void error(struct orlix_tcti_feature_error *e, enum orlix_tcti_feature_error_code code, size_t offset, const char *fmt, ...)
{
	va_list ap;
	if (!e || e->code)
		return;
...
static int token_add(struct parser *p, enum json_kind kind, size_t start)
{
	if (p->count == MAX_TOKENS || grow((void **)&p->tokens, &p->capacity,
		...
		error(p->error, ORLIX_TCTI_FEATURE_LIMIT, p->at, "JSON token limit");
		return -1;
	}
...
invalid:
	free(text); error(p->error, ORLIX_TCTI_FEATURE_INVALID_JSON, p->at, "invalid JSON string"); return -1;
oom:
	free(text); error(p->error, ORLIX_TCTI_FEATURE_NO_MEMORY, p->at, "JSON string"); return -1;
}
...
static int json_container(struct parser *p, enum json_kind kind, char close)
{
	...
	for (;;) {
		...
		if (kind == JSON_OBJECT) {
			...
			key = json_string(p);
			if (key < 0) return -1;
			if (duplicate_key(p, object, key)) { error(p->error, ORLIX_TCTI_FEATURE_DUPLICATE_KEY, p->tokens[key].start, "duplicate JSON key"); return -1; }
			whitespace(p);
			if (p->at == p->n || p->s[p->at++] != ':') goto invalid;
	...
	return object;
invalid:
	error(p->error, ORLIX_TCTI_FEATURE_INVALID_JSON, p->at, "malformed JSON container");
	return -1;
}
...
static int json_primitive(struct parser *p)
{
	...
	return index;
invalid:
	error(p->error, ORLIX_TCTI_FEATURE_INVALID_JSON, p->at, "invalid JSON primitive");
	return -1;
}
...
static int json_value(struct parser *p)
{
	int result;
	whitespace(p);
	if (++p->depth > MAX_DEPTH) { error(p->error, ORLIX_TCTI_FEATURE_LIMIT, p->at, "JSON depth"); return -1; }
	...
	p->depth--;
	if (result < 0 && !p->error->code) error(p->error, ORLIX_TCTI_FEATURE_INVALID_JSON, p->at, "missing JSON value");
	return result;
}
...
static int node_add(struct importer *in, struct orlix_tcti_feature_node node, uint32_t *out)
{
	if (in->model->node_count == UINT32_MAX || grow((void **)&in->model->nodes, &in->model->node_capacity, in->model->node_count + 1, sizeof(*in->model->nodes))) {
		free(node.text); error(in->error, ORLIX_TCTI_FEATURE_NO_MEMORY, 0, "feature nodes"); return -1;
	}
...
static int operator_kind(struct importer *in, int object, enum orlix_tcti_feature_node_kind *kind)
{
	int op = field(in, object, "op");
	if (string_eq(in, op, "&&")) *kind = ORLIX_TCTI_FEATURE_AND;
	else if (string_eq(in, op, "||")) *kind = ORLIX_TCTI_FEATURE_OR;
	else if (string_eq(in, op, "==")) *kind = ORLIX_TCTI_FEATURE_EQ;
	else if (string_eq(in, op, "!=")) *kind = ORLIX_TCTI_FEATURE_NE;
	else if (string_eq(in, op, "<")) *kind = ORLIX_TCTI_FEATURE_LT;
	else if (string_eq(in, op, ">")) *kind = ORLIX_TCTI_FEATURE_GT;
	else if (string_eq(in, op, ">=")) *kind = ORLIX_TCTI_FEATURE_GE;
	else if (string_eq(in, op, "IN")) *kind = ORLIX_TCTI_FEATURE_IN;
	else if (string_eq(in, op, "==>")) *kind = ORLIX_TCTI_FEATURE_IMPLIES;
	else if (string_eq(in, op, "<=>")) *kind = ORLIX_TCTI_FEATURE_IFF;
	else { error(in->error, ORLIX_TCTI_FEATURE_UNSUPPORTED_GRAMMAR, in->tokens[object].start, "unsupported operator"); return -1; }
	return 0;
}
...
static int field_null_qualifier(struct importer *in, int object,
	const char *name, struct orlix_tcti_feature_field_qualifier *qualifier)
{
	...
	if (!primitive_eq(in, value, "null")) {
		error(in->error, ORLIX_TCTI_FEATURE_UNSUPPORTED_GRAMMAR,
			value >= 0 && (size_t)value < in->count ?
			in->tokens[value].start : in->tokens[object].start,
			"unsupported Types.Field qualifier");
		return -1;
	}
	qualifier->kind = ORLIX_TCTI_FEATURE_FIELD_QUALIFIER_NULL;
	qualifier->provenance = (struct orlix_tcti_feature_provenance) {
...
{
	int type, value;
	struct orlix_tcti_feature_node n;
	if (object < 0 || (size_t)object >= in->count || in->tokens[object].kind != JSON_OBJECT) goto malformed;
	if (++in->depth > MAX_DEPTH) { error(in->error, ORLIX_TCTI_FEATURE_LIMIT, in->tokens[object].start, "AST depth"); return -1; }
	type = field(in, object, "_type");
	n = (struct orlix_tcti_feature_node){ .left = ORLIX_TCTI_FEATURE_NODE_NONE, .right = ORLIX_TCTI_FEATURE_NODE_NONE, .first_child = ORLIX_TCTI_FEATURE_NODE_NONE, .provenance = { in->tokens[object].start, in->tokens[object].end - in->tokens[object].start } };
	if (string_eq(in, type, "AST.Bool")) { value = field(in, object, "value"); if (!primitive_eq(in, value, "true") && !primitive_eq(in, value, "false")) goto malformed_depth; n.kind = ORLIX_TCTI_FEATURE_BOOL; n.integer = in->s[in->tokens[value].start] == 't'; }
	else if (string_eq(in, type, "AST.Identifier") || string_eq(in, type, "Values.Value")) { n.kind = string_eq(in, type, "AST.Identifier") ? ORLIX_TCTI_FEATURE_IDENTIFIER : ORLIX_TCTI_FEATURE_VALUE; n.text = string_copy(in, field(in, object, "value")); if (!n.text) goto malformed_depth; }
	else if (string_eq(in, type, "AST.Integer")) { n.kind = ORLIX_TCTI_FEATURE_INTEGER; if (primitive_integer(in, field(in, object, "value"), &n.integer)) goto malformed_depth; }
	else if (string_eq(in, type, "AST.DotAtom") || string_eq(in, type, "AST.Set")) { n.kind = string_eq(in, type, "AST.DotAtom") ? ORLIX_TCTI_FEATURE_DOT_ATOM : ORLIX_TCTI_FEATURE_SET; if (node_children(in, field(in, object, "values"), &n.first_child, &n.child_count)) goto fail_depth; }
	else if (string_eq(in, type, "AST.UnaryOp")) { if (!string_eq(in, field(in, object, "op"), "!")) { error(in->error, ORLIX_TCTI_FEATURE_UNSUPPORTED_GRAMMAR, in->tokens[object].start, "unsupported unary operator"); goto fail_depth; } n.kind = ORLIX_TCTI_FEATURE_NOT; if (node(in, field(in, object, "expr"), &n.left)) goto fail_depth; }
	...
	else if (string_eq(in, type, "AST.Function")) { if (string_eq(in, field(in, object, "name"), "UInt")) n.kind = ORLIX_TCTI_FEATURE_UINT; else if (string_eq(in, field(in, object, "name"), "SInt")) n.kind = ORLIX_TCTI_FEATURE_SINT; else { error(in->error, ORLIX_TCTI_FEATURE_UNSUPPORTED_GRAMMAR, in->tokens[object].start, "unsupported function"); goto fail_depth; } if (node_children(in, field(in, object, "arguments"), &n.first_child, &n.child_count) || n.child_count != 1) { error(in->error, ORLIX_TCTI_FEATURE_INVALID_SOURCE, in->tokens[object].start, "function arguments"); goto fail_depth; } }
	else if (string_eq(in, type, "Types.Field")) {
		n.kind = ORLIX_TCTI_FEATURE_FIELD;
		...
		n.field.provenance = (struct orlix_tcti_feature_provenance) {
	...
	else { error(in->error, ORLIX_TCTI_FEATURE_UNSUPPORTED_GRAMMAR, in->tokens[object].start, "unsupported AST type"); goto fail_depth; }
	in->depth--; return node_add(in, n, out);
malformed_depth:
	error(in->error, ORLIX_TCTI_FEATURE_INVALID_SOURCE, in->tokens[object].start, "malformed AST node");
fail_depth:
	free(n.text); free(n.field.state); free(n.field.register_name);
	free(n.field.selector); in->depth--; return -1;
malformed:
	error(in->error, ORLIX_TCTI_FEATURE_INVALID_SOURCE, object >= 0 && (size_t)object < in->count ? in->tokens[object].start : 0, "malformed AST node"); return -1;
}
...
int orlix_tcti_target_feature_model_import(const char *s, size_t n,
	struct orlix_tcti_feature_model *model, struct orlix_tcti_feature_error *e)
{
	struct orlix_tcti_feature_error local = { 0 };
	struct parser p = { .s = s, .n = n, .error = e ? e : &local };
	struct importer in;
	int root, parameters, constraints;
	size_t i, j;
	char digest[65];
	if (!s || !n || !model) { error(p.error, ORLIX_TCTI_FEATURE_INVALID_ARGUMENT, 0, "input and model required"); return -1; }
	if (n > MAX_INPUT) { error(p.error, ORLIX_TCTI_FEATURE_LIMIT, 0, "input limit"); return -1; }
	memset(model, 0, sizeof(*model));
	root = json_value(&p); whitespace(&p);
	if (root < 0 || p.at != n) { if (!p.error->code) error(p.error, ORLIX_TCTI_FEATURE_INVALID_JSON, p.at, "trailing JSON"); goto fail; }
	in = (struct importer){ .s = s, .tokens = p.tokens, .count = p.count, .model = model, .error = p.error };
	if (metadata(&in, root)) { error(p.error, ORLIX_TCTI_FEATURE_PIN_MISMATCH, 0, "Features.json metadata pin"); goto fail; }
	...
	for (i = 0; i < p.tokens[parameters].size; i++) {
		...
		struct orlix_tcti_feature_parameter x = { .name = string_copy(&in, field(&in, parameter, "name")), .first_constraint = (uint32_t)model->constraint_count, .provenance = { p.tokens[parameter].start, p.tokens[parameter].end - p.tokens[parameter].start } };
	...
	if (model->parameter_count != ORLIX_TCTI_FEATURE_PARAMETER_COUNT || model->constraint_count != ORLIX_TCTI_FEATURE_CONSTRAINT_COUNT) { error(p.error, ORLIX_TCTI_FEATURE_COUNT_MISMATCH, 0, "Features.json counts"); goto fail; }
	sha256_hex(s, n, digest);
	if (memcmp(digest, PIN, sizeof(digest))) { error(p.error, ORLIX_TCTI_FEATURE_PIN_MISMATCH, 0, "Features.json SHA-256 pin"); goto fail; }
	for (i = 0; i < p.count; i++) free(p.tokens[i].text);
	free(p.tokens); return 0;
bad:
	error(p.error, ORLIX_TCTI_FEATURE_INVALID_SOURCE, 0, "Features.json shape");
fail:
	for (i = 0; i < p.count; i++) free(p.tokens[i].text);
	free(p.tokens); orlix_tcti_target_feature_model_destroy(model); return -1;
}
