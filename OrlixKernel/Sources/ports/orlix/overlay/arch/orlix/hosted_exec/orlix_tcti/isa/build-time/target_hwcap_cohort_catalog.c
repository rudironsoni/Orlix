...
#define AUXV_HWCAP ORLIX_TCTI_HWCAP_CATALOG_HWCAP
...
#define ORLIX_TCTI_A64_PROFILE(major, minor, hwcap, hwcap2) \
...
#define ORLIX_TCTI_A64_SOURCE_COUNTS(...)
#define ORLIX_TCTI_A64_FEATURE(...)
#define ORLIX_TCTI_A64_COUNTS(...)
#define ORLIX_TCTI_A64_ENCODING(...)
#define ORLIX_TCTI_A64_CONDITION(...)
#include "../isa/inventory.def"
#undef ORLIX_TCTI_A64_CONDITION
#undef ORLIX_TCTI_A64_ENCODING
#undef ORLIX_TCTI_A64_COUNTS
#undef ORLIX_TCTI_A64_FEATURE
#undef ORLIX_TCTI_A64_SOURCE_COUNTS
#undef ORLIX_TCTI_A64_SOURCE
#undef ORLIX_TCTI_A64_PROFILE

#define ORLIX_TCTI_A64_SOURCE_MANIFEST_SOURCE(release, build, ref, schema, sha, count, ...) \
...
static const struct orlix_tcti_target_ledger_source_row pinned_source_rows[] = {
#include "../isa/source_manifest.def"
};
#undef ORLIX_TCTI_A64_SOURCE_MANIFEST_ROW
#undef ORLIX_TCTI_A64_SOURCE_MANIFEST_SOURCE
...
static int leaf_has_feature(const struct orlix_tcti_target_inventory *inventory,
...
static unsigned long *word(struct orlix_tcti_hwcap_cohort_catalog *catalog,
			   enum orlix_tcti_hwcap_catalog_word capability_word,
			   bool authorized)
{
	if (capability_word == ORLIX_TCTI_HWCAP_CATALOG_HWCAP)
		return authorized ? &catalog->authorized_hwcap :
				    &catalog->mapped_hwcap;
	if (capability_word == ORLIX_TCTI_HWCAP_CATALOG_HWCAP2)
		return authorized ? &catalog->authorized_hwcap2 :
				    &catalog->mapped_hwcap2;
	return NULL;
}

static enum orlix_tcti_hwcap_catalog_blocker binding_blocker(
	const struct orlix_tcti_target_ledger_classification_row *classification,
	bool proof_resolved)
{
	if (classification->classification ==
	    ORLIX_TCTI_TARGET_LEDGER_UNCLASSIFIED)
		return ORLIX_TCTI_HWCAP_CATALOG_UNCLASSIFIED;
	if (empty(classification->proof))
		return ORLIX_TCTI_HWCAP_CATALOG_MISSING_PROOF;
	if (!proof_resolved)
		return ORLIX_TCTI_HWCAP_CATALOG_UNRESOLVED_PROOF;
	return ORLIX_TCTI_HWCAP_CATALOG_PROVED;
}

static int fail(struct orlix_tcti_hwcap_cohort_catalog *catalog,
		enum orlix_tcti_hwcap_catalog_error *error,
		enum orlix_tcti_hwcap_catalog_error value)
{
	...
	enum orlix_tcti_hwcap_catalog_error *error)
{
	...
	if (error)
		*error = ORLIX_TCTI_HWCAP_CATALOG_OK;
	if (!catalog)
		return -1;
	...
	if (inventory->leaf_count != ORLIX_TCTI_A64_TARGET_LEAF_COUNT ||
	    pinned_source_declared_count != ORLIX_TCTI_A64_TARGET_LEAF_COUNT ||
	...
	if (!catalog->cohorts || !catalog->bindings)
		return fail(catalog, error, ORLIX_TCTI_HWCAP_CATALOG_NO_MEMORY);
	...
	for (mapping_index = 0; mapping_index < mapping_count;
	     mapping_index++) {
		const struct orlix_tcti_hwcap_catalog_mapping *mapping =
			&mappings[mapping_index];
		struct orlix_tcti_hwcap_catalog_cohort *cohort =
		...
		*mapped |= mapping->bit;
		...
		for (leaf_index = 0; leaf_index < inventory->leaf_count;
		     leaf_index++) {
			struct orlix_tcti_hwcap_catalog_binding *binding;
			bool has_feature;
...
int orlix_tcti_hwcap_cohort_catalog_build(
	...
	enum orlix_tcti_hwcap_catalog_error *error)
{
	return orlix_tcti_hwcap_cohort_catalog_build_with_mappings(
