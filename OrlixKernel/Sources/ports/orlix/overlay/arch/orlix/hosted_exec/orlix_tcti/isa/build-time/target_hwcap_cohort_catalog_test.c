...
struct fixture {
	...
	struct orlix_tcti_target_ledger_classification_row *classifications;
	bool *proof_resolved;
};
...
static unsigned long mapped_hwcap(
	const struct orlix_tcti_hwcap_catalog_mapping *mappings, size_t count)
{
	unsigned long value = 0;
	size_t index;

	for (index = 0; index < count; index++)
		if (mappings[index].word == ORLIX_TCTI_HWCAP_CATALOG_HWCAP)
...
static int fixture_initialize(struct fixture *fixture)
{
	...
	fixture->inventory.leaf_count = ORLIX_TCTI_A64_TARGET_LEAF_COUNT;
	...
	if (!fixture->inventory.leaves || !fixture->inventory.expressions ||
		...
		return -1;
	source = orlix_tcti_hwcap_catalog_pinned_source(&source_count);
	...
	for (index = 0; index < fixture->inventory.leaf_count; index++) {
		...
		fixture->inventory.leaves[index].condition =
			index < fixture->inventory.expression_count ?
				(uint32_t)index : ORLIX_TCTI_TARGET_EXPR_NONE;
...
static void current_catalog_maps_every_advertised_bit_to_source(void)
{
	...
	orlix_tcti_hwcap_cohort_catalog_destroy(&catalog);
...
static void blockers_are_retained_per_exact_leaf(void)
{
	...
	for (index = 0; index < catalog.binding_count; index++) {
		const struct orlix_tcti_hwcap_catalog_binding *binding =
			&catalog.bindings[index];

		if (binding->blocker == ORLIX_TCTI_HWCAP_CATALOG_PROVED)
			continue;
		blockers++;
...
static void duplicate_and_missing_mappings_fail(void)
{
	...
	struct orlix_tcti_hwcap_cohort_catalog catalog;
	enum orlix_tcti_hwcap_catalog_error error;
	const struct orlix_tcti_hwcap_catalog_mapping *current;
	struct orlix_tcti_hwcap_catalog_mapping mappings[11];
	size_t mapping_count;
	unsigned long previous_unproved_hwcap;

	EXPECT(fixture_initialize(&fixture) == 0);
	current = orlix_tcti_hwcap_catalog_current_mappings(&mapping_count);
	...
	EXPECT(orlix_tcti_hwcap_cohort_catalog_build_with_mappings(
		       &fixture.inventory, fixture.classifications,
		       fixture.inventory.leaf_count, fixture.proof_resolved,
		       mappings, mapping_count,
		       previous_unproved_hwcap,
		       orlix_tcti_hwcap_catalog_current_hwcap2(), &catalog,
		       &error) == -1);
	EXPECT(error == ORLIX_TCTI_HWCAP_CATALOG_DUPLICATE_MAPPING);
	...
	EXPECT(orlix_tcti_hwcap_cohort_catalog_build_with_mappings(
		       ...
		       orlix_tcti_hwcap_catalog_current_hwcap2(), &catalog,
		       &error) == -1);
	EXPECT(error == ORLIX_TCTI_HWCAP_CATALOG_MISSING_MAPPING);
	...
	EXPECT(orlix_tcti_hwcap_cohort_catalog_build_with_mappings(
		       ...
		       orlix_tcti_hwcap_catalog_current_hwcap2(), &catalog,
		       &error) == -1);
	EXPECT(error == ORLIX_TCTI_HWCAP_CATALOG_EMPTY_COHORT);
	fixture_destroy(&fixture);
}
...
static void source_identity_drift_fails(void)
{
	...
	EXPECT(orlix_tcti_hwcap_cohort_catalog_build(
		       ...
		       fixture.inventory.leaf_count, fixture.proof_resolved,
		       &catalog, &error) == -1);
	EXPECT(error == ORLIX_TCTI_HWCAP_CATALOG_SOURCE_MISMATCH);
...
static void normalized_feature_grammar_builds_exact_cohorts(void)
{
	...
	fixture.inventory.expressions[10].kind = ORLIX_TCTI_TARGET_EXPR_FEATURE;
	fixture.inventory.expressions[10].text = "FEAT_UNKNOWN_TO_HWCAP";
	fixture.inventory.expressions[11].kind = ORLIX_TCTI_TARGET_EXPR_OR;
	...
	fixture.inventory.expressions[12].kind = ORLIX_TCTI_TARGET_EXPR_NOT;
	fixture.inventory.expressions[12].left = 3;
	fixture.inventory.expressions[13].kind = ORLIX_TCTI_TARGET_EXPR_OR;
	...
	fixture.inventory.expressions[14].kind = ORLIX_TCTI_TARGET_EXPR_AND;
	fixture.inventory.expressions[14].left = 1;
	fixture.inventory.expressions[14].right = 2;
	fixture.inventory.expressions[15].kind = ORLIX_TCTI_TARGET_EXPR_NOT;
	fixture.inventory.expressions[15].left = 1;
	fixture.inventory.expressions[16].kind = ORLIX_TCTI_TARGET_EXPR_NOT;
	fixture.inventory.expressions[16].left = 2;
	fixture.inventory.expressions[17].kind = ORLIX_TCTI_TARGET_EXPR_AND;
	fixture.inventory.expressions[17].left = 15;
	fixture.inventory.expressions[17].right = 16;
	fixture.inventory.expressions[18].kind = ORLIX_TCTI_TARGET_EXPR_OR;
	...
	fixture.inventory.expressions[19].kind = ORLIX_TCTI_TARGET_EXPR_OR;
	...
	fixture.inventory.expressions[20].kind = ORLIX_TCTI_TARGET_EXPR_BOOL;
	fixture.inventory.expressions[20].boolean = true;
	fixture.inventory.leaves[21].condition = 20;
	fixture.inventory.expressions[21].kind = ORLIX_TCTI_TARGET_EXPR_FEATURE;
	...
	for (mapping_index = 0; mapping_index < mapping_count;
	     mapping_index++) {
		const struct orlix_tcti_hwcap_catalog_cohort *cohort =
...
static void cyclic_feature_condition_fails_closed(void)
{
	...
	fixture.inventory.expressions[10].kind = ORLIX_TCTI_TARGET_EXPR_NOT;
	...
	EXPECT(orlix_tcti_hwcap_cohort_catalog_build(
		       ...
		       fixture.inventory.leaf_count, fixture.proof_resolved,
		       &catalog, &error) == -1);
	EXPECT(error == ORLIX_TCTI_HWCAP_CATALOG_INVALID_CONDITION);
	fixture_destroy(&fixture);
}
	...
	const char *instructions_path)
{
	...
	if (!json)
		return;
	EXPECT(orlix_tcti_target_inventory_import(json, length, &inventory,
					    &import_error) == 0);
	free(json);
	...
	EXPECT(orlix_tcti_target_proof_registry_validate(
	...
	EXPECT(orlix_tcti_hwcap_catalog_current_hwcap() == 0);
	EXPECT(orlix_tcti_hwcap_catalog_current_hwcap2() == 0);
	mappings = orlix_tcti_hwcap_catalog_current_mappings(&mapping_count);
	EXPECT(orlix_tcti_hwcap_cohort_catalog_build(
	...
	for (index = 0; index < catalog.binding_count; index++) {
		...
		EXPECT(binding->classification ==
		       current_classifications[binding->leaf_index]
			       .classification);
		EXPECT(binding->blocker != ORLIX_TCTI_HWCAP_CATALOG_PROVED);
	}
	orlix_tcti_hwcap_cohort_catalog_destroy(&catalog);
	free(proof_resolved);
	orlix_tcti_target_inventory_destroy(&inventory);
