...
#define ORLIX_TCTI_TARGET_MAX_INPUT_BYTES (128U * 1024U * 1024U)
/* The pinned tree exceeds four million JSON tokens near EOF. */
#define ORLIX_TCTI_TARGET_MAX_TOKENS 5000000U
#define ORLIX_TCTI_TARGET_MAX_JSON_DEPTH 256U
#define ORLIX_TCTI_TARGET_MAX_AST_DEPTH 256U
#define ORLIX_TCTI_TARGET_MAX_TREE_DEPTH 256U
/* Tokens for the 110.1 MiB pinned tree require more than 128 MiB. */
#define ORLIX_TCTI_TARGET_MAX_ALLOCATION (256U * 1024U * 1024U)
/* A 32-bit encoding can contain no more than 32 non-overlapping fields. */
#define ORLIX_TCTI_TARGET_MAX_OPERANDS (ORLIX_TCTI_A64_TARGET_LEAF_COUNT * 32U)
#define ORLIX_TCTI_TARGET_MAX_OPERAND_NAME_BYTES (8U * 1024U * 1024U)
...
static void set_error(struct orlix_tcti_target_import_error *error,
		      enum orlix_tcti_target_import_error_code code, size_t offset,
		      const char *format, ...)
{
	va_list arguments;

	if (!error || error->code != ORLIX_TCTI_TARGET_IMPORT_OK)
		return;
	error->code = code;
	error->offset = offset;
		   ...
		   size_t element_size)
{
	...
	if (next > SIZE_MAX / element_size || next * element_size >
	    ORLIX_TCTI_TARGET_MAX_ALLOCATION)
		return -1;
			...
			size_t start)
{
	...
	if (parser->count >= ORLIX_TCTI_TARGET_MAX_TOKENS) {
		set_error(parser->error, ORLIX_TCTI_TARGET_IMPORT_INPUT_LIMIT,
	...
	if (reserve((void **)&parser->tokens, &parser->capacity,
		...
		set_error(parser->error, ORLIX_TCTI_TARGET_IMPORT_NO_MEMORY,
...
static int parse_string(struct parser *parser)
{
	...
	while (parser->position < parser->length) {
		...
		if (byte < 0x20) {
			set_error(parser->error, ORLIX_TCTI_TARGET_IMPORT_INVALID_JSON,
				  parser->position - 1,
	...
	set_error(parser->error, ORLIX_TCTI_TARGET_IMPORT_INVALID_JSON, quote,
...
static int parse_primitive(struct parser *parser)
{
	...
	if (parser->position == start) {
		set_error(parser->error, ORLIX_TCTI_TARGET_IMPORT_INVALID_JSON, start,
...
static int parse_array(struct parser *parser)
{
	...
	if (index < 0)
		return -1;
	if (++parser->depth > ORLIX_TCTI_TARGET_MAX_JSON_DEPTH) {
		set_error(parser->error, ORLIX_TCTI_TARGET_IMPORT_DEPTH_LIMIT,
...
static int add_expression(struct importer *importer,
			  struct orlix_tcti_target_expr expression, uint32_t *index)
{
			    ...
			    bool feature, uint32_t *result)
{
	...
	return add_expression(importer, (struct orlix_tcti_target_expr) {
		.kind = feature ? ORLIX_TCTI_TARGET_EXPR_FEATURE :
				 ORLIX_TCTI_TARGET_EXPR_OPERAND,
		.left = ORLIX_TCTI_TARGET_EXPR_NONE,
		.right = ORLIX_TCTI_TARGET_EXPR_NONE,
		.first_item = ORLIX_TCTI_TARGET_EXPR_NONE,
		.text = text,
	}, result);
}
		     ...
		     uint32_t *result)
{
	...
	return add_expression(importer, (struct orlix_tcti_target_expr) {
		.kind = ORLIX_TCTI_TARGET_EXPR_SET,
		.left = ORLIX_TCTI_TARGET_EXPR_NONE,
		.right = ORLIX_TCTI_TARGET_EXPR_NONE,
		.first_item = first,
				  ...
				  uint32_t *result)
{
	...
	if (token_equals(importer->json, &importer->tokens[type],
		...
		return add_expression(importer, (struct orlix_tcti_target_expr) {
			.kind = ORLIX_TCTI_TARGET_EXPR_BOOL,
			.left = ORLIX_TCTI_TARGET_EXPR_NONE,
			.right = ORLIX_TCTI_TARGET_EXPR_NONE,
			.first_item = ORLIX_TCTI_TARGET_EXPR_NONE,
			.boolean = boolean,
	...
	if (token_equals(importer->json, &importer->tokens[type],
		...
		return add_expression(importer, (struct orlix_tcti_target_expr) {
			.kind = ORLIX_TCTI_TARGET_EXPR_VALUE,
			.left = ORLIX_TCTI_TARGET_EXPR_NONE,
			.right = ORLIX_TCTI_TARGET_EXPR_NONE,
			.first_item = ORLIX_TCTI_TARGET_EXPR_NONE,
			.text = text,
	...
	if (token_equals(importer->json, &importer->tokens[type],
		...
		return add_expression(importer, (struct orlix_tcti_target_expr) {
			.kind = ORLIX_TCTI_TARGET_EXPR_NOT,
			.left = child,
			.right = ORLIX_TCTI_TARGET_EXPR_NONE,
			.first_item = ORLIX_TCTI_TARGET_EXPR_NONE,
		}, result);
	...
	if (token_equals(importer->json, &importer->tokens[type],
		...
		if (token_equals(importer->json, &importer->tokens[operation],
				 "&&"))
			kind = ORLIX_TCTI_TARGET_EXPR_AND;
		else if (token_equals(importer->json,
				      &importer->tokens[operation], "||"))
			kind = ORLIX_TCTI_TARGET_EXPR_OR;
		else if (token_equals(importer->json,
				      &importer->tokens[operation], "=="))
			kind = ORLIX_TCTI_TARGET_EXPR_EQ;
		else if (token_equals(importer->json,
				      &importer->tokens[operation], "!="))
			kind = ORLIX_TCTI_TARGET_EXPR_NE;
		else if (token_equals(importer->json,
				      &importer->tokens[operation], "IN"))
			kind = ORLIX_TCTI_TARGET_EXPR_IN;
		else
			goto unsupported_binary;
...
unsupported_binary:
	...
	set_error(importer->error, ORLIX_TCTI_TARGET_IMPORT_UNSUPPORTED_GRAMMAR,
			    ...
			    uint32_t *result)
{
	...
	if (++importer->expression_depth > ORLIX_TCTI_TARGET_MAX_AST_DEPTH) {
...
static void discard_operands_from(struct orlix_tcti_target_inventory *inventory,
				  size_t first)
{
	while (inventory->operand_count > first) {
		struct orlix_tcti_target_operand *operand =
		       ...
		       uint32_t variable_mask)
{
	...
	if (name_bytes > ORLIX_TCTI_TARGET_MAX_OPERAND_NAME_BYTES ||
	    ...
	    inventory->operand_count >= ORLIX_TCTI_TARGET_MAX_OPERANDS) {
		free(name);
	...
	for (index = 0; index < inventory->operand_count; index++) {
		const struct orlix_tcti_target_operand *prior =
			     ...
			     size_t source_offset, size_t source_length)
{
	...
	if (name_bytes > ORLIX_TCTI_TARGET_MAX_OPERAND_NAME_BYTES ||
	    ...
	    ORLIX_TCTI_TARGET_MAX_OPERAND_NAME_BYTES - name_bytes ||
	    inventory->fixed_operand_count >= ORLIX_TCTI_TARGET_MAX_OPERANDS) {
		free(name);
		set_error(importer->error, ORLIX_TCTI_TARGET_IMPORT_INPUT_LIMIT,
			  importer->tokens[name_token].start,
	...
	for (index = 0; index < inventory->fixed_operand_count; index++) {
		const struct orlix_tcti_target_fixed_operand *prior =
...
{
	struct orlix_tcti_target_inventory *inventory = importer->inventory;
	struct orlix_tcti_target_instruction_alias alias = {0};
	int name = object_find(importer, node_index, "name");
	int operation = object_find(importer, node_index, "operation_id");
	int condition_object = object_find(importer, node_index, "condition");
	int preferred = object_find(importer, node_index, "preferred");
				     ...
				     uint32_t parent_condition)
{
	...
	if (condition_object < 0 ||
		...
		return -1;
	if (parent_condition == ORLIX_TCTI_TARGET_EXPR_NONE)
		combined = local;
	else if (add_expression(importer, (struct orlix_tcti_target_expr) {
			.kind = ORLIX_TCTI_TARGET_EXPR_AND,
...
static int import_operation_semantic_member(struct importer *importer,
					    ...
					    bool semantic_body,
					    struct orlix_tcti_target_operation *operation)
{
	int key;
	int value;
	...
	if (semantic_body) {
		...
		orlix_tcti_target_inventory_sha256(importer->json + value_offset,
					      value_length,
...
static int import_operations(struct importer *importer, int root)
{
	...
	if (operations < 0 || importer->tokens[operations].kind != JSON_OBJECT) {
		set_error(importer->error, ORLIX_TCTI_TARGET_IMPORT_INVALID_SOURCE, 0,
	...
	for (index = 0; index < importer->tokens[operations].size; index++) {
		int key;
		int value;
		struct orlix_tcti_target_operation operation = { 0 };
		int type;
		...
		if (!operation.id || !operation.source_length ||
		    orlix_tcti_target_inventory_operation(importer->inventory, operation.id)) {
			free(operation.id);
			free(operation.alias_operation_id);
			set_error(importer->error, ORLIX_TCTI_TARGET_IMPORT_INVALID_SOURCE,
				  importer->tokens[key].start,
...
static struct orlix_tcti_target_operation *mutable_operation(
	struct orlix_tcti_target_inventory *inventory, const char *id)
{
	size_t index;
					 ...
					 const char *id)
{
	if (!inventory)
		return NULL;
	return mutable_operation((struct orlix_tcti_target_inventory *)inventory, id);
}
...
static int resolve_operation_alias(struct importer *importer, const char *id)
{
	struct orlix_tcti_target_inventory *inventory = importer->inventory;
	struct orlix_tcti_target_operation *operation;
	struct orlix_tcti_target_operation **path;
	size_t steps = 0;
	size_t index;
...
static int resolve_reachable_operation_aliases(struct importer *importer)
{
	...
	for (index = 0; index < inventory->leaf_count; index++) {
		struct orlix_tcti_target_operation *operation = mutable_operation(
	...
	for (index = 0; index < inventory->instruction_alias_count; index++) {
		struct orlix_tcti_target_instruction_alias *alias =
	...
	struct orlix_tcti_target_import_error *error)
{
	...
	if (length > ORLIX_TCTI_TARGET_MAX_INPUT_BYTES) {
		set_error(error, ORLIX_TCTI_TARGET_IMPORT_INPUT_LIMIT, 0,
	...
	if (root < 0 || parser.position != parser.length) {
		if (!error->code)
			set_error(error, ORLIX_TCTI_TARGET_IMPORT_INVALID_JSON,
				  parser.position,
	...
	if (a64 < 0) {
		set_error(error, ORLIX_TCTI_TARGET_IMPORT_INVALID_SOURCE, 0,
			  "Instructions.json lacks the A64 instruction set");
		goto out_inventory;
	}
	if (import_node(&importer, a64, ORLIX_TCTI_TARGET_EXPR_NONE))
		goto out_inventory;
	if (inventory->leaf_count != ORLIX_TCTI_A64_TARGET_LEAF_COUNT) {
		set_error(error, ORLIX_TCTI_TARGET_IMPORT_COUNT_MISMATCH, 0,
