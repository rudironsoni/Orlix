...
#define UNCLASSIFIED ORLIX_TCTI_TARGET_LEDGER_UNCLASSIFIED
#define REQUIRED ORLIX_TCTI_TARGET_LEDGER_REQUIRED_EL0
#define REQUIRED_EL0 ORLIX_TCTI_TARGET_LEDGER_REQUIRED_EL0
#define NON_EL0 ORLIX_TCTI_TARGET_LEDGER_NON_EL0
#define ARCHITECTURALLY_UNDEFINED ORLIX_TCTI_TARGET_LEDGER_ARCH_UNDEFINED_OR_UNALLOCATED
#define ARCH_UNDEFINED_OR_UNALLOCATED ORLIX_TCTI_TARGET_LEDGER_ARCH_UNDEFINED_OR_UNALLOCATED
#define ALIAS_OR_DUPLICATE ORLIX_TCTI_TARGET_LEDGER_ALIAS_OR_DUPLICATE
...
static const struct orlix_tcti_target_ledger_source_row source_rows[] = {
#include "../isa/source_manifest.def"
};
#undef ORLIX_TCTI_A64_SOURCE_MANIFEST_ROW
#undef ORLIX_TCTI_A64_SOURCE_MANIFEST_SOURCE

#define ORLIX_TCTI_A64_TARGET_CLASSIFICATION(name, classification, relation, canonical, evidence, proof) \
...
static void error(struct orlix_tcti_target_ledger_result *result,
		  enum orlix_tcti_target_ledger_error code, size_t index)
{
	if (result->first_error == ORLIX_TCTI_TARGET_LEDGER_OK) {
...
static size_t find_source(const struct orlix_tcti_target_ledger_source_row *source,
...
static size_t find_classification(const struct orlix_tcti_target_ledger_classification_row *classification,
				  size_t count, const char *name)
{
	size_t i;
...
static void validate_proof_reference(
	const struct orlix_tcti_target_ledger_source_row *source_row,
	const struct orlix_tcti_target_ledger_classification_row *classification_row,
	...
	struct orlix_tcti_target_ledger_result *result)
{
	...
	enum orlix_tcti_target_proof_registry_error proof_error;
	enum orlix_tcti_target_ledger_error ledger_error;

	proof_error = orlix_tcti_target_proof_registry_lookup(proof_registry,
							 proof_registry_count,
							 &reference);
	if (proof_error == ORLIX_TCTI_TARGET_PROOF_REGISTRY_OK)
		return;
	result->proof_gaps++;
	switch (proof_error) {
	case ORLIX_TCTI_TARGET_PROOF_REGISTRY_UNKNOWN_PROOF:
		ledger_error = ORLIX_TCTI_TARGET_LEDGER_UNKNOWN_PROOF;
		break;
	case ORLIX_TCTI_TARGET_PROOF_REGISTRY_CLASSIFICATION_MISMATCH:
		ledger_error = ORLIX_TCTI_TARGET_LEDGER_PROOF_CLASSIFICATION_MISMATCH;
		break;
	case ORLIX_TCTI_TARGET_PROOF_REGISTRY_FAMILY_MISMATCH:
		ledger_error = ORLIX_TCTI_TARGET_LEDGER_PROOF_FAMILY_MISMATCH;
		break;
	case ORLIX_TCTI_TARGET_PROOF_REGISTRY_INSUFFICIENT_OBLIGATIONS:
		ledger_error = ORLIX_TCTI_TARGET_LEDGER_PROOF_INSUFFICIENT_OBLIGATIONS;
		break;
	case ORLIX_TCTI_TARGET_PROOF_REGISTRY_UNKNOWN_FAMILY_REQUIREMENTS:
		ledger_error =
			ORLIX_TCTI_TARGET_LEDGER_UNKNOWN_PROOF_FAMILY_REQUIREMENTS;
		break;
	case ORLIX_TCTI_TARGET_PROOF_REGISTRY_BINDING_MISMATCH:
		ledger_error = ORLIX_TCTI_TARGET_LEDGER_PROOF_BINDING_MISMATCH;
		break;
	default:
		ledger_error = ORLIX_TCTI_TARGET_LEDGER_INVALID_PROOF_REGISTRY;
		break;
	}
...
static void validate_registry_bindings(
	...
	const struct orlix_tcti_target_proof_registry_entry *proof_registry,
	size_t proof_registry_count, struct orlix_tcti_target_ledger_result *result)
{
	size_t entry_index;
	...
	for (entry_index = 0; entry_index < proof_registry_count; entry_index++) {
		...
		for (binding_index = 0; binding_index < entry->binding_count;
			...
			if (strcmp(source[source_index].mnemonic, binding->mnemonic) ||
				...
				error(result, ORLIX_TCTI_TARGET_LEDGER_STALE_PROOF_BINDING,
				      entry_index);
				...
				struct orlix_tcti_target_ledger_result *result)
{
	...
	if (!source || source_count != required_count ||
	    source_count > ORLIX_TCTI_A64_TARGET_LEAF_COUNT) {
		error(result, ORLIX_TCTI_TARGET_LEDGER_BAD_COUNT, source_count);
		return -1;
	}
	if (!classification || classification_count != required_count ||
	    classification_count > ORLIX_TCTI_A64_TARGET_LEAF_COUNT) {
		error(result, ORLIX_TCTI_TARGET_LEDGER_BAD_COUNT,
		      classification_count);
		return -1;
	}
	if (orlix_tcti_target_proof_registry_validate(proof_registry,
						 proof_registry_count,
						 &proof_error)) {
		error(result, ORLIX_TCTI_TARGET_LEDGER_INVALID_PROOF_REGISTRY, 0);
		return -1;
	}
	...
	for (i = 0; i < source_count; i++) {
		if (source[i].ordinal != i)
			error(result, ORLIX_TCTI_TARGET_LEDGER_BAD_ORDINAL, i);
		for (j = 0; j < i; j++)
			if (!strcmp(source[i].name, source[j].name))
				error(result, ORLIX_TCTI_TARGET_LEDGER_DUPLICATE_SOURCE_NAME, i);
	}
	for (i = 0; i < classification_count; i++) {
		const struct orlix_tcti_target_ledger_classification_row *row =
			&classification[i];

		if (empty(row->name) ||
		    find_source(source, source_count, row->name) == source_count)
			error(result, ORLIX_TCTI_TARGET_LEDGER_STALE_CLASSIFICATION, i);
		for (j = 0; j < i; j++)
			if (!strcmp(row->name, classification[j].name))
				error(result, ORLIX_TCTI_TARGET_LEDGER_DUPLICATE_CLASSIFICATION_NAME, i);
	}
	for (i = 0; i < source_count; i++) {
		const struct orlix_tcti_target_ledger_classification_row *row;
		...
		if (j == classification_count) {
			error(result, ORLIX_TCTI_TARGET_LEDGER_MISSING_CLASSIFICATION, i);
			continue;
		}
		row = &classification[j];
		if (row->classification == ORLIX_TCTI_TARGET_LEDGER_UNCLASSIFIED) {
			result->unclassified_rows++;
			error(result, ORLIX_TCTI_TARGET_LEDGER_ERROR_UNCLASSIFIED, i);
			continue;
		}
		if (row->classification > ORLIX_TCTI_TARGET_LEDGER_ALIAS_OR_DUPLICATE) {
			error(result, ORLIX_TCTI_TARGET_LEDGER_INVALID_CLASSIFICATION, i);
			continue;
		}
		...
		if (row->classification == ORLIX_TCTI_TARGET_LEDGER_REQUIRED_EL0) {
		...
		if (empty(row->evidence) || empty(row->proof)) {
			result->proof_gaps++;
			error(result, empty(row->evidence) ? ORLIX_TCTI_TARGET_LEDGER_MISSING_EVIDENCE :
			      ORLIX_TCTI_TARGET_LEDGER_MISSING_PROOF, i);
		...
		if (row->classification == ORLIX_TCTI_TARGET_LEDGER_ALIAS_OR_DUPLICATE) {
			...
			relation_name = row->relation == ORLIX_TCTI_A64_TARGET_RELATION_ALIAS ?
				"alias" : "duplicate";
			...
			if (empty(row->relation_evidence) ||
				...
				error(result,
				      ORLIX_TCTI_TARGET_LEDGER_MISSING_RELATION_EVIDENCE, i);
			else if (relation_length < 0 ||
				 relation_length >= (int)sizeof(expected_relation) ||
				 strcmp(row->relation_evidence, expected_relation) ||
				 orlix_tcti_target_proof_source_evidence_validate(
					 row->relation_source,
					 row->relation_source_sha256,
					 row->relation_evidence))
				error(result,
				      ORLIX_TCTI_TARGET_LEDGER_INVALID_RELATION_EVIDENCE, i);
			if (row->relation != ORLIX_TCTI_A64_TARGET_RELATION_ALIAS &&
			    row->relation != ORLIX_TCTI_A64_TARGET_RELATION_DUPLICATE) {
				error(result, ORLIX_TCTI_TARGET_LEDGER_INVALID_RELATION, i);
				continue;
			}
			if (empty(row->canonical_name)) {
				error(result, ORLIX_TCTI_TARGET_LEDGER_MISSING_CANONICAL, i);
				continue;
			}
			...
			if (target == classification_count)
				error(result, ORLIX_TCTI_TARGET_LEDGER_UNKNOWN_CANONICAL, i);
			else if (target == j)
				error(result, ORLIX_TCTI_TARGET_LEDGER_SELF_CANONICAL, i);
			else if (classification[target].classification == ORLIX_TCTI_TARGET_LEDGER_ALIAS_OR_DUPLICATE)
				error(result, ORLIX_TCTI_TARGET_LEDGER_NONCANONICAL_TARGET, i);
			else {
				...
				if (canonical_source == source_count) {
					error(result,
					      ORLIX_TCTI_TARGET_LEDGER_UNKNOWN_CANONICAL, i);
					continue;
				}
				if (strcmp(source[i].condition_tcnd_hex,
					   source[canonical_source].condition_tcnd_hex))
					error(result,
					      ORLIX_TCTI_TARGET_LEDGER_CONDITION_MISMATCH, i);
				if (strcmp(row->proof, classification[target].proof))
					error(result,
					      ORLIX_TCTI_TARGET_LEDGER_ALIAS_PROOF_MISMATCH, i);
		...
		} else if (row->relation != ORLIX_TCTI_A64_TARGET_RELATION_NONE || !empty(row->canonical_name)) {
			error(result, ORLIX_TCTI_TARGET_LEDGER_UNEXPECTED_CANONICAL, i);
		}
	}
				     ...
				     struct orlix_tcti_target_ledger_result *result)
{
	...
	if (inventory->leaf_count != count) {
		error(result, ORLIX_TCTI_TARGET_LEDGER_BAD_COUNT, inventory->leaf_count);
		return -1;
	}
	for (i = 0; i < count; i++) {
		struct orlix_tcti_target_condition_bytes bytes = { 0 };
		const struct orlix_tcti_target_leaf *leaf = &inventory->leaves[i];
		const struct orlix_tcti_target_ledger_source_row *row = &source[i];
		...
		if (strcmp(row->name, leaf->name) || strcmp(row->mnemonic, leaf->mnemonic) ||
			...
			error(result, ORLIX_TCTI_TARGET_LEDGER_STALE_SOURCE_FACT, i);
			continue;
		}
		if (orlix_tcti_target_condition_serialize(inventory, leaf->condition, &bytes, NULL) ||
		    !hex_matches(row->condition_tcnd_hex, bytes.data, bytes.length))
			error(result, ORLIX_TCTI_TARGET_LEDGER_STALE_SOURCE_CONDITION, i);
