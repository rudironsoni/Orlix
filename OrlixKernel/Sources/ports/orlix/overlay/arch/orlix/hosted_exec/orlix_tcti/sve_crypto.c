		...
		struct orlix_tcti_sve_crypto_instruction *decoded)
{
	...
	for (i = 0; i < ARRAY_SIZE(sve_crypto_encodings); i++) {
		...
		if (encoding->op == ORLIX_TCTI_SVE_CRYPTO_XAR) {
			u8 tsize = (((instruction >> 22) & 0x3U) << 2) |
				((instruction >> 19) & 0x3U);
		...
		const struct orlix_tcti_sve_crypto_instruction *decoded)
{
	...
	if (!state || !regs || !user_simd || !decoded ||
	    ...
	    decoded->zm >= ORLIX_TCTI_SVE_ZREG_COUNT ||
	    decoded->zk >= ORLIX_TCTI_SVE_ZREG_COUNT || !decoded->nregs ||
	    decoded->zd + decoded->nregs > ORLIX_TCTI_SVE_ZREG_COUNT)
		return -EINVAL;
	if (decoded->op == ORLIX_TCTI_SVE_CRYPTO_XAR &&
	    (decoded->element_bytes != 1 && decoded->element_bytes != 2 &&
	     decoded->element_bytes != 4 && decoded->element_bytes != 8))
		return -EINVAL;
	/* Feature admission belongs to the decoded-instruction execution profile.
	 * The executor only validates its own state and the caller's availability.
	 */
	...
	if (decoded->zm != decoded->zd && decoded->zm != decoded->zn)
