...
static bool orlix_tcti_sve_valid_vl(u16 vl_bytes)
{
...
static bool orlix_tcti_sve_valid_element_bytes(u8 element_bytes)
{
...
{
	if (!orlix_tcti_sve_valid_element_bytes(element_bytes))
		return false;
...
static bool orlix_tcti_sve_predicate_lane_active(const struct orlix_tcti_sve_state *state,
...
static u64 orlix_tcti_sve_read_element(const u8 *vector, u16 offset,
				  u8 element_bytes)
{
	u64 value = 0;
	u8 byte;
...
static void orlix_tcti_sve_write_element(u8 *vector, u16 offset, u8 element_bytes,
				   u64 value)
{
	u8 byte;
...
static u64 orlix_tcti_sve_element_mask(u8 element_bytes)
{
	return element_bytes == sizeof(u64) ? U64_MAX :
		GENMASK_ULL(element_bytes * BITS_PER_BYTE - 1, 0);
}

static s64 orlix_tcti_sve_signed_element(u64 value, u8 element_bytes)
{
	return sign_extend64(value & orlix_tcti_sve_element_mask(element_bytes),
			     element_bytes * BITS_PER_BYTE - 1);
}

static int orlix_tcti_sve_binary_result(enum orlix_tcti_sve_integer_binary_op op,
				  u64 left, u64 right, u8 element_bytes,
				  u64 *result)
{
	u64 mask = orlix_tcti_sve_element_mask(element_bytes);
	s64 signed_left = orlix_tcti_sve_signed_element(left, element_bytes);
	s64 signed_right = orlix_tcti_sve_signed_element(right, element_bytes);
...
static void orlix_tcti_sve_sync_v_to_z(struct orlix_tcti_sve_state *state,
...
static void orlix_tcti_sve_sync_z_to_v(const struct orlix_tcti_sve_state *state,
				 unsigned long *user_simd, u8 reg)
{
...
{
	if (!state || !user_simd || !orlix_tcti_sve_valid_vl(vl_bytes))
		return -EINVAL;
			...
			const unsigned long *source_simd)
{
	if (!destination || !destination_simd || !source || !source_simd ||
	    !source->valid || !orlix_tcti_sve_valid_vl(source->vl_bytes))
		return -EINVAL;
				...
				u8 element_bytes)
{
	...
	if (!state || !user_simd || !state->valid ||
	    !orlix_tcti_sve_valid_vl(state->vl_bytes) ||
	    !orlix_tcti_sve_integer_binary_op_supports_element_bytes(op,
							 element_bytes) ||
	    state->vl_bytes % element_bytes || zd >= ORLIX_TCTI_SVE_ZREG_COUNT ||
	    zn >= ORLIX_TCTI_SVE_ZREG_COUNT || zm >= ORLIX_TCTI_SVE_ZREG_COUNT ||
	    pg >= ORLIX_TCTI_SVE_PREG_COUNT ||
	    (predication != ORLIX_TCTI_SVE_PREDICATE_MERGING &&
	     predication != ORLIX_TCTI_SVE_PREDICATE_ZEROING))
		return -EINVAL;

	/* Validate the operation before modifying any architected state. */
	ret = orlix_tcti_sve_binary_result(op, 0, 0, element_bytes, &ignored);
	if (ret)
		return ret;

	/* Vn aliases Zn[127:0], so synchronize every architectural operand. */
	orlix_tcti_sve_sync_v_to_z(state, user_simd, zd);
	if (zn != zd)
		orlix_tcti_sve_sync_v_to_z(state, user_simd, zn);
	if (zm != zd && zm != zn)
		orlix_tcti_sve_sync_v_to_z(state, user_simd, zm);

	for (offset = 0; offset < state->vl_bytes; offset += element_bytes) {
		u64 result;

		if (orlix_tcti_sve_predicate_lane_active(state, pg, offset)) {
			ret = orlix_tcti_sve_binary_result(op,
				orlix_tcti_sve_read_element(state->z[zn], offset,
						      element_bytes),
				orlix_tcti_sve_read_element(state->z[zm], offset,
						      element_bytes), element_bytes, &result);
			if (ret)
				return ret;
			orlix_tcti_sve_write_element(state->z[zd], offset, element_bytes,
					       result);
		} else if (predication == ORLIX_TCTI_SVE_PREDICATE_ZEROING) {
			memset(&state->z[zd][offset], 0, element_bytes);
		}
	}
	orlix_tcti_sve_sync_z_to_v(state, user_simd, zd);
					...
					u8 element_bytes)
{
	...
	if (!available)
		return -EOPNOTSUPP;
	ret = orlix_tcti_sve_predicated_integer_binary(state, user_simd, op, predication,
						 zd, pg,
						 zn, zm, element_bytes);
	if (!ret)
		regs->pc += sizeof(u32);
