				   ...
				   unsigned long *fault_address)
{
	...
	case ORLIX_TCTI_DECODE_POINTER_AUTHENTICATION:
		return orlix_tcti_execute_pointer_authentication(mm, regs, decoded,
							 fault_address);
	default:
		return -EOPNOTSUPP;
	}
...
static const struct orlix_tcti_pauth_key *
orlix_tcti_pauth_selected_key(enum orlix_tcti_pauth_key_select select)
{
	switch (select) {
	case ORLIX_TCTI_PAUTH_KEY_APIA:
		return &current->thread.user_pauth.apia;
	case ORLIX_TCTI_PAUTH_KEY_APIB:
		return &current->thread.user_pauth.apib;
	case ORLIX_TCTI_PAUTH_KEY_APDA:
		return &current->thread.user_pauth.apda;
	case ORLIX_TCTI_PAUTH_KEY_APDB:
		return &current->thread.user_pauth.apdb;
	case ORLIX_TCTI_PAUTH_KEY_APGA:
		return &current->thread.user_pauth.apga;
	default:
		return NULL;
	}
}

static u64 orlix_tcti_pauth_read_gpr_zero(const struct pt_regs *regs, u8 reg)
{
	return reg == 31 ? 0 : regs->regs[reg];
}

static void orlix_tcti_pauth_write_gpr_zero(struct pt_regs *regs, u8 reg,
					     u64 value)
{
	if (reg != 31)
		regs->regs[reg] = value;
...
static u64 orlix_tcti_pauth_modifier(const struct pt_regs *regs,
				     const struct orlix_tcti_decoded_instruction *decoded)
{
	...
	if (decoded->pauth_modifier_is_sp)
		return regs->sp;
	if (decoded->pauth_modifier_reg_is_rm)
		return orlix_tcti_pauth_read_gpr_zero(regs, decoded->rm);
	return orlix_tcti_pauth_read_gpr_zero(regs, decoded->rn);
}

static u64 orlix_tcti_pauth_modifier2(const struct pt_regs *regs,
				      const struct orlix_tcti_decoded_instruction *decoded)
{
	...
	unsigned long *fault_address)
{
	...
	if (!user_mode(regs))
		return -EOPNOTSUPP;
	key = orlix_tcti_pauth_selected_key(decoded->pauth_key);
	if (!key)
		return -EINVAL;
	...
	modifier = orlix_tcti_pauth_modifier(regs, decoded);
	if (use_modifier2)
		modifier2 = orlix_tcti_pauth_modifier2(regs, decoded);

	switch (decoded->pauth_op) {
	case ORLIX_TCTI_PAUTH_ADD:
		pointer = orlix_tcti_pauth_read_gpr_zero(regs, decoded->rd);
		result = orlix_tcti_pauth_add(pointer, modifier, modifier2,
			use_modifier2, key);
		orlix_tcti_pauth_write_gpr_zero(regs, decoded->rd, result);
		regs->pc += sizeof(u32);
		return 0;
	...
	case ORLIX_TCTI_PAUTH_STRIP:
		pointer = orlix_tcti_pauth_read_gpr_zero(regs, decoded->rd);
		orlix_tcti_pauth_write_gpr_zero(regs, decoded->rd,
			orlix_tcti_pauth_strip(pointer));
		regs->pc += sizeof(u32);
		return 0;
	case ORLIX_TCTI_PAUTH_GENERIC:
		pointer = orlix_tcti_pauth_read_gpr_zero(regs, decoded->rn);
		result = orlix_tcti_pauth_compute_qarma5(pointer, modifier, key);
		orlix_tcti_pauth_write_gpr_zero(regs, decoded->rd,
			result & GENMASK_ULL(63, 32));
		regs->pc += sizeof(u32);
		return 0;
	case ORLIX_TCTI_PAUTH_BRANCH:
		pointer = decoded->pauth_return ? regs->regs[30] :
			orlix_tcti_pauth_read_gpr_zero(regs, decoded->rn);
		result = orlix_tcti_pauth_authenticate(pointer, modifier, modifier2,
			use_modifier2, key_b, key);
		if (decoded->link)
			regs->regs[30] = regs->pc + sizeof(u32);
	...
	case ORLIX_TCTI_PAUTH_LOAD: {
		...
		address = orlix_tcti_pauth_authenticate(address, 0, 0, false,
			key_b, key);
		...
		orlix_tcti_pauth_write_gpr_zero(regs, decoded->rt, value);
