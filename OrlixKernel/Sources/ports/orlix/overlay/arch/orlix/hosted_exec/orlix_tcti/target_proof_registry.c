...
#define KNOWN_OBLIGATION_MASK \
	(ORLIX_TCTI_TARGET_PROOF_OBLIGATION_DECODE | \
	 ...
	 ORLIX_TCTI_TARGET_PROOF_OBLIGATION_SYSTEM_PT_CONTROL)
...
#define UNDEFINED_REJECTION_OBLIGATIONS \
	(ORLIX_TCTI_TARGET_PROOF_OBLIGATION_DECODE | \
	 ...
	 ORLIX_TCTI_TARGET_PROOF_OBLIGATION_MEMORY | ORLIX_TCTI_TARGET_PROOF_OBLIGATION_PC | \
	 ORLIX_TCTI_TARGET_PROOF_OBLIGATION_FAULTS)
#define BASE_EXCEPTION_STATE_OBLIGATIONS \
	(ORLIX_TCTI_TARGET_PROOF_OBLIGATION_FLAGS | \
	 ORLIX_TCTI_TARGET_PROOF_OBLIGATION_FP_SIMD | \
	 ORLIX_TCTI_TARGET_PROOF_OBLIGATION_SVE | \
	 ORLIX_TCTI_TARGET_PROOF_OBLIGATION_SME_TASK_STATE)
#define BASE_EXCEPTION_OBLIGATIONS \
	(UNDEFINED_REJECTION_OBLIGATIONS | BASE_EXCEPTION_STATE_OBLIGATIONS)
...
#define PRODUCTION_CAPTURE_FAMILY_OBLIGATIONS \
	(BASELINE_OBLIGATIONS | ORLIX_TCTI_TARGET_PROOF_OBLIGATION_REGISTERS | \
	 ORLIX_TCTI_TARGET_PROOF_OBLIGATION_PC)
#define BRANCH_CONTROL_STATE_OBLIGATIONS \
	(BRANCH_CONTROL_OBLIGATIONS | \
	 ORLIX_TCTI_TARGET_PROOF_OBLIGATION_MEMORY | \
	 BASE_EXCEPTION_STATE_OBLIGATIONS)
#define BRANCH_CONTROL_EXCEPTION_OBLIGATIONS \
...
#define KUNIT_BUILD_SOURCE_SHA256 \
	"4f47d729d0b1af52a33fe0b26c01b550d25b0d2b64161f1dc0e2b4a726c44c59"
...
#define KSELFTEST_SIGNAL_SOURCE_SHA256 \
	"27bb8376c93bc5bbe6aa62b74bad6db78bea712b277d14fac5a8b52fddd9500e"
...
static const struct kunit_source_provenance kunit_sources[] = {
	...
	{ SOURCE_LEAF_CLASSIFICATION_SOURCE,
	  "8e9ada735a003f648b9c9b217b1f7874ec4f8d81ecbbc7d2f4aa288b5a1d0bd0",
...
#include "target_production_capture_family.def"
#undef ORLIX_TCTI_PROOF_FAMILY_METADATA
#define ORLIX_TCTI_PROOF_FAMILY_EXCEPTION_CASE(source, suite, symbol, array, case) \
	{ source, suite, symbol, array, case, BASE_EXCEPTION_STATE_OBLIGATIONS | \
	  ORLIX_TCTI_TARGET_PROOF_OBLIGATION_MEMORY | \
	  ORLIX_TCTI_TARGET_PROOF_OBLIGATION_FAULTS },
#include "target_production_capture_family.def"
#undef ORLIX_TCTI_PROOF_FAMILY_EXCEPTION_CASE
	{ DECODE_SOURCE, DECODE_SUITE, DECODE_SUITE_SYMBOL, DECODE_CASE_ARRAY,
...
source_leaf_rejection_registry_operations[] = {
	...
	{ .proof_id = "kunit:source-leaf-tenter-feat-tev-absent",
	  .classification = 3, .base_exception = true },
...
static bool build_source_leaf_rejection_registry(void)
{
	...
	for (index = 0;
		...
		if (operation->base_exception)
			obligations = BASE_EXCEPTION_OBLIGATIONS;
...
static const struct orlix_tcti_target_proof_case
branch_control_exception_cases[] = {
	{ production_capture_family_cases[0].name, ORLIX_TCTI_TARGET_PROOF_OBLIGATION_DECODE |
		ORLIX_TCTI_TARGET_PROOF_OBLIGATION_LEGAL_ENCODINGS },
	{ production_capture_family_cases[1].name, ORLIX_TCTI_TARGET_PROOF_OBLIGATION_REGISTERS |
		ORLIX_TCTI_TARGET_PROOF_OBLIGATION_PC },
#define ORLIX_TCTI_PROOF_FAMILY_EXCEPTION_CASE(source, suite, symbol, array, case) \
	{ case, BASE_EXCEPTION_STATE_OBLIGATIONS | \
	  ORLIX_TCTI_TARGET_PROOF_OBLIGATION_MEMORY },
#include "target_production_capture_family.def"
#undef ORLIX_TCTI_PROOF_FAMILY_EXCEPTION_CASE
};
...
static unsigned int production_capture_family_exception_kind(const char *proof_id)
{
#define ORLIX_TCTI_PROOF_FAMILY_EXCEPTION_OPERATION(proof, operation, fault) \
	if (!strcmp(proof_id, proof)) return (fault) + 1U;
#include "target_production_capture_family.def"
#undef ORLIX_TCTI_PROOF_FAMILY_EXCEPTION_OPERATION
	return 0U;
}

static unsigned int production_capture_family_exception_operation_kind(
	const char *operation_id)
{
#define ORLIX_TCTI_PROOF_FAMILY_EXCEPTION_OPERATION(proof, operation, fault) \
	if (!strcmp(operation_id, operation)) return (fault) + 1U;
#include "target_production_capture_family.def"
#undef ORLIX_TCTI_PROOF_FAMILY_EXCEPTION_OPERATION
	return 0U;
}
...
static bool build_production_capture_family_registry(void)
{
	...
	for (index = 0; index < ARRAY_COUNT(source_bound_proofs); index++) {
		...
		*binding = (struct orlix_tcti_target_proof_binding) {
			...
			.encoding_pattern = source->encoding_pattern,
			.condition_tcnd_hex = source->condition_tcnd_hex,
			.kunit_case_mask = operation->exception_kind ?
				ORLIX_TCTI_PROOF_U64_C(0x7) :
				ORLIX_TCTI_PROOF_U64_C(0x3),
			.source_ordinal = source->ordinal,
		};
	...
	for (index = 0;
		...
		const struct production_capture_family_registry_operation *operation =
			&production_capture_family_registry_operations[index];
		unsigned int exception_kind =
			production_capture_family_exception_kind(operation->proof_id);
		bool exception = exception_kind != 0U;
		orlix_tcti_proof_u32 obligations = exception_kind == 1U ?
			(UNDEFINED_REJECTION_OBLIGATIONS &
			 ~ORLIX_TCTI_TARGET_PROOF_OBLIGATION_FAULTS) |
			BASE_EXCEPTION_STATE_OBLIGATIONS : exception ?
			BASE_EXCEPTION_OBLIGATIONS :
		...
		proof_registry_entries[entry_base + index] =
			(struct orlix_tcti_target_proof_registry_entry) {
				...
				.classification_mask =
					...
					.obligations = obligations,
				.linux_interface =
					exception ? ORLIX_TCTI_TARGET_PROOF_LINUX_INTERFACE_REQUIRED :
					ORLIX_TCTI_TARGET_PROOF_LINUX_INTERFACE_NOT_APPLICABLE,
					.kunit_source = production_capture_family.source,
					.kunit_suite = production_capture_family.suite,
					.kunit_cases = exception ? branch_control_exception_cases :
					production_capture_family_cases,
					.kunit_case_count =
						exception ? ARRAY_COUNT(branch_control_exception_cases) :
						ARRAY_COUNT(production_capture_family_cases),
					.bindings = &production_capture_family_registry_bindings[
					operation->binding_offset],
				.binding_count = operation->binding_count,
				.kselftest = exception ? &branch_control_exception_interface_kselftest : NULL,
					.unproved_obligations = obligations,
			};
	}
	...
	orlix_tcti_proof_u32 *requirements)
{
	...
	if ((classification == 2 || classification == 3) &&
	    (!strcmp(operation_id, "UDF_perm_undef") ||
	     !strcmp(operation_id, "TENTER") ||
	     !strcmp(operation_id, "HVC") || !strcmp(operation_id, "SMC") ||
	     !strcmp(operation_id, "DCPS1") || !strcmp(operation_id, "DCPS2") ||
	     !strcmp(operation_id, "DCPS3"))) {
		*requirements = BASE_EXCEPTION_OBLIGATIONS;
		return 0;
	}
	if (classification == 2) {
	...
	if (classification != 1)
		return -1;
	if (production_capture_family_exception_operation_kind(operation_id)) {
		if (production_capture_family_exception_operation_kind(operation_id) == 1U)
			*requirements = (UNDEFINED_REJECTION_OBLIGATIONS &
				~ORLIX_TCTI_TARGET_PROOF_OBLIGATION_FAULTS) |
				BASE_EXCEPTION_STATE_OBLIGATIONS;
		else
