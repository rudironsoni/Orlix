...
#define ORLIX_TCTI_A64_SOURCE(release, build, ref, schema, sha256, url) \
...
#define ORLIX_TCTI_A64_RUNTIME_ENCODING(name, mnemonic, operation_id, mask, \
			  pattern, witness)
#define ORLIX_TCTI_A64_CONDITION(name, kind)
...
#define ORLIX_TCTI_A64_RUNTIME_ENCODING(name, mnemonic, operation_id, mask, \
			  pattern, witness)
#define ORLIX_TCTI_A64_CONDITION(name, kind) { stringify(name), (kind) },
...
#define ORLIX_TCTI_A64_TARGET_RELATION_NONE
#define ORLIX_TCTI_A64_TARGET_RELATION_ALIAS
...
static int emit_header(FILE *output)
{
	...
	fprintf(output,
		...
		"struct orlix_tcti_a64_encoding_inventory_entry {\n"
		"\tconst char *name;\n"
		"\tconst char *mnemonic;\n"
		"\tconst char *operation_id;\n"
		"\tu32 mask;\n"
		...
		"\tu32 witness;\n"
		"};\n\n"
		"enum orlix_tcti_a64_condition_kind {\n"
		"\tORLIX_TCTI_A64_CONDITION_RM_NOT_31,\n"
		"\tORLIX_TCTI_A64_CONDITION_IMMH_NOT_ZERO,\n"
		"\tORLIX_TCTI_A64_CONDITION_OPTION_NOT_3,\n"
		"\tORLIX_TCTI_A64_CONDITION_RT_BELOW_24,\n"
		"\tORLIX_TCTI_A64_CONDITION_SYSTEM_ENCODING,\n"
		"};\n\n"
		"struct orlix_tcti_a64_condition_inventory_entry {\n"
		"\tconst char *name;\n"
		"\tenum orlix_tcti_a64_condition_kind kind;\n"
		"};\n\n",
	...
	fprintf(output, "static const struct orlix_tcti_a64_encoding_inventory_entry\n"
		"orlix_tcti_a64_encoding_inventory[] = {\n");
	for (i = 0; i < sizeof(inventory) / sizeof(inventory[0]); i++)
	...
	fprintf(output, "static const struct orlix_tcti_a64_encoding_inventory_entry\n"
		"orlix_tcti_a64_unsupported_encoding_inventory[] = {\n");
	for (i = 0; i < sizeof(inventory) / sizeof(inventory[0]); i++)
		if (!inventory_entry_is_required(&inventory[i]))
			emit_entry(output, &inventory[i]);
	fprintf(output, "};\n\n");

	fprintf(output, "static const struct orlix_tcti_a64_condition_inventory_entry\n"
		"orlix_tcti_a64_condition_inventory[] = {\n");
	for (i = 0;
	     i < sizeof(inventory_conditions) / sizeof(inventory_conditions[0]);
	     i++)
		fprintf(output, "\t{ \"%s\", %u },\n",
