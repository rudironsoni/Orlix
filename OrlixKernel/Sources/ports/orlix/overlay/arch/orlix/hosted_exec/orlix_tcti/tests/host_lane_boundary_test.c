 ...
 * host tests, KUnit, and the kernel OrlixTCTI objects consume generated C data and
...
static bool raw_arm_source_content(const char *text)
{
	static const char *const signatures[] = {
		"orlix_tcti_target_inventory_import(",
		"orlix_tcti_target_feature_model_import(",
		"orlix_tcti_register_model_import(",
 ...
 * arch/orlix.  It may live in this one directory, while ordinary OrlixTCTI source
		...
		if (lstat(path, &status) != 0) {
			...
			if (text) {
				...
				if (raw_source && !build_time_inventory_source(path))
					fail(path, "raw Arm source machinery must live in orlix_tcti/isa/build-time");
				free(text);
			}
						  ...
						  size_t target_count, const char *path)
{
	...
	if (!target || target->visited)
		return;
	target->visited = true;
	if (!strcmp(target->name, "orlix-tcti-isa-maintainer-source-check") ||
	    !strcmp(target->name, "orlix-tcti-isa-refresh")) {
...
static void check_makefile_lanes(const char *path)
{
	...
	check_host_dependency_closure(find_target(targets, target_count,
						  "orlix-tcti-isa-host-tests"), targets,
				      target_count, path);
	for (target_index = 0; target_index < target_count; target_index++)
		targets[target_index].visited = false;
	check_host_dependency_closure(find_target(targets, target_count,
						  "orlix-tcti-isa-audit"), targets,
				      target_count, path);
	free(text);
}

static void check_generated_artifacts(const char *orlix_tcti_root)
{
	static const char *const artifacts[] = {
		"isa/inventory.def", "isa/source_manifest.def",
		"isa/proof_registry_projection.def",
		"isa/target_classification_input.def",
		"isa/generations/current/target_classification.def",
	...
	for (index = 0; index < ARRAY_SIZE(artifacts); index++) {
		char *path = join_path(orlix_tcti_root, artifacts[index]);
		if (!path) {
			fail(orlix_tcti_root, "out of memory");
			return;
		}
...
static void check_kunit_makefiles(const char *orlix_tcti_root)
{
	...
	for (index = 0; index < ARRAY_SIZE(makefiles); index++) {
		char *path = join_path(orlix_tcti_root, makefiles[index]);
...
int main(int argc, char **argv)
{
	...
	orlix_tcti_root = join_path(argv[1],
		"OrlixKernel/Sources/ports/orlix/overlay/arch/orlix/hosted_exec/orlix_tcti");
	overlay_root = join_path(argv[1],
		"OrlixKernel/Sources/ports/orlix/overlay");
	makefile = join_path(argv[1], "Makefile");
	if (!overlay_root || !orlix_tcti_root || !makefile) {
		fail(argv[1], "out of memory");
		free(makefile);
		free(orlix_tcti_root);
		free(overlay_root);
		return 1;
	}
	check_makefile_lanes(makefile);
	check_kunit_makefiles(orlix_tcti_root);
	check_generated_artifacts(orlix_tcti_root);
	...
	puts("OrlixTCTI host-lane boundary test: PASS");
	return 0;
}
