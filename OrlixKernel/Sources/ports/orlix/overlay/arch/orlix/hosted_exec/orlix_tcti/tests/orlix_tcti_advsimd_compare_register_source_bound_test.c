...
#define ORLIX_TCTI_ADVSIMD_COMPARE_SVC	0xd4000001U
#define ORLIX_TCTI_ADVSIMD_COMPARE_VARIABLE_MASK	0x40df03ffU

struct orlix_tcti_advsimd_compare_leaf {
	u16 source_ordinal;
...
static const struct orlix_tcti_advsimd_compare_leaf orlix_tcti_advsimd_compare_leaves[] = {
	{ 3900U, "CMGT_asimdsame_only", "CMGT", "CMGT_advsimd_reg",
	  0xbf20fc00U, 0x0e203400U, ORLIX_TCTI_SIMD_COMPARE_CMGT },
	{ 3901U, "CMGE_asimdsame_only", "CMGE", "CMGE_advsimd_reg",
	  0xbf20fc00U, 0x0e203c00U, ORLIX_TCTI_SIMD_COMPARE_CMGE },
	{ 3911U, "CMTST_asimdsame_only", "CMTST", "CMTST_advsimd",
	  0xbf20fc00U, 0x0e208c00U, ORLIX_TCTI_SIMD_COMPARE_CMTST },
	{ 3942U, "CMHI_asimdsame_only", "CMHI", "CMHI_advsimd",
	  0xbf20fc00U, 0x2e203400U, ORLIX_TCTI_SIMD_COMPARE_CMHI },
	{ 3943U, "CMHS_asimdsame_only", "CMHS", "CMHS_advsimd",
	  0xbf20fc00U, 0x2e203c00U, ORLIX_TCTI_SIMD_COMPARE_CMHS },
	{ 3953U, "CMEQ_asimdsame_only", "CMEQ", "CMEQ_advsimd_reg",
	  0xbf20fc00U, 0x2e208c00U, ORLIX_TCTI_SIMD_COMPARE_CMEQ },
};

struct orlix_tcti_advsimd_compare_context {
	struct mm_struct *mm;
...
struct orlix_tcti_advsimd_compare_registers {
	u8 rd;
	u8 rn;
	u8 rm;
};

struct orlix_tcti_advsimd_compare_operands {
	u64 left[2];
	u64 right[2];
};

static const struct orlix_tcti_advsimd_compare_registers
orlix_tcti_advsimd_compare_overlap_cases[] = {
	{ 3U, 4U, 5U },
	{ 6U, 6U, 7U },
	{ 8U, 9U, 8U },
	{ 10U, 11U, 11U },
...
static const struct orlix_tcti_advsimd_compare_operands
orlix_tcti_advsimd_compare_operand_cases[] = {
	{
...
static const char *orlix_tcti_advsimd_compare_artifact_string(
...
static int orlix_tcti_advsimd_compare_test_init(struct kunit *test)
{
	struct orlix_tcti_advsimd_compare_context *context;
...
static void orlix_tcti_advsimd_compare_test_exit(struct kunit *test)
{
	struct orlix_tcti_advsimd_compare_context *context = test->priv;
...
static u32 orlix_tcti_advsimd_compare_instruction(
	const struct orlix_tcti_advsimd_compare_leaf *leaf, u8 q, u8 size, u8 rd,
	u8 rn, u8 rm)
{
...
static int orlix_tcti_advsimd_compare_load_instruction(struct kunit *test,
						 u32 instruction)
{
	struct orlix_tcti_advsimd_compare_context *context = test->priv;
	const u32 program[] = { instruction, ORLIX_TCTI_ADVSIMD_COMPARE_SVC };
	int ret;
...
static void orlix_tcti_advsimd_compare_seed_regs(struct pt_regs *regs,
...
static u64 orlix_tcti_advsimd_compare_lane_mask(u8 lane_bytes)
{
	return lane_bytes == sizeof(u64) ? U64_MAX :
		GENMASK_ULL(lane_bytes * 8 - 1, 0);
}

static bool orlix_tcti_advsimd_compare_lane_matches(
	enum orlix_tcti_simd_vector_compare_op operation, u64 left, u64 right,
	u8 lane_bits)
{
	switch (operation) {
...
static void orlix_tcti_advsimd_compare_expected(
	u64 result[2], enum orlix_tcti_simd_vector_compare_op operation,
	const u64 left[2], const u64 right[2], u8 lane_bytes, u8 result_bytes)
{
	u64 mask = orlix_tcti_advsimd_compare_lane_mask(lane_bytes);
	...
	for (lane = 0; lane < lane_count; lane++) {
		...
		if (orlix_tcti_advsimd_compare_lane_matches(operation, left_lane,
						       right_lane,
						       lane_bytes * 8))
...
static void orlix_tcti_advsimd_compare_source_bindings(struct kunit *test)
{
	...
	KUNIT_ASSERT_EQ(test, 6U, ARRAY_SIZE(orlix_tcti_advsimd_compare_leaves));
	for (index = 0; index < ARRAY_SIZE(orlix_tcti_advsimd_compare_leaves);
	     index++) {
		const struct orlix_tcti_advsimd_compare_leaf *leaf =
			&orlix_tcti_advsimd_compare_leaves[index];
		const struct orlix_tcti_target_instruction_artifact_leaf *source =
			&artifact->leaves[leaf->source_ordinal];
		const char *name = orlix_tcti_advsimd_compare_artifact_string(
			artifact, source->name_offset);
		const char *mnemonic = orlix_tcti_advsimd_compare_artifact_string(
			artifact, source->mnemonic_offset);
		const char *operation = orlix_tcti_advsimd_compare_artifact_string(
		...
		KUNIT_EXPECT_EQ(test, ORLIX_TCTI_ADVSIMD_COMPARE_VARIABLE_MASK,
				~source->encoding_mask);
	}
}

static void orlix_tcti_advsimd_compare_operand_corpus_distinguishes_predicates(
	struct kunit *test)
{
	size_t leaf_index;
	u8 q;
	...
	for (leaf_index = 0;
	     leaf_index < ARRAY_SIZE(orlix_tcti_advsimd_compare_leaves);
	     leaf_index++) {
		const struct orlix_tcti_advsimd_compare_leaf *leaf =
			&orlix_tcti_advsimd_compare_leaves[leaf_index];
		...
		for (q = 0; q < 2; q++) {
			for (size = 0; size < 4; size++) {
				...
				for (operand_index = 0;
					...
					orlix_tcti_advsimd_compare_operand_cases);
				     operand_index++) {
					const struct orlix_tcti_advsimd_compare_operands *operands =
						&orlix_tcti_advsimd_compare_operand_cases[
							operand_index];
					u8 lane_bytes = BIT(size);
					u8 result_bytes = q ? 2 * sizeof(u64) :
							      sizeof(u64);
					u64 mask = orlix_tcti_advsimd_compare_lane_mask(
						lane_bytes);
					u8 lane;
					...
					for (lane = 0; lane < result_bytes / lane_bytes;
						...
						u64 right =
							...
							mask;
						bool matches =
							orlix_tcti_advsimd_compare_lane_matches(
...
static void orlix_tcti_advsimd_compare_legal_fields_decode(struct kunit *test)
{
	size_t leaf_index;
	u8 q;
	u8 size;
				...
				for (reg = 0; reg < 32; reg++) {
					struct orlix_tcti_decoded_instruction decoded =
						orlix_tcti_decode_aarch64(
							orlix_tcti_advsimd_compare_instruction(
								leaf, q, size, reg,
								(reg + 1U) & 31U,
								(reg + 2U) & 31U));
...
static void orlix_tcti_advsimd_compare_reserved_1d_rejected(struct kunit *test)
{
	struct orlix_tcti_advsimd_compare_context *context = test->priv;
	size_t leaf_index;
	...
	for (leaf_index = 0;
		...
		const struct orlix_tcti_advsimd_compare_leaf *leaf =
			&orlix_tcti_advsimd_compare_leaves[leaf_index];
		u32 instruction = orlix_tcti_advsimd_compare_instruction(
			leaf, 0, 3, 3, 4, 5);
		const u32 expected_program[] = {
			instruction, ORLIX_TCTI_ADVSIMD_COMPARE_SVC,
		};
		...
		ret = orlix_tcti_advsimd_compare_load_instruction(test, instruction);
		...
		orlix_tcti_advsimd_compare_seed_regs(&regs, context->instructions,
						instruction);
		expected_regs = regs;
...
static void orlix_tcti_advsimd_compare_source_leaves_resume(struct kunit *test)
{
	struct orlix_tcti_advsimd_compare_context *context = test->priv;
	size_t leaf_index;
	u8 q;
			...
			for (size = 0; size < 4; size++) {
				...
				for (case_index = 0;
					...
					size_t operand_index =
						...
						ARRAY_SIZE(
							orlix_tcti_advsimd_compare_operand_cases);
					const struct orlix_tcti_advsimd_compare_registers
						*registers =
							&orlix_tcti_advsimd_compare_overlap_cases
								[overlap_index];
					const struct orlix_tcti_advsimd_compare_operands
						*operands =
							&orlix_tcti_advsimd_compare_operand_cases
								[operand_index];
					u32 instruction =
					...
					const u32 expected_program[] = {
						instruction,
						ORLIX_TCTI_ADVSIMD_COMPARE_SVC,
					};
					...
					ret = orlix_tcti_advsimd_compare_load_instruction(
						test, instruction);
					KUNIT_ASSERT_EQ(test, 0, ret);
					...
					memcpy(expected_simd,
					       current->thread.user_simd,
					       sizeof(expected_simd));
					orlix_tcti_advsimd_compare_expected(
						expected_result,
						leaf->operation,
					...
					current->thread.user_fpsr = BIT(27) |
								    BIT(4);
					orlix_tcti_advsimd_compare_seed_regs(
					...
					KUNIT_EXPECT_EQ(test,
							...
							result.pc);
					KUNIT_EXPECT_EQ(
						test, ORLIX_TCTI_ADVSIMD_COMPARE_SVC,
						result.instruction);
					KUNIT_EXPECT_MEMEQ(
						test, expected_simd,
						current->thread.user_simd,
...
static struct kunit_case orlix_tcti_advsimd_compare_source_bound_cases[] = {
	KUNIT_CASE(orlix_tcti_advsimd_compare_source_bindings),
	KUNIT_CASE(orlix_tcti_advsimd_compare_operand_corpus_distinguishes_predicates),
	KUNIT_CASE(orlix_tcti_advsimd_compare_legal_fields_decode),
	KUNIT_CASE(orlix_tcti_advsimd_compare_reserved_1d_rejected),
	KUNIT_CASE(orlix_tcti_advsimd_compare_source_leaves_resume),
	{}
};

static struct kunit_suite orlix_tcti_advsimd_compare_source_bound_suite = {
	.name = "orlix-tcti-advsimd-compare-register-source-bound",
	.init = orlix_tcti_advsimd_compare_test_init,
	.exit = orlix_tcti_advsimd_compare_test_exit,
	.test_cases = orlix_tcti_advsimd_compare_source_bound_cases,
};

kunit_test_suite(orlix_tcti_advsimd_compare_source_bound_suite);
