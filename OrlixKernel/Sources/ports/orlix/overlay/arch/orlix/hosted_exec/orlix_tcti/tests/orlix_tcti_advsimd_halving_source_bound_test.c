 ...
 * underflow for UHSUB.
 */
#define ORLIX_TCTI_ADVSIMD_HALVING_SVC 0xd4000001U
#define ORLIX_TCTI_ADVSIMD_HALVING_RD 4U
#define ORLIX_TCTI_ADVSIMD_HALVING_RN 5U
#define ORLIX_TCTI_ADVSIMD_HALVING_RM 6U

struct orlix_tcti_advsimd_halving_leaf {
  u16 source_ordinal;
...
static const struct orlix_tcti_advsimd_halving_leaf orlix_tcti_advsimd_halving_leaves[] = {
...
static const struct orlix_tcti_advsimd_source_manifest_leaf *
orlix_tcti_advsimd_halving_source_leaf(u32 ordinal) {
  size_t index;
...
static u32
orlix_tcti_advsimd_halving_instruction(const struct orlix_tcti_advsimd_halving_leaf *leaf,
                                 u8 q, u8 size, u8 rd, u8 rn, u8 rm) {
  return leaf->source_pattern | ((u32)q << 30) | ((u32)size << 22) |
         ((u32)rm << 16) | ((u32)rn << 5) | rd;
}

static unsigned long orlix_tcti_advsimd_halving_map_instruction(struct kunit *test,
                                                          u32 instruction) {
  const u32 program[] = {instruction, ORLIX_TCTI_ADVSIMD_HALVING_SVC};
...
static s64 orlix_tcti_advsimd_floor_div2(s64 value) {
  return value >= 0 ? value / 2 : -(((-value) + 1) / 2);
}

static u64 orlix_tcti_advsimd_halving_expected(
	enum orlix_tcti_simd_vector_arithmetic_op operation,
                                         u64 left, u64 right, u8 bits) {
  u64 mask = GENMASK_ULL(bits - 1, 0);

  switch (operation) {
  case ORLIX_TCTI_SIMD_ARITH_SHADD:
    return orlix_tcti_advsimd_floor_div2(sign_extend64(left, bits - 1) +
                                   sign_extend64(right, bits - 1)) &
           mask;
  case ORLIX_TCTI_SIMD_ARITH_SRHADD:
    return orlix_tcti_advsimd_floor_div2(sign_extend64(left, bits - 1) +
                                   sign_extend64(right, bits - 1) + 1) &
           mask;
  case ORLIX_TCTI_SIMD_ARITH_SHSUB:
    return orlix_tcti_advsimd_floor_div2(sign_extend64(left, bits - 1) -
                                   sign_extend64(right, bits - 1)) &
           mask;
  case ORLIX_TCTI_SIMD_ARITH_UHADD:
    return (left + right) / 2;
  case ORLIX_TCTI_SIMD_ARITH_URHADD:
    return (left + right + 1) / 2;
  case ORLIX_TCTI_SIMD_ARITH_UHSUB:
    return orlix_tcti_advsimd_floor_div2((s64)left - (s64)right) & mask;
  default:
    return 0;
  }
}

static u64 orlix_tcti_advsimd_halving_lane_input(u8 bits, u8 lane, bool right) {
...
static void orlix_tcti_advsimd_halving_fill_register(u64 register_words[2], u8 bytes,
  ...
  for (lane = 0; lane < lane_count; lane++) {
    ...
    register_words[word] |=
        orlix_tcti_advsimd_halving_lane_input(bytes * 8, lane, right) << shift;
  }
}

static void orlix_tcti_advsimd_halving_expected_register(
	u64 expected_words[2], enum orlix_tcti_simd_vector_arithmetic_op operation,
  ...
  for (lane = 0; lane < lane_count; lane++) {
    ...
    expected_words[word] |=
        orlix_tcti_advsimd_halving_expected(operation, lhs, rhs, bytes * 8) << shift;
  }
}

static void orlix_tcti_advsimd_halving_source_leaves_decode(struct kunit *test) {
  size_t index;
  u8 size;
  u8 q;

  KUNIT_ASSERT_EQ(test, 6U, ARRAY_SIZE(orlix_tcti_advsimd_halving_leaves));
  for (index = 0; index < ARRAY_SIZE(orlix_tcti_advsimd_halving_leaves); index++) {
    const struct orlix_tcti_advsimd_halving_leaf *leaf =
        &orlix_tcti_advsimd_halving_leaves[index];
    const struct orlix_tcti_advsimd_source_manifest_leaf *source =
        orlix_tcti_advsimd_halving_source_leaf(leaf->source_ordinal);
    ...
    for (q = 0; q < 2; q++) {
      for (size = 0; size < 3; size++) {
        struct orlix_tcti_decoded_instruction decoded =
            orlix_tcti_decode_aarch64(orlix_tcti_advsimd_halving_instruction(
                leaf, q, size, ORLIX_TCTI_ADVSIMD_HALVING_RD, ORLIX_TCTI_ADVSIMD_HALVING_RN,
                ORLIX_TCTI_ADVSIMD_HALVING_RM));
...
static void
orlix_tcti_advsimd_halving_reserved_64bit_lanes_reject(struct kunit *test) {
  size_t index;
  u8 q;
  ...
  for (index = 0; index < ARRAY_SIZE(orlix_tcti_advsimd_halving_leaves); index++) {
    ...
    for (q = 0; q < 2; q++) {
      struct orlix_tcti_decoded_instruction decoded =
          orlix_tcti_decode_aarch64(orlix_tcti_advsimd_halving_instruction(
              leaf, q, 3, ORLIX_TCTI_ADVSIMD_HALVING_RD, ORLIX_TCTI_ADVSIMD_HALVING_RN,
              ORLIX_TCTI_ADVSIMD_HALVING_RM));

      KUNIT_EXPECT_EQ_MSG(test, ORLIX_TCTI_DECODE_UNSUPPORTED, decoded.decode_class,
                          "%s q=%u", leaf->source_name, q);
    }
  }
}

static void orlix_tcti_advsimd_halving_source_leaves_execute(struct kunit *test) {
  size_t index;
  u8 size;
  u8 q;
  ...
  for (index = 0; index < ARRAY_SIZE(orlix_tcti_advsimd_halving_leaves); index++) {
    ...
    for (q = 0; q < 2; q++) {
      for (size = 0; size < 3; size++) {
        u32 instruction = orlix_tcti_advsimd_halving_instruction(
            leaf, q, size, ORLIX_TCTI_ADVSIMD_HALVING_RD, ORLIX_TCTI_ADVSIMD_HALVING_RN,
            ORLIX_TCTI_ADVSIMD_HALVING_RM);
        unsigned long mapped =
            orlix_tcti_advsimd_halving_map_instruction(test, instruction);
        struct pt_regs regs = {};
        ...
        orlix_tcti_advsimd_halving_fill_register(left, BIT(size), false,
                                           result_bytes);
        orlix_tcti_advsimd_halving_fill_register(right, BIT(size), true,
                                           result_bytes);
        current->thread.user_simd[ORLIX_TCTI_ADVSIMD_HALVING_RN * 2] = left[0];
        current->thread.user_simd[ORLIX_TCTI_ADVSIMD_HALVING_RN * 2 + 1] = left[1];
        current->thread.user_simd[ORLIX_TCTI_ADVSIMD_HALVING_RM * 2] = right[0];
        current->thread.user_simd[ORLIX_TCTI_ADVSIMD_HALVING_RM * 2 + 1] = right[1];
        memcpy(expected_simd, current->thread.user_simd, sizeof(expected_simd));
        orlix_tcti_advsimd_halving_expected_register(
            &expected_simd[ORLIX_TCTI_ADVSIMD_HALVING_RD * 2], leaf->operation, left,
            right, BIT(size), result_bytes);
        if (leaf->operation == ORLIX_TCTI_SIMD_ARITH_UHSUB) {
          u64 lane_mask = GENMASK_ULL(BIT(size) * 8 - 1, 0);

          KUNIT_EXPECT_EQ(test, lane_mask,
                          expected_simd[ORLIX_TCTI_ADVSIMD_HALVING_RD * 2] &
                              lane_mask);
        }
        ...
        KUNIT_EXPECT_EQ(test, ORLIX_TCTI_ADVSIMD_HALVING_SVC, result.instruction);
...
static struct kunit_case orlix_tcti_advsimd_halving_source_bound_cases[] = {
    KUNIT_CASE(orlix_tcti_advsimd_halving_source_leaves_decode),
    KUNIT_CASE(orlix_tcti_advsimd_halving_reserved_64bit_lanes_reject),
    KUNIT_CASE(orlix_tcti_advsimd_halving_source_leaves_execute),
    {}};

static int orlix_tcti_advsimd_halving_source_bound_init(struct kunit *test)
{
...
static void orlix_tcti_advsimd_halving_source_bound_exit(struct kunit *test)
{
...
static struct kunit_suite orlix_tcti_advsimd_halving_source_bound_suite = {
    .name = "orlix-tcti-advsimd-halving-source-bound",
    .init = orlix_tcti_advsimd_halving_source_bound_init,
    .exit = orlix_tcti_advsimd_halving_source_bound_exit,
    .test_cases = orlix_tcti_advsimd_halving_source_bound_cases,
};

kunit_test_suite(orlix_tcti_advsimd_halving_source_bound_suite);
