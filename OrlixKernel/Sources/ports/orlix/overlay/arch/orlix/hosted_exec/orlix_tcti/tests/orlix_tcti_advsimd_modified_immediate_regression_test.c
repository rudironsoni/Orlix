...
struct orlix_tcti_advsimd_modimm_leaf {
  u16 source_ordinal;
...
static const struct orlix_tcti_advsimd_modimm_leaf orlix_tcti_advsimd_modimm_leaves[] = {
    {3987, "MVNI_asimdimm_L_sl", 0xbff89c00U, 0x2f000400U,
     ORLIX_TCTI_SIMD_MODIMM_MVNI, 0x000000a5000000a5ULL, 0xffffff5affffff5aULL,
     0xffffff5affffff5aULL},
    {3988, "BIC_asimdimm_L_sl", 0xbff89c00U, 0x2f001400U, ORLIX_TCTI_SIMD_MODIMM_BIC,
     0x000000a5000000a5ULL, 0x1122334011223340ULL, 0x5566770855667708ULL},
    {3989, "MVNI_asimdimm_L_hl", 0xbff8dc00U, 0x2f008400U,
     ORLIX_TCTI_SIMD_MODIMM_MVNI, 0x00a500a500a500a5ULL, 0xff5aff5aff5aff5aULL,
     0xff5aff5aff5aff5aULL},
    {3990, "BIC_asimdimm_L_hl", 0xbff8dc00U, 0x2f009400U, ORLIX_TCTI_SIMD_MODIMM_BIC,
     0x00a500a500a500a5ULL, 0x1102334011023340ULL, 0x5542770855427708ULL},
    {3991, "MVNI_asimdimm_M_sm", 0xbff8ec00U, 0x2f00c400U,
     ORLIX_TCTI_SIMD_MODIMM_MVNI, 0xa5ffa5ffa5ffa5ffULL, 0x5a005a005a005a00ULL,
     0x5a005a005a005a00ULL},
};

static const struct orlix_tcti_advsimd_source_manifest_leaf *
orlix_tcti_advsimd_modimm_source_leaf(u32 ordinal) {
  size_t index;
...
static u32
orlix_tcti_advsimd_modimm_instruction(const struct orlix_tcti_advsimd_modimm_leaf *leaf,
                                bool q, u8 imm8, u8 rd) {
  return leaf->source_pattern | (q ? BIT(30) : 0) |
         ((u32)(imm8 & 0xe0U) << 11) | ((u32)(imm8 & 0x1fU) << 5) | rd;
}

static unsigned long orlix_tcti_advsimd_modimm_map_program(struct kunit *test,
                                                     u32 instruction) {
  const u32 program[] = {instruction, ORLIX_TCTI_ADVSIMD_MODIMM_SVC};
...
static void orlix_tcti_advsimd_modimm_decode_source_leaves(struct kunit *test) {
  static const u16 expected_ordinals[] = {3987, 3988, 3989, 3990, 3991};
  size_t index;

  KUNIT_ASSERT_EQ(test, 5U, ARRAY_SIZE(orlix_tcti_advsimd_modimm_leaves));
  KUNIT_ASSERT_EQ(test, ARRAY_SIZE(expected_ordinals),
                  ARRAY_SIZE(orlix_tcti_advsimd_modimm_leaves));
  for (index = 0; index < ARRAY_SIZE(orlix_tcti_advsimd_modimm_leaves); index++) {
    const struct orlix_tcti_advsimd_modimm_leaf *leaf =
        &orlix_tcti_advsimd_modimm_leaves[index];
    const struct orlix_tcti_advsimd_source_manifest_leaf *source =
        orlix_tcti_advsimd_modimm_source_leaf(leaf->source_ordinal);
    struct orlix_tcti_decoded_instruction decoded =
        orlix_tcti_decode_aarch64(orlix_tcti_advsimd_modimm_instruction(
            leaf, false, 0xa5U, ORLIX_TCTI_ADVSIMD_MODIMM_RD));
    ...
    KUNIT_EXPECT_EQ(test, ORLIX_TCTI_ADVSIMD_MODIMM_RD, decoded.rd);
    KUNIT_EXPECT_EQ(test, sizeof(u64), decoded.result_size);
    KUNIT_EXPECT_EQ(test, sizeof(u64), decoded.access_size);

    decoded = orlix_tcti_decode_aarch64(orlix_tcti_advsimd_modimm_instruction(
        leaf, true, 0xa5U, ORLIX_TCTI_ADVSIMD_MODIMM_RD));
    KUNIT_EXPECT_EQ(test, ORLIX_TCTI_DECODE_SIMD_MODIFIED_IMMEDIATE,
                    decoded.decode_class);
    KUNIT_EXPECT_EQ(test, leaf->operation, decoded.simd_modified_immediate_op);
    KUNIT_EXPECT_EQ(test, leaf->immediate, decoded.logical_immediate);
...
static void
orlix_tcti_advsimd_modimm_exhaustive_source_encodings(struct kunit *test) {
  size_t index;
    ...
    for (q = 0; q < 2; q++) {
      ...
      for (imm8 = 0; imm8 < 256; imm8++) {
        ...
        for (rd = 0; rd < 32; rd++) {
          ...
          KUNIT_EXPECT_EQ_MSG(test, ORLIX_TCTI_DECODE_SIMD_MODIFIED_IMMEDIATE,
                              decoded.decode_class,
                              "source=%u id=%s imm8=%#x rd=%u q=%u",
                              source->ordinal, source->id, imm8, rd, q);
...
static void orlix_tcti_advsimd_modimm_execute_source_leaves(struct kunit *test) {
  size_t index;
  ...
  for (index = 0; index < ARRAY_SIZE(orlix_tcti_advsimd_modimm_leaves); index++) {
    ...
    for (q = 0; q < 2; q++) {
      u32 instruction = orlix_tcti_advsimd_modimm_instruction(leaf, q, 0xa5U,
                                                        ORLIX_TCTI_ADVSIMD_MODIMM_RD);
      unsigned long mapped = orlix_tcti_advsimd_modimm_map_program(test, instruction);
      struct pt_regs regs = {};
      ...
      current->thread.user_simd[ORLIX_TCTI_ADVSIMD_MODIMM_RD * 2] =
          0x1122334411223344ULL;
      current->thread.user_simd[ORLIX_TCTI_ADVSIMD_MODIMM_RD * 2 + 1] =
          0x5566778855667788ULL;
      memcpy(expected_simd, current->thread.user_simd, sizeof(expected_simd));
      expected_simd[ORLIX_TCTI_ADVSIMD_MODIMM_RD * 2] = leaf->expected_low;
      expected_simd[ORLIX_TCTI_ADVSIMD_MODIMM_RD * 2 + 1] =
      ...
      KUNIT_EXPECT_EQ_MSG(test, ORLIX_TCTI_EXIT_SYSCALL, result.reason, "%s q=%u",
                          leaf->source_name, q);
      KUNIT_EXPECT_EQ(test, 0L, result.status);
      KUNIT_EXPECT_EQ(test, mapped + sizeof(u32), result.pc);
      KUNIT_EXPECT_EQ(test, ORLIX_TCTI_ADVSIMD_MODIMM_SVC, result.instruction);
...
static struct kunit_case orlix_tcti_advsimd_modimm_regression_cases[] = {
    KUNIT_CASE(orlix_tcti_advsimd_modimm_decode_source_leaves),
    KUNIT_CASE(orlix_tcti_advsimd_modimm_exhaustive_source_encodings),
...
kunit_test_suite(orlix_tcti_advsimd_modimm_regression_suite);
