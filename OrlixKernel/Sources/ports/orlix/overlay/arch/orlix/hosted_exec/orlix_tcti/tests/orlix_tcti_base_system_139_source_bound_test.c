...
static void orlix_tcti_base_system_139_two_context_dmb_ordering_interaction(
	struct kunit *test);
static void orlix_tcti_base_system_139_wfxt_is_held_without_active_profile(
	struct kunit *test);
...
static void orlix_tcti_base_system_139_dmb_observation_rejects_disabled_path(
	struct kunit *test)
{
	struct orlix_tcti_barrier_observation before;
	struct orlix_tcti_barrier_observation after;
	struct pt_regs regs = { };
	struct pt_regs saved;
	struct orlix_tcti_result result;
	atomic_t handoff = ATOMIC_INIT(0);
	const u32 *instructions = (const u32 *)orlix_tcti_native_dmb_ish;
	unsigned long mapped;

	/* The fixed production entry must begin with DMB ISH, not merely contain it. */
	KUNIT_ASSERT_EQ(test, 0xd5033bbfU, READ_ONCE(instructions[0]));

	/* The production DMB ISH entry publishes this only after its native DMB. */
	orlix_tcti_native_dmb_ish_set_handoff(&handoff);
	orlix_tcti_native_barrier_observation(&before);
	mapped = base_system_139_map(test, 0xd50330bfU | (11U << 8));
	regs.pc = mapped;
	...
	KUNIT_EXPECT_EQ(test, 1, atomic_read(&handoff));
	orlix_tcti_native_barrier_observation(&after);
	KUNIT_EXPECT_EQ(test, before.sequence + 1, after.sequence);
	KUNIT_EXPECT_EQ(test, 1U, after.op);
	KUNIT_EXPECT_EQ(test, 11U, after.option);
	KUNIT_EXPECT_FALSE(test, after.nxs);
	KUNIT_EXPECT_EQ(test, saved.pc + 2 * sizeof(u32), regs.pc);
	KUNIT_EXPECT_EQ(test, 0, vm_munmap(mapped, PAGE_SIZE));

	/* DSB is a valid native barrier, but cannot credit the DMB-ISH handoff. */
	mapped = base_system_139_map(test, 0xd503309fU | (11U << 8));
	regs = saved;
	regs.pc = mapped;
	result = orlix_tcti_resume_user(current, &regs, current->mm);
	KUNIT_EXPECT_EQ(test, ORLIX_TCTI_EXIT_SYSCALL, result.reason);
	...
	/* A valid guest DMB with observation disabled receives no handoff credit. */
	orlix_tcti_native_dmb_ish_set_handoff(NULL);
	mapped = base_system_139_map(test, 0xd50330bfU | (11U << 8));
	regs = saved;
	regs.pc = mapped;
	result = orlix_tcti_resume_user(current, &regs, current->mm);
...
static void orlix_tcti_base_system_139_feature_hints_are_held_without_active_profile(
	struct kunit *test)
{
	...
	struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, mapped + 2 * sizeof(u32), result.pc);
	KUNIT_EXPECT_EQ(test, 0, vm_munmap(mapped, PAGE_SIZE));
	orlix_tcti_base_system_139_wfxt_is_held_without_active_profile(test);
}
...
static void orlix_tcti_base_system_139_sevl_survives_schedule_switch(
	struct kunit *test)
{
	struct base_system_139_event_worker worker = { .issue_sevl = true };
	struct task_struct *task;
	...
	task = kthread_run(base_system_139_event_worker, &worker,
		"orlix-tcti-sevl-switch");
	KUNIT_ASSERT_FALSE(test, IS_ERR(task));
	KUNIT_ASSERT_NE(test, 0UL,
		wait_for_completion_timeout(&worker.ready, HZ));
	/* wait_for_completion() in the worker forced a real scheduler switch. */
	KUNIT_EXPECT_TRUE(test, worker.initially_consumed);
	complete(&worker.release);
	KUNIT_ASSERT_NE(test, 0UL,
		wait_for_completion_timeout(&worker.done, HZ));
	KUNIT_EXPECT_FALSE(test, worker.sev_consumed);
}
	...
	struct kunit *test)
{
	...
	/* Deliver remotely while the target is asleep in wait_for_completion(). */
...
static void orlix_tcti_base_system_139_sb_is_held_without_active_profile(
	struct kunit *test)
{
...
static void orlix_tcti_base_system_139_dsb_nxs_is_held_without_active_profile(
	struct kunit *test)
{
...
static void orlix_tcti_base_system_139_pstate_flags_are_held_without_active_profile(
	struct kunit *test)
{
	static const struct {
		u32 instruction;
...
static struct kunit_case orlix_tcti_base_system_139_source_bound_cases[] = {
	...
	KUNIT_CASE(orlix_tcti_base_system_139_feature_hints_are_held_without_active_profile),
	...
	KUNIT_CASE(orlix_tcti_base_system_139_sevl_survives_schedule_switch),
	...
	KUNIT_CASE(orlix_tcti_base_system_139_dmb_observation_rejects_disabled_path),
	KUNIT_CASE(orlix_tcti_base_system_139_isb_advances_instruction_context),
	KUNIT_CASE(orlix_tcti_base_system_139_dsb_nxs_is_held_without_active_profile),
	KUNIT_CASE(orlix_tcti_base_system_139_sb_is_held_without_active_profile),
	KUNIT_CASE(orlix_tcti_base_system_139_pstate_flags_are_held_without_active_profile),
