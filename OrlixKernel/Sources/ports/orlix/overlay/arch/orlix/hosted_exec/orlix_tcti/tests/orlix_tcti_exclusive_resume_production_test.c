 ...
 * orlix_tcti_resume_user().  Store-exclusive leaves are preceded by the matching
 * exclusive load, which establishes the architectural reservation through
 * the same production path.
 */
#define ORLIX_TCTI_EXCLUSIVE_RESUME_SVC 0xd4000001U
#define ORLIX_TCTI_EXCLUSIVE_RN 10U
#define ORLIX_TCTI_EXCLUSIVE_RS 6U
#define ORLIX_TCTI_EXCLUSIVE_RT 8U
#define ORLIX_TCTI_EXCLUSIVE_RT2 9U

struct orlix_tcti_exclusive_resume_case {
	const char *name;
...
static const struct orlix_tcti_exclusive_resume_case orlix_tcti_exclusive_resume_cases[] = {
...
static const char *orlix_tcti_exclusive_resume_artifact_string(
...
static void orlix_tcti_exclusive_resume_assert_source(
	struct kunit *test, const struct orlix_tcti_exclusive_resume_case *entry,
	u32 instruction, const struct orlix_tcti_decoded_instruction *decoded)
{
	...
	source_name = orlix_tcti_exclusive_resume_artifact_string(artifact,
							    leaf->name_offset);
	operation = orlix_tcti_exclusive_resume_artifact_string(
...
static int orlix_tcti_exclusive_resume_production_test_init(struct kunit *test)
{
...
static void orlix_tcti_exclusive_resume_production_test_exit(struct kunit *test)
{
...
static unsigned long orlix_tcti_exclusive_resume_map(struct kunit *test, int prot)
{
...
static u32 orlix_tcti_exclusive_resume_instruction(u32 encoding, bool pair, bool load,
					     u8 rt)
{
	u32 fields = (ORLIX_TCTI_EXCLUSIVE_RN << 5) | rt;

	if (load)
		fields |= 31U << 16;
	else
		fields |= ORLIX_TCTI_EXCLUSIVE_RS << 16;
	if (pair)
		fields |= (rt + 1) << 10;
	else
		fields |= 31U << 10;
...
static int orlix_tcti_exclusive_resume_write_program(unsigned long address,
...
static u64 orlix_tcti_exclusive_resume_mask(u8 size)
{
	return size == sizeof(u64) ? U64_MAX : (1ULL << (size * 8)) - 1;
}

static void orlix_tcti_exclusive_resume_init_regs(struct pt_regs *regs,
					    unsigned long instructions,
					    unsigned long data)
{
	*regs = (struct pt_regs){};
	regs->regs[ORLIX_TCTI_EXCLUSIVE_RN] = data;
...
static void orlix_tcti_exclusive_resume_executes_every_base_leaf(struct kunit *test)
{
	...
	const u64 expected_pstate =
		PSR_MODE_EL0t | PSR_N_BIT | PSR_Z_BIT | PSR_C_BIT | PSR_V_BIT;
	unsigned long instructions =
		orlix_tcti_exclusive_resume_map(test, PROT_READ | PROT_WRITE);
	unsigned long data =
		orlix_tcti_exclusive_resume_map(test, PROT_READ | PROT_WRITE);
	size_t index;
	...
	if (IS_ERR_VALUE(instructions) || IS_ERR_VALUE(data))
		return;
	for (index = 0; index < ARRAY_SIZE(orlix_tcti_exclusive_resume_cases);
	     index++) {
		const struct orlix_tcti_exclusive_resume_case *entry =
			&orlix_tcti_exclusive_resume_cases[index];
		u32 program[3];
		...
		program[words++] = orlix_tcti_exclusive_resume_instruction(
			entry->load ? entry->encoding : entry->setup_encoding,
			entry->pair, true, entry->load ? ORLIX_TCTI_EXCLUSIVE_RT : 0);
		if (!entry->load)
			program[words++] = orlix_tcti_exclusive_resume_instruction(
				entry->encoding, entry->pair, false,
				ORLIX_TCTI_EXCLUSIVE_RT);
		program[words++] = ORLIX_TCTI_EXCLUSIVE_RESUME_SVC;
		decoded = orlix_tcti_decode_aarch64(program[entry->load ? 0 : 1]);
		KUNIT_ASSERT_EQ_MSG(test, ORLIX_TCTI_DECODE_LOAD_STORE_EXCLUSIVE,
		...
		KUNIT_ASSERT_EQ_MSG(test, entry->pair, decoded.pair,
				    "%s source=%u", entry->name,
				    entry->source_ordinal);
		orlix_tcti_exclusive_resume_assert_source(
			test, entry, program[entry->load ? 0 : 1], &decoded);
		bytes = decoded.access_size * (decoded.pair ? 2 : 1);
		mask = orlix_tcti_exclusive_resume_mask(decoded.access_size);
		ret = orlix_tcti_exclusive_resume_write_program(instructions, program,
							  words);
		...
		regs.regs[ORLIX_TCTI_EXCLUSIVE_RT] = desired[0] & mask;
		regs.regs[ORLIX_TCTI_EXCLUSIVE_RT2] = desired[1] & mask;
		...
		KUNIT_EXPECT_EQ_MSG(test,
				    ...
				    entry->source_ordinal);
		KUNIT_EXPECT_EQ_MSG(test, ORLIX_TCTI_EXCLUSIVE_RESUME_SVC,
		...
		if (entry->load) {
			KUNIT_EXPECT_EQ(test, old[0] & mask,
					regs.regs[ORLIX_TCTI_EXCLUSIVE_RT]);
			if (entry->pair)
...
static void
orlix_tcti_exclusive_resume_store_faults_preserve_state(struct kunit *test)
{
	const u64 old[] = {0x8123456789abcdefULL, 0x1020304050607080ULL};
	const u64 desired[] = {0x0fedcba987654321ULL, 0x8877665544332211ULL};
	unsigned long instructions =
	...
	for (index = 0; index < ARRAY_SIZE(orlix_tcti_exclusive_resume_cases);
		...
		if (entry->load)
			continue;
		decoded = orlix_tcti_decode_aarch64(orlix_tcti_exclusive_resume_instruction(
			entry->encoding, entry->pair, false,
			ORLIX_TCTI_EXCLUSIVE_RT));
		KUNIT_ASSERT_EQ_MSG(test, ORLIX_TCTI_DECODE_LOAD_STORE_EXCLUSIVE,
				    decoded.decode_class, "%s source=%u",
		...
		orlix_tcti_exclusive_resume_assert_source(
			test, entry,
			orlix_tcti_exclusive_resume_instruction(entry->encoding,
							  entry->pair, false,
							  ORLIX_TCTI_EXCLUSIVE_RT),
			&decoded);
		...
		bytes = decoded.access_size * (decoded.pair ? 2 : 1);
		mask = orlix_tcti_exclusive_resume_mask(decoded.access_size);
		setup_program[0] = orlix_tcti_exclusive_resume_instruction(
			entry->setup_encoding, entry->pair, true, 0);
		setup_program[1] = ORLIX_TCTI_EXCLUSIVE_RESUME_SVC;
		ret = orlix_tcti_exclusive_resume_write_program(
		...
		store_program[0] = orlix_tcti_exclusive_resume_instruction(
			entry->encoding, entry->pair, false, ORLIX_TCTI_EXCLUSIVE_RT);
		store_program[1] = ORLIX_TCTI_EXCLUSIVE_RESUME_SVC;
		ret = orlix_tcti_exclusive_resume_write_program(
		...
		KUNIT_EXPECT_EQ_MSG(test, data, result.fault_address,
				    "%s source=%u", entry->name,
				    entry->source_ordinal);
		KUNIT_EXPECT_EQ_MSG(test, ORLIX_TCTI_ACCESS_WRITE,
...
static struct kunit_case orlix_tcti_exclusive_resume_production_test_cases[] = {
	KUNIT_CASE(orlix_tcti_exclusive_resume_executes_every_base_leaf),
	KUNIT_CASE(orlix_tcti_exclusive_resume_store_faults_preserve_state),
	{}};

static struct kunit_suite orlix_tcti_exclusive_resume_production_test_suite = {
	.name = "orlix-tcti-exclusive-resume-production",
	.init = orlix_tcti_exclusive_resume_production_test_init,
	.exit = orlix_tcti_exclusive_resume_production_test_exit,
	.test_cases = orlix_tcti_exclusive_resume_production_test_cases,
};
kunit_test_suite(orlix_tcti_exclusive_resume_production_test_suite);
