...
struct orlix_tcti_lse_rmw_case {
...
struct orlix_tcti_lse_exclusive_state {
...
static const struct orlix_tcti_lse_rmw_case orlix_tcti_lse_rmw_cases[] = {
	{ ORLIX_TCTI_LSE_ATOMIC_ADD, 0, 0x12, 0x25 },
	{ ORLIX_TCTI_LSE_ATOMIC_CLR, 1, 0x3f, 0x15 },
	{ ORLIX_TCTI_LSE_ATOMIC_EOR, 2, 0x3c, 0x25 },
	{ ORLIX_TCTI_LSE_ATOMIC_SET, 3, 0x12, 0x25 },
	{ ORLIX_TCTI_LSE_ATOMIC_SMAX, 4, U64_MAX, 1 },
	{ ORLIX_TCTI_LSE_ATOMIC_SMIN, 5, 0x7fffffffffffffffULL, U64_MAX },
	{ ORLIX_TCTI_LSE_ATOMIC_UMAX, 6, 1, U64_MAX },
	{ ORLIX_TCTI_LSE_ATOMIC_UMIN, 7, U64_MAX, 1 },
	{ ORLIX_TCTI_LSE_ATOMIC_SWP, 8, 0x12, 0x25 },
};

static u32 orlix_tcti_lse_order(u32 instruction, bool cas, u8 order)
{
	if (cas) {
		if (order & 1U)
			instruction |= BIT(22);
...
static unsigned long orlix_tcti_lse_map(struct kunit *test)
{
...
static void orlix_tcti_lse_expect_exclusive(
	struct kunit *test, const struct orlix_tcti_lse_exclusive_state *expected)
{
...
static u64 orlix_tcti_lse_mask(u8 width)
{
	return width == sizeof(u64) ? U64_MAX :
		(1ULL << (width * 8)) - 1;
}

static u64 orlix_tcti_lse_rmw_result(const struct orlix_tcti_lse_rmw_case *rmw,
			       u64 initial, u64 operand, u8 width)
{
	u64 mask = orlix_tcti_lse_mask(width);
...
static void orlix_tcti_lse_write_pair(struct kunit *test, unsigned long address,
				const u64 values[2], u8 width)
{
	...
	ret = orlix_tcti_write_user_data(current->mm, address, encoded, 2 * width);
	KUNIT_ASSERT_EQ(test, 0, ret);
}

static void orlix_tcti_lse_expect_pair(struct kunit *test, unsigned long address,
				 const u64 expected[2], u8 width)
{
	...
	ret = orlix_tcti_read_user_data(current->mm, address, encoded, 2 * width);
	KUNIT_ASSERT_EQ(test, 0, ret);
	memcpy(&observed[0], encoded, width);
	memcpy(&observed[1], encoded + width, width);
	KUNIT_EXPECT_EQ(test, expected[0] & orlix_tcti_lse_mask(width),
			observed[0]);
	KUNIT_EXPECT_EQ(test, expected[1] & orlix_tcti_lse_mask(width),
			observed[1]);
}

static void orlix_tcti_expect_lse(struct kunit *test, u32 instruction,
			    enum orlix_tcti_lse_atomic_op op, u8 width, bool acquire,
...
static void orlix_tcti_lse_decode_cas_leaves(struct kunit *test)
{
	u8 size;
	u8 order;
	...
	for (size = 0; size < 4; size++) {
		for (order = 0; order < 4; order++) {
			...
			instruction = orlix_tcti_lse_order(instruction, true, order);
			orlix_tcti_expect_lse(test, instruction, ORLIX_TCTI_LSE_ATOMIC_CAS,
					1U << size, order & 1U, order & 2U, false);
		}
	}
}

static void orlix_tcti_lse_decode_casp_leaves(struct kunit *test)
{
	u8 size;
	u8 order;
...
static void orlix_tcti_lse_decode_rmw_and_swp_leaves(struct kunit *test)
{
	static const enum orlix_tcti_lse_atomic_op ops[] = {
	...
	for (op = 0; op < ARRAY_SIZE(ops); op++) {
		for (size = 0; size < 4; size++) {
			for (order = 0; order < 4; order++) {
				...
				instruction = orlix_tcti_lse_order(instruction, false, order);
				orlix_tcti_expect_lse(test, instruction, ops[op], 1U << size,
...
static void orlix_tcti_lse_execute_cas_scalar_matrix(struct kunit *test)
{
	unsigned long mapped = orlix_tcti_lse_map(test);
	u8 size;
	u8 order;
...
static void orlix_tcti_lse_execute_casp_matrix(struct kunit *test)
{
	unsigned long mapped = orlix_tcti_lse_map(test);
	u8 size;
	u8 order;
	...
	for (size = 0; size < 2; size++) {
		...
		for (order = 0; order < 4; order++) {
			...
			regs.pstate = PSR_MODE_EL0t | PSR_N_BIT | PSR_Z_BIT |
				PSR_C_BIT | PSR_V_BIT;
			regs.pc = 0x3000;
			orlix_tcti_lse_write_pair(test, mapped, initial, width);
			ret = orlix_tcti_switch_debug_execute_decoded(
				current->mm, &regs, &decoded, &fault_address);
			KUNIT_ASSERT_EQ(test, 0, ret);
			orlix_tcti_lse_expect_pair(test, mapped, replacement, width);
			...
			ret = orlix_tcti_switch_debug_execute_decoded(
				current->mm, &regs, &decoded, &fault_address);
			KUNIT_ASSERT_EQ(test, 0, ret);
			orlix_tcti_lse_expect_pair(test, mapped, initial, width);
			KUNIT_EXPECT_EQ(test, initial[0], regs.regs[6]);
			KUNIT_EXPECT_EQ(test, initial[1], regs.regs[7]);
...
static void orlix_tcti_lse_execute_rmw_matrix(struct kunit *test)
{
	unsigned long mapped = orlix_tcti_lse_map(test);
	size_t op;
	u8 size;
	...
	for (op = 0; op < ARRAY_SIZE(orlix_tcti_lse_rmw_cases); op++) {
		const struct orlix_tcti_lse_rmw_case *rmw =
			&orlix_tcti_lse_rmw_cases[op];
		...
		for (size = 0; size < 4; size++) {
			...
			if (rmw->op == ORLIX_TCTI_LSE_ATOMIC_SMAX) {
				initial = sign;
				operand = 1;
			} else if (rmw->op == ORLIX_TCTI_LSE_ATOMIC_SMIN) {
				initial = sign - 1;
				operand = mask;
			} else if (rmw->op == ORLIX_TCTI_LSE_ATOMIC_UMAX) {
				initial = 0;
				operand = mask;
			} else if (rmw->op == ORLIX_TCTI_LSE_ATOMIC_UMIN) {
				initial = mask;
				operand = 0;
			}
			expected = orlix_tcti_lse_rmw_result(
			...
			for (order = 0; order < 4; order++) {
				...
				instruction = orlix_tcti_lse_order(
				...
				regs.pc = 0x5000;
				ret = orlix_tcti_write_user_data(
...
static void orlix_tcti_lse_execute_alias_zero_and_sp(struct kunit *test)
{
	unsigned long mapped = orlix_tcti_lse_map(test);
	struct orlix_tcti_decoded_instruction decoded;
	struct pt_regs regs = {};
...
static void orlix_tcti_lse_execute_fault_preserves_state(struct kunit *test)
{
	unsigned long mapped = orlix_tcti_lse_map(test);
...
static void orlix_tcti_lse_execute_updates_exclusive_state(struct kunit *test)
{
	static const struct orlix_tcti_lse_exclusive_state active = {
		.address = 0x12345000,
	...
	static const struct orlix_tcti_lse_exclusive_state cleared;
	unsigned long mapped = orlix_tcti_lse_map(test);
	struct orlix_tcti_decoded_instruction decoded;
	struct pt_regs regs = {};
	u64 initial;
	...
	ret = orlix_tcti_write_user_data(current->mm, mapped, &initial,
				   sizeof(initial));
	KUNIT_ASSERT_EQ(test, 0, ret);
	orlix_tcti_lse_set_exclusive(&active);
	ret = orlix_tcti_switch_debug_execute_decoded(
		current->mm, &regs, &decoded, NULL);
	KUNIT_ASSERT_EQ(test, 0, ret);
	ret = orlix_tcti_read_user_data(current->mm, mapped, &observed,
	...
	KUNIT_ASSERT_EQ(test, 0, ret);
	KUNIT_EXPECT_EQ(test, initial, observed);
	orlix_tcti_lse_expect_exclusive(test, &cleared);
	...
	ret = orlix_tcti_switch_debug_execute_decoded(
		current->mm, &regs, &decoded, NULL);
	KUNIT_ASSERT_EQ(test, 0, ret);
	orlix_tcti_lse_expect_exclusive(test, &active);
	...
	regs.pc = 0xe000;
	orlix_tcti_lse_write_pair(test, mapped, pair_initial, sizeof(u64));
	orlix_tcti_lse_set_exclusive(&active);
	ret = orlix_tcti_switch_debug_execute_decoded(
		current->mm, &regs, &decoded, NULL);
	KUNIT_ASSERT_EQ(test, 0, ret);
	...
	orlix_tcti_lse_set_exclusive(&cleared);
	KUNIT_EXPECT_EQ(test, 0, vm_munmap(mapped, PAGE_SIZE));
}

static void orlix_tcti_lse_same_value_cas_invalidates_reservations(
	struct kunit *test)
{
	static const struct orlix_tcti_lse_exclusive_state active = {
		.address = 0x34567000,
	...
	ret = orlix_tcti_write_user_data(current->mm, mapped, &scalar_initial,
	...
	orlix_tcti_lse_expect_exclusive(test, &cleared);
	ret = orlix_tcti_read_user_data(current->mm, mapped, &scalar_observed,
	...
	orlix_tcti_lse_expect_exclusive(test, &cleared);
	orlix_tcti_lse_expect_pair(test, mapped, pair_initial, sizeof(u64));
	stored = true;
	ret = orlix_tcti_store_exclusive_user_data(
...
static void orlix_tcti_lse_resume_reports_alignment_faults(struct kunit *test)
{
	static const struct {
		u32 instruction;
	...
	static const struct orlix_tcti_lse_exclusive_state cleared;
	unsigned long instructions = orlix_tcti_lse_map(test);
	unsigned long data = orlix_tcti_lse_map(test);
	...
	for (i = 0; i < ARRAY_SIZE(cases); i++) {
		...
		result = orlix_tcti_switch_debug_resume_user(
...
static void orlix_tcti_lse_decode_rejects_reserved_neighbors(struct kunit *test)
{
	u8 size;
	u8 order;
	u8 op;
	...
	for (size = 0; size < 4; size++) {
		for (order = 0; order < 4; order++) {
			...
			cas = orlix_tcti_lse_order(cas, true, order);
			rmw = orlix_tcti_lse_order(rmw, false, order);
			decoded = orlix_tcti_decode_aarch64(cas ^ BIT(10));
			KUNIT_EXPECT_EQ_MSG(test, ORLIX_TCTI_DECODE_UNSUPPORTED,
					    decoded.decode_class,
					    "instruction %#lx", cas ^ BIT(10));
			decoded = orlix_tcti_decode_aarch64(rmw ^ BIT(10));
			KUNIT_EXPECT_EQ_MSG(test, ORLIX_TCTI_DECODE_UNSUPPORTED,
					    decoded.decode_class,
	...
	for (size = 0; size < 2; size++) {
		for (order = 0; order < 4; order++) {
			...
			casp = orlix_tcti_lse_order(casp, true, order);
			decoded = orlix_tcti_decode_aarch64(casp ^ BIT(10));
			KUNIT_EXPECT_EQ_MSG(test, ORLIX_TCTI_DECODE_UNSUPPORTED,
					    decoded.decode_class,
...
static void orlix_tcti_lse_decode_preserves_allocated_exclusive_neighbors(
	struct kunit *test)
{
	...
	for (i = 0; i < ARRAY_SIZE(instructions); i++) {
		...
		KUNIT_EXPECT_EQ_MSG(test, ORLIX_TCTI_DECODE_LOAD_STORE_EXCLUSIVE,
				    decoded.decode_class,
...
static void orlix_tcti_lse_decode_rejects_invalid_casp_pairs(struct kunit *test)
{
	u8 size;
	u8 order;
	u8 rs;
 ...
 * orlix_tcti_resume_user() path used by an EL0 task. The direct execution tests
 ...
 */
static u32 orlix_tcti_lse_exclusive_instruction(u8 size, bool ordered, bool load,
	...
	struct kunit *test)
{
	...
	for (form_index = 0; form_index < ARRAY_SIZE(forms); form_index++) {
		...
		for (size = first_size; size <= last_size; size++) {
			...
			const u64 mask = orlix_tcti_lse_mask(width);
			const u64 initial[2] = {
				0x1122334455667788ULL & mask,
				0x8877665544332211ULL & mask,
			};
			...
			const u32 program[] = {
				orlix_tcti_lse_exclusive_instruction(
			...
			ret = orlix_tcti_lse_write_resume_program(instructions, program,
							   ARRAY_SIZE(program));
...
static struct kunit_case orlix_tcti_lse_decode_test_cases[] = {
	KUNIT_CASE(orlix_tcti_lse_decode_cas_leaves),
	KUNIT_CASE(orlix_tcti_lse_decode_casp_leaves),
	KUNIT_CASE(orlix_tcti_lse_decode_rmw_and_swp_leaves),
	KUNIT_CASE(orlix_tcti_lse_execute_cas_scalar_matrix),
	KUNIT_CASE(orlix_tcti_lse_execute_casp_matrix),
	KUNIT_CASE(orlix_tcti_lse_execute_rmw_matrix),
	KUNIT_CASE(orlix_tcti_lse_execute_alias_zero_and_sp),
	KUNIT_CASE(orlix_tcti_lse_execute_fault_preserves_state),
	KUNIT_CASE(orlix_tcti_lse_execute_updates_exclusive_state),
	KUNIT_CASE(orlix_tcti_lse_same_value_cas_invalidates_reservations),
	KUNIT_CASE(orlix_tcti_lse_resume_reports_alignment_faults),
	KUNIT_CASE(orlix_tcti_lse_decode_rejects_reserved_neighbors),
	KUNIT_CASE(orlix_tcti_lse_decode_preserves_allocated_exclusive_neighbors),
	KUNIT_CASE(orlix_tcti_lse_decode_rejects_invalid_casp_pairs),
	KUNIT_CASE(orlix_tcti_lse_resume_executes_exclusive_and_ordered_matrix),
	{}
};
...
kunit_test_suite(orlix_tcti_lse_decode_test_suite);
