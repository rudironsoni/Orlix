...
static u32 orlix_tcti_move_wide_instruction(const struct orlix_tcti_move_wide_leaf *leaf,
...
static const struct orlix_tcti_move_wide_leaf *
orlix_tcti_move_wide_source_leaf(u32 instruction)
{
	size_t index;
...
static u64 orlix_tcti_move_wide_mask(const struct orlix_tcti_move_wide_leaf *leaf)
{
	return leaf->is_64bit ? U64_MAX : U32_MAX;
}

static u64 orlix_tcti_move_wide_expected(const struct orlix_tcti_move_wide_leaf *leaf,
					  u8 hw, u16 immediate, u64 old)
{
	...
	return result & orlix_tcti_move_wide_mask(leaf);
}

static void orlix_tcti_move_wide_expect_decode(struct kunit *test,
					 const struct orlix_tcti_move_wide_leaf *leaf,
					 ...
					 u8 rd)
{
	...
	KUNIT_ASSERT_EQ_MSG(test, ORLIX_TCTI_DECODE_MOVE_WIDE_IMMEDIATE,
...
static unsigned long orlix_tcti_move_wide_map_program(struct kunit *test,
						 u32 instruction)
{
...
static void orlix_tcti_move_wide_execute(struct kunit *test,
				   const struct orlix_tcti_move_wide_leaf *leaf,
				   u8 hw, u16 immediate, u8 rd)
{
	u32 instruction = orlix_tcti_move_wide_instruction(leaf, hw, immediate, rd);
	unsigned long mapped = orlix_tcti_move_wide_map_program(test, instruction);
	unsigned int pass;
	...
	for (pass = 0; pass < 2; pass++) {
		...
		before = regs;
		expected = orlix_tcti_move_wide_expected(
...
static void orlix_tcti_move_wide_source_bindings(struct kunit *test)
{
	size_t index;

	KUNIT_ASSERT_EQ(test, 6U, ARRAY_SIZE(orlix_tcti_move_wide_leaves));
	for (index = 0; index < ARRAY_SIZE(orlix_tcti_move_wide_leaves); index++) {
		const struct orlix_tcti_move_wide_leaf *leaf =
			&orlix_tcti_move_wide_leaves[index];
		...
		for (previous = 0; previous < index; previous++)
			KUNIT_EXPECT_NE(test, leaf->source_pattern,
				orlix_tcti_move_wide_leaves[previous].source_pattern);
	}
}

static void orlix_tcti_move_wide_complete_legal_decode_matrix(struct kunit *test)
{
	...
	for (index = 0; index < ARRAY_SIZE(orlix_tcti_move_wide_leaves); index++) {
		...
		for (hw = 0; hw < (leaf->is_64bit ? 4 : 2); hw++) {
			...
			for (immediate_index = 0;
			     ...
			     immediate_index++)
				for (rd = 0; rd < 32; rd++)
					orlix_tcti_move_wide_expect_decode(
						test, leaf,
						orlix_tcti_move_wide_instruction(
							leaf, hw,
...
static void orlix_tcti_move_wide_all_immediates_decode(struct kunit *test)
{
	size_t index;
...
static void orlix_tcti_move_wide_production_path_semantics(struct kunit *test)
{
	...
	for (index = 0; index < ARRAY_SIZE(orlix_tcti_move_wide_leaves); index++) {
		...
		for (hw = 0; hw < (leaf->is_64bit ? 4 : 2); hw++) {
			...
			for (immediate_index = 0;
			     immediate_index < ARRAY_SIZE(immediate_vectors);
			     immediate_index++) {
				orlix_tcti_move_wide_execute(test, leaf, hw,
...
static void orlix_tcti_move_wide_fixed_bit_neighbours_and_reserved(struct kunit *test)
{
	size_t index;
	...
	for (index = 0; index < ARRAY_SIZE(orlix_tcti_move_wide_leaves); index++) {
		...
		for (bit = 0; bit < 32; bit++) {
			...
			instruction = orlix_tcti_move_wide_instruction(leaf, 0, 1, 3) ^
				BIT(bit);
			neighbour = orlix_tcti_move_wide_source_leaf(instruction);
			decoded = orlix_tcti_decode_aarch64(instruction);
			if (neighbour) {
				KUNIT_EXPECT_EQ(test, ORLIX_TCTI_DECODE_MOVE_WIDE_IMMEDIATE,
						decoded.decode_class);
				KUNIT_EXPECT_EQ(test, neighbour->operation,
						decoded.move_wide_op);
				continue;
			}
			KUNIT_EXPECT_NE_MSG(test, ORLIX_TCTI_DECODE_MOVE_WIDE_IMMEDIATE,
					    decoded.decode_class,
	...
	for (index = 0; index < 2; index++) {
		...
		for (hw = 0; hw < 4; hw++)
			KUNIT_EXPECT_EQ(test, ORLIX_TCTI_DECODE_UNSUPPORTED,
				orlix_tcti_decode_aarch64(base | BIT(29) |
	...
	for (index = 0; index < 3; index++) {
		...
		for (hw = 2; hw < 4; hw++)
			KUNIT_EXPECT_EQ(test, ORLIX_TCTI_DECODE_UNSUPPORTED,
				orlix_tcti_decode_aarch64(
					orlix_tcti_move_wide_instruction(
						&orlix_tcti_move_wide_leaves[index], hw, 0, 0))
					.decode_class);
	}
}
...
static int orlix_tcti_move_wide_source_bound_test_init(struct kunit *test)
{
...
static void orlix_tcti_move_wide_source_bound_test_exit(struct kunit *test)
{
...
struct kunit_suite orlix_tcti_move_wide_source_bound_test_suite = {
	.name = "orlix-tcti-move-wide-source-bound",
	.init = orlix_tcti_move_wide_source_bound_test_init,
	.exit = orlix_tcti_move_wide_source_bound_test_exit,
	.test_cases = orlix_tcti_move_wide_source_bound_test_cases,
};

kunit_test_suite(orlix_tcti_move_wide_source_bound_test_suite);
