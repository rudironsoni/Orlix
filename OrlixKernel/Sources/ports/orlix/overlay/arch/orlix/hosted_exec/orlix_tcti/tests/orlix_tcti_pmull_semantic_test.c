...
#define ORLIX_TCTI_PMULL_PATTERN	0x0e20e000U
 ...
 */
static u16 orlix_tcti_pmull_test_u8(u8 left, u8 right)
{
	u16 result = 0;
	u8 bit;
...
static void orlix_tcti_pmull_test_u64(u64 left, u64 right, u64 *low, u64 *high)
{
	u64 result_low = 0;
	u64 result_high = 0;
	u8 bit;
...
static void orlix_tcti_pmull_test_expected(u8 size, u64 left, u64 right,
				      u64 *low, u64 *high)
{
	if (!size) {
		u64 result_low = 0;
		...
		for (lane = 0; lane < 8; lane++) {
			u16 product = orlix_tcti_pmull_test_u8(left >> (lane * 8),
						  right >> (lane * 8));
	...
	orlix_tcti_pmull_test_u64(left, right, low, high);
}

static u32 orlix_tcti_pmull_test_instruction(u8 size, u8 q, u8 rd, u8 rn, u8 rm)
{
	return ORLIX_TCTI_PMULL_PATTERN | ((u32)size << 22) |
		(q ? BIT(30) : 0) | ((u32)rm << 16) | ((u32)rn << 5) | rd;
}

static void orlix_tcti_pmull_test_initialize_state(u32 instruction)
{
	u8 word;
...
static void orlix_tcti_pmull_test_expect_execution(struct kunit *test, u8 size,
					      u8 q, u8 rd, u8 rn, u8 rm)
{
	u32 instruction = orlix_tcti_pmull_test_instruction(size, q, rd, rn, rm);
	...
	KUNIT_ASSERT_EQ(test, ORLIX_TCTI_SIMD_ARITH_PMULL,
	...
	before_regs = regs;
	orlix_tcti_pmull_test_initialize_state(instruction);
	...
	orlix_tcti_pmull_test_expected(size, left, right, &expected_low,
					  &expected_high);
...
static void orlix_tcti_pmull_semantic_all_register_combinations(struct kunit *test)
{
	u8 size;
	u8 q;
	u8 rd;
	...
	for (size = 0; size <= 3; size += 3)
		for (q = 0; q < 2; q++)
			for (rd = 0; rd < 32; rd++)
				for (rn = 0; rn < 32; rn++)
					for (rm = 0; rm < 32; rm++)
						orlix_tcti_pmull_test_expect_execution(test, size,
									q, rd, rn, rm);
}

static void orlix_tcti_pmull_semantic_known_mathematical_vectors(struct kunit *test)
{
	static const struct {
		u8 size;
		u64 left;
	...
	for (index = 0; index < ARRAY_SIZE(vectors); index++) {
		...
		orlix_tcti_pmull_test_expected(vectors[index].size, vectors[index].left,
					  vectors[index].right, &low, &high);
		KUNIT_EXPECT_EQ(test, vectors[index].low, low);
		KUNIT_EXPECT_EQ(test, vectors[index].high, high);

		for (q = 0; q < 2; q++) {
			u32 instruction = orlix_tcti_pmull_test_instruction(
...
static void orlix_tcti_pmull_semantic_rejects_reserved_sizes(struct kunit *test)
{
	u8 size;
	u8 q;
	...
	for (size = 1; size <= 2; size++)
		for (q = 0; q < 2; q++) {
			struct orlix_tcti_decoded_instruction decoded = orlix_tcti_decode_aarch64(
				orlix_tcti_pmull_test_instruction(size, q, 17, 9, 3));
...
static struct kunit_case orlix_tcti_pmull_semantic_cases[] = {
	KUNIT_CASE(orlix_tcti_pmull_semantic_all_register_combinations),
	KUNIT_CASE(orlix_tcti_pmull_semantic_known_mathematical_vectors),
	KUNIT_CASE(orlix_tcti_pmull_semantic_rejects_reserved_sizes),
	{}
};

static struct kunit_suite orlix_tcti_pmull_semantic_test_suite = {
	.name = "orlix-tcti-pmull-semantic",
	.test_cases = orlix_tcti_pmull_semantic_cases,
};

kunit_test_suite(orlix_tcti_pmull_semantic_test_suite);
