...
struct orlix_tcti_el0_feature_profile_entry {
	const char *feature;
	bool present;
	const char *provenance;
};

#define ORLIX_TCTI_A64_EL0_FEATURE_PROFILE(feature, state, provenance) \
	{ #feature, false, provenance },
static const struct orlix_tcti_el0_feature_profile_entry
	orlix_tcti_el0_feature_profile[] = {
#include "../isa/el0_feature_profile.def"
};
#undef ORLIX_TCTI_A64_EL0_FEATURE_PROFILE

static const struct orlix_tcti_el0_feature_profile_entry *
orlix_tcti_el0_feature_profile_find(const char *feature)
{
	size_t index;

	for (index = 0; index < ARRAY_SIZE(orlix_tcti_el0_feature_profile); index++)
		if (!strcmp(orlix_tcti_el0_feature_profile[index].feature, feature))
			return &orlix_tcti_el0_feature_profile[index];
	return NULL;
}
...
{
	return leaf->ordinal == 2166U || leaf->ordinal == 2235U ||
	       (leaf->ordinal >= 2228U && leaf->ordinal <= 2234U);
}

static void orlix_tcti_source_leaf_catalog_has_exact_proof_and_encoding_sets(
	struct kunit *test)
{
	static const u32 expected_ordinals[] = {
		2166U, 2235U, 2228U, 2229U, 2232U, 2233U, 2234U,
		2299U, 2300U, 2301U, 2303U,
	};
	static const char * const expected_proof_ids[] = {
	...
	KUNIT_EXPECT_EQ(test, 10U,
			orlix_tcti_source_leaf_rejection_proof_count());
	KUNIT_ASSERT_EQ(test, ARRAY_SIZE(expected_ordinals),
			orlix_tcti_source_leaf_rejection_encoding_count());
	for (index = 0; index < ARRAY_SIZE(expected_ordinals); index++) {
		...
		KUNIT_EXPECT_EQ_MSG(test, expected_ordinals[index], leaf->ordinal,
				    "catalog encoding index %zu", index);
		KUNIT_EXPECT_STREQ_MSG(test, expected_proof_ids[index], leaf->proof_id,
				       "catalog source ordinal %u", leaf->ordinal);
	}
}
...
static void orlix_tcti_source_leaf_base_exceptions_emit_typed_observations(
	struct kunit *test)
{
	const struct orlix_tcti_el0_feature_profile_entry *tev;
	size_t leaf_index;

	tev = orlix_tcti_el0_feature_profile_find("FEAT_TEV");
	KUNIT_ASSERT_NOT_NULL(test, tev);
	KUNIT_EXPECT_FALSE(test, tev->present);
	KUNIT_EXPECT_STREQ(test, "orlix-el0-profile:v1:FEAT_TEV=absent",
			   tev->provenance);

	for (leaf_index = 0;
	     leaf_index < orlix_tcti_source_leaf_rejection_encoding_count(); leaf_index++) {
		struct orlix_tcti_source_leaf_rejection entry;
		const struct orlix_tcti_source_leaf_rejection *leaf =
...
static void orlix_tcti_source_leaf_rejections_match_pinned_tuples(struct kunit *test)
{
	...
	for (index = 0; index < orlix_tcti_source_leaf_rejection_encoding_count(); index++) {
		struct orlix_tcti_source_leaf_rejection entry;
		const struct orlix_tcti_source_leaf_rejection *leaf =
			orlix_tcti_source_leaf_rejection_at(index, &entry);
		u32 variable_mask;
...
static struct kunit_case orlix_tcti_source_leaf_classification_test_cases[] = {
	...
	KUNIT_CASE(orlix_tcti_source_leaf_catalog_has_exact_proof_and_encoding_sets),
