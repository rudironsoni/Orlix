// SPDX-License-Identifier: GPL-2.0-only
#ifdef ORLIX_TCTI_RUNTIME_PROJECTION_HOST_TEST
#include <errno.h>
#include <string.h>
#else
#include <linux/errno.h>
#include <linux/kernel.h>
#include <linux/string.h>
#include <asm/isa.h>
#include <target_inventory.h>
#include "target_runtime_capability_cohort_artifact.h"
#endif

#include "runtime_projection.h"

#ifndef ORLIX_TCTI_RUNTIME_PROJECTION_HOST_TEST
struct orlix_tcti_runtime_source_bound_proof {
	unsigned int ordinal;
	const char *proof_id;
};

#define ORLIX_TCTI_A64_SOURCE_BOUND_PROOF(ordinal, proof_id) \
	{ ordinal, proof_id },
static const struct orlix_tcti_runtime_source_bound_proof
orlix_tcti_runtime_source_bound_proofs[] = {
#include "../isa/source_bound_proof.def"
};
#undef ORLIX_TCTI_A64_SOURCE_BOUND_PROOF
#endif

#ifndef ORLIX_TCTI_RUNTIME_PROJECTION_HOST_TEST
#define stringify_1(value) #value
#define stringify(value) stringify_1(value)
#define AUXV_HWCAP ORLIX_TCTI_RUNTIME_CAPABILITY_HWCAP
#define AUXV_HWCAP2 ORLIX_TCTI_RUNTIME_CAPABILITY_HWCAP2
#define ORLIX_TCTI_A64_RUNTIME_CAPABILITY(word, bit, feature, extension) \
	{ (word), (bit), stringify(feature) },
static const struct orlix_tcti_runtime_projection_capability
orlix_tcti_runtime_capability_mappings[] = {
#include "../isa/runtime_profile.def"
};
#undef ORLIX_TCTI_A64_RUNTIME_CAPABILITY
#undef AUXV_HWCAP2
#undef AUXV_HWCAP
#undef stringify
#undef stringify_1
#endif

static bool orlix_tcti_runtime_leaf_is_classified(
	const struct orlix_tcti_runtime_projection_leaf *leaf)
{
	return leaf->classification > ORLIX_TCTI_RUNTIME_LEAF_UNCLASSIFIED &&
	       leaf->classification <= ORLIX_TCTI_RUNTIME_LEAF_ALIAS_OR_DUPLICATE;
}

static bool orlix_tcti_runtime_leaf_has_feature(
	const struct orlix_tcti_runtime_projection_leaf *leaf, const char *feature)
{
	size_t i;

	for (i = 0; i < leaf->feature_count; i++)
		if (leaf->features[i] && !strcmp(leaf->features[i], feature))
			return true;
	return false;
}

#ifndef ORLIX_TCTI_RUNTIME_PROJECTION_HOST_TEST
static bool orlix_tcti_runtime_leaf_is_source_bound(unsigned int ordinal,
	const char *proof_id)
{
	size_t index;

	if (!proof_id || !proof_id[0])
		return false;
	for (index = 0; index < sizeof(orlix_tcti_runtime_source_bound_proofs) /
				      sizeof(orlix_tcti_runtime_source_bound_proofs[0]); index++)
		if (orlix_tcti_runtime_source_bound_proofs[index].ordinal == ordinal &&
		    !strcmp(orlix_tcti_runtime_source_bound_proofs[index].proof_id,
			    proof_id))
			return true;
	return false;
}
#endif

static int orlix_tcti_runtime_ledger_read_leaf(const void *context, size_t index,
	struct orlix_tcti_runtime_projection_leaf *leaf)
{
	const struct orlix_tcti_runtime_projection_ledger *ledger = context;

	if (!ledger || !leaf || index >= ledger->leaf_count)
		return -EINVAL;
	*leaf = ledger->leaves[index];
	return 0;
}

static unsigned long *orlix_tcti_runtime_result_word(
	struct orlix_tcti_runtime_projection_result *result,
	enum orlix_tcti_runtime_capability_word word, bool proved)
{
	if (word == ORLIX_TCTI_RUNTIME_CAPABILITY_HWCAP)
		return proved ? &result->proved_hwcap : &result->mapped_hwcap;
	if (word == ORLIX_TCTI_RUNTIME_CAPABILITY_HWCAP2)
		return proved ? &result->proved_hwcap2 : &result->mapped_hwcap2;
	return NULL;
}

/*
 * Callers inspect this record to diagnose a rejected projection.  Initialize
 * it before validating the rest of the request so an early -EINVAL never
 * exposes prior stack contents as audit state.  The values below are merely
 * request metadata.  They do not discharge a target, feature, or proof
 * obligation.
 */
static void orlix_tcti_runtime_projection_result_init(
	struct orlix_tcti_runtime_projection_result *result,
	const struct orlix_tcti_runtime_projection_provider *provider,
	const struct orlix_tcti_runtime_projection_profile *profile)
{
	if (!result)
		return;

	memset(result, 0, sizeof(*result));
	if (profile) {
		result->advertised_hwcap = profile->hwcap;
		result->advertised_hwcap2 = profile->hwcap2;
	}
	if (provider)
		result->target_leaf_count = provider->leaf_count;
}

int orlix_tcti_runtime_projection_audit_provider(
	const struct orlix_tcti_runtime_projection_provider *provider,
	const struct orlix_tcti_runtime_projection_profile *profile,
	const struct orlix_tcti_runtime_projection_capability *capabilities,
	size_t capability_count,
	struct orlix_tcti_runtime_projection_result *result)
{
	bool feature_present[ORLIX_TCTI_RUNTIME_PROJECTION_MAX_CAPABILITY_MAPPINGS] = {
		false,
	};
	bool feature_proved[ORLIX_TCTI_RUNTIME_PROJECTION_MAX_CAPABILITY_MAPPINGS];
	unsigned long seen_hwcap = 0;
	unsigned long seen_hwcap2 = 0;
	size_t i;

	orlix_tcti_runtime_projection_result_init(result, provider, profile);
	if (!result || !provider || !provider->read_leaf || !profile ||
	    (!capabilities && capability_count) ||
	    !provider->leaf_count)
		return -EINVAL;
	if (provider->leaf_count != ORLIX_TCTI_RUNTIME_PROJECTION_MAX_TARGET_LEAVES ||
	    !capability_count ||
	    capability_count >
		    ORLIX_TCTI_RUNTIME_PROJECTION_MAX_CAPABILITY_MAPPINGS)
		return -EINVAL;
	for (i = 0; i < capability_count; i++) {
		const struct orlix_tcti_runtime_projection_capability *mapping =
			&capabilities[i];
		unsigned long *seen;

		if (!mapping->feature || !mapping->feature[0] ||
		    !mapping->bit || (mapping->bit & (mapping->bit - 1)))
			return -EINVAL;
		if (mapping->word == ORLIX_TCTI_RUNTIME_CAPABILITY_HWCAP)
			seen = &seen_hwcap;
		else if (mapping->word == ORLIX_TCTI_RUNTIME_CAPABILITY_HWCAP2)
			seen = &seen_hwcap2;
		else
			return -EINVAL;
		if (*seen & mapping->bit)
			return -EINVAL;
		*seen |= mapping->bit;
		feature_proved[i] = true;
	}

	for (i = 0; i < provider->leaf_count; i++) {
		struct orlix_tcti_runtime_projection_leaf leaf;
		bool leaf_proved;
		size_t feature;
		size_t capability;

		if (provider->read_leaf(provider->context, i, &leaf))
			return -EINVAL;
		if (!leaf.name || !leaf.name[0] ||
		    leaf.feature_count >
			    ORLIX_TCTI_RUNTIME_PROJECTION_MAX_FEATURES_PER_LEAF ||
		    (leaf.feature_count && !leaf.features))
			return -EINVAL;
		for (feature = 0; feature < leaf.feature_count; feature++) {
			size_t previous;

			if (!leaf.features[feature] ||
			    !leaf.features[feature][0])
				return -EINVAL;
			for (previous = 0; previous < feature; previous++)
				if (!strcmp(leaf.features[feature],
					    leaf.features[previous]))
					return -EINVAL;
		}
		if (orlix_tcti_runtime_leaf_is_classified(&leaf))
			result->classified_leaf_count++;
		if (leaf.source_bound)
			result->source_bound_leaf_count++;
		leaf_proved = !leaf.unresolved_feature_semantics &&
			orlix_tcti_runtime_leaf_is_classified(&leaf) && leaf.proof &&
			leaf.proof[0] && leaf.source_bound && leaf.proved;
		if (!leaf_proved)
			result->unproved_leaf_count++;
		for (capability = 0; capability < capability_count;
		     capability++) {
			if (!orlix_tcti_runtime_leaf_has_feature(
					&leaf, capabilities[capability].feature))
				continue;
			feature_present[capability] = true;
			if (!leaf_proved)
				feature_proved[capability] = false;
		}
	}

	for (i = 0; i < capability_count; i++) {
		const struct orlix_tcti_runtime_projection_capability *mapping =
			&capabilities[i];
		unsigned long *mapped;
		unsigned long *proved;
		unsigned long advertised;
		bool cohort_present;
		bool cohort_proved;

		mapped = orlix_tcti_runtime_result_word(result, mapping->word, false);
		proved = orlix_tcti_runtime_result_word(result, mapping->word, true);
		if (!mapped || !proved || (*mapped & mapping->bit))
			return -EINVAL;

		*mapped |= mapping->bit;
		advertised =
			mapping->word == ORLIX_TCTI_RUNTIME_CAPABILITY_HWCAP ?
				result->advertised_hwcap :
				result->advertised_hwcap2;
		cohort_present = feature_present[i];
		cohort_proved = cohort_present && feature_proved[i];
		/*
		 * The capability table is an authoritative projection contract, even
		 * while the corresponding HWCAP bit is intentionally zero. A stale
		 * mapping must therefore fail before a later profile change can expose it.
		 */
		if (!cohort_present)
			result->missing_feature_mapping_count++;
		if (cohort_proved)
			*proved |= mapping->bit;
		if (!(advertised & mapping->bit)) {
			result->unadvertised_mapping_count++;
			if (!cohort_proved)
				result->unadvertised_incomplete_feature_count++;
		}
		result->mapping_count++;
	}

	result->unmapped_advertised_hwcap =
		result->advertised_hwcap & ~result->mapped_hwcap;
	result->unmapped_advertised_hwcap2 =
		result->advertised_hwcap2 & ~result->mapped_hwcap2;
	result->advertised_without_proof_hwcap =
		result->advertised_hwcap & ~result->proved_hwcap;
	result->advertised_without_proof_hwcap2 =
		result->advertised_hwcap2 & ~result->proved_hwcap2;

	if (result->unmapped_advertised_hwcap ||
	    result->unmapped_advertised_hwcap2 ||
	    result->advertised_without_proof_hwcap ||
	    result->advertised_without_proof_hwcap2 ||
	    result->missing_feature_mapping_count)
		return -EINVAL;
	return 0;
}

int orlix_tcti_runtime_projection_audit_ledger(
	const struct orlix_tcti_runtime_projection_ledger *ledger,
	const struct orlix_tcti_runtime_projection_profile *profile,
	const struct orlix_tcti_runtime_projection_capability *capabilities,
	size_t capability_count,
	struct orlix_tcti_runtime_projection_result *result)
{
	const struct orlix_tcti_runtime_projection_provider provider = {
		.context = ledger,
		.leaf_count = ledger ? ledger->leaf_count : 0,
		.read_leaf = orlix_tcti_runtime_ledger_read_leaf,
	};

	orlix_tcti_runtime_projection_result_init(result, &provider, profile);
	if (!ledger || !ledger->leaves ||
	    ledger->leaf_count != ORLIX_TCTI_RUNTIME_PROJECTION_MAX_TARGET_LEAVES ||
	    ledger->target_leaf_count !=
		ORLIX_TCTI_RUNTIME_PROJECTION_MAX_TARGET_LEAVES)
		return -EINVAL;
	return orlix_tcti_runtime_projection_audit_provider(
		&provider, profile, capabilities, capability_count, result);
}

#ifndef ORLIX_TCTI_RUNTIME_PROJECTION_HOST_TEST
static int orlix_tcti_runtime_generated_string(u32 offset, const char **text)
{
	const char *value;

	if (!text || offset >= ORLIX_TCTI_A64_GENERATED_STRING_POOL_SIZE)
		return -EINVAL;
	value = (const char *)orlix_tcti_a64_generated_strings + offset;
	if (!memchr(value, '\0', ORLIX_TCTI_A64_GENERATED_STRING_POOL_SIZE - offset))
		return -EINVAL;
	*text = value;
	return 0;
}

static int orlix_tcti_runtime_generated_read_leaf(const void *context, size_t index,
	struct orlix_tcti_runtime_projection_leaf *leaf)
{
	const struct orlix_tcti_a64_generated_row *row;
	const char *name;
	const char *condition;
	const char *proof;
	const char *const *features;
	size_t feature_count;
	bool unresolved_feature_semantics;
	enum orlix_tcti_runtime_leaf_classification classification;

	if (!context || !leaf || index >= ORLIX_TCTI_A64_GENERATED_SOURCE_COUNT ||
	    orlix_tcti_a64_generated_metadata.source_count !=
		ORLIX_TCTI_A64_GENERATED_SOURCE_COUNT)
		return -EINVAL;
	row = &orlix_tcti_a64_generated_rows[index];
	if (row->ordinal != index ||
	    orlix_tcti_runtime_generated_string(row->name, &name) ||
	    orlix_tcti_runtime_generated_string(row->condition, &condition) ||
	    orlix_tcti_runtime_generated_string(row->proof, &proof) ||
	    orlix_tcti_runtime_capability_cohort_leaf_features(row->ordinal,
		    &features, &feature_count, &unresolved_feature_semantics) ||
	    !condition[0])
		return -EINVAL;
	switch (row->classification) {
	case 0:
		classification = ORLIX_TCTI_RUNTIME_LEAF_UNCLASSIFIED;
		break;
	case 1:
		classification = ORLIX_TCTI_RUNTIME_LEAF_REQUIRED_EL0;
		break;
	case 2:
		classification = ORLIX_TCTI_RUNTIME_LEAF_NON_EL0;
		break;
	case 3:
		classification =
			ORLIX_TCTI_RUNTIME_LEAF_ARCH_UNDEFINED_OR_UNALLOCATED;
		break;
	case 4:
		classification = ORLIX_TCTI_RUNTIME_LEAF_ALIAS_OR_DUPLICATE;
		break;
	default:
		return -EINVAL;
	}
	/* Source binding is necessary for promotion, but it is not execution proof. */
	*leaf = (struct orlix_tcti_runtime_projection_leaf) {
		.name = name,
		.features = features,
		.feature_count = feature_count,
		.unresolved_feature_semantics = unresolved_feature_semantics,
		.proof = proof,
		.classification = classification,
		.source_bound = orlix_tcti_runtime_leaf_is_source_bound(row->ordinal,
			proof),
		.proved = false,
	};
	return 0;
}

int orlix_tcti_runtime_projection_audit(
	struct orlix_tcti_runtime_projection_result *result)
{
	const struct orlix_tcti_runtime_projection_provider provider = {
		.context = &orlix_tcti_a64_generated_metadata,
		.leaf_count = ORLIX_TCTI_A64_GENERATED_SOURCE_COUNT,
		.read_leaf = orlix_tcti_runtime_generated_read_leaf,
	};
	const struct orlix_tcti_runtime_projection_profile profile = {
		.hwcap = ORLIX_EL0_HWCAP,
		.hwcap2 = ORLIX_EL0_HWCAP2,
	};
	struct orlix_tcti_runtime_capability_cohort_validation_result cohort_result;

	if (orlix_tcti_runtime_capability_cohort_artifact_validate(
		    orlix_tcti_runtime_capability_cohort_artifact_canonical(),
		    &cohort_result)) {
		orlix_tcti_runtime_projection_result_init(result, &provider, &profile);
		return -EINVAL;
	}

	return orlix_tcti_runtime_projection_audit_provider(
		&provider, &profile, orlix_tcti_runtime_capability_mappings,
		ARRAY_SIZE(orlix_tcti_runtime_capability_mappings), result);
}
#endif
