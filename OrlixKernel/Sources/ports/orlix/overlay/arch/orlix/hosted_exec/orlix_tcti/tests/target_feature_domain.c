...
#define ORLIX_TCTI_FEATURE_DOMAIN_MAX_ATOM_DEPTH 256U

static int fail(struct orlix_tcti_feature_domain_diagnostic *diagnostic,
...
static int tcnd_hex_byte(const char *hex, size_t byte_index,
	unsigned char *value)
{
	unsigned int high;
	unsigned int low;

	if (!hex || tcnd_hex_nibble(hex[byte_index * 2U], &high) ||
	    tcnd_hex_nibble(hex[byte_index * 2U + 1U], &low))
		return -1;
	*value = (unsigned char)((high << 4) | low);
	return 0;
}

static int tcnd_hex_equals_bytes(const char *hex, size_t byte_offset,
	size_t byte_length, const char *text, size_t text_length)
{
	size_t index;

	if (!hex || !text || text_length != byte_length)
		return 0;
	for (index = 0; index < byte_length; index++) {
		unsigned char byte;

		if (tcnd_hex_byte(hex, byte_offset + index, &byte) ||
...
static int operand_shape_valid(
...
{
	orlix_tcti_feature_artifact_u32 field_mask;
	...
	field_mask = operand->width == 32U ?
		(orlix_tcti_feature_artifact_u32)~0U :
		(((orlix_tcti_feature_artifact_u32)1U << operand->width) - 1U) <<
			operand->start;
...
static int operand_condition_matches_leaf(
	const struct orlix_tcti_target_instruction_artifact_leaf *leaf,
	orlix_tcti_feature_artifact_u32 condition_offset,
	orlix_tcti_feature_artifact_u32 condition_length)
{
...
static int artifact_operand_name(
	...
	orlix_tcti_feature_artifact_u32 offset, const char **name, size_t *length)
{
	const char *end;
...
static int fixed_operand_locator_valid(
	...
	const struct orlix_tcti_target_instruction_artifact_fixed_operand *operand)
{
	const struct orlix_tcti_target_instruction_artifact *canonical =
	...
	if (!artifact || !canonical || !operand || !artifact->source_sha256 ||
	    ...
	    operand->source_offset > (orlix_tcti_feature_artifact_u32)~0U -
...
static void encode_operand_value(
	...
	orlix_tcti_feature_artifact_u32 value, orlix_tcti_feature_artifact_u8 start,
	orlix_tcti_feature_artifact_u8 width, const char **text, size_t *length)
{
	size_t index;
	...
	const char *hex)
{
	...
	for (index = 0; index < leaf->condition_length; index++) {
		...
		if (tcnd_hex_byte(hex, index, &byte) ||
		    byte != artifact->condition_pool[leaf->condition_offset + index])
			return 0;
	}
	return 1;
}
...
{
	struct orlix_tcti_target_instruction_operand_assignment *assignment = context;
	const struct orlix_tcti_target_instruction_artifact *artifact;
	const struct orlix_tcti_target_instruction_artifact_leaf *leaf;
	const struct orlix_tcti_target_instruction_artifact_operand *variable = NULL;
	const struct orlix_tcti_target_instruction_artifact_fixed_operand *fixed = NULL;
	size_t index;

	if (!assignment || !value || !value_length ||
	    !(artifact = assignment->artifact) ||
	    assignment->leaf_index >= artifact->leaf_count)
		return -1;
	leaf = &artifact->leaves[assignment->leaf_index];
	if (!tcnd_hex_matches_leaf_condition(artifact, leaf, tcnd_hex) ||
	    leaf->operand_first > artifact->operand_count ||
	    leaf->operand_count > artifact->operand_count - leaf->operand_first ||
	    (assignment->instruction & leaf->encoding_mask) !=
		leaf->encoding_pattern)
		return -1;
	for (index = leaf->operand_first;
	     index < leaf->operand_first + leaf->operand_count; index++) {
		const struct orlix_tcti_target_instruction_artifact_operand *operand =
...
static int reference_valid(const struct orlix_tcti_feature_artifact *artifact,
			   orlix_tcti_feature_artifact_u32 reference)
{
	return reference != ORLIX_TCTI_FEATURE_ARTIFACT_NODE_NONE &&
		reference < artifact->counts.node_count;
}

static int child_valid(const struct orlix_tcti_feature_artifact *artifact,
...
static int scalar(const struct orlix_tcti_feature_domain_value *value)
{
	return value->kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_BOOL ||
		value->kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_SIGNED ||
		value->kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_UNSIGNED ||
		value->kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_ATOM ||
		value->kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_DOT_ATOM;
}

static int integer_valid(const struct orlix_tcti_feature_domain_integer *integer);
static int integer_negative(const struct orlix_tcti_feature_domain_integer *integer);

static int callback_value_valid(const struct orlix_tcti_feature_domain_value *value)
{
	return value && ((value->kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_BOOL &&
		 value->boolean <= 1U) ||
		((value->kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_SIGNED ||
		  value->kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_UNSIGNED) &&
...
static orlix_tcti_feature_artifact_u64 width_mask(orlix_tcti_feature_artifact_u8 width)
{
	if (width >= 64U)
		return ~(orlix_tcti_feature_artifact_u64)0;
	return ((orlix_tcti_feature_artifact_u64)1U << width) - 1U;
}
...
static int integer_unsigned_order(const struct orlix_tcti_feature_domain_integer *left,
	const struct orlix_tcti_feature_domain_integer *right)
{
...
static void integer_extend(const struct orlix_tcti_feature_domain_integer *source,
	orlix_tcti_feature_artifact_u8 width, int sign_extend,
	struct orlix_tcti_feature_domain_integer *result)
{
	*result = *source;
	result->width = width;
	if (sign_extend && integer_negative(source) && width != source->width &&
		...
		if (width > 64U)
			result->high = ~(orlix_tcti_feature_artifact_u64)0;
...
static int integer_negative(const struct orlix_tcti_feature_domain_integer *integer)
{
	orlix_tcti_feature_artifact_u8 bit = integer->width - 1U;

	if (bit >= 64U)
		return !!(integer->high & ((orlix_tcti_feature_artifact_u64)1U <<
			(bit - 64U)));
	return !!(integer->low & ((orlix_tcti_feature_artifact_u64)1U << bit));
}
...
{
	struct orlix_tcti_feature_domain_integer integer = { 0 };
	size_t index;
	...
	if (index == 1U || index - 1U > 128U || text[index] != '\'' ||
	    text[index + 1U])
		return -1;
	integer.width = (orlix_tcti_feature_artifact_u8)(index - 1U);
...
int orlix_tcti_feature_domain_parse_uint_literal(const char *text,
	struct orlix_tcti_feature_domain_value *value)
{
	return parse_integer_literal(text, ORLIX_TCTI_FEATURE_DOMAIN_VALUE_UNSIGNED,
		value);
}
...
int orlix_tcti_feature_domain_parse_sint_literal(const char *text,
	struct orlix_tcti_feature_domain_value *value)
{
	return parse_integer_literal(text, ORLIX_TCTI_FEATURE_DOMAIN_VALUE_SIGNED,
		value);
}

static int atom_node_length(const struct orlix_tcti_feature_artifact *artifact,
			    orlix_tcti_feature_artifact_u32 index,
			    orlix_tcti_feature_artifact_u32 depth,
			    orlix_tcti_feature_artifact_u64 *length)
{
	const struct orlix_tcti_feature_artifact_node *node;
	orlix_tcti_feature_artifact_u32 child;

	if (depth >= ORLIX_TCTI_FEATURE_DOMAIN_MAX_ATOM_DEPTH ||
	    index >= artifact->counts.node_count)
		return -1;
	node = &artifact->nodes[index];
	if (node->kind == ORLIX_TCTI_FEATURE_ARTIFACT_VALUE) {
		if (!present(node->text))
			return -1;
		*length = strlen(node->text);
		return 0;
	}
	if (node->kind != ORLIX_TCTI_FEATURE_ARTIFACT_DOT_ATOM || !node->child_count)
		return -1;
	*length = 0;
	for (child = 0; child < node->child_count; child++) {
		orlix_tcti_feature_artifact_u64 part;
		...
		if (atom_node_length(artifact, target, depth + 1U, &part) ||
		    (child && *length == ~(orlix_tcti_feature_artifact_u64)0) ||
		    *length > ~(orlix_tcti_feature_artifact_u64)0 - part - (child ? 1U : 0U))
			return -1;
...
static int atom_node_byte(const struct orlix_tcti_feature_artifact *artifact,
			  orlix_tcti_feature_artifact_u32 index,
			  orlix_tcti_feature_artifact_u64 offset,
			  orlix_tcti_feature_artifact_u32 depth, unsigned char *byte)
{
	const struct orlix_tcti_feature_artifact_node *node;
	orlix_tcti_feature_artifact_u32 child;
...
static int atom_length(const struct orlix_tcti_feature_artifact *artifact,
		       const struct orlix_tcti_feature_domain_value *value,
		       orlix_tcti_feature_artifact_u64 *length)
{
	if (value->kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_ATOM) {
		if (!present(value->text))
			return -1;
		*length = strlen(value->text);
		return 0;
	}
	if (value->kind != ORLIX_TCTI_FEATURE_DOMAIN_VALUE_DOT_ATOM)
		return -1;
	return atom_node_length(artifact, value->node_index, 0, length);
}

static int atom_byte(const struct orlix_tcti_feature_artifact *artifact,
		     const struct orlix_tcti_feature_domain_value *value,
		     orlix_tcti_feature_artifact_u64 offset, unsigned char *byte)
{
	if (value->kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_ATOM) {
		size_t length;
...
static int atom_equal(const struct orlix_tcti_feature_artifact *artifact,
		      const struct orlix_tcti_feature_domain_value *left,
		      const struct orlix_tcti_feature_domain_value *right, int *equal)
{
	orlix_tcti_feature_artifact_u64 left_length;
	orlix_tcti_feature_artifact_u64 right_length;
	orlix_tcti_feature_artifact_u64 index;
...
static int values_equal(const struct orlix_tcti_feature_artifact *artifact,
			const struct orlix_tcti_feature_domain_value *left,
			const struct orlix_tcti_feature_domain_value *right, int *equal)
{
	if (!scalar(left) || !scalar(right))
		return -1;
	if ((left->kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_ATOM ||
	     left->kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_DOT_ATOM) &&
	...
	const struct orlix_tcti_feature_domain_value *right, int *order)
{
	...
	struct orlix_tcti_feature_domain_integer normalized_left;
	struct orlix_tcti_feature_domain_integer normalized_right;

	if (!left || !right || !order || left->kind != right->kind ||
	    (left->kind != ORLIX_TCTI_FEATURE_DOMAIN_VALUE_SIGNED &&
	...
	if (left->kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_UNSIGNED) {
...
static int evaluate_node(const struct orlix_tcti_feature_artifact *artifact,
			 orlix_tcti_feature_artifact_u32 index,
			 ...
			 struct orlix_tcti_feature_domain_diagnostic *diagnostic)
{
	...
	if (index >= artifact->counts.node_count)
		return fail(diagnostic, ORLIX_TCTI_FEATURE_DOMAIN_REFERENCE, index);
	if (depth >= scratch->max_depth)
		return fail(diagnostic, ORLIX_TCTI_FEATURE_DOMAIN_DEPTH, index);
	if (scratch->active[index])
		return fail(diagnostic, ORLIX_TCTI_FEATURE_DOMAIN_CYCLE, index);
	...
	*value = (struct orlix_tcti_feature_domain_value) { 0 };
	...
	case ORLIX_TCTI_FEATURE_ARTIFACT_BOOL:
		...
		value->boolean = (orlix_tcti_feature_artifact_u8)node->integer;
		break;
	...
	case ORLIX_TCTI_FEATURE_ARTIFACT_INTEGER:
		value->kind = ORLIX_TCTI_FEATURE_DOMAIN_VALUE_SIGNED;
		value->integer = (struct orlix_tcti_feature_domain_integer) {
			.low = (orlix_tcti_feature_artifact_u64)node->integer,
			.width = 64U,
		};
		break;
	...
	case ORLIX_TCTI_FEATURE_ARTIFACT_SET:
		if (!node->child_count)
			goto node_error;
		value->kind = ORLIX_TCTI_FEATURE_DOMAIN_VALUE_SET;
		value->node_index = index;
		break;
	...
	case ORLIX_TCTI_FEATURE_ARTIFACT_SINT:
		...
		if (node->kind == ORLIX_TCTI_FEATURE_ARTIFACT_UINT) {
			if (left.kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_UNSIGNED)
				*value = left;
			else if (left.kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_SIGNED &&
				 !integer_negative(&left.integer)) {
				value->kind = ORLIX_TCTI_FEATURE_DOMAIN_VALUE_UNSIGNED;
				value->integer = left.integer;
			} else if (left.kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_ATOM &&
				   !orlix_tcti_feature_domain_parse_uint_literal(left.text, value)) {
				break;
			} else if (left.kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_SIGNED)
				goto overflow;
			else
			...
			*value = left;
		else if (left.kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_UNSIGNED) {
			value->kind = ORLIX_TCTI_FEATURE_DOMAIN_VALUE_SIGNED;
			value->integer = left.integer;
		} else if (left.kind == ORLIX_TCTI_FEATURE_DOMAIN_VALUE_ATOM &&
			   !orlix_tcti_feature_domain_parse_sint_literal(left.text, value)) {
			break;
		} else
			goto type_error;
		break;
	...
	case ORLIX_TCTI_FEATURE_ARTIFACT_IFF:
		if (!reference_valid(artifact, node->left) ||
			...
			goto reference_or_child_error;
		if (left.kind != ORLIX_TCTI_FEATURE_DOMAIN_VALUE_BOOL ||
		    right.kind != ORLIX_TCTI_FEATURE_DOMAIN_VALUE_BOOL)
			goto type_error;
		value->kind = ORLIX_TCTI_FEATURE_DOMAIN_VALUE_BOOL;
		if (node->kind == ORLIX_TCTI_FEATURE_ARTIFACT_AND)
			value->boolean = left.boolean && right.boolean;
		else if (node->kind == ORLIX_TCTI_FEATURE_ARTIFACT_OR)
			value->boolean = left.boolean || right.boolean;
		else if (node->kind == ORLIX_TCTI_FEATURE_ARTIFACT_IMPLIES)
			value->boolean = !left.boolean || right.boolean;
		else
			value->boolean = left.boolean == right.boolean;
		break;
	...
	case ORLIX_TCTI_FEATURE_ARTIFACT_GE:
		...
		if (node->kind == ORLIX_TCTI_FEATURE_ARTIFACT_EQ ||
			...
			if (orlix_tcti_feature_domain_compare_numeric(&left, &right, &relation))
				goto type_error;
			if (node->kind == ORLIX_TCTI_FEATURE_ARTIFACT_LT)
				relation = relation < 0;
			else if (node->kind == ORLIX_TCTI_FEATURE_ARTIFACT_GT)
				relation = relation > 0;
			else
				relation = relation >= 0;
		}
	...
	case ORLIX_TCTI_FEATURE_ARTIFACT_IN: {
		...
		value->kind = ORLIX_TCTI_FEATURE_DOMAIN_VALUE_BOOL;
		value->boolean = (orlix_tcti_feature_artifact_u8)matched;
		break;
	}
	default:
		goto node_error;
...
callback_value:
	scratch->active[index] = 0;
	return fail(diagnostic, ORLIX_TCTI_FEATURE_DOMAIN_CALLBACK_VALUE, index);
overflow:
	scratch->active[index] = 0;
	return fail(diagnostic, ORLIX_TCTI_FEATURE_DOMAIN_OVERFLOW, index);
type_error:
	scratch->active[index] = 0;
	return fail(diagnostic, ORLIX_TCTI_FEATURE_DOMAIN_TYPE, index);
reference_or_child_error:
	if (diagnostic && diagnostic->error != ORLIX_TCTI_FEATURE_DOMAIN_OK) {
		scratch->active[index] = 0;
		return -1;
	}
	scratch->active[index] = 0;
...
node_error:
	scratch->active[index] = 0;
	return fail(diagnostic, ORLIX_TCTI_FEATURE_DOMAIN_NODE, index);
}
...
{
	if (diagnostic)
		*diagnostic = (struct orlix_tcti_feature_domain_diagnostic) {
			.error = ORLIX_TCTI_FEATURE_DOMAIN_OK,
			.node_index = ORLIX_TCTI_FEATURE_ARTIFACT_NODE_NONE,
		};
	if (!artifact || !environment || !scratch || !value || !scratch->active ||
	    scratch->active_count < artifact->counts.node_count ||
	    !scratch->max_depth)
		return fail(diagnostic, ORLIX_TCTI_FEATURE_DOMAIN_INVALID_ARGUMENT,
			    ORLIX_TCTI_FEATURE_ARTIFACT_NODE_NONE);
	if (!artifact->nodes || !artifact->children ||
	    root >= artifact->counts.node_count)
		return fail(diagnostic, ORLIX_TCTI_FEATURE_DOMAIN_REFERENCE, root);
	...
	struct orlix_tcti_feature_domain_diagnostic *diagnostic)
{
	...
	*satisfied = 1;
	for (index = 0; index < artifact->counts.constraint_count; index++) {
		...
		if (orlix_tcti_feature_domain_evaluate(artifact,
				artifact->constraints[index].node_index, environment,
				scratch, &value, diagnostic))
			return -1;
		if (value.kind != ORLIX_TCTI_FEATURE_DOMAIN_VALUE_BOOL)
			return fail(diagnostic, ORLIX_TCTI_FEATURE_DOMAIN_TYPE,
	...
	struct orlix_tcti_feature_domain_union_result *result)
{
	...
	if (!result)
		return -1;
	*result = (struct orlix_tcti_feature_domain_union_result) {
		.first_unsupported = {
			.error = ORLIX_TCTI_FEATURE_DOMAIN_OK,
			.node_index = ORLIX_TCTI_FEATURE_ARTIFACT_NODE_NONE,
		},
	...
	for (index = 0; index < candidate_count; index++) {
		...
		if (!candidates[index].environment ||
		    orlix_tcti_feature_domain_evaluate_constraints(artifact,
...
static int tcnd_fail(struct tcnd_reader *reader,
			enum orlix_tcti_feature_domain_tcnd_error error, size_t offset)
{
		     ...
		     unsigned char *value)
{
	...
	if (offset >= reader->byte_count ||
		...
		return tcnd_fail(reader, ORLIX_TCTI_FEATURE_DOMAIN_TCND_ENCODING, offset);
	*value = (unsigned char)((high << 4) | low);
	return 0;
}

static int tcnd_u32be(struct tcnd_reader *reader, size_t *offset,
		      size_t limit, orlix_tcti_feature_artifact_u32 *value)
{
	unsigned char bytes[4];
	size_t index;

	if (*offset > limit || limit - *offset < sizeof(bytes))
		return tcnd_fail(reader, ORLIX_TCTI_FEATURE_DOMAIN_TCND_ENCODING,
				 *offset);
	for (index = 0; index < sizeof(bytes); index++)
		if (tcnd_byte(reader, *offset + index, &bytes[index]))
			return -1;
	*offset += sizeof(bytes);
	*value = ((orlix_tcti_feature_artifact_u32)bytes[0] << 24) |
		 ((orlix_tcti_feature_artifact_u32)bytes[1] << 16) |
		 ((orlix_tcti_feature_artifact_u32)bytes[2] << 8) |
		 (orlix_tcti_feature_artifact_u32)bytes[3];
	return 0;
}
	...
	*offset += (size_t)length;
		...
		return 0;
	{
		...
		if (match < 0)
			return -1;
		if (!match)
		return tcnd_fail(reader, ORLIX_TCTI_FEATURE_DOMAIN_TCND_UNKNOWN_FEATURE,
				 start);
	}
	if (reader->diagnostic)
...
{
	unsigned char tag;
	orlix_tcti_feature_artifact_u32 payload_length;
	size_t payload_end;
	...
	if (depth >= ORLIX_TCTI_TARGET_CONDITION_MAX_DEPTH || *offset >= limit ||
		...
		return tcnd_fail(reader, depth >= ORLIX_TCTI_TARGET_CONDITION_MAX_DEPTH ?
				 ORLIX_TCTI_FEATURE_DOMAIN_TCND_DEPTH :
				 ORLIX_TCTI_FEATURE_DOMAIN_TCND_ENCODING, *offset);
	...
	struct orlix_tcti_feature_domain_tcnd_diagnostic *diagnostic)
{
	...
	if (diagnostic)
		*diagnostic = (struct orlix_tcti_feature_domain_tcnd_diagnostic) {
			.error = ORLIX_TCTI_FEATURE_DOMAIN_TCND_OK,
		};
	if (!artifact || !artifact->parameters || !tcnd_hex)
		return tcnd_fail(&(struct tcnd_reader) { .diagnostic = diagnostic },
				 ORLIX_TCTI_FEATURE_DOMAIN_TCND_INVALID_ARGUMENT, 0);
	hex_length = strlen(tcnd_hex);
	if (!hex_length || (hex_length & 1U) || hex_length / 2U < sizeof(header) ||
	    hex_length / 2U > ORLIX_TCTI_TARGET_CONDITION_MAX_SERIALIZED_BYTES)
		return tcnd_fail(&(struct tcnd_reader) { .diagnostic = diagnostic },
				 ORLIX_TCTI_FEATURE_DOMAIN_TCND_ENCODING, 0);
	...
	for (index = 0; index < sizeof(header); index++) {
		...
		if (actual != header[index])
			return tcnd_fail(&reader, ORLIX_TCTI_FEATURE_DOMAIN_TCND_ENCODING, index);
	}
...
{
	orlix_tcti_feature_artifact_u32 encoded_length;
	...
	unsigned int depth, int *matched)
{
	...
	*matched = 0;
	...
	if (offset != set->set_end)
		return tcnd_fail_value(reader, ORLIX_TCTI_FEATURE_DOMAIN_TCND_ENCODING,
			offset);
	return 0;
}
	...
	const struct tcnd_value *left, const struct tcnd_value *right, int *equal)
{
	...
	if (left->kind != right->kind ||
		...
		return tcnd_fail_value(reader, ORLIX_TCTI_FEATURE_DOMAIN_TCND_TYPE, 0);
	...
	struct tcnd_value *value)
{
	...
	if (depth >= ORLIX_TCTI_TARGET_CONDITION_MAX_DEPTH || *offset >= limit ||
	    ...
	    tcnd_u32be(reader, offset, limit, &payload_length) ||
	    (size_t)payload_length > limit - *offset)
		return tcnd_fail_value(reader, depth >= ORLIX_TCTI_TARGET_CONDITION_MAX_DEPTH ?
			ORLIX_TCTI_FEATURE_DOMAIN_TCND_DEPTH : ORLIX_TCTI_FEATURE_DOMAIN_TCND_ENCODING,
			*offset);
	...
	*value = (struct tcnd_value) { 0 };
	...
	case ORLIX_TCTI_TARGET_CONDITION_FEATURE: {
		...
		if (!known)
			return tcnd_fail_value(reader,
				ORLIX_TCTI_FEATURE_DOMAIN_TCND_UNKNOWN_FEATURE, start);
		if (!environment->feature || environment->feature(environment->context,
			reader->hex, start, length, &value->boolean) ||
		    value->boolean > 1U)
			return tcnd_fail_value(reader,
				ORLIX_TCTI_FEATURE_DOMAIN_TCND_MISSING_FEATURE, start);
		value->kind = TCND_VALUE_BOOL;
		return 0;
	}
	case ORLIX_TCTI_TARGET_CONDITION_OPERAND: {
		...
		if (!environment->operand || environment->operand(environment->context,
			...
			return tcnd_fail_value(reader,
				ORLIX_TCTI_FEATURE_DOMAIN_TCND_MISSING_OPERAND, start);
	...
	case ORLIX_TCTI_TARGET_CONDITION_SET: {
		...
		while (count--) {
			...
			if (item.kind != TCND_VALUE_TEXT)
				return tcnd_fail_value(reader,
					ORLIX_TCTI_FEATURE_DOMAIN_TCND_TYPE, *offset);
		}
		if (*offset != payload_end)
			return tcnd_fail_value(reader, ORLIX_TCTI_FEATURE_DOMAIN_TCND_ENCODING,
				*offset);
	...
	case ORLIX_TCTI_TARGET_CONDITION_NOT:
		...
		if (*offset != payload_end || left.kind != TCND_VALUE_BOOL)
			return tcnd_fail_value(reader, ORLIX_TCTI_FEATURE_DOMAIN_TCND_TYPE,
				*offset);
		value->kind = TCND_VALUE_BOOL;
		value->boolean = !left.boolean;
		return 0;
	...
	case ORLIX_TCTI_TARGET_CONDITION_OR:
		...
		if (*offset != payload_end || left.kind != TCND_VALUE_BOOL ||
			...
			return tcnd_fail_value(reader, ORLIX_TCTI_FEATURE_DOMAIN_TCND_TYPE,
				*offset);
		value->kind = TCND_VALUE_BOOL;
		value->boolean = tag == ORLIX_TCTI_TARGET_CONDITION_AND ?
	...
	case ORLIX_TCTI_TARGET_CONDITION_NE: {
		...
		if (*offset != payload_end ||
			...
			return -1;
		value->kind = TCND_VALUE_BOOL;
		value->boolean = tag == ORLIX_TCTI_TARGET_CONDITION_EQ ? equal : !equal;
		return 0;
	}
	case ORLIX_TCTI_TARGET_CONDITION_IN: {
		int matched;
	...
	struct orlix_tcti_feature_domain_tcnd_diagnostic *diagnostic)
{
	...
	if (value.kind != TCND_VALUE_BOOL)
		return tcnd_fail(&reader, ORLIX_TCTI_FEATURE_DOMAIN_TCND_TYPE, offset);
	*satisfied = value.boolean;
	return 0;
}
	...
	size_t candidate_count, struct orlix_tcti_feature_domain_tcnd_union_result *result)
{
	...
	if (!result)
		return -1;
	*result = (struct orlix_tcti_feature_domain_tcnd_union_result) {
		.first_unsupported = {
			.error = ORLIX_TCTI_FEATURE_DOMAIN_TCND_OK,
		},
	};
	...
	for (index = 0; index < candidate_count; index++) {
		...
		if (!candidates[index].environment ||
		    orlix_tcti_feature_domain_evaluate_tcnd(artifact, tcnd_hex,
