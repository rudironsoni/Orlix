...
struct fixture {
	...
	struct orlix_tcti_target_instruction_artifact_leaf leaves[
		ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_LEAF_COUNT];
	struct orlix_tcti_target_instruction_artifact_operand operands[
		ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_LEAF_COUNT + 1U];
	struct orlix_tcti_target_instruction_artifact_instruction_alias instruction_aliases[
		ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_INSTRUCTION_ALIAS_COUNT];
	struct orlix_tcti_target_instruction_artifact_operation_alias operation_aliases[
				  ...
				  size_t length)
{
	...
	for (index = 0; index < ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_LEAF_COUNT;
	     index++) {
	...
	for (index = 0;
	     index < ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_INSTRUCTION_ALIAS_COUNT;
	     index++) {
	...
	unsigned int leaf_index)
{
	...
	if (leaf_index + 1U >= ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_LEAF_COUNT)
		abort();
	memmove(&fixture->operands[leaf_index + 1U],
		&fixture->operands[leaf_index],
		(ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_LEAF_COUNT - leaf_index) *
	...
	for (index = leaf_index + 1U;
	     index < ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_LEAF_COUNT; index++)
...
static void fixture_initialize(struct fixture *fixture)
{
	...
	for (index = 0; index < ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_LEAF_COUNT;
		...
		if (index < ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_OPERATION_ALIAS_COUNT)
	...
	for (index = 0; index <
		ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_INSTRUCTION_ALIAS_COUNT; index++) {
		char name[24];
		char operation[24];
	...
	for (index = 0; index <
		ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_OPERATION_ALIAS_COUNT; index++) {
		char declared[24];
	...
	fixture->artifact = (struct orlix_tcti_target_instruction_artifact) {
		...
		.operand_count = ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_LEAF_COUNT,
...
static void test_header_and_provenance_rejections(void)
{
	...
	expect_error(&fixture, ORLIX_TCTI_TARGET_INSTRUCTION_ARTIFACT_POOL_INVALID);
}
...
static void test_string_and_condition_rejections(void)
{
	...
	expect_error(&fixture, ORLIX_TCTI_TARGET_INSTRUCTION_ARTIFACT_STRING_INVALID);
	fixture_initialize(&fixture);
...
static void test_leaf_and_span_rejections(void)
{
	...
	expect_error(&fixture, ORLIX_TCTI_TARGET_INSTRUCTION_ARTIFACT_LEAF_INVALID);
	fixture_initialize(&fixture);
	fixture.leaves[17].operand_first++;
	expect_error(&fixture, ORLIX_TCTI_TARGET_INSTRUCTION_ARTIFACT_SPAN_INVALID);
	fixture_initialize(&fixture);
...
static void test_alias_rejections_and_denominator(void)
{
	...
	expect_alias_error(&fixture,
		ORLIX_TCTI_TARGET_INSTRUCTION_ARTIFACT_COUNT_MISMATCH, UINT32_MAX);
	EXPECT(fixture.artifact.leaf_count ==
		ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_LEAF_COUNT);
	...
	fixture.leaves[0].operation_offset = fixture.leaves[
		ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_OPERATION_ALIAS_COUNT].operation_offset;
	fixture.instruction_aliases[0].declared_operation_offset =
		fixture.instruction_aliases[
			ORLIX_TCTI_A64_INSTRUCTION_ARTIFACT_OPERATION_ALIAS_COUNT].declared_operation_offset;
	expect_alias_error(&fixture,
		ORLIX_TCTI_TARGET_INSTRUCTION_ARTIFACT_ALIAS_INVALID, 0);
	...
	expect_alias_error(&fixture,
		ORLIX_TCTI_TARGET_INSTRUCTION_ARTIFACT_ALIAS_IDENTITY_INVALID, 3);
	...
	expect_alias_error(&fixture,
		ORLIX_TCTI_TARGET_INSTRUCTION_ARTIFACT_ALIAS_INVALID, 3);
...
int main(void)
{
	...
	EXPECT(orlix_tcti_target_instruction_artifact_validate(NULL, &result) == -1);
	EXPECT(result.error == ORLIX_TCTI_TARGET_INSTRUCTION_ARTIFACT_INVALID_ARGUMENT);
