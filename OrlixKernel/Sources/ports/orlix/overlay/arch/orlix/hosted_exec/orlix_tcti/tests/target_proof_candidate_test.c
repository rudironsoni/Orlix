...
static int scalar_obligations_translate_without_claiming_proof(void)
{
	...
	EXPECT(orlix_tcti_target_proof_candidate_translate_scalar(
	...
	EXPECT(obligations ==
		...
		ORLIX_TCTI_TARGET_PROOF_CANDIDATE_OBLIGATION_SYSTEM_STATE));
	EXPECT(blockers ==
	       ORLIX_TCTI_TARGET_PROOF_CANDIDATE_BLOCKER_SYSTEM_STATE);
	EXPECT(orlix_tcti_target_proof_candidate_translate_scalar(
		       1U << 31, &obligations, &blockers) == -1);
	EXPECT(orlix_tcti_target_proof_candidate_translate_scalar(
		       ORLIX_TCTI_SCALAR_OBLIGATION_DECODE, NULL, &blockers) == -1);
	EXPECT(orlix_tcti_target_proof_candidate_translate_scalar(
		       ORLIX_TCTI_SCALAR_OBLIGATION_DECODE, &obligations, NULL) == -1);
	EXPECT(orlix_tcti_target_proof_candidate_translate_scalar(
...
static int lse_obligations_translate_without_claiming_proof(void)
{
	...
	EXPECT(orlix_tcti_target_proof_candidate_translate_lse(
	...
	EXPECT(orlix_tcti_target_proof_candidate_translate_lse(
		       1U << 31, &obligations, &blockers) == -1);
	EXPECT(orlix_tcti_target_proof_candidate_translate_lse(
		       ORLIX_TCTI_LSE_OPERATION_OBLIGATION_DECODE, NULL,
		       &blockers) == -1);
	EXPECT(orlix_tcti_target_proof_candidate_translate_lse(
		       ORLIX_TCTI_LSE_OPERATION_OBLIGATION_DECODE, &obligations,
		       NULL) == -1);
	EXPECT(orlix_tcti_target_proof_candidate_translate_lse(
		       0, &obligations, &blockers) == -1);
	return 0;
...
{
	enum orlix_tcti_target_proof_candidate_error error;
	const struct orlix_tcti_target_proof_candidate_set *again;
	const struct orlix_tcti_target_proof_candidate_set *set;
	...
	set = orlix_tcti_target_proof_candidates(&error);
	EXPECT(set != NULL);
	EXPECT(error == ORLIX_TCTI_TARGET_PROOF_CANDIDATE_OK);
	again = orlix_tcti_target_proof_candidates(NULL);
	EXPECT(again == set);
	EXPECT(set->state == ORLIX_TCTI_TARGET_PROOF_CANDIDATE_UNPROVED);
	EXPECT(set->groups != NULL);
	EXPECT(set->group_count > 0);
	EXPECT(set->group_count <= ORLIX_TCTI_TARGET_PROOF_CANDIDATE_MAX_GROUPS);
	EXPECT(set->bindings != NULL);
	EXPECT(set->binding_count == expected_bindings);
	EXPECT(set->binding_count <= ORLIX_TCTI_TARGET_PROOF_CANDIDATE_MAX_BINDINGS);
	EXPECT((set->blockers &
		ORLIX_TCTI_TARGET_PROOF_CANDIDATE_BLOCKER_LSE2) != 0);
	EXPECT((set->blockers &
		ORLIX_TCTI_TARGET_PROOF_CANDIDATE_BLOCKER_INCOMPLETE_PROVENANCE) != 0);
	EXPECT(set->scalar_catalog_sha256 != NULL);
	EXPECT(strcmp(set->scalar_catalog_sha256,
		      ORLIX_TCTI_SCALAR_OPERATION_CATALOG_REVIEWED_SHA256) == 0);
	EXPECT(set->lse_catalog_sha256 == NULL);

	for (group_index = 0; group_index < set->group_count; group_index++) {
		const struct orlix_tcti_target_proof_candidate_group *group =
			&set->groups[group_index];

		EXPECT(group->state == ORLIX_TCTI_TARGET_PROOF_CANDIDATE_UNPROVED);
		...
		for (binding_index = group->binding_offset;
			...
			const struct orlix_tcti_target_proof_candidate_binding *binding =
