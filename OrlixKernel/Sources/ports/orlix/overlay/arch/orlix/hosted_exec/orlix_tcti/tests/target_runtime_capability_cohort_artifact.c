...
#define ORLIX_TCTI_RUNTIME_COHORT_DEF \
	"../isa/target_runtime_capability_cohort_artifact.def"

#define ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_SOURCE(...)
#define ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_COUNTS(...)
#define ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_LEAF(i, f, c) [i] = { i, f, c },
#define ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_MEMBERSHIP(...)
static const struct orlix_tcti_runtime_capability_cohort_leaf canonical_leaves[] = {
#include ORLIX_TCTI_RUNTIME_COHORT_DEF
};
#undef ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_SOURCE
#undef ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_COUNTS
#undef ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_LEAF
#undef ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_MEMBERSHIP

#define ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_SOURCE(...)
#define ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_COUNTS(...)
#define ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_LEAF(...)
#define ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_MEMBERSHIP(i, l, p, n, d, o, z) \
	[i] = { i, l, p, n, d, o, z },
static const struct orlix_tcti_runtime_capability_cohort_membership
...
static const struct orlix_tcti_runtime_capability_cohort_artifact canonical_artifact = {
#define ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_SOURCE(a, b, r, s, ih, il, fh, fl) \
	.source = { a, b, r, s, ih, il, fh, fl },
#define ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_COUNTS(l, m, c, u) \
	.counts = { l, m, c, u },
#define ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_LEAF(...)
#define ORLIX_TCTI_A64_RUNTIME_CAPABILITY_COHORT_MEMBERSHIP(...)
#include ORLIX_TCTI_RUNTIME_COHORT_DEF
...
static int fail(
	...
	enum orlix_tcti_runtime_capability_cohort_error error, u32 leaf, u32 membership)
{
	if (result) {
		result->error = error;
...
static int source_matches(
	const struct orlix_tcti_runtime_capability_cohort_source *source)
{
	...
	return source->architecture && source->build && source->reference &&
		...
		features->counts.parameter_count ==
			ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_FEATURE_PARAMETER_COUNT;
}
	...
	struct orlix_tcti_runtime_capability_cohort_validation_result *result)
{
	...
	if (result)
		*result = (struct orlix_tcti_runtime_capability_cohort_validation_result) {
			.error = ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_VALID,
		};
	if (!artifact || !artifact->leaves || !artifact->memberships)
		return fail(result, ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_INVALID_ARGUMENT,
			    0, 0);
	if (!source_matches(&artifact->source))
		return fail(result, ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_SOURCE_MISMATCH,
			    0, 0);
	if (artifact->counts.leaf_count !=
			ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_LEAF_COUNT ||
	    artifact->counts.membership_count !=
			ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_MEMBERSHIP_COUNT ||
	    ...
	    ARRAY_SIZE(canonical_memberships) !=
			ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_MEMBERSHIP_COUNT)
		return fail(result, ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_COUNT_MISMATCH,
			    0, 0);
	...
	for (leaf_index = 0; leaf_index < artifact->counts.leaf_count;
		...
		const struct orlix_tcti_runtime_capability_cohort_leaf *canonical =
		...
		for (membership_index = expected_first;
			...
			const struct orlix_tcti_runtime_capability_cohort_membership
				*canonical_member =
...
{
	switch (error) {
	case ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_VALID:
		return "valid";
	case ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_INVALID_ARGUMENT:
		return "invalid argument";
	case ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_SOURCE_MISMATCH:
		return "source mismatch";
	case ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_COUNT_MISMATCH:
		return "count mismatch";
	case ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_LEAF_MISMATCH:
		return "leaf mismatch";
	case ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_MEMBERSHIP_MISMATCH:
		return "membership mismatch";
	case ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_FEATURE_MISMATCH:
		return "feature mismatch";
	case ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_DISPOSITION_MISMATCH:
		return "disposition mismatch";
	case ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_SOURCE_SPAN_MISMATCH:
