...
static int canonical_is_complete_and_unresolved(void)
{
	...
	if (orlix_tcti_runtime_capability_cohort_artifact_validate(artifact, &result)) {
		fprintf(stderr, "canonical cohort invalid: %s leaf=%u member=%u\n",
			orlix_tcti_runtime_capability_cohort_error_name(result.error),
			result.leaf_index, result.membership_index);
		return -1;
	}
	CHECK(result.error == ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_VALID);
...
static int canonical_leaf_view_is_bounded(void)
{
	...
	CHECK(!orlix_tcti_runtime_capability_cohort_leaf_features(
		0, &features, &feature_count, &unresolved));
	CHECK(features);
	CHECK(feature_count == 2);
	CHECK(unresolved);
	CHECK(orlix_tcti_runtime_capability_cohort_leaf_features(
		ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_LEAF_COUNT, &features,
...
{
	const struct orlix_tcti_runtime_capability_cohort_artifact *canonical =
		orlix_tcti_runtime_capability_cohort_artifact_canonical();
	struct orlix_tcti_runtime_capability_cohort_artifact artifact = *canonical;
	struct orlix_tcti_runtime_capability_cohort_validation_result result;

	artifact.source.instructions_sha256 = "stale";
	CHECK(orlix_tcti_runtime_capability_cohort_artifact_validate(&artifact,
							      &result) < 0);
	CHECK(result.error == ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_SOURCE_MISMATCH);
	artifact = *canonical;
	artifact.counts.membership_count--;
	CHECK(orlix_tcti_runtime_capability_cohort_artifact_validate(&artifact,
							      &result) < 0);
	CHECK(result.error == ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_COUNT_MISMATCH);
	artifact = *canonical;
...
static int leaf_and_membership_drift_fail(void)
{
	const struct orlix_tcti_runtime_capability_cohort_artifact *canonical =
		orlix_tcti_runtime_capability_cohort_artifact_canonical();
	struct orlix_tcti_runtime_capability_cohort_artifact artifact = *canonical;
	struct orlix_tcti_runtime_capability_cohort_leaf *leaves;
	struct orlix_tcti_runtime_capability_cohort_membership *memberships;
	struct orlix_tcti_runtime_capability_cohort_validation_result result;
	...
	CHECK(orlix_tcti_runtime_capability_cohort_artifact_validate(&artifact,
							      &result) < 0);
	CHECK(result.error == ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_LEAF_MISMATCH);
	...
	CHECK(orlix_tcti_runtime_capability_cohort_artifact_validate(&artifact,
							      &result) < 0);
	CHECK(result.error == ORLIX_TCTI_RUNTIME_CAPABILITY_COHORT_FEATURE_MISMATCH);
	memberships[0] = canonical->memberships[0];
	...
	memberships[0].disposition =
		(enum orlix_tcti_runtime_capability_cohort_disposition)1;
	CHECK(orlix_tcti_runtime_capability_cohort_artifact_validate(&artifact,
							      &result) < 0);
	CHECK(result.error ==
