...
static struct orlix_tcti_scalar_operation_catalog_entry *catalog_copy(size_t *count)
{
	const struct orlix_tcti_scalar_operation_catalog_entry *entries =
		orlix_tcti_scalar_operation_catalog_entries(count);
	struct orlix_tcti_scalar_operation_catalog_entry *copy;
...
static int catalog_is_exact_and_unproved(void)
{
	...
	entries = orlix_tcti_scalar_operation_catalog_entries(&count);
	EXPECT(entries != NULL);
	EXPECT(count == ORLIX_TCTI_SCALAR_OPERATION_CATALOG_EXPECTED_COUNT);
	EXPECT(orlix_tcti_scalar_operation_catalog_sha256(entries, count,
						    catalog_sha256) == 0);
	...
	EXPECT(!strcmp(catalog_sha256,
		       ORLIX_TCTI_SCALAR_OPERATION_CATALOG_REVIEWED_SHA256));
	if (orlix_tcti_scalar_operation_catalog_validate(entries, count, &error) < 0) {
		fprintf(stderr, "catalog validation error: %u\n", error);
		return -1;
	}
	EXPECT(error == ORLIX_TCTI_SCALAR_OPERATION_CATALOG_OK);
...
static int missing_binding_is_rejected(void)
{
	...
	EXPECT(copy != NULL);
	copy[0].leaf_name = "";
	EXPECT(orlix_tcti_scalar_operation_catalog_validate(copy, count, &error) < 0);
	EXPECT(error == ORLIX_TCTI_SCALAR_OPERATION_CATALOG_MISSING_BINDING);
	free(copy);
	return 0;
}
...
static int missing_leaf_is_rejected(void)
{
	...
	entries = orlix_tcti_scalar_operation_catalog_entries(&count);
	EXPECT(count > 1);
	EXPECT(orlix_tcti_scalar_operation_catalog_validate(entries, count - 1,
						      &error) < 0);
	EXPECT(error == ORLIX_TCTI_SCALAR_OPERATION_CATALOG_WRONG_COUNT);
	return 0;
}
...
static int duplicate_leaf_is_rejected(void)
{
	...
	EXPECT(orlix_tcti_scalar_operation_source_sha256(
		       &copy[1], copy[1].source_sha256) == 0);
	EXPECT(orlix_tcti_scalar_operation_catalog_validate(copy, count, &error) < 0);
	EXPECT(error == ORLIX_TCTI_SCALAR_OPERATION_CATALOG_DUPLICATE_LEAF);
	free(copy);
	return 0;
}
...
static int stale_source_is_rejected(void)
{
	...
	EXPECT(orlix_tcti_scalar_operation_catalog_validate(copy, count, &error) < 0);
	EXPECT(error == ORLIX_TCTI_SCALAR_OPERATION_CATALOG_STALE_SOURCE);
	free(copy);
	return 0;
}
...
static int malformed_digest_lengths_are_rejected(void)
{
	...
	EXPECT(orlix_tcti_scalar_operation_catalog_validate(copy, count, &error) < 0);
	EXPECT(error == ORLIX_TCTI_SCALAR_OPERATION_CATALOG_STALE_SOURCE);
	memcpy(copy, orlix_tcti_scalar_operation_catalog_entries(NULL),
	       count * sizeof(*copy));
...
{
	const struct orlix_tcti_scalar_operation_catalog_entry *entries;
	struct orlix_tcti_scalar_operation_catalog_entry first;
	struct orlix_tcti_scalar_operation_catalog_entry second;
	char first_sha256[ORLIX_TCTI_SCALAR_OPERATION_SHA256_HEX_SIZE];
	char second_sha256[ORLIX_TCTI_SCALAR_OPERATION_SHA256_HEX_SIZE];

	entries = orlix_tcti_scalar_operation_catalog_entries(NULL);
	EXPECT(entries != NULL);
	first = entries[0];
	second = entries[0];
	first.leaf_name = "ab";
	...
	second.leaf_name = "a";
	second.mnemonic = "bc";
	EXPECT(orlix_tcti_scalar_operation_source_sha256(&first, first_sha256) == 0);
	EXPECT(orlix_tcti_scalar_operation_source_sha256(&second, second_sha256) == 0);
...
static int coarse_family_is_rejected(void)
{
	...
	EXPECT(orlix_tcti_scalar_operation_source_sha256(
		       &copy[0], copy[0].source_sha256) == 0);
	EXPECT(orlix_tcti_scalar_operation_catalog_validate(copy, count, &error) < 0);
	EXPECT(error == ORLIX_TCTI_SCALAR_OPERATION_CATALOG_COARSE_OPERATION);
	free(copy);
	return 0;
}
...
static int unknown_exact_operation_is_rejected(void)
{
	...
	EXPECT(orlix_tcti_scalar_operation_source_sha256(
		       &copy[0], copy[0].source_sha256) == 0);
	EXPECT(orlix_tcti_scalar_operation_catalog_validate(copy, count, &error) < 0);
	EXPECT(error == ORLIX_TCTI_SCALAR_OPERATION_CATALOG_UNKNOWN_OPERATION);
	free(copy);
	return 0;
}
...
static int changed_obligations_are_rejected(void)
{
	...
	copy = catalog_copy(&count);
	EXPECT(copy != NULL);
	copy[0].obligations ^= ORLIX_TCTI_SCALAR_OBLIGATION_FLAGS;
	EXPECT(orlix_tcti_scalar_operation_catalog_validate(copy, count, &error) < 0);
	EXPECT(error == ORLIX_TCTI_SCALAR_OPERATION_CATALOG_INVALID_OBLIGATIONS);
	free(copy);
	return 0;
}
...
static int stale_candidate_provenance_is_rejected(void)
{
	...
	EXPECT(orlix_tcti_scalar_operation_catalog_validate(copy, count, &error) < 0);
	EXPECT(error == ORLIX_TCTI_SCALAR_OPERATION_CATALOG_INVALID_PROVENANCE);
	free(copy);
	return 0;
}
...
static int candidate_source_digest_mutation_is_rejected(void)
{
	struct orlix_tcti_scalar_operation_catalog_entry *copy;
	enum orlix_tcti_scalar_operation_catalog_error error;
	char mutated_sha256[ORLIX_TCTI_SCALAR_OPERATION_SHA256_HEX_SIZE];
	size_t count;
...
static int candidate_cannot_claim_proof(void)
{
	...
	EXPECT(copy != NULL);
	copy[0].proof_state =
		(enum orlix_tcti_scalar_operation_proof_state)1;
	EXPECT(orlix_tcti_scalar_operation_catalog_validate(copy, count, &error) < 0);
	EXPECT(error == ORLIX_TCTI_SCALAR_OPERATION_CATALOG_ALREADY_PROVED);
	free(copy);
	return 0;
}
