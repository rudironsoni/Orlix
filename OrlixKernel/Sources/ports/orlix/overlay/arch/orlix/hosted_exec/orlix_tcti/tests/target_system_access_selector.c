...
static int value_width(enum orlix_tcti_system_access_selector_form form,
		       enum orlix_tcti_system_access_selector_field field,
		       uint8_t *width)
{
	if (!width)
		return -EINVAL;
	if (field > ORLIX_TCTI_SYSTEM_ACCESS_SELECTOR_FIELD_OP2)
		return -EINVAL;
	if ((form == ORLIX_TCTI_SYSTEM_ACCESS_SELECTOR_SYS ||
	     form == ORLIX_TCTI_SYSTEM_ACCESS_SELECTOR_SYSL) &&
	    field == ORLIX_TCTI_SYSTEM_ACCESS_SELECTOR_FIELD_OP0)
		return -EINVAL;

	switch (field) {
	case ORLIX_TCTI_SYSTEM_ACCESS_SELECTOR_FIELD_OP0:
		*width = 2U;
		return 0;
	case ORLIX_TCTI_SYSTEM_ACCESS_SELECTOR_FIELD_OP1:
	case ORLIX_TCTI_SYSTEM_ACCESS_SELECTOR_FIELD_OP2:
		*width = 3U;
		return 0;
	case ORLIX_TCTI_SYSTEM_ACCESS_SELECTOR_FIELD_CRN:
	case ORLIX_TCTI_SYSTEM_ACCESS_SELECTOR_FIELD_CRM:
		*width = 4U;
		return 0;
	default:
		return -EINVAL;
...
static int equation_width(const struct orlix_tcti_system_access_equation_value *equation,
			  uint8_t expected_width)
{
	...
	if (!equation || !equation->identifier || !equation->identifier_length ||
	    ...
	    equation->slice_count > ORLIX_TCTI_SYSTEM_ACCESS_VALUE_MAX_SLICES)
		return -EINVAL;
	for (index = 0; index < equation->slice_count; index++) {
		const struct orlix_tcti_system_access_slice *slice =
...
static int normalize_atom(
	enum orlix_tcti_system_access_selector_field field, uint8_t expected_width,
	uint8_t lsb, const struct orlix_tcti_system_access_source_value *source,
	struct orlix_tcti_system_access_normalized_value *normalized)
{
	...
	normalized->lsb = lsb;
	switch (source->kind) {
	case ORLIX_TCTI_SYSTEM_ACCESS_SOURCE_VALUE:
		if (orlix_tcti_system_access_value_parse(source->value.literal,
				source->value.length, expected_width, &value))
			return -EINVAL;
		normalized->kind = ORLIX_TCTI_SYSTEM_ACCESS_NORMALIZED_VALUE;
		normalized->value = value.value;
		return 0;
	case ORLIX_TCTI_SYSTEM_ACCESS_SOURCE_EQUATION:
		if (equation_width(&source->equation, expected_width))
			return -EINVAL;
		normalized->kind = ORLIX_TCTI_SYSTEM_ACCESS_NORMALIZED_EQUATION;
		normalized->equation = source->equation;
		return 0;
	case ORLIX_TCTI_SYSTEM_ACCESS_SOURCE_GROUP:
	default:
		return -EINVAL;
	}
}
	...
	enum orlix_tcti_system_access_selector_field field, uint8_t *width)
{
	...
	if (orlix_tcti_system_access_selector_width(form, &selector_width))
		return -EINVAL;
	...
	struct orlix_tcti_system_access_normalized_value *normalized)
{
	...
	if (!source || !scratch || !normalized ||
	    orlix_tcti_system_access_selector_field_width(form, field, &expected_width))
		return -EINVAL;
	if (source->kind != ORLIX_TCTI_SYSTEM_ACCESS_SOURCE_GROUP)
		return normalize_atom(field, expected_width, 0U, source, normalized);
	if (!source->group.members || !source->group.member_count ||
	    source->group.member_count > ORLIX_TCTI_SYSTEM_ACCESS_VALUE_MAX_GROUP_MEMBERS ||
	...
	for (index = 0; index < source->group.member_count; index++) {
		const struct orlix_tcti_system_access_source_value *member =
			&source->group.members[index];
		struct orlix_tcti_system_access_normalized_value *output =
			&scratch->members[scratch->member_count];
		uint8_t member_width;

		if (member->kind == ORLIX_TCTI_SYSTEM_ACCESS_SOURCE_GROUP)
			goto invalid;
		if (member->kind == ORLIX_TCTI_SYSTEM_ACCESS_SOURCE_VALUE) {
		...
		} else if (member->kind == ORLIX_TCTI_SYSTEM_ACCESS_SOURCE_EQUATION) {
			if (!member->equation.slice_count ||
			    member->equation.slice_count >
				ORLIX_TCTI_SYSTEM_ACCESS_VALUE_MAX_SLICES)
				goto invalid;
			member_width = 0;
	...
	normalized->kind = ORLIX_TCTI_SYSTEM_ACCESS_NORMALIZED_GROUP;
	normalized->field = field;
	normalized->width = expected_width;
	normalized->lsb = 0U;
	...
	enum orlix_tcti_system_access_selector_form form, uint8_t *width)
{
	...
	switch (form) {
	case ORLIX_TCTI_SYSTEM_ACCESS_SELECTOR_MRS:
	case ORLIX_TCTI_SYSTEM_ACCESS_SELECTOR_MSR:
		*width = ORLIX_TCTI_SYSTEM_ACCESS_MRS_MSR_SELECTOR_WIDTH;
		return 0;
	case ORLIX_TCTI_SYSTEM_ACCESS_SELECTOR_SYS:
	case ORLIX_TCTI_SYSTEM_ACCESS_SELECTOR_SYSL:
		*width = ORLIX_TCTI_SYSTEM_ACCESS_SYS_SYSL_SELECTOR_WIDTH;
		return 0;
	default:
		return -EINVAL;
	}
	...
	orlix_tcti_system_access_selector_callback callback, void *context)
{
	...
	if (!callback || orlix_tcti_system_access_selector_width(form, &width))
		return -EINVAL;
	count = 1U << width;
