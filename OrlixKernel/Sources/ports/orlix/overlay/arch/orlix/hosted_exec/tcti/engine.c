 ...
 */
static bool tcti_decoded_requires_cssc(
...
static bool tcti_decoded_requires_fp16(
	const struct tcti_decoded_instruction *decoded)
{
...
static bool tcti_decoded_runtime_available(
	const struct tcti_decoded_instruction *decoded)
{
	if (tcti_decoded_requires_fp16(decoded))
		return decoded->decode_class == TCTI_DECODE_SIMD_VECTOR_ARITHMETIC ?
			(ELF_HWCAP & HWCAP_ASIMDHP) :
			(ELF_HWCAP & HWCAP_FPHP);
	return !tcti_decoded_requires_cssc(decoded) ||
...
static bool tcti_decoded_for_program_pc(
	const struct tcti_gadget_word *program, size_t program_words,
	...
	struct tcti_decoded_instruction *decoded)
{
	...
	if (offset + 1 + TCTI_DECODED_INSTRUCTION_WORDS > program_words)
		return false;
	...
	*instruction_count = 0;
	...
	for (count = 0; count < TCTI_MAX_BLOCK_INSTRUCTIONS; count++) {
		...
		if (decoded.decode_class == TCTI_DECODE_UNSUPPORTED ||
		    !tcti_decoded_runtime_available(&decoded))
				    ...
				    struct mm_struct *mm)
{
	...
	for (;;) {
		...
		if (block) {
			...
			enum tcti_access block_fault_access = TCTI_ACCESS_FETCH;
		...
		pr_debug("Orlix TCTI: block exec task=%s pid=%d start_pc=%#llx end_pc=%#llx before_lr=%#llx after_lr=%#llx before_sp=%#llx after_sp=%#llx insn=%#x ret=%d words=%u count=%u cached=1\n",
			...
			if (ret == -EFAULT || ret == -EACCES) {
				pr_info("Orlix TCTI: cached block fault task=%s pid=%d pc=%#llx ret=%d fault=%#lx insn=%#x code_generation=%llu words=%u\n",
		...
		if (ret == -EFAULT || ret == -EACCES) {
			fault_decoded = tcti_decoded_for_program_pc(
