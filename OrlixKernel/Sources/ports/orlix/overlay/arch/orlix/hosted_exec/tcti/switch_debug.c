...
{
	return tcti_fp64_is_nan(value) && !(value & BIT_ULL(51));
}
...
static u32 tcti_s64_to_fp32_bits(s64 value)
{
	u64 magnitude = value < 0 ? 0 - (u64)value : (u64)value;
	u32 result = tcti_u64_to_fp32_bits(magnitude);
...
static int tcti_fp32_bits_to_s32_zero(u32 value, u64 *result)
{
	bool negative = value & BIT(31);
	u32 exponent_bits = (value >> 23) & 0xffU;
	u32 fraction = value & GENMASK(22, 0);
...
static int tcti_simd_indexed_operand(
	const struct tcti_decoded_instruction *decoded, u64 result[2])
{
	u8 lane_count;
	u8 byte_offset;
	u8 word;
	...
	result[0] = tcti_replicate_simd_element(element,
						decoded->access_size);
	result[1] = result[0];
	return 0;
}
...
static u64 tcti_simd_mixed_saturating_add_lane(u64 destination, u64 source,
					       u8 bits,
...
static const u8 tcti_sm4_sbox[256] = {
...
static u32 tcti_sm4_substitute(u32 value)
{
	return (u32)tcti_sm4_sbox[value & 0xffU] |
	       (u32)tcti_sm4_sbox[(value >> 8) & 0xffU] << 8 |
	       (u32)tcti_sm4_sbox[(value >> 16) & 0xffU] << 16 |
	       (u32)tcti_sm4_sbox[value >> 24] << 24;
}
	...
	struct pt_regs *regs, const struct tcti_decoded_instruction *decoded)
{
	...
	if (decoded->simd_arithmetic_op >= TCTI_SIMD_ARITH_SHA1C &&
	    decoded->simd_arithmetic_op <= TCTI_SIMD_ARITH_SHA1SU1) {
		u64 destination[2] = {
		...
		if (decoded->simd_arithmetic_op == TCTI_SIMD_ARITH_SHA1H) {
			if (decoded->access_size != sizeof(u32) ||
			    decoded->result_size != sizeof(u32) ||
			    !decoded->simd_scalar)
				return -EOPNOTSUPP;
			tcti_write_simd_fp_register(decoded->rd, sizeof(u32),
						    ror32(n[0], 2), 0);
		} else if (decoded->simd_arithmetic_op ==
			   TCTI_SIMD_ARITH_SHA1SU1) {
			u32 temporary[4] = {
	...
	if (decoded->simd_arithmetic_op == TCTI_SIMD_ARITH_SM4E ||
	    decoded->simd_arithmetic_op == TCTI_SIMD_ARITH_SM4EKEY) {
		u64 source_n[2] = {
			current->thread.user_simd[decoded->rn * 2],
			current->thread.user_simd[decoded->rn * 2 + 1],
		};
		...
		for (round = 0; round < 4; round++) {
			u32 transformed = tcti_sm4_substitute(
	...
	if (decoded->simd_arithmetic_op >= TCTI_SIMD_ARITH_ADDHN &&
	    decoded->simd_arithmetic_op <= TCTI_SIMD_ARITH_RSUBHN) {
		u64 left[2];
		u64 right[2];
		u64 narrowed = 0;
		u8 narrow_size = decoded->access_size / 2;
		u8 lane_count;
		u8 operation = decoded->simd_arithmetic_op - TCTI_SIMD_ARITH_ADDHN;
	...
	if (decoded->simd_arithmetic_op == TCTI_SIMD_ARITH_SUQADD ||
	    decoded->simd_arithmetic_op == TCTI_SIMD_ARITH_USQADD) {
		u64 destination[2];
		u64 source[2];
		u64 result[2] = {};
		u8 lane_count;
		bool unsigned_destination = decoded->simd_arithmetic_op ==
					    TCTI_SIMD_ARITH_USQADD;
		bool saturated = false;
		...
		for (lane = 0; lane < lane_count; lane++) {
			...
			u64 lane_result = tcti_simd_mixed_saturating_add_lane(
	...
	if (decoded->simd_arithmetic_op >= TCTI_SIMD_ARITH_FABD &&
		...
		if (decoded->simd_indexed) {
			ret = tcti_simd_indexed_operand(decoded, right);
			if (ret)
				return ret;
		} else {
			right[0] = current->thread.user_simd[decoded->rm * 2];
	...
	if (decoded->simd_arithmetic_op >= TCTI_SIMD_ARITH_SQDMULL &&
		...
		if (decoded->simd_indexed) {
			...
			ret = tcti_simd_indexed_operand(decoded, indexed);
			if (ret)
				return ret;
	...
	if (decoded->simd_arithmetic_op >= TCTI_SIMD_ARITH_SMULL &&
	    decoded->simd_arithmetic_op <= TCTI_SIMD_ARITH_UMLSL) {
		u64 left;
		u64 right;
		u64 result[2] = {};
		u8 result_size = decoded->access_size * 2;
		...
		bool signed_multiply =
			decoded->simd_arithmetic_op == TCTI_SIMD_ARITH_SMULL ||
			decoded->simd_arithmetic_op == TCTI_SIMD_ARITH_SMLAL ||
			decoded->simd_arithmetic_op == TCTI_SIMD_ARITH_SMLSL;
		bool accumulate =
			decoded->simd_arithmetic_op != TCTI_SIMD_ARITH_SMULL &&
			decoded->simd_arithmetic_op != TCTI_SIMD_ARITH_UMULL;
		bool subtract =
			decoded->simd_arithmetic_op == TCTI_SIMD_ARITH_SMLSL ||
			decoded->simd_arithmetic_op == TCTI_SIMD_ARITH_UMLSL;
		int ret;
	...
	if (decoded->simd_arithmetic_op >= TCTI_SIMD_ARITH_SABD &&
	    decoded->simd_arithmetic_op <= TCTI_SIMD_ARITH_UABA) {
		u64 accumulator[2];
		u64 left[2];
		u64 right[2];
		u64 result[2] = {};
		u64 mask;
		u8 lane_count;
		u8 operation = decoded->simd_arithmetic_op - TCTI_SIMD_ARITH_SABD;
		bool is_unsigned = operation & 1U;
		bool accumulate = operation & 2U;
	...
	struct pt_regs *regs, const struct tcti_decoded_instruction *decoded)
{
	...
	if (decoded->simd_reduction_op >= TCTI_SIMD_REDUCTION_FADDP &&
	    decoded->simd_reduction_op <= TCTI_SIMD_REDUCTION_FMINV) {
		u64 source[2];
		u64 fp_result[2] = {};
		int ret;
		...
		source[0] = current->thread.user_simd[decoded->rn * 2];
		source[1] = current->thread.user_simd[decoded->rn * 2 + 1];
		ret = tcti_native_simd_fp_reduction(
	...
	struct pt_regs *regs, const struct tcti_decoded_instruction *decoded)
{
	...
	case TCTI_FP_MOVE_SIMD_HIGH_TO_GPR:
		...
		tcti_write_gpr_or_zero(regs, decoded->rd, sizeof(u64), value);
		break;
	case TCTI_FP_MOVE_GPR_TO_SIMD_HIGH:
		value = tcti_read_gpr_or_zero(regs, decoded->rn, sizeof(u64));
		tcti_write_simd_lane(decoded->rd, 1, sizeof(u64), value);
		break;
...
static int tcti_execute_fp_round(const struct tcti_decoded_instruction *decoded,
...
#define TCTI_EXECUTE_FRINT_S(instruction) \
	({ \
		...
		switch (decoded->fp1_op) {
		case TCTI_FP1_FRINTN:
			TCTI_EXECUTE_FRINT_S("frintn");
			break;
		case TCTI_FP1_FRINTP:
			TCTI_EXECUTE_FRINT_S("frintp");
			break;
		case TCTI_FP1_FRINTM:
			TCTI_EXECUTE_FRINT_S("frintm");
			break;
		case TCTI_FP1_FRINTZ:
			TCTI_EXECUTE_FRINT_S("frintz");
			break;
		case TCTI_FP1_FRINTA:
			TCTI_EXECUTE_FRINT_S("frinta");
			break;
		case TCTI_FP1_FRINTX:
			TCTI_EXECUTE_FRINT_S("frintx");
			break;
		case TCTI_FP1_FRINTI:
			TCTI_EXECUTE_FRINT_S("frinti");
			break;
		default:
			goto unsupported;
		}
#undef TCTI_EXECUTE_FRINT_S
		*result = result32;
	} else {
#define TCTI_EXECUTE_FRINT_D(instruction) \
	({ \
		asm volatile( \
			"fmov d0, %1\n" \
			instruction " d0, d0\n" \
		...
		switch (decoded->fp1_op) {
		case TCTI_FP1_FRINTN:
			TCTI_EXECUTE_FRINT_D("frintn");
			break;
		case TCTI_FP1_FRINTP:
			TCTI_EXECUTE_FRINT_D("frintp");
			break;
		case TCTI_FP1_FRINTM:
			TCTI_EXECUTE_FRINT_D("frintm");
			break;
		case TCTI_FP1_FRINTZ:
			TCTI_EXECUTE_FRINT_D("frintz");
			break;
		case TCTI_FP1_FRINTA:
			TCTI_EXECUTE_FRINT_D("frinta");
			break;
		case TCTI_FP1_FRINTX:
			TCTI_EXECUTE_FRINT_D("frintx");
			break;
		case TCTI_FP1_FRINTI:
			TCTI_EXECUTE_FRINT_D("frinti");
			break;
		default:
			goto unsupported;
		}
#undef TCTI_EXECUTE_FRINT_D
	}
	...
	struct pt_regs *regs, const struct tcti_decoded_instruction *decoded)
{
	...
	case TCTI_FP1_FRINTX:
	case TCTI_FP1_FRINTI:
		if (tcti_execute_fp_round(decoded,
...
#undef TCTI_EXECUTE_FP2_D
	...
	if (decoded->fp2_op == TCTI_FP2_FMUL) {
		...
		ret = tcti_native_simd_fp_three_same(
			TCTI_SIMD_ARITH_FMUL, false, decoded->access_size,
	...
	struct pt_regs *regs, const struct tcti_decoded_instruction *decoded)
{
	...
	if (decoded->fp_conditional &&
	    !tcti_condition_passed(regs, decoded->condition)) {
		tcti_set_nzcv_from_immediate(regs, decoded->nzcv);
		regs->pc += sizeof(u32);
		return 0;
	}
	...
	if (decoded->access_size == sizeof(u16)) {
		...
		invalid_operation =
			(decoded->fp_signal_all_nans &&
			 (tcti_fp32_is_nan(left) || tcti_fp32_is_nan(right))) ||
			tcti_fp32_is_signaling_nan(left) ||
			tcti_fp32_is_signaling_nan(right);
	} else if (decoded->access_size == sizeof(u64)) {
		result = tcti_compare_fp64(left, right);
		invalid_operation =
			(decoded->fp_signal_all_nans &&
			 (tcti_fp64_is_nan(left) || tcti_fp64_is_nan(right))) ||
			tcti_fp64_is_signaling_nan(left) ||
			tcti_fp64_is_signaling_nan(right);
	} else {
		return -EOPNOTSUPP;
	}
...
#undef TCTI_EXECUTE_FCVT_X
	...
	if (decoded->fp_int_op == TCTI_FP_INT_SCVTF &&
		...
		value = tcti_read_gpr_or_zero(regs, decoded->rn, sizeof(u64));
		if (decoded->result_size == sizeof(u32))
			result = tcti_s64_to_fp32_bits((s64)value);
		else if (decoded->result_size == sizeof(u64))
			result = tcti_s64_to_fp64_bits((s64)value);
		else
			return -EOPNOTSUPP;
		tcti_write_simd_fp_register(decoded->rd, decoded->result_size,
					    result, 0);
	...
	if ((decoded->fp_int_op == TCTI_FP_INT_FCVTZS_SIMD ||
	     decoded->fp_int_op == TCTI_FP_INT_FCVTZU_SIMD) &&
	    decoded->result_size == decoded->access_size) {
		value = current->thread.user_simd[decoded->rn * 2];
		if (decoded->access_size == sizeof(u32)) {
			if (decoded->fp_int_op == TCTI_FP_INT_FCVTZS_SIMD) {
				if (tcti_fp32_bits_to_s32_zero((u32)value, &result))
					return -EOPNOTSUPP;
			} else if (tcti_fp32_bits_to_u64_zero((u32)value,
							      &result)) {
				return -EOPNOTSUPP;
			}
		} else if (decoded->access_size == sizeof(u64)) {
			if (decoded->fp_int_op == TCTI_FP_INT_FCVTZS_SIMD) {
				if (tcti_fp64_bits_to_s64_zero(value, &result))
					return -EOPNOTSUPP;
			} else if (tcti_fp64_bits_to_u64_zero(value, &result)) {
				return -EOPNOTSUPP;
			}
		} else {
			return -EOPNOTSUPP;
