...
#define TEST_LEAF_CAPACITY (TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT + 1)

static const char *const base_features[] = { "FEAT_BASE" };
static const char *const alternative_features[] = { "FEAT_ALT" };
static const struct tcti_inventory_condition conditions[] = {
	{ "base", "IsFeatureImplemented(FEAT_BASE)", base_features, 1 },
	{ "alternative", "IsFeatureImplemented(FEAT_ALT)",
	  alternative_features, 1 },
};
static const struct tcti_inventory_capability capabilities[] = {
	{ "FEAT_BASE", TCTI_INVENTORY_HWCAP, 1U << 0 },
	{ "FEAT_ALT", TCTI_INVENTORY_HWCAP2, 1U << 3 },
};
static struct tcti_inventory_leaf leaves[TEST_LEAF_CAPACITY];
...
static void reset_leaves(size_t count)
{
	...
	for (index = 0; index < count; index++) {
		...
		leaves[index] = (struct tcti_inventory_leaf) {
			.name = names[index],
			.classification = TCTI_INVENTORY_REQUIRED_EL0,
			.condition_id = "base",
			.proof = "leaf_kunit",
			.proved = true,
		};
...
static int validate_target(struct tcti_inventory_result *result)
{
	return tcti_inventory_validate_target(
		leaves, TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT,
		conditions, 2, result);
}

static int target_count_is_fixed_and_runtime_independent(void)
{
	const struct tcti_inventory_runtime_profile profile = { .hwcap = 1U << 0 };
	struct tcti_inventory_result target_result;
	struct tcti_inventory_result runtime_result;

	reset_leaves(TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT);
	EXPECT_EQ(0, validate_target(&target_result));
	EXPECT_EQ(0, tcti_inventory_validate_runtime_projection(
		leaves, TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT,
		conditions, 2, capabilities, 2, &profile, &runtime_result));
	EXPECT_EQ(TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT,
...
static int wrong_target_counts_fail(void)
{
	...
	reset_leaves(TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT - 1);
	EXPECT_EQ(-1, tcti_inventory_validate_target(leaves,
		TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT - 1,
		conditions, 2, &result));
	EXPECT_EQ(TCTI_INVENTORY_ERROR_TARGET_COUNT, result.first_error);
	reset_leaves(TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT + 1);
	EXPECT_EQ(-1, tcti_inventory_validate_target(leaves,
		TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT + 1,
		conditions, 2, &result));
...
static int unproved_unadvertised_leaf_fails(void)
{
	...
	EXPECT_EQ(TCTI_INVENTORY_ERROR_UNPROVED_LEAF, result.first_error);
	return 0;
}
...
static int null_metadata_fails_safely(void)
{
	...
	EXPECT_EQ(-1, tcti_inventory_validate_target(NULL,
		TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT,
		conditions, 2, &result));
	EXPECT_EQ(TCTI_INVENTORY_ERROR_TARGET_COUNT, result.first_error);
	reset_leaves(TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT);
	leaves[0].proof = NULL;
	EXPECT_EQ(-1, validate_target(&result));
	EXPECT_EQ(TCTI_INVENTORY_ERROR_MISSING_PROOF, result.first_error);
...
static int alias_self_cycle_chain_and_missing_fail(void)
{
	...
	reset_leaves(TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT);
	leaves[0].classification = TCTI_INVENTORY_ALIAS_OR_DUPLICATE;
	leaves[0].alias_target = leaves[0].name;
	EXPECT_EQ(-1, validate_target(&result));
	EXPECT_EQ(TCTI_INVENTORY_ERROR_SELF_ALIAS, result.first_error);

	reset_leaves(TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT);
	leaves[0].classification = TCTI_INVENTORY_ALIAS_OR_DUPLICATE;
	leaves[1].classification = TCTI_INVENTORY_ALIAS_OR_DUPLICATE;
	leaves[0].alias_target = leaves[1].name;
	leaves[1].alias_target = leaves[0].name;
	EXPECT_EQ(-1, validate_target(&result));
	EXPECT_EQ(TCTI_INVENTORY_ERROR_ALIAS_TARGET_NOT_CANONICAL,
		  result.first_error);
	...
	EXPECT_EQ(-1, validate_target(&result));
	EXPECT_EQ(TCTI_INVENTORY_ERROR_UNKNOWN_ALIAS_TARGET, result.first_error);
	return 0;
}
...
static int aliases_require_proof_compatible_and_proved_canonical(void)
{
	...
	leaves[0].condition_id = "alternative";
	EXPECT_EQ(-1, validate_target(&result));
	EXPECT_EQ(TCTI_INVENTORY_ERROR_ALIAS_CONDITION_MISMATCH,
		  result.first_error);
	return 0;
}
...
{
	struct tcti_inventory_capability invalid = {
		.word = TCTI_INVENTORY_HWCAP,
		.mask = 1U,
	};
	const struct tcti_inventory_runtime_profile profile = { .hwcap = 1U };
	struct tcti_inventory_result result;

	reset_leaves(TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT);
	EXPECT_EQ(-1, tcti_inventory_validate_runtime_projection(leaves,
		TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT, conditions, 2,
		&invalid, 1, &profile, &result));
	EXPECT_EQ(TCTI_INVENTORY_ERROR_EMPTY_CAPABILITY_FEATURE,
		  result.first_error);
	return 0;
}
...
{
	const struct tcti_inventory_condition invalid[] = {
	...
	EXPECT_EQ(-1, tcti_inventory_validate_target(leaves,
		TCTI_INVENTORY_AUTHORITATIVE_TARGET_LEAF_COUNT, invalid, 1,
		&result));
	EXPECT_EQ(TCTI_INVENTORY_ERROR_EMPTY_CONDITION_ID, result.first_error);
	return 0;
}
