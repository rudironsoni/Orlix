...
static int reserve(struct serializer *serializer, size_t extra)
{
	...
	if (extra > TCTI_TARGET_CONDITION_MAX_SERIALIZED_BYTES -
		    serializer->length) {
		serializer->error = TCTI_TARGET_CONDITION_SERIALIZE_TOO_LARGE;
		return -1;
	}
	...
	while (capacity < needed) {
		if (capacity > TCTI_TARGET_CONDITION_MAX_SERIALIZED_BYTES / 2U) {
			capacity = TCTI_TARGET_CONDITION_MAX_SERIALIZED_BYTES;
			break;
		}
		capacity *= 2U;
	}
	...
	if (!data) {
		serializer->error = TCTI_TARGET_CONDITION_SERIALIZE_NO_MEMORY;
		return -1;
	}
...
static int tag_for_kind(enum tcti_target_expr_kind kind, uint8_t *tag)
{
	switch (kind) {
	case TCTI_TARGET_EXPR_BOOL: *tag = TCTI_TARGET_CONDITION_BOOL; return 0;
	case TCTI_TARGET_EXPR_FEATURE: *tag = TCTI_TARGET_CONDITION_FEATURE; return 0;
	case TCTI_TARGET_EXPR_OPERAND: *tag = TCTI_TARGET_CONDITION_OPERAND; return 0;
	case TCTI_TARGET_EXPR_VALUE: *tag = TCTI_TARGET_CONDITION_VALUE; return 0;
	case TCTI_TARGET_EXPR_SET: *tag = TCTI_TARGET_CONDITION_SET; return 0;
	case TCTI_TARGET_EXPR_NOT: *tag = TCTI_TARGET_CONDITION_NOT; return 0;
	case TCTI_TARGET_EXPR_AND: *tag = TCTI_TARGET_CONDITION_AND; return 0;
	case TCTI_TARGET_EXPR_OR: *tag = TCTI_TARGET_CONDITION_OR; return 0;
	case TCTI_TARGET_EXPR_EQ: *tag = TCTI_TARGET_CONDITION_EQ; return 0;
	case TCTI_TARGET_EXPR_NE: *tag = TCTI_TARGET_CONDITION_NE; return 0;
	case TCTI_TARGET_EXPR_IN: *tag = TCTI_TARGET_CONDITION_IN; return 0;
	}
	return -1;
}
				...
				size_t depth)
{
	...
	if (depth >= TCTI_TARGET_CONDITION_MAX_DEPTH) {
		serializer->error = TCTI_TARGET_CONDITION_SERIALIZE_TOO_DEEP;
		return -1;
	}
	if (index == TCTI_TARGET_EXPR_NONE ||
	    index >= serializer->inventory->expression_count) {
		serializer->error = TCTI_TARGET_CONDITION_SERIALIZE_INVALID_EXPRESSION;
		return -1;
	}
	...
	enum tcti_target_condition_serialize_error *error)
{
	...
	if (error)
		*error = TCTI_TARGET_CONDITION_SERIALIZE_OK;
	if (!inventory || !bytes) {
		if (error)
			*error = TCTI_TARGET_CONDITION_SERIALIZE_INVALID_ARGUMENT;
		return -1;
	}
...
void tcti_target_condition_bytes_destroy(struct tcti_target_condition_bytes *bytes)
{
	if (!bytes)
		return;
	free(bytes->data);
