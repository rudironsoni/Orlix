...
static enum tcti_decode_class asr_class(enum asr_family f)
{
	return f == ASR_SHIFTED	   ? TCTI_DECODE_ADD_SUB_SHIFTED_REGISTER
	       : f == ASR_EXTENDED ? TCTI_DECODE_ADD_SUB_EXTENDED_REGISTER
...
static unsigned long asr_map(struct kunit *test, u32 insn)
{
	...
	ret = tcti_write_user_data(current->mm, m, p, sizeof(p));
...
static void asr_resume_semantics(struct kunit *test)
{
	...
	for (i = 0; i < ARRAY_SIZE(leaves); i++) {
		...
		before = r;
		{
			...
			result = tcti_resume_user(current, &r, current->mm);
...
static void asr_reserved_structured_exits(struct kunit *test)
{
	...
	for (i = 0; i < ARRAY_SIZE(leaves); i++) {
		...
		r.syscallno = NO_SYSCALL;
		before = r;
		x = tcti_resume_user(current, &r, current->mm);
...
static struct kunit_case asr_cases[] = {
    ...
    {}};
struct kunit_suite tcti_add_sub_register_source_bound_test_suite = {
    .name = "orlix-tcti-add-sub-register-source-bound",
    .test_cases = asr_cases,
};
kunit_test_suite(tcti_add_sub_register_source_bound_test_suite);
