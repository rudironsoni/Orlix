...
#define TCTI_ADVSIMD_ADD_SUB_SVC 0xd4000001U
#define TCTI_ADVSIMD_ADD_SUB_RD 4U
#define TCTI_ADVSIMD_ADD_SUB_RN 5U
#define TCTI_ADVSIMD_ADD_SUB_RM 6U

struct tcti_advsimd_add_sub_leaf {
	u16 source_ordinal;
...
static u32
tcti_advsimd_add_sub_instruction(const struct tcti_advsimd_add_sub_leaf *leaf,
...
static unsigned long tcti_advsimd_add_sub_map_instruction(struct kunit *test,
							  u32 instruction)
{
	const u32 program[] = { instruction, TCTI_ADVSIMD_ADD_SUB_SVC };
...
static void tcti_advsimd_add_sub_expected(
	u64 result[2], enum tcti_simd_vector_arithmetic_op operation,
	const u64 left[2], const u64 right[2], u8 lane_bytes, u8 result_bytes)
{
	...
	for (lane = 0; lane < lane_count; lane++) {
		...
		u64 value = operation == TCTI_SIMD_ARITH_ADD ? lhs + rhs :
							       lhs - rhs;
...
static void tcti_advsimd_add_sub_seed_regs(struct pt_regs *regs,
...
static void tcti_advsimd_add_sub_source_leaves_decode(struct kunit *test)
{
	...
	KUNIT_ASSERT_EQ(test, 4U, ARRAY_SIZE(tcti_advsimd_add_sub_leaves));
	for (index = 0; index < ARRAY_SIZE(tcti_advsimd_add_sub_leaves);
	     index++) {
		const struct tcti_advsimd_add_sub_leaf *leaf =
			&tcti_advsimd_add_sub_leaves[index];
		const struct tcti_target_instruction_artifact_leaf *source =
			&artifact->leaves[leaf->source_ordinal];
		const char *source_name = tcti_advsimd_add_sub_artifact_string(
			artifact, source->name_offset);
		const char *source_operation =
			tcti_advsimd_add_sub_artifact_string(
		...
		if (leaf->scalar) {
			struct tcti_decoded_instruction decoded =
				tcti_decode_aarch64(
					tcti_advsimd_add_sub_instruction(
						leaf, 0, 3,
						TCTI_ADVSIMD_ADD_SUB_RD,
						TCTI_ADVSIMD_ADD_SUB_RN,
						TCTI_ADVSIMD_ADD_SUB_RM));
...
static void
tcti_advsimd_add_sub_reserved_vector_1d_preserves_state(struct kunit *test)
{
	size_t leaf_index;

	for (leaf_index = 2;
	     leaf_index < ARRAY_SIZE(tcti_advsimd_add_sub_leaves);
	     leaf_index++) {
		const struct tcti_advsimd_add_sub_leaf *leaf =
			&tcti_advsimd_add_sub_leaves[leaf_index];
		u32 instruction = tcti_advsimd_add_sub_instruction(
			leaf, 0, 3, TCTI_ADVSIMD_ADD_SUB_RD,
			TCTI_ADVSIMD_ADD_SUB_RN, TCTI_ADVSIMD_ADD_SUB_RM);
		unsigned long mapped =
			tcti_advsimd_add_sub_map_instruction(test, instruction);
		...
		tcti_advsimd_add_sub_seed_regs(&regs, mapped, instruction);
		before = regs;
...
static void tcti_advsimd_add_sub_source_leaves_execute(struct kunit *test)
{
	static const struct {
		u8 rd;
		u8 rn;
		...
		{ TCTI_ADVSIMD_ADD_SUB_RD, TCTI_ADVSIMD_ADD_SUB_RN,
		  TCTI_ADVSIMD_ADD_SUB_RM },
		{ TCTI_ADVSIMD_ADD_SUB_RN, TCTI_ADVSIMD_ADD_SUB_RN,
		  TCTI_ADVSIMD_ADD_SUB_RM },
		{ TCTI_ADVSIMD_ADD_SUB_RM, TCTI_ADVSIMD_ADD_SUB_RN,
		  TCTI_ADVSIMD_ADD_SUB_RM },
	};
		...
		for (q = 0; q < q_limit; q++) {
			...
			for (size = first_size; size < 4; size++) {
				...
				for (alias_index = 0;
					...
					unsigned long mapped =
						tcti_advsimd_add_sub_map_instruction(
							test, instruction);
					...
					current->thread.user_fpsr = BIT(27) |
								    BIT(4);
					tcti_advsimd_add_sub_seed_regs(
...
static struct kunit_case tcti_advsimd_add_sub_source_bound_cases[] = {
	KUNIT_CASE(tcti_advsimd_add_sub_source_leaves_decode),
	KUNIT_CASE(tcti_advsimd_add_sub_reserved_vector_1d_preserves_state),
	KUNIT_CASE(tcti_advsimd_add_sub_source_leaves_execute),
	{}
};

static struct kunit_suite tcti_advsimd_add_sub_source_bound_suite = {
	.name = "orlix-tcti-advsimd-add-sub-source-bound",
	.test_cases = tcti_advsimd_add_sub_source_bound_cases,
};

kunit_test_suite(tcti_advsimd_add_sub_source_bound_suite);
