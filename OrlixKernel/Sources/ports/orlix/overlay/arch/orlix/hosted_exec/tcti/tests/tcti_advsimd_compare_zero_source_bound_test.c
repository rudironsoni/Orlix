...
struct compare_zero_leaf {
	...
	u32 mask;
	u32 pattern;
	enum tcti_simd_vector_compare_op compare_op;
};

static const struct compare_zero_leaf compare_zero_leaves[] = {
	{ 3791U, "CMGT_asimdmisc_Z", "CMGT", "CMGT_advsimd_zero",
	  0xbf3ffc00U, 0x0e208800U, TCTI_SIMD_COMPARE_CMGT },
	{ 3792U, "CMEQ_asimdmisc_Z", "CMEQ", "CMEQ_advsimd_zero",
	  0xbf3ffc00U, 0x0e209800U, TCTI_SIMD_COMPARE_CMEQ },
	{ 3793U, "CMLT_asimdmisc_Z", "CMLT", "CMLT_advsimd",
	  0xbf3ffc00U, 0x0e20a800U, TCTI_SIMD_COMPARE_CMLT },
	{ 3824U, "CMGE_asimdmisc_Z", "CMGE", "CMGE_advsimd_zero",
	  0xbf3ffc00U, 0x2e208800U, TCTI_SIMD_COMPARE_CMGE },
	{ 3825U, "CMLE_asimdmisc_Z", "CMLE", "CMLE_advsimd",
	  0xbf3ffc00U, 0x2e209800U, TCTI_SIMD_COMPARE_CMLE },
};
...
static bool compare_zero_matches(enum tcti_simd_vector_compare_op operation,
						...
						const u32 expected[2])
{
	...
	ret = tcti_read_user_data(current->mm, address, observed, sizeof(observed));
	...
	for (index = 0; index < ARRAY_SIZE(compare_zero_leaves); index++) {
		...
		for (q = 0; q < 2; q++) {
			for (size = 0; size < 4; size++) {
				...
				KUNIT_EXPECT_EQ_MSG(test, TCTI_DECODE_SIMD_VECTOR_COMPARE,
						    decoded.decode_class,
						    "%s q=%u size=%u",
						    leaf->name, q, size);
