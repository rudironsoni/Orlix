...
#define TCTI_ADVSIMD_MIN_MAX_SVC 0xd4000001U
#define TCTI_ADVSIMD_MIN_MAX_VARIABLE_MASK 0x40df03ffU

struct tcti_advsimd_min_max_leaf {
	u16 source_ordinal;
...
static const struct tcti_advsimd_min_max_leaf tcti_advsimd_min_max_leaves[] = {
	{ 3906U, "SMAX_asimdsame_only", "SMAX", "SMAX_advsimd", 0xbf20fc00U,
	  0x0e206400U, TCTI_SIMD_ARITH_SMAX, false, false },
	{ 3907U, "SMIN_asimdsame_only", "SMIN", "SMIN_advsimd", 0xbf20fc00U,
	  0x0e206c00U, TCTI_SIMD_ARITH_SMIN, false, true },
	{ 3948U, "UMAX_asimdsame_only", "UMAX", "UMAX_advsimd", 0xbf20fc00U,
	  0x2e206400U, TCTI_SIMD_ARITH_UMAX, true, false },
	{ 3949U, "UMIN_asimdsame_only", "UMIN", "UMIN_advsimd", 0xbf20fc00U,
	  0x2e206c00U, TCTI_SIMD_ARITH_UMIN, true, true },
};

struct tcti_advsimd_min_max_context {
	struct mm_struct *mm;
...
struct tcti_advsimd_min_max_registers {
	u8 rd;
	u8 rn;
	u8 rm;
};

static const struct tcti_advsimd_min_max_registers
	tcti_advsimd_min_max_overlap_cases[] = {
		{ 3U, 4U, 5U },	   { 6U, 6U, 7U },    { 8U, 9U, 8U },
		{ 10U, 11U, 11U }, { 12U, 12U, 12U }, { 31U, 0U, 30U },
	};

static const char *tcti_advsimd_min_max_artifact_string(
...
static int tcti_advsimd_min_max_test_init(struct kunit *test)
{
	struct tcti_advsimd_min_max_context *context;
...
static void tcti_advsimd_min_max_test_exit(struct kunit *test)
{
	struct tcti_advsimd_min_max_context *context = test->priv;
...
static u32
tcti_advsimd_min_max_instruction(const struct tcti_advsimd_min_max_leaf *leaf,
...
static int tcti_advsimd_min_max_load_instruction(struct kunit *test,
						 u32 instruction)
{
	struct tcti_advsimd_min_max_context *context = test->priv;
	const u32 program[] = { instruction, TCTI_ADVSIMD_MIN_MAX_SVC };
	int ret;
...
static void tcti_advsimd_min_max_seed_regs(struct pt_regs *regs,
...
static void tcti_advsimd_min_max_expected(
	u64 result[2], const struct tcti_advsimd_min_max_leaf *leaf,
...
static void tcti_advsimd_min_max_source_bindings(struct kunit *test)
{
	...
	KUNIT_ASSERT_EQ(test, 4U, ARRAY_SIZE(tcti_advsimd_min_max_leaves));
	for (index = 0; index < ARRAY_SIZE(tcti_advsimd_min_max_leaves);
	     index++) {
		const struct tcti_advsimd_min_max_leaf *leaf =
			&tcti_advsimd_min_max_leaves[index];
		const struct tcti_target_instruction_artifact_leaf *source =
			&artifact->leaves[leaf->source_ordinal];
		const char *name = tcti_advsimd_min_max_artifact_string(
			artifact, source->name_offset);
		const char *mnemonic = tcti_advsimd_min_max_artifact_string(
			artifact, source->mnemonic_offset);
		const char *operation = tcti_advsimd_min_max_artifact_string(
		...
		KUNIT_EXPECT_EQ(test, TCTI_ADVSIMD_MIN_MAX_VARIABLE_MASK,
				~source->encoding_mask);
	}
}

static void tcti_advsimd_min_max_all_legal_fields_decode(struct kunit *test)
{
	size_t leaf_index;
	u8 q;
	u8 size;
	...
	for (leaf_index = 0;
	     leaf_index < ARRAY_SIZE(tcti_advsimd_min_max_leaves);
	     leaf_index++) {
		const struct tcti_advsimd_min_max_leaf *leaf =
			&tcti_advsimd_min_max_leaves[leaf_index];
			...
			for (size = 0; size < 3; size++) {
				for (reg = 0; reg < 32; reg++) {
					struct tcti_decoded_instruction decoded =
						tcti_decode_aarch64(
							tcti_advsimd_min_max_instruction(
								leaf, q, size,
								reg,
								(reg + 1U) &
									31U,
...
static void tcti_advsimd_min_max_reserved_forms_reject(struct kunit *test)
{
	struct tcti_advsimd_min_max_context *context = test->priv;
	size_t leaf_index;
	u8 q;
	...
	for (leaf_index = 0;
		...
		for (q = 0; q < 2; q++) {
			u32 instruction = tcti_advsimd_min_max_instruction(
				leaf, q, 3, 3, 4, 5);
			const u32 expected_program[] = {
				instruction,
				TCTI_ADVSIMD_MIN_MAX_SVC,
			};
			...
			ret = tcti_advsimd_min_max_load_instruction(
				test, instruction);
			...
			tcti_advsimd_min_max_seed_regs(
...
static void tcti_advsimd_min_max_source_leaves_resume(struct kunit *test)
{
	struct tcti_advsimd_min_max_context *context = test->priv;
			...
			for (size = 0; size < 3; size++) {
				for (overlap_index = 0;
				     ...
				     ARRAY_SIZE(
					     tcti_advsimd_min_max_overlap_cases);
				     overlap_index++) {
					const struct tcti_advsimd_min_max_registers
						*registers =
							&tcti_advsimd_min_max_overlap_cases
								[overlap_index];
					u32 instruction =
...
static struct kunit_case tcti_advsimd_min_max_source_bound_cases[] = {
	KUNIT_CASE(tcti_advsimd_min_max_source_bindings),
	KUNIT_CASE(tcti_advsimd_min_max_all_legal_fields_decode),
	KUNIT_CASE(tcti_advsimd_min_max_reserved_forms_reject),
	KUNIT_CASE(tcti_advsimd_min_max_source_leaves_resume),
	{}
};

static struct kunit_suite tcti_advsimd_min_max_source_bound_suite = {
	.name = "orlix-tcti-advsimd-integer-min-max-source-bound",
	.init = tcti_advsimd_min_max_test_init,
	.exit = tcti_advsimd_min_max_test_exit,
	.test_cases = tcti_advsimd_min_max_source_bound_cases,
};

kunit_test_suite(tcti_advsimd_min_max_source_bound_suite);
