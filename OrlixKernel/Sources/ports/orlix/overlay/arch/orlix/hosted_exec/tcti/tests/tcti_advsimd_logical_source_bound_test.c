...
#define TCTI_ADVSIMD_LOGICAL_SVC	0xd4000001U
#define TCTI_ADVSIMD_LOGICAL_VARIABLE_MASK	0x401f03ffU

struct tcti_advsimd_logical_leaf {
	u16 source_ordinal;
...
static const struct tcti_advsimd_logical_leaf tcti_advsimd_logical_leaves[] = {
	{ 3925U, "AND_asimdsame_only", "AND", "AND_advsimd",
	  0xbfe0fc00U, 0x0e201c00U, TCTI_LOGICAL_AND },
	{ 3927U, "BIC_asimdsame_only", "BIC", "BIC_advsimd_reg",
	  0xbfe0fc00U, 0x0e601c00U, TCTI_LOGICAL_BIC },
	{ 3934U, "ORR_asimdsame_only", "ORR", "ORR_advsimd_reg",
	  0xbfe0fc00U, 0x0ea01c00U, TCTI_LOGICAL_ORR },
	{ 3936U, "ORN_asimdsame_only", "ORN", "ORN_advsimd",
	  0xbfe0fc00U, 0x0ee01c00U, TCTI_LOGICAL_ORN },
	{ 3966U, "EOR_asimdsame_only", "EOR", "EOR_advsimd",
	  0xbfe0fc00U, 0x2e201c00U, TCTI_LOGICAL_EOR },
	{ 3968U, "BSL_asimdsame_only", "BSL", "BSL_advsimd",
	  0xbfe0fc00U, 0x2e601c00U, TCTI_LOGICAL_BSL },
	{ 3976U, "BIT_asimdsame_only", "BIT", "BIT_advsimd",
	  0xbfe0fc00U, 0x2ea01c00U, TCTI_LOGICAL_BIT },
	{ 3978U, "BIF_asimdsame_only", "BIF", "BIF_advsimd",
	  0xbfe0fc00U, 0x2ee01c00U, TCTI_LOGICAL_BIF },
};

struct tcti_advsimd_logical_context {
	struct mm_struct *mm;
...
struct tcti_advsimd_logical_registers {
	u8 rd;
	u8 rn;
	u8 rm;
};

static const struct tcti_advsimd_logical_registers
tcti_advsimd_logical_overlap_cases[] = {
	{ 3U, 4U, 5U },
	{ 6U, 6U, 7U },
	{ 8U, 9U, 8U },
	{ 10U, 11U, 11U },
...
static const char *tcti_advsimd_logical_artifact_string(
...
static int tcti_advsimd_logical_test_init(struct kunit *test)
{
	struct tcti_advsimd_logical_context *context;
...
static void tcti_advsimd_logical_test_exit(struct kunit *test)
{
	struct tcti_advsimd_logical_context *context = test->priv;
...
static u32 tcti_advsimd_logical_instruction(
	const struct tcti_advsimd_logical_leaf *leaf, u8 q, u8 rd, u8 rn,
	u8 rm)
{
...
static int tcti_advsimd_logical_load_instruction(
	struct kunit *test, u32 instruction)
{
	struct tcti_advsimd_logical_context *context = test->priv;
	const u32 program[] = { instruction, TCTI_ADVSIMD_LOGICAL_SVC };
	int ret;
...
static void tcti_advsimd_logical_seed_regs(struct pt_regs *regs,
...
static u64 tcti_advsimd_logical_word_expected(enum tcti_logical_op operation,
...
static void tcti_advsimd_logical_source_bindings(struct kunit *test)
{
	...
	KUNIT_ASSERT_EQ(test, 8U, ARRAY_SIZE(tcti_advsimd_logical_leaves));
	for (index = 0; index < ARRAY_SIZE(tcti_advsimd_logical_leaves);
	     index++) {
		const struct tcti_advsimd_logical_leaf *leaf =
			&tcti_advsimd_logical_leaves[index];
		const struct tcti_target_instruction_artifact_leaf *source =
			&artifact->leaves[leaf->source_ordinal];
		const char *name = tcti_advsimd_logical_artifact_string(
			artifact, source->name_offset);
		const char *mnemonic = tcti_advsimd_logical_artifact_string(
			artifact, source->mnemonic_offset);
		const char *operation = tcti_advsimd_logical_artifact_string(
		...
		KUNIT_EXPECT_EQ(test, TCTI_ADVSIMD_LOGICAL_VARIABLE_MASK,
				~source->encoding_mask);
	}
}

static void tcti_advsimd_logical_all_legal_fields_decode(struct kunit *test)
{
	size_t leaf_index;
	u8 q;
	u8 reg;
	...
	for (leaf_index = 0;
	     leaf_index < ARRAY_SIZE(tcti_advsimd_logical_leaves);
	     leaf_index++) {
		const struct tcti_advsimd_logical_leaf *leaf =
			&tcti_advsimd_logical_leaves[leaf_index];

		for (q = 0; q < 2; q++) {
			for (reg = 0; reg < 32; reg++) {
				struct tcti_decoded_instruction decoded =
					tcti_decode_aarch64(
						tcti_advsimd_logical_instruction(
							leaf, q, reg,
							(reg + 1U) & 31U,
							(reg + 2U) & 31U));
...
static void tcti_advsimd_logical_source_leaves_resume(struct kunit *test)
{
	struct tcti_advsimd_logical_context *context = test->priv;
	size_t leaf_index;
	u8 q;
	...
	for (leaf_index = 0;
		...
		for (q = 0; q < 2; q++) {
			for (overlap_index = 0;
			     overlap_index <
				ARRAY_SIZE(tcti_advsimd_logical_overlap_cases);
			     overlap_index++) {
				const struct tcti_advsimd_logical_registers *registers =
					&tcti_advsimd_logical_overlap_cases[
						overlap_index];
				u32 instruction = tcti_advsimd_logical_instruction(
					leaf, q, registers->rd, registers->rn,
					registers->rm);
				const u32 expected_program[] = {
					instruction, TCTI_ADVSIMD_LOGICAL_SVC,
				};
				...
				ret = tcti_advsimd_logical_load_instruction(test,
								       instruction);
				...
				for (word = 0; word < (q ? 2U : 1U); word++) {
					...
					expected_simd[registers->rd * 2U + word] =
						tcti_advsimd_logical_word_expected(
				...
				tcti_advsimd_logical_seed_regs(
				...
				KUNIT_EXPECT_EQ_MSG(test, TCTI_EXIT_SYSCALL,
						    result.reason,
						    "%s q=%u overlap=%zu",
						    leaf->source_name, q,
						    overlap_index);
				...
				KUNIT_EXPECT_EQ(test,
						...
						result.pc);
				KUNIT_EXPECT_EQ(test, TCTI_ADVSIMD_LOGICAL_SVC,
						result.instruction);
...
static struct kunit_case tcti_advsimd_logical_source_bound_cases[] = {
	KUNIT_CASE(tcti_advsimd_logical_source_bindings),
	KUNIT_CASE(tcti_advsimd_logical_all_legal_fields_decode),
	KUNIT_CASE(tcti_advsimd_logical_source_leaves_resume),
	{}
};

static struct kunit_suite tcti_advsimd_logical_source_bound_suite = {
	.name = "orlix-tcti-advsimd-logical-source-bound",
	.init = tcti_advsimd_logical_test_init,
	.exit = tcti_advsimd_logical_test_exit,
	.test_cases = tcti_advsimd_logical_source_bound_cases,
};

kunit_test_suite(tcti_advsimd_logical_source_bound_suite);
