...
struct advsimd_mul_leaf {
	...
	u32 mask;
	u32 pattern;
	enum tcti_simd_vector_arithmetic_op arithmetic_op;
};
...
static const struct advsimd_mul_leaf advsimd_mul_leaves[] = {
	{ 3912U, "MLA_asimdsame_only", "MLA", "MLA_advsimd_vec",
	  0xbf20fc00U, 0x0e209400U, TCTI_SIMD_ARITH_MLA },
	{ 3913U, "MUL_asimdsame_only", "MUL", "MUL_advsimd_vec",
	  0xbf20fc00U, 0x0e209c00U, TCTI_SIMD_ARITH_MUL },
	{ 3954U, "MLS_asimdsame_only", "MLS", "MLS_advsimd_vec",
	  0xbf20fc00U, 0x2e209400U, TCTI_SIMD_ARITH_MLS },
};
				    ...
				    const u32 expected[2])
{
	...
	KUNIT_ASSERT_EQ(test, 0, tcti_read_user_data(current->mm, address,
				 ...
				 u8 lane_bytes, u8 result_bytes)
{
	...
	for (lane = 0; lane < result_bytes / lane_bytes; lane++) {
		...
		u64 value = operation == TCTI_SIMD_ARITH_MLA ? acc + product :
			operation == TCTI_SIMD_ARITH_MLS ? acc - product : product;
		...
		for (q = 0; q < 2; q++)
			for (size = 0; size < 4; size++) {
				struct tcti_decoded_instruction decoded =
					tcti_decode_aarch64(advsimd_mul_instruction(leaf, q, size,
										  4, 5, 6));
