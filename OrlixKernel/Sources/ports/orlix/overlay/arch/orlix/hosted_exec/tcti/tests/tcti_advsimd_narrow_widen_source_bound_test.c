 ...
 */
#define TCTI_TEST_XTN_MASK		0xbf3ffc00U
#define TCTI_TEST_XTN_PATTERN		0x0e212800U
#define TCTI_TEST_SHLL_MASK		0xbf3ffc00U
#define TCTI_TEST_SHLL_PATTERN		0x2e213800U
#define TCTI_TEST_SHIFT_LONG_MASK	0xbf80fc00U
#define TCTI_TEST_SSHLL_PATTERN		0x0f00a400U
#define TCTI_TEST_USHLL_PATTERN		0x2f00a400U
#define TCTI_TEST_SQXTN_PATTERN		0x0e214800U
#define TCTI_TEST_SQXTUN_PATTERN		0x0e212800U
#define TCTI_TEST_SCALAR_SQXTN_PATTERN	0x5e214800U
#define TCTI_TEST_SCALAR_SQXTUN_PATTERN	0x5e212800U
#define TCTI_TEST_SATURATING_NARROW_MASK	0x9f3ffc00U
#define TCTI_TEST_SCALAR_SATURATING_NARROW_MASK	0xdf3ffc00U
...
{
	struct tcti_advsimd_narrow_widen_context *context;
...
static void tcti_advsimd_narrow_widen_test_exit(struct kunit *test)
{
...
static int tcti_test_load_instruction(struct kunit *test, u32 instruction)
{
	struct tcti_advsimd_narrow_widen_context *context = test->priv;
	const u32 program[] = { instruction, TCTI_TEST_SVC };
	int ret;
...
tcti_test_resume_instruction(struct kunit *test, struct pt_regs *regs,
				     u32 instruction)
{
	struct tcti_advsimd_narrow_widen_context *context = test->priv;
	int ret;
...
static u64 tcti_test_read_lane(const u64 words[2], u8 size, u8 lane)
{
	u8 byte = lane * size;
	u8 word = byte / sizeof(u64);
	u8 shift = (byte % sizeof(u64)) * 8;

	return (words[word] >> shift) & tcti_test_lane_mask(size);
}

static void tcti_test_write_lane(u64 words[2], u8 size, u8 lane, u64 value)
{
	u8 byte = lane * size;
	u8 word = byte / sizeof(u64);
	u8 shift = (byte % sizeof(u64)) * 8;
	u64 mask = tcti_test_lane_mask(size) << shift;

	words[word] = (words[word] & ~mask) |
		      ((value & tcti_test_lane_mask(size)) << shift);
}

static void tcti_test_initialize_registers(struct pt_regs *regs, u32 instruction)
{
	unsigned int index;
...
{
	u32 instruction = TCTI_TEST_XTN_PATTERN | ((u32)size << 22) |
	...
	regs.pc = ((struct tcti_advsimd_narrow_widen_context *)test->priv)->instructions;
	regs.pstate |= PSR_MODE_EL0t;
	regs.syscallno = NO_SYSCALL;
	before_regs = regs;
	...
	for (lane = 0; lane < lane_count; lane++) {
		...
		u64 value = tcti_test_read_lane(source, source_size, lane);

		tcti_test_write_lane(narrowed, result_size, lane, value);
	}
	if (upper)
	...
	tcti_test_expect_resume_success(test, &result, &before_regs, &regs);
	...
	for (size = 0; size < 3; size++) {
		for (upper = 0; upper < 2; upper++) {
			for (reg = 0; reg < 32; reg++)
				tcti_test_execute_xtn_case(
					test, size, upper, (reg * 7 + 3) & 0x1fU,
					reg);
			tcti_test_execute_xtn_case(test, size, upper, 17, 17);
		}
	}
}

static void tcti_xtn_source_leaf_rejects_reserved_size(struct kunit *test)
{
	u8 upper;
	u8 rn;
	u8 rd;
	...
	for (upper = 0; upper < 2; upper++) {
		for (rn = 0; rn < 32; rn++) {
			for (rd = 0; rd < 32; rd += 31) {
				u32 instruction = TCTI_TEST_XTN_PATTERN |
...
static void tcti_test_execute_shift_long_case(struct kunit *test, bool shll,
					      ...
					      u8 rn, u8 rd)
{
	...
	u32 pattern = shll ? TCTI_TEST_SHLL_PATTERN :
		      sign ? TCTI_TEST_SSHLL_PATTERN :
			     TCTI_TEST_USHLL_PATTERN;
	...
	for (lane = 0; lane < lane_count; lane++) {
		...
		u64 value = tcti_test_read_lane(
			source, source_size, (upper ? lane_count : 0) + lane);

		if (!shll && sign)
			value = sign_extend64(value, source_bits - 1);
		tcti_test_write_lane(result_words, 2 * source_size, lane,
	...
	exec_result = tcti_test_resume_instruction(test, &regs, instruction);

	tcti_test_expect_resume_success(test, &exec_result, &before_regs, &regs);
	KUNIT_EXPECT_MEMEQ(test, expected_simd, current->thread.user_simd,
			   sizeof(expected_simd));
	KUNIT_EXPECT_EQ(test, 1UL, current->thread.user_simd_valid);
}
		...
		for (upper = 0; upper < 2; upper++) {
			for (size = 0; size < 3; size++) {
				...
				for (shift = 0; shift < source_bits; shift++) {
					...
					tcti_test_execute_shift_long_case(
...
static void tcti_shift_long_source_leaves_reject_reserved_immediates(
	struct kunit *test)
{
	...
	for (sign = 0; sign < 2; sign++) {
		for (upper = 0; upper < 2; upper++) {
			for (immediate_index = 0;
			     ...
			     immediate_index++) {
				u32 pattern = sign ? TCTI_TEST_SSHLL_PATTERN :
						     TCTI_TEST_USHLL_PATTERN;
...
static void tcti_shll_alias_matches_unsigned_shift_long(struct kunit *test)
{
	u8 upper;
	u8 size;
	u8 reg;
	...
	for (upper = 0; upper < 2; upper++) {
		for (size = 0; size < 3; size++) {
			u32 instruction = TCTI_TEST_SHLL_PATTERN |
				((u32)size << 22) | (upper ? BIT(30) : 0) |
				(13U << 5) | 14U;
			struct tcti_decoded_instruction decoded =
				tcti_decode_aarch64(instruction);

			KUNIT_ASSERT_EQ(test, TCTI_TEST_SHLL_PATTERN,
				instruction & TCTI_TEST_SHLL_MASK);
...
static void tcti_shll_source_leaf_rejects_reserved_size(struct kunit *test)
{
	u8 upper;
	u8 rn;
...
static u64 tcti_test_saturating_narrow_lane(
	enum tcti_simd_vector_arithmetic_op operation, u8 source_size, u64 value,
	bool *saturated)
{
	...
	minimum = operation == TCTI_SIMD_ARITH_SQXTN ?
		-(s64)BIT_ULL(result_bits - 1) : 0;
	maximum = operation == TCTI_SIMD_ARITH_SQXTN ?
...
static u64 tcti_test_saturating_narrow_input(
	enum tcti_simd_vector_arithmetic_op operation, u8 source_size, u8 lane)
{
	u8 result_bits = source_size * 4;
	u64 result_mask = GENMASK_ULL(result_bits - 1, 0);
	...
	switch (lane & 3U) {
	case 0:
		return operation == TCTI_SIMD_ARITH_SQXTN ? signed_max :
			operation == TCTI_SIMD_ARITH_SQXTUN ? 0 : result_mask;
	case 1:
		return operation == TCTI_SIMD_ARITH_SQXTN ? signed_max + 1 :
			result_mask + 1;
	case 2:
		return operation == TCTI_SIMD_ARITH_SQXTN ?
			-(s64)signed_min : -1;
	default:
		return operation == TCTI_SIMD_ARITH_SQXTN ?
			-(s64)signed_min - 1 : 1;
	...
	bool scalar, bool upper, u8 size, u8 rn, u8 rd)
{
	...
	pattern = scalar ? TCTI_TEST_SCALAR_SQXTN_PATTERN :
		TCTI_TEST_SQXTN_PATTERN;
	if (operation == TCTI_SIMD_ARITH_SQXTUN)
		pattern = scalar ? TCTI_TEST_SCALAR_SQXTUN_PATTERN :
			TCTI_TEST_SQXTUN_PATTERN;
	instruction = pattern | ((u32)size << 22) |
		(operation == TCTI_SIMD_ARITH_UQXTN ? BIT(29) : 0) |
	...
	for (lane = 0; lane < 2 * sizeof(u64) / source_size; lane++)
		tcti_test_write_lane(source, source_size, lane,
			tcti_test_saturating_narrow_input(operation, source_size, lane));
	...
	for (lane = 0; lane < lane_count; lane++) {
		u64 value = tcti_test_read_lane(source, source_size, lane);
		u64 result = tcti_test_saturating_narrow_lane(operation,
...
static void tcti_saturating_narrow_source_leaves_execute_all_legal_variants(
	struct kunit *test)
{
	static const enum tcti_simd_vector_arithmetic_op operations[] = {
	...
	for (operation = 0; operation < ARRAY_SIZE(operations); operation++) {
		for (size = 0; size < 3; size++) {
			for (upper = 0; upper < 2; upper++)
				tcti_test_execute_saturating_narrow_case(test,
	...
	struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, TCTI_DECODE_UNSUPPORTED,
		tcti_decode_aarch64(TCTI_TEST_SQXTUN_PATTERN |
			(7U << 5) | 8U).decode_class);
	tcti_test_expect_rejected_instruction(test, TCTI_TEST_SQXTUN_PATTERN |
			(7U << 5) | 8U);
	KUNIT_EXPECT_EQ(test, TCTI_DECODE_UNSUPPORTED,
		tcti_decode_aarch64(TCTI_TEST_SCALAR_SQXTUN_PATTERN |
			(7U << 5) | 8U).decode_class);
	tcti_test_expect_rejected_instruction(test,
		TCTI_TEST_SCALAR_SQXTUN_PATTERN | (7U << 5) | 8U);
}

static void tcti_narrow_widen_resume_reports_instruction_fetch_fault(
	struct kunit *test)
{
	struct tcti_advsimd_narrow_widen_context *context = test->priv;
	struct pt_regs regs = {};
...
static struct kunit_case tcti_advsimd_narrow_widen_source_bound_cases[] = {
	...
	KUNIT_CASE(tcti_shift_long_source_leaves_reject_reserved_immediates),
	KUNIT_CASE(tcti_shll_alias_matches_unsigned_shift_long),
	KUNIT_CASE(tcti_shll_source_leaf_rejects_reserved_size),
	KUNIT_CASE(tcti_saturating_narrow_source_leaves_execute_all_legal_variants),
	KUNIT_CASE(tcti_saturating_narrow_source_leaves_reject_reserved_encodings),
	KUNIT_CASE(tcti_narrow_widen_resume_reports_instruction_fetch_fault),
	{}
};

static struct kunit_suite tcti_advsimd_narrow_widen_source_bound_suite = {
	.name = "orlix-tcti-advsimd-narrow-widen-source-bound",
	.init = tcti_advsimd_narrow_widen_test_init,
	.exit = tcti_advsimd_narrow_widen_test_exit,
	.test_cases = tcti_advsimd_narrow_widen_source_bound_cases,
};

kunit_test_suite(tcti_advsimd_narrow_widen_source_bound_suite);
