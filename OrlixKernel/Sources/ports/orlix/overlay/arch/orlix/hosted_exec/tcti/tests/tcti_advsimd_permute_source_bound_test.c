...
#define PERMUTE_SVC 0xd4000001U
...
struct permute_leaf {
	...
	u32 mask;
	u32 pattern;
	enum tcti_simd_element_move_op decoded_operation;
};

static const struct permute_leaf permute_leaves[] = {
	{ 3684U, "UZP1_asimdperm_only", "UZP1", "UZP1_advsimd",
	  0xbf20fc00U, 0x0e001800U, TCTI_SIMD_ELEMENT_MOVE_UZP1 },
	{ 3685U, "TRN1_asimdperm_only", "TRN1", "TRN1_advsimd",
	  0xbf20fc00U, 0x0e002800U, TCTI_SIMD_ELEMENT_MOVE_TRN1 },
	{ 3686U, "ZIP1_asimdperm_only", "ZIP1", "ZIP1_advsimd",
	  0xbf20fc00U, 0x0e003800U, TCTI_SIMD_ELEMENT_MOVE_ZIP1 },
	{ 3687U, "UZP2_asimdperm_only", "UZP2", "UZP2_advsimd",
	  0xbf20fc00U, 0x0e005800U, TCTI_SIMD_ELEMENT_MOVE_UZP2 },
	{ 3688U, "TRN2_asimdperm_only", "TRN2", "TRN2_advsimd",
	  0xbf20fc00U, 0x0e006800U, TCTI_SIMD_ELEMENT_MOVE_TRN2 },
	{ 3689U, "ZIP2_asimdperm_only", "ZIP2", "ZIP2_advsimd",
	  0xbf20fc00U, 0x0e007800U, TCTI_SIMD_ELEMENT_MOVE_ZIP2 },
};

struct permute_context {
...
static int permute_test_init(struct kunit *test)
{
	struct permute_context *context;
...
static void permute_test_exit(struct kunit *test)
{
	struct permute_context *context = test->priv;
...
static const char *permute_artifact_string(
...
static u32 permute_instruction(const struct permute_leaf *leaf, u8 q, u8 size,
			       u8 rd, u8 rn, u8 rm)
{
...
static unsigned long permute_map_instruction(struct kunit *test, u32 instruction)
{
	const u32 program[] = { instruction, PERMUTE_SVC };
...
static void permute_seed_regs(struct pt_regs *regs, unsigned long pc,
			      u32 instruction)
{
	u8 reg;
...
static u8 permute_get_byte(const u64 simd[], u8 vector, u8 byte)
{
	return (simd[vector * 2 + byte / sizeof(u64)] >>
		((byte % sizeof(u64)) * 8U)) & 0xffU;
}

static void permute_set_byte(u64 simd[], u8 vector, u8 byte, u8 value)
{
...
static void permute_oracle(const struct permute_leaf *leaf, u8 q, u8 size,
	...
	for (destination_lane = 0; destination_lane < lane_count;
		...
		case TCTI_SIMD_ELEMENT_MOVE_UZP2:
			...
			source_lane = (destination_lane % half) * 2U +
				(leaf->decoded_operation == TCTI_SIMD_ELEMENT_MOVE_UZP2);
			break;
		...
		case TCTI_SIMD_ELEMENT_MOVE_TRN2:
			...
			source_lane = (destination_lane / 2U) * 2U +
				(leaf->decoded_operation == TCTI_SIMD_ELEMENT_MOVE_TRN2);
			break;
		case TCTI_SIMD_ELEMENT_MOVE_ZIP1:
		case TCTI_SIMD_ELEMENT_MOVE_ZIP2:
			source_vector = destination_lane & 1U ? rm : rn;
			source_lane = destination_lane / 2U +
				(leaf->decoded_operation == TCTI_SIMD_ELEMENT_MOVE_ZIP2 ?
				 half : 0U);
			break;
		default:
			return;
		...
		for (byte = 0; byte < lane_bytes; byte++) {
			...
			permute_set_byte(expected, rd,
				destination_lane * lane_bytes + byte,
				permute_get_byte(before, source_vector, source_byte));
		}
	}
}

static void permute_source_binding_and_decode_domains(struct kunit *test)
{
	...
	for (leaf_index = 0; leaf_index < ARRAY_SIZE(permute_leaves);
	     leaf_index++) {
		const struct permute_leaf *leaf = &permute_leaves[leaf_index];
		const struct tcti_target_instruction_artifact_leaf *source =
			&artifact->leaves[leaf->ordinal];
		const char *name = permute_artifact_string(artifact,
			source->name_offset);
		const char *mnemonic = permute_artifact_string(artifact,
			source->mnemonic_offset);
		const char *operation = permute_artifact_string(artifact,
		...
		for (q = 0; q < 2; q++) {
			for (size = 0; size < 4; size++) {
				...
				u32 instruction = permute_instruction(leaf, q, size,
					4U, 5U, 6U);
		...
		for (reg = 0; reg < 32; reg++) {
			...
			decoded = tcti_decode_aarch64(permute_instruction(leaf, 1U, 3U,
				reg, 1U, 2U));
...
static void permute_reserved_size_rejects_without_state_change(struct kunit *test)
{
	size_t leaf_index;
	...
	for (leaf_index = 0; leaf_index < ARRAY_SIZE(permute_leaves);
		...
		for (rd = 0; rd < 32; rd++) {
			...
			rm = (rd * 13U + 5U) & 31U;
			{
				u32 instruction = permute_instruction(leaf, 0U, 3U,
					rd, rn, rm);
				const u32 expected_code[] = { instruction, PERMUTE_SVC };
				u32 observed_code[ARRAY_SIZE(expected_code)] = {};
				unsigned long mapped = permute_map_instruction(test,
					instruction);
				...
				permute_seed_regs(&regs, mapped, instruction);
				before_regs = regs;
				...
				KUNIT_EXPECT_EQ(test, 0, tcti_read_user_data(current->mm,
...
static void permute_source_leaves_execute_all_arrangements(struct kunit *test)
{
	static const struct {
		u8 rd;
		u8 rn;
			...
			for (size = 0; size < 4; size++) {
				...
				for (arrangement_index = 0;
					...
					for (index = 0; index < 16; index++) {
						permute_set_byte(current->thread.user_simd, rn,
							index, 0x10U + index);
						permute_set_byte(current->thread.user_simd, rm,
							index, 0x90U + index);
					}
					memcpy(before_simd, current->thread.user_simd,
						sizeof(before_simd));
					permute_oracle(leaf, q, size, rd, rn, rm, before_simd,
						expected_simd);
					...
					KUNIT_EXPECT_EQ(test, PERMUTE_SVC, result.instruction);
...
static struct kunit_case permute_cases[] = {
	KUNIT_CASE(permute_source_binding_and_decode_domains),
	KUNIT_CASE(permute_reserved_size_rejects_without_state_change),
	KUNIT_CASE(permute_source_leaves_execute_all_arrangements),
	{}
};

static struct kunit_suite permute_suite = {
	.name = "orlix-tcti-advsimd-permute-source-bound",
	.init = permute_test_init,
	.exit = permute_test_exit,
	.test_cases = permute_cases,
};

kunit_test_suite(permute_suite);
