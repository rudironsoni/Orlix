...
struct tcti_atomic_128_value {
	u64 low;
	u64 high;
};

struct tcti_atomic_128_case {
	enum tcti_atomic_memory_operation operation;
	struct tcti_atomic_128_value expected;
};

static unsigned long tcti_atomic_128_map(struct kunit *test, size_t size,
					 int prot)
{
...
static void tcti_atomic_128_expect_memory(
	struct kunit *test, unsigned long mapped,
	const struct tcti_atomic_128_value *expected)
{
	struct tcti_atomic_128_value observed = {};
	int ret;
...
static void tcti_atomic_128_rmw_asymmetric_lanes(struct kunit *test)
{
	const struct tcti_atomic_128_value initial = {
		.low = 0x0123456789abcdefULL,
		.high = 0xfedcba9876543210ULL,
	};
	const struct tcti_atomic_128_value operand = {
		.low = 0x0f0f55ffaa5500ffULL,
		.high = 0x33cc0ff0f00f5aa5ULL,
	};
	const struct tcti_atomic_128_case cases[] = {
		{
	...
	unsigned long mapped =
		tcti_atomic_128_map(test, PAGE_SIZE, PROT_READ | PROT_WRITE);
	size_t i;

	for (i = 0; i < ARRAY_SIZE(cases); i++) {
		struct tcti_atomic_128_value old = {};
		...
		ret = tcti_write_user_data(current->mm, mapped, &initial,
					   sizeof(initial));
		KUNIT_ASSERT_EQ(test, 0, ret);
		ret = tcti_atomic_user_data(
			current->mm, mapped, cases[i].operation,
			TCTI_ATOMIC_MEMORY_ACQ_REL, NULL, &operand, &old,
			sizeof(old), &exchanged);
		KUNIT_ASSERT_EQ(test, 0, ret);
		KUNIT_EXPECT_TRUE(test, exchanged);
		KUNIT_EXPECT_MEMEQ(test, &initial, &old, sizeof(old));
		tcti_atomic_128_expect_memory(test, mapped, &cases[i].expected);
	}

	KUNIT_EXPECT_EQ(test, 0, vm_munmap(mapped, PAGE_SIZE));
}

static void tcti_atomic_128_same_value_writes_break_reservations(
	struct kunit *test)
{
	const struct tcti_atomic_128_value initial = {
	...
	const struct tcti_atomic_128_value zero = {};
	...
	for (i = 0; i < ARRAY_SIZE(operations); i++) {
		const struct tcti_atomic_128_value *operand =
			operations[i] == TCTI_ATOMIC_MEMORY_SWP ? &initial : &zero;
		struct tcti_atomic_128_value observed = {};
		struct tcti_atomic_128_value desired = {
			.low = ~initial.low,
			.high = ~initial.high,
		};
		unsigned long pfn;
		...
		ret = tcti_atomic_user_data(
			...
			TCTI_ATOMIC_MEMORY_RELAXED, NULL, operand, &observed,
		...
		tcti_atomic_128_expect_memory(test, mapped, &initial);
	}

	KUNIT_EXPECT_EQ(test, 0, vm_munmap(mapped, PAGE_SIZE));
}

static void tcti_atomic_128_unsupported_ops_do_not_mutate(struct kunit *test)
{
	const struct tcti_atomic_128_value initial = {
	...
	KUNIT_ASSERT_EQ(test, 0, ret);
	for (i = 0; i < ARRAY_SIZE(operations); i++) {
		const struct tcti_atomic_128_value old_sentinel = {
			.low = U64_MAX,
			.high = U64_MAX,
		};
		struct tcti_atomic_128_value old = old_sentinel;
		bool exchanged = true;

		ret = tcti_atomic_user_data(
			current->mm, mapped, operations[i],
			TCTI_ATOMIC_MEMORY_RELAXED, NULL, &operand, &old,
			sizeof(old), &exchanged);
...
static void tcti_atomic_128_faults_do_not_partially_mutate(struct kunit *test)
{
	const struct tcti_atomic_128_value operand = {
		.low = U64_MAX,
		.high = U64_MAX,
	...
	u8 observed[32];
	unsigned long mapped =
		tcti_atomic_128_map(test, 2 * PAGE_SIZE,
				    PROT_READ | PROT_WRITE);
	unsigned long readonly =
		tcti_atomic_128_map(test, PAGE_SIZE, PROT_READ);
	bool exchanged = true;
	int ret;
	...
	KUNIT_ASSERT_EQ(test, 0, ret);
	ret = tcti_atomic_user_data(
		current->mm, mapped + 1, TCTI_ATOMIC_MEMORY_SET,
		TCTI_ATOMIC_MEMORY_RELAXED, NULL, &operand, &old, sizeof(old),
		&exchanged);
	...
	ret = tcti_write_user_data(current->mm, mapped + PAGE_SIZE - 16,
	...
	ret = tcti_atomic_user_data(
		...
		TCTI_ATOMIC_MEMORY_SET, TCTI_ATOMIC_MEMORY_RELAXED, NULL,
		&operand, &old, sizeof(old), &exchanged);
	KUNIT_EXPECT_EQ(test, -EFAULT, ret);
	KUNIT_EXPECT_TRUE(test, exchanged);
	KUNIT_EXPECT_MEMEQ(test, &old_sentinel, &old, sizeof(old));
	ret = tcti_read_user_data(current->mm, mapped + PAGE_SIZE - 16,
	...
	exchanged = true;
	ret = tcti_atomic_user_data(
		current->mm, readonly, TCTI_ATOMIC_MEMORY_SET,
		TCTI_ATOMIC_MEMORY_RELAXED, NULL, &operand, &old, sizeof(old),
		&exchanged);
	KUNIT_EXPECT_EQ(test, -EACCES, ret);
	KUNIT_EXPECT_TRUE(test, exchanged);
	KUNIT_EXPECT_MEMEQ(test, &old_sentinel, &old, sizeof(old));
	tcti_atomic_128_expect_memory(
		test, readonly, &(const struct tcti_atomic_128_value) {});

	KUNIT_EXPECT_EQ(test, 0, vm_munmap(readonly, PAGE_SIZE));
	KUNIT_EXPECT_EQ(test, 0, vm_munmap(mapped, 2 * PAGE_SIZE));
}

static struct kunit_case tcti_atomic_128_rmw_test_cases[] = {
	KUNIT_CASE(tcti_atomic_128_rmw_asymmetric_lanes),
	KUNIT_CASE(tcti_atomic_128_same_value_writes_break_reservations),
	KUNIT_CASE(tcti_atomic_128_unsupported_ops_do_not_mutate),
	KUNIT_CASE(tcti_atomic_128_faults_do_not_partially_mutate),
	{}
};

static struct kunit_suite tcti_atomic_128_rmw_test_suite = {
	.name = "orlix-tcti-atomic-128-rmw",
	.test_cases = tcti_atomic_128_rmw_test_cases,
};

kunit_test_suite(tcti_atomic_128_rmw_test_suite);
