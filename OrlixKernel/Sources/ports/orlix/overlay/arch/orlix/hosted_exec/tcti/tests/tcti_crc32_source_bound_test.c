...
static void crc_source_leaves_bind_canonical_artifacts(struct kunit *test)
{
	...
	for (index = 0; index < ARRAY_SIZE(crc_source_leaves); index++) {
		...
		struct tcti_decoded_instruction decoded =
			tcti_decode_aarch64(crc_source_instruction(leaf));
		...
		KUNIT_EXPECT_EQ(test, leaf->castagnoli ? TCTI_DP2_CRC32C :
				TCTI_DP2_CRC32, decoded.dp2_op);
	...
	for (index = 0; index < ARRAY_SIZE(crc_source_leaves); index++) {
		...
		for (bit = 0; bit < 32; bit++) {
			...
			if (crc_source_leaf_for_instruction(instruction))
				continue;
			if (tcti_decode_aarch64(instruction).decode_class !=
