...
struct tcti_cssc_data_processing_leaf {
	const char *source_name;
	u32 source_mask;
	u32 source_pattern;
	bool is_64bit;
 ...
 */
static const struct tcti_cssc_data_processing_leaf
tcti_cssc_data_processing_leaves[] = {
...
static u32 tcti_cssc_data_processing_instruction(
	const struct tcti_cssc_data_processing_leaf *leaf, u8 rm, u8 rn, u8 rd)
{
	return leaf->source_pattern | (leaf->two_source ? (u32)rm << 16 : 0) |
		((u32)rn << 5) | rd;
}

static u64 tcti_cssc_data_processing_expected(
	const struct tcti_cssc_data_processing_leaf *leaf, u64 left, u64 right)
{
...
static void tcti_cssc_data_processing_source_fingerprints(struct kunit *test)
{
	size_t index;

	for (index = 0; index < ARRAY_SIZE(tcti_cssc_data_processing_leaves);
	     index++) {
		const struct tcti_cssc_data_processing_leaf *leaf =
			&tcti_cssc_data_processing_leaves[index];
		struct tcti_decoded_instruction decoded = tcti_decode_aarch64(
			tcti_cssc_data_processing_instruction(leaf, 3, 2, 1));

		KUNIT_EXPECT_EQ_MSG(test, leaf->source_pattern,
				    leaf->source_pattern & leaf->source_mask,
				    "%s source fingerprint", leaf->source_name);
		KUNIT_ASSERT_EQ_MSG(test,
			leaf->two_source ? TCTI_DECODE_DATA_PROCESSING_2SOURCE :
			TCTI_DECODE_DATA_PROCESSING_1SOURCE, decoded.decode_class,
			"%s", leaf->source_name);
		KUNIT_EXPECT_EQ(test, leaf->is_64bit, decoded.is_64bit);
		KUNIT_EXPECT_EQ(test, 2, decoded.rn);
		KUNIT_EXPECT_EQ(test, 1, decoded.rd);
...
static void tcti_cssc_data_processing_all_legal_register_fields(
	struct kunit *test)
{
	size_t leaf_index;
	u8 rm;
	...
	for (leaf_index = 0;
	     leaf_index < ARRAY_SIZE(tcti_cssc_data_processing_leaves);
	     leaf_index++) {
		const struct tcti_cssc_data_processing_leaf *leaf =
			&tcti_cssc_data_processing_leaves[leaf_index];
		...
		for (rm = 0; rm < (leaf->two_source ? 32 : 1); rm++) {
			for (rn = 0; rn < 32; rn++) {
				for (rd = 0; rd < 32; rd++) {
					struct tcti_decoded_instruction decoded =
						tcti_decode_aarch64(
							tcti_cssc_data_processing_instruction(
								leaf, rm, rn, rd));
...
static void tcti_cssc_data_processing_execute_boundaries(struct kunit *test)
{
	...
	for (leaf_index = 0;
		...
		for (left_index = 0; left_index < ARRAY_SIZE(values); left_index++) {
			for (right_index = 0;
				...
				u64 pstate = PSR_MODE_EL0t | PSR_N_BIT | PSR_C_BIT |
					PSR_V_BIT;
				u64 expected = tcti_cssc_data_processing_expected(
...
static void tcti_cssc_data_processing_zero_registers(struct kunit *test)
{
	size_t index;
	...
	for (index = 0; index < ARRAY_SIZE(tcti_cssc_data_processing_leaves);
		...
		struct pt_regs regs = {};
		struct tcti_decoded_instruction decoded = tcti_decode_aarch64(
			tcti_cssc_data_processing_instruction(leaf, 31, 31, 31));
		u64 pstate = PSR_MODE_EL0t | PSR_Z_BIT | PSR_C_BIT;
		int ret;
...
static unsigned long tcti_cssc_data_processing_map_program(
...
static void tcti_cssc_data_processing_resume_user_rejects_unavailable_feature(
	struct kunit *test)
{
	size_t index;
	...
	for (index = 0; index < ARRAY_SIZE(tcti_cssc_data_processing_leaves);
		...
		unsigned long mapped = tcti_cssc_data_processing_map_program(
			test, tcti_cssc_data_processing_instruction(leaf, 3, 2, 1));
		struct pt_regs regs = {};
		...
		KUNIT_ASSERT_EQ_MSG(test, TCTI_EXIT_UNSUPPORTED_INSTRUCTION,
				    result.reason,
				    "%s", leaf->source_name);
...
static void
tcti_cssc_min_max_immediate_resume_user_rejects_unavailable_feature(
	struct kunit *test)
{
	size_t index;
	...
	for (index = 0;
	     index < ARRAY_SIZE(tcti_cssc_min_max_immediate_leaves); index++) {
		const struct tcti_cssc_min_max_immediate_leaf *leaf =
			&tcti_cssc_min_max_immediate_leaves[index];
		u8 immediate = 0x80;
		u64 source = leaf->is_64bit ? (u64)S64_MIN : 0x80000000ULL;
		unsigned long mapped = tcti_cssc_data_processing_map_program(
			test, tcti_cssc_min_max_immediate_instruction(leaf, immediate,
								      2, 1));
		struct pt_regs regs = {};
		struct pt_regs before;
		struct tcti_result result;
		...
		KUNIT_EXPECT_EQ(test, mapped, result.pc);
		KUNIT_EXPECT_EQ(test,
			tcti_cssc_min_max_immediate_instruction(leaf, immediate, 2, 1),
			result.instruction);
		KUNIT_EXPECT_MEMEQ(test, &before, &regs, sizeof(regs));
		KUNIT_EXPECT_EQ(test, 0, vm_munmap(mapped, PAGE_SIZE));
	}
...
static void tcti_cssc_data_processing_rejects_reserved_opcodes(
	struct kunit *test)
{
	static const u32 instructions[] = {
...
static struct kunit_case tcti_cssc_min_max_immediate_test_cases[] = {
	...
	KUNIT_CASE(tcti_cssc_min_max_immediate_resume_user_rejects_unavailable_feature),
	KUNIT_CASE(tcti_cssc_min_max_immediate_rejects_non_cssc_encodings),
	KUNIT_CASE(tcti_cssc_data_processing_source_fingerprints),
	KUNIT_CASE(tcti_cssc_data_processing_all_legal_register_fields),
	KUNIT_CASE(tcti_cssc_data_processing_execute_boundaries),
	KUNIT_CASE(tcti_cssc_data_processing_zero_registers),
	KUNIT_CASE(tcti_cssc_data_processing_resume_user_rejects_unavailable_feature),
	KUNIT_CASE(tcti_cssc_data_processing_rejects_reserved_opcodes),
	{}
};
