...
struct tcti_integer_conditional_leaf {
	u16 ordinal;
	const char *name;
	u32 mask;
	u32 pattern;
...
static const struct tcti_integer_conditional_leaf
    tcti_integer_conditional_leaves[] = {
...
static unsigned long tcti_integer_conditional_map(struct kunit *test,
						  u32 instruction) {
...
static struct tcti_result tcti_integer_conditional_run(struct kunit *test,
						       u32 instruction,
						       struct pt_regs *regs,
						       unsigned long *mapped) {
	struct tcti_result result;

	*mapped = tcti_integer_conditional_map(test, instruction);
	regs->pc = *mapped;
...
static void tcti_integer_conditional_source_bindings(struct kunit *test) {
	size_t index;

	for (index = 0; index < ARRAY_SIZE(tcti_integer_conditional_leaves);
	     index++) {
		const struct tcti_integer_conditional_leaf *leaf =
		    &tcti_integer_conditional_leaves[index];
		struct tcti_decoded_instruction decoded =
		    tcti_decode_aarch64(leaf->pattern);
...
static u32 tcti_integer_conditional_dp2(u32 pattern, u8 rd, u8 rn, u8 rm)
{
	return pattern | ((u32)rm << 16) | ((u32)rn << 5) | rd;
}

static u32 tcti_integer_conditional_compare(u32 pattern, u8 rn, u8 rm_or_imm,
...
static u32 tcti_integer_conditional_select(u32 pattern, u8 rd, u8 rn, u8 rm,
					    u8 condition)
{
...
static u32 tcti_integer_conditional_multiply(u32 pattern, u8 rd, u8 rn,
					      u8 rm, u8 ra)
{
...
static void tcti_integer_conditional_expect_preserved(
...
static void
tcti_integer_conditional_divide_corners_production_path(struct kunit *test) {
	static const struct {
		u32 instruction;
		u64 dividend;
		u64 divisor;
	...
	for (index = 0; index < ARRAY_SIZE(cases); index++) {
		...
		regs.pstate =
		    ...
		    PSR_N_BIT | PSR_C_BIT;
		before = regs;
		tcti_integer_conditional_run(test,
			tcti_integer_conditional_dp2(cases[index].instruction, 0, 1, 2),
					     &regs, &mapped);
...
static void tcti_integer_conditional_all_divide_leaves_production_path(
	struct kunit *test)
{
	static const struct {
		u32 pattern;
	...
	for (index = 0; index < ARRAY_SIZE(cases); index++) {
		...
		before = regs;
		tcti_integer_conditional_run(test,
			tcti_integer_conditional_dp2(cases[index].pattern, 0, 1, 2),
			&regs, &mapped);
		KUNIT_EXPECT_EQ(test, cases[index].expected, regs.regs[0]);
		tcti_integer_conditional_expect_preserved(test, &before, &regs,
			true);
...
static void tcti_integer_conditional_compare_leaves_production_path(
	struct kunit *test)
{
	static const struct {
		u32 pattern;
	...
	for (index = 0; index < ARRAY_SIZE(cases); index++) {
		...
		before = regs;
		tcti_integer_conditional_run(test,
			tcti_integer_conditional_compare(cases[index].pattern, 1,
...
static void tcti_integer_conditional_select_leaves_production_path(
	struct kunit *test)
{
	static const struct {
		u32 pattern;
	...
	for (index = 0; index < ARRAY_SIZE(cases); index++) {
		...
		before = regs;
		tcti_integer_conditional_run(test,
			tcti_integer_conditional_select(cases[index].pattern, 0, 1,
				2, 0), &regs, &mapped);
...
static void tcti_integer_conditional_multiply_leaves_production_path(
	struct kunit *test)
{
	static const struct {
		u32 pattern;
	...
	for (index = 0; index < ARRAY_SIZE(cases); index++) {
		...
		before = regs;
		tcti_integer_conditional_run(test,
			tcti_integer_conditional_multiply(cases[index].pattern, 0, 1,
				2, ra), &regs, &mapped);
		KUNIT_EXPECT_EQ(test, cases[index].expected, regs.regs[0]);
		tcti_integer_conditional_expect_preserved(test, &before, &regs,
...
static void tcti_integer_conditional_x31_is_zero_and_not_writable(
	struct kunit *test)
{
	struct pt_regs regs = {};
	struct pt_regs before;
	...
	before = regs;
	tcti_integer_conditional_run(test,
		tcti_integer_conditional_select(0x9a800000U, 0, 31, 2, 0),
		&regs, &mapped);
	...
	before = regs;
	tcti_integer_conditional_run(test,
		tcti_integer_conditional_select(0x1a800000U, 31, 1, 2, 0),
		&regs, &mapped);
...
static void tcti_integer_conditional_reserved_encodings_fail_before_state(
    struct kunit *test) {
...
static struct kunit_case tcti_integer_conditional_source_bound_test_cases[] = {
    KUNIT_CASE(tcti_integer_conditional_source_bindings),
    KUNIT_CASE(tcti_integer_conditional_divide_corners_production_path),
    KUNIT_CASE(tcti_integer_conditional_all_divide_leaves_production_path),
    KUNIT_CASE(tcti_integer_conditional_compare_leaves_production_path),
    KUNIT_CASE(tcti_integer_conditional_select_leaves_production_path),
    KUNIT_CASE(tcti_integer_conditional_multiply_leaves_production_path),
    KUNIT_CASE(tcti_integer_conditional_x31_is_zero_and_not_writable),
    KUNIT_CASE(tcti_integer_conditional_reserved_encodings_fail_before_state),
    {}};

struct kunit_suite tcti_integer_conditional_source_bound_test_suite = {
    .name = "orlix-tcti-integer-conditional-source-bound",
    .test_cases = tcti_integer_conditional_source_bound_test_cases,
};

kunit_test_suite(tcti_integer_conditional_source_bound_test_suite);
