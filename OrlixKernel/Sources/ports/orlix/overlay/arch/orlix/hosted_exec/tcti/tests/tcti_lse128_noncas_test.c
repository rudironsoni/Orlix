...
static const struct tcti_lse128_leaf tcti_lse128_leaves[] = {
	{ TCTI_LSE_ATOMIC_CLR, 0, 0x19201000U },
	{ TCTI_LSE_ATOMIC_SET, 0, 0x19203000U },
	{ TCTI_LSE_ATOMIC_SWP, 0, 0x19208000U },
	{ TCTI_LSE_ATOMIC_CLR, 2, 0x19601000U },
	{ TCTI_LSE_ATOMIC_SET, 2, 0x19603000U },
	{ TCTI_LSE_ATOMIC_SWP, 2, 0x19608000U },
	{ TCTI_LSE_ATOMIC_CLR, 1, 0x19a01000U },
	{ TCTI_LSE_ATOMIC_SET, 1, 0x19a03000U },
	{ TCTI_LSE_ATOMIC_SWP, 1, 0x19a08000U },
	{ TCTI_LSE_ATOMIC_CLR, 3, 0x19e01000U },
	{ TCTI_LSE_ATOMIC_SET, 3, 0x19e03000U },
	{ TCTI_LSE_ATOMIC_SWP, 3, 0x19e08000U },
};

static u32 tcti_lse128_instruction(enum tcti_lse_atomic_op op, u8 order,
				   u8 rs, u8 rn, u8 rt)
{
	u8 encoding;
...
static unsigned long tcti_lse128_map(struct kunit *test)
{
...
tcti_lse128_result(enum tcti_lse_atomic_op op,
			   const struct tcti_lse128_value *initial,
			   const struct tcti_lse128_value *operand)
{
	struct tcti_lse128_value result;
	...
	case TCTI_LSE_ATOMIC_SWP:
		...
		break;
	default:
		result = (struct tcti_lse128_value) {};
		break;
	}
...
static void tcti_lse128_decode_all_leaves(struct kunit *test)
{
	size_t i;

	for (i = 0; i < ARRAY_SIZE(tcti_lse128_leaves); i++) {
		const struct tcti_lse128_leaf *leaf = &tcti_lse128_leaves[i];
		u32 instruction = leaf->encoding | (6U << 16) | (7U << 5) | 8U;
		struct tcti_decoded_instruction decoded =
			tcti_decode_aarch64(instruction);

		KUNIT_ASSERT_EQ(test, TCTI_DECODE_LSE_ATOMIC,
...
static void tcti_lse128_rejects_reserved_operations(
	struct kunit *test)
{
	...
	for (i = 0; i < ARRAY_SIZE(invalid_ops); i++)
		KUNIT_EXPECT_EQ(test, TCTI_DECODE_UNSUPPORTED,
			tcti_decode_aarch64(LSE128_RMW_BASE |
				((u32)invalid_ops[i] << 12) | (6U << 16) |
				(7U << 5) | 8U).decode_class);
}

static void tcti_lse128_accepts_independent_odd_registers(
	struct kunit *test)
{
	struct tcti_decoded_instruction decoded;

	decoded = tcti_decode_aarch64(tcti_lse128_instruction(
		TCTI_LSE_ATOMIC_CLR, 0, 5, 8, 11));
	...
	KUNIT_EXPECT_EQ(test, TCTI_DECODE_UNSUPPORTED,
		tcti_decode_aarch64(tcti_lse128_instruction(
			TCTI_LSE_ATOMIC_CLR, 0, 5, 8, 31)).decode_class);
	KUNIT_EXPECT_EQ(test, TCTI_DECODE_UNSUPPORTED,
		tcti_decode_aarch64(tcti_lse128_instruction(
			TCTI_LSE_ATOMIC_CLR, 0, 31, 8, 5)).decode_class);
	KUNIT_EXPECT_EQ(test, TCTI_DECODE_UNSUPPORTED,
		tcti_decode_aarch64(tcti_lse128_instruction(
			TCTI_LSE_ATOMIC_CLR, 0, 5, 8, 5)).decode_class);
}

static void tcti_lse128_execute_all_leaves(struct kunit *test)
{
	const struct tcti_lse128_value initial = {
	...
	unsigned long mapped = tcti_lse128_map(test);
	size_t i;

	for (i = 0; i < ARRAY_SIZE(tcti_lse128_leaves); i++) {
		const struct tcti_lse128_leaf *leaf = &tcti_lse128_leaves[i];
		struct tcti_decoded_instruction decoded = tcti_decode_aarch64(
			tcti_lse128_instruction(leaf->op, leaf->order, 6, 10, 8));
		struct tcti_lse128_value expected =
			tcti_lse128_result(leaf->op, &initial, &operand);
...
static void tcti_lse128_rejects_overlapping_result_lanes(struct kunit *test)
{
	struct tcti_decoded_instruction decoded = tcti_decode_aarch64(
		tcti_lse128_instruction(TCTI_LSE_ATOMIC_SET, 3, 6, 10, 6));

	KUNIT_EXPECT_EQ(test, TCTI_DECODE_UNSUPPORTED, decoded.decode_class);
}

static void tcti_lse128_fixed_bit_near_misses_are_not_decoded(struct kunit *test)
{
	u32 instruction = tcti_lse128_instruction(TCTI_LSE_ATOMIC_SWP,
						   0, 6, 10, 8);

	KUNIT_EXPECT_NE(test, TCTI_DECODE_LSE_ATOMIC,
		tcti_decode_aarch64(instruction ^ BIT(21)).decode_class);
	KUNIT_EXPECT_NE(test, TCTI_DECODE_LSE_ATOMIC,
		tcti_decode_aarch64(instruction ^ BIT(10)).decode_class);
}

static void tcti_lse128_unaligned_fault_preserves_state(struct kunit *test)
{
	const struct tcti_lse128_value initial = {
	...
	unsigned long mapped = tcti_lse128_map(test);
	struct tcti_decoded_instruction decoded = tcti_decode_aarch64(
		tcti_lse128_instruction(TCTI_LSE_ATOMIC_SWP, 0, 6, 10, 8));
	struct tcti_lse128_value observed = {};
	struct pt_regs regs = {};
...
static struct kunit_case tcti_lse128_noncas_cases[] = {
	KUNIT_CASE(tcti_lse128_decode_all_leaves),
	KUNIT_CASE(tcti_lse128_rejects_reserved_operations),
	KUNIT_CASE(tcti_lse128_accepts_independent_odd_registers),
	KUNIT_CASE(tcti_lse128_execute_all_leaves),
	KUNIT_CASE(tcti_lse128_rejects_overlapping_result_lanes),
	KUNIT_CASE(tcti_lse128_fixed_bit_near_misses_are_not_decoded),
	KUNIT_CASE(tcti_lse128_unaligned_fault_preserves_state),
	{}
};

struct kunit_suite tcti_lse128_noncas_test_suite = {
	.name = "orlix_tcti_lse128_noncas",
	.test_cases = tcti_lse128_noncas_cases,
};

kunit_test_suite(tcti_lse128_noncas_test_suite);
