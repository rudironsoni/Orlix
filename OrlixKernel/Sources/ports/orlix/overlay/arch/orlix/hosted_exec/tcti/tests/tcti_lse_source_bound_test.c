...
static u64 lse_source_bound_rmw_result(enum tcti_lse_atomic_op operation,
...
static void lse_source_bound_catalog_is_exact_base_lse(struct kunit *test)
{
	...
	KUNIT_ASSERT_EQ(test, TCTI_LSE_OPERATION_CATALOG_DIRECT_LEAF_COUNT,
			count);
	KUNIT_ASSERT_EQ(test, 0,
		tcti_lse_operation_catalog_validate(
	...
	for (index = 0; index < count; index++) {
		...
		if (entry->cohort != TCTI_LSE_OPERATION_COHORT_BASE_LSE)
			continue;
		KUNIT_EXPECT_TRUE(test, entry->direct_source_leaf);
		KUNIT_EXPECT_EQ(test, TCTI_LSE_OPERATION_STRUCTURAL_DECODER_ONLY,
				entry->implementation_status);
		KUNIT_EXPECT_EQ(test, TCTI_LSE_OPERATION_COHORT_BASE_LSE,
				entry->implementation_cohort);
		KUNIT_EXPECT_EQ(test, TCTI_LSE_OPERATION_PROOF_NONE,
				entry->proof_status);
		KUNIT_EXPECT_EQ(test, TCTI_LSE_OPERATION_COHORT_NONE,
				entry->proof_cohort);
...
static void lse_source_bound_decodes_every_base_leaf(struct kunit *test)
{
	...
	for (index = 0; index < count; index++) {
		...
		u32 instruction;
		enum tcti_lse_atomic_op expected;
		...
		KUNIT_EXPECT_EQ_MSG(test, TCTI_DECODE_LSE_ATOMIC,
		...
		expected = entry->semantic_class ==
			...
			(((instruction >> 12) & 0xfU) == 8U ?
			 TCTI_LSE_ATOMIC_SWP :
			 TCTI_LSE_ATOMIC_ADD + ((instruction >> 12) & 0xfU));
...
static void lse_source_bound_executes_every_base_leaf(struct kunit *test)
{
	...
	for (index = 0; index < count; index++) {
		...
		if (decoded.pair) {
			...
			ret = tcti_write_user_data(current->mm, mapped, old,
					  2 * width);
		} else {
			ret = tcti_write_user_data(current->mm, mapped, old, width);
		}
		...
		if (decoded.pair)
			...
			ret = tcti_read_user_data(current->mm, mapped, observed, width);
		KUNIT_ASSERT_EQ_MSG(test, 0, ret, "%s", entry->source_leaf);
		if (decoded.lse_atomic_op == TCTI_LSE_ATOMIC_CAS) {
...
{
	const struct tcti_lse_operation_catalog_entry *entries;
	const struct tcti_lse_operation_catalog_entry *lse2;
	size_t count;
	size_t index;
	size_t base_count = 0;

	entries = tcti_lse_operation_catalog(&count);
	lse2 = tcti_lse_operation_catalog_lse2_blocker();
	KUNIT_ASSERT_NOT_NULL(test, entries);
	KUNIT_ASSERT_NOT_NULL(test, lse2);
	for (index = 0; index < count; index++)
		if (entries[index].cohort == TCTI_LSE_OPERATION_COHORT_BASE_LSE)
			base_count++;
	KUNIT_EXPECT_EQ(test, 168U, base_count);
	KUNIT_EXPECT_FALSE(test, lse2->direct_source_leaf);
	KUNIT_EXPECT_EQ(test, TCTI_LSE_OPERATION_COHORT_LSE2_SEMANTIC_VARIANT,
			lse2->cohort);
	KUNIT_EXPECT_EQ(test, TCTI_LSE_OPERATION_PROOF_NONE,
			lse2->proof_status);
}

static struct kunit_case tcti_lse_source_bound_test_cases[] = {
...
struct kunit_suite tcti_lse_source_bound_test_suite = {
	.name = "orlix-tcti-lse-source-bound",
	.test_cases = tcti_lse_source_bound_test_cases,
};

kunit_test_suite(tcti_lse_source_bound_test_suite);
