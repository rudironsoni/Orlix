...
static void osls_unprivileged_load_store_production_path(struct kunit *test)
{
	...
	ret = tcti_read_user_data(current->mm, data + 1, &observed8,
				  sizeof(observed8));
	KUNIT_ASSERT_EQ(test, 0, ret);
	...
	byte = 0x80U;
	ret = tcti_write_user_data(current->mm, data + 2, &byte, sizeof(byte));
	KUNIT_ASSERT_EQ(test, 0, ret);
	memset(&regs, 0, sizeof(regs));
	regs.regs[1] = data;
	...
	ret = tcti_write_user_data(current->mm, data + 3, &byte, sizeof(byte));
	KUNIT_ASSERT_EQ(test, 0, ret);
	memset(&regs, 0, sizeof(regs));
	regs.regs[0] = ~0ULL;
	regs.regs[1] = data;
	...
	ret = tcti_read_user_data(current->mm, data + 4, &observed16,
				  sizeof(observed16));
	KUNIT_ASSERT_EQ(test, 0, ret);
	...
	half = 0x8001U;
	ret = tcti_write_user_data(current->mm, data + 5, &half, sizeof(half));
	KUNIT_ASSERT_EQ(test, 0, ret);
	memset(&regs, 0, sizeof(regs));
	regs.regs[0] = ~0ULL;
	regs.regs[1] = data;
	...
	ret = tcti_write_user_data(current->mm, data + 6, &half, sizeof(half));
	KUNIT_ASSERT_EQ(test, 0, ret);
	memset(&regs, 0, sizeof(regs));
	regs.regs[1] = data;
	...
	ret = tcti_write_user_data(current->mm, data + 7, &half, sizeof(half));
	KUNIT_ASSERT_EQ(test, 0, ret);
	memset(&regs, 0, sizeof(regs));
	regs.regs[1] = data;
	...
	ret = tcti_read_user_data(current->mm, data + 8, &observed32,
				  sizeof(observed32));
	KUNIT_ASSERT_EQ(test, 0, ret);
	...
	ret = tcti_write_user_data(current->mm, data + 9, &observed32,
				   sizeof(observed32));
	KUNIT_ASSERT_EQ(test, 0, ret);
	memset(&regs, 0, sizeof(regs));
	regs.regs[0] = ~0ULL;
	...
	ret = tcti_write_user_data(current->mm, data + 10, &observed32,
				   sizeof(observed32));
	KUNIT_ASSERT_EQ(test, 0, ret);
	memset(&regs, 0, sizeof(regs));
	regs.regs[1] = data;
	...
	ret = tcti_read_user_data(current->mm, data + 11, &observed64,
				  sizeof(observed64));
	KUNIT_ASSERT_EQ(test, 0, ret);
	...
	ret = tcti_write_user_data(current->mm, data + 12, &word, sizeof(word));
	KUNIT_ASSERT_EQ(test, 0, ret);
	memset(&regs, 0, sizeof(regs));
	regs.regs[1] = data;
	...
	ret = tcti_read_user_data(current->mm, data + 16, &zero, sizeof(zero));
	KUNIT_ASSERT_EQ(test, 0, ret);
	KUNIT_EXPECT_EQ(test, 0ULL, zero);
	KUNIT_EXPECT_EQ(test, data + 16, regs.sp);
	KUNIT_EXPECT_EQ(test, 0, vm_munmap(mapped, PAGE_SIZE));

	ret = tcti_write_user_data(current->mm, data + 16, &word, sizeof(word));
	KUNIT_ASSERT_EQ(test, 0, ret);
	memset(&regs, 0, sizeof(regs));
	regs.sp = data + 16;
