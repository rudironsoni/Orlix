				...
				const struct pt_regs *regs, unsigned long text)
{
	...
	KUNIT_EXPECT_EQ(test, TCTI_ACCESS_NONE, result->fault_access);
	...
	for (index = 0; index < ARRAY_SIZE(sfro_expected); index++) {
		...
		for (option = 2; option <= 7; option++) {
			...
			for (shift = 0; shift < 2; shift++) {
				...
				if (expected->load) {
					ret = tcti_write_user_data(current->mm, address, bytes,
				...
				before = regs;
				{
					struct tcti_result result =
						tcti_resume_user(current, &regs, current->mm);
...
static void sfro_sp_v31_and_xzr_execute_without_aliasing(struct kunit *test)
{
	...
	ret = tcti_write_user_data(current->mm, data + 128, bytes, sizeof(bytes));
	KUNIT_ASSERT_EQ(test, 0, ret);
	...
	regs.syscallno = NO_SYSCALL;
	{
		struct tcti_result result = tcti_resume_user(current, &regs,
							     current->mm);
	...
	ret = tcti_read_user_data(current->mm, data + 160, observed,
				  sizeof(observed));
	KUNIT_EXPECT_EQ(test, 0, ret);
	...
	struct kunit *test)
{
	...
	KUNIT_ASSERT_NOT_NULL(test, ldr_q);
	KUNIT_ASSERT_NOT_NULL(test, str_q);
	ret = tcti_write_user_data(current->mm, address, &sentinel,
				  sizeof(sentinel));
	KUNIT_ASSERT_EQ(test, 0, ret);
...
static struct kunit_suite tcti_simd_fp_register_offset_source_bound_suite = {
...
kunit_test_suite(tcti_simd_fp_register_offset_source_bound_suite);
