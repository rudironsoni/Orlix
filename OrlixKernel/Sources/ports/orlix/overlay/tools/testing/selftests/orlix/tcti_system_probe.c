...
#define TCTI_FPCR_WRITABLE_MASK	(((uint64_t)0x1f << 22) | \
				 ((uint64_t)1 << 15) | \
				 ((uint64_t)0x1f << 8))
#define TCTI_FPSR_WRITABLE_MASK	(((uint64_t)1 << 27) | \
				 ((uint64_t)1 << 7) | 0x1f)
...
static bool fpcr_mask_is_enforced(void)
{
	...
	observed = read_fpcr();
	write_fpcr(saved);
	return observed == TCTI_FPCR_WRITABLE_MASK;
}
...
static bool fpsr_mask_is_enforced(void)
{
	...
	observed = read_fpsr();
	write_fpsr(saved);
	return observed == TCTI_FPSR_WRITABLE_MASK;
}
