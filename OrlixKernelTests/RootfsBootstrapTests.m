/*
 * OrlixKernelTests - RootfsBootstrapTests.m
 *
 ...
 * through Orlix's path mediation.
 */
...
- (void)testVirtualEtcPasswdExists {
    ...
    /* Verify file is accessible by opening it via Orlix's owner open_impl/read_impl */
    int fd = open_impl(@"/etc/passwd".UTF8String, O_RDONLY, 0);
    XCTAssertTrue(fd >= 0, @"open_impl /etc/passwd through Orlix should succeed");
    if (fd >= 0) {
        close_impl(fd);
    }
}
...
- (void)testVirtualEtcGroupExists {
    ...
    XCTAssertTrue(fd >= 0, @"open_impl /etc/group through Orlix should succeed");
    if (fd >= 0) {
        close_impl(fd);
    }
}
...
- (void)testVirtualEtcHostsExists {
    ...
    XCTAssertTrue(fd >= 0, @"open_impl /etc/hosts through Orlix should succeed");
    if (fd >= 0) {
        close_impl(fd);
    }
}
...
- (void)testVirtualEtcResolvConfExists {
    ...
    XCTAssertTrue(fd >= 0, @"open_impl /etc/resolv.conf through Orlix should succeed");
    if (fd >= 0) {
        close_impl(fd);
    }
}
...
- (void)testVirtualEtcPasswdContent {
    ...
    /* Verify orlix user entry exists */
    XCTAssertTrue(strstr(buf, "orlix:x:1000:1000:") != NULL,
                  @"/etc/passwd should contain orlix user entry");
}
...
- (void)testVirtualEtcGroupContent {
    ...
    /* Verify orlix group entry exists */
    XCTAssertTrue(strstr(buf, "orlix:x:1000:") != NULL,
                  @"/etc/group should contain orlix group entry");
}
