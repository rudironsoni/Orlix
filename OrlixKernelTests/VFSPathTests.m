...
- (void)testSyntheticChildStatHandlesSupportedProcFilesAndRejectsUnsupportedSys {
    ...
    XCTAssertEqual(errno, ENOENT, @"Orlix lstat should set ENOENT for unsupported synthetic child paths");
}
...
- (void)testRecursiveUmountUpdatesProcMountinfoForPropagatedPeers {
    extern int vfs_contract_recursive_umount_updates_proc_mountinfo_for_propagated_peers(void);
    XCTAssertEqual(vfs_contract_recursive_umount_updates_proc_mountinfo_for_propagated_peers(), 0,
...
- (void)testStickyDirectoryBlocksNonOwnerRename {
    extern int vfs_contract_sticky_directory_blocks_nonowner_rename(void);
    XCTAssertEqual(vfs_contract_sticky_directory_blocks_nonowner_rename(), 0,
