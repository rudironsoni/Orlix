...
$(ORLIXOS_COREUTILS_TEST_INIT_BINARY): $(ORLIXOS_COREUTILS_TEST_INIT_SOURCE) $(ORLIXOS_MLIBC_SYSROOT)/.orlixmlibc-sysroot-ready $(ORLIXOS_MLIBC_RTLIB) $(PROJECT_DIR)/Sources/make/coreutils-test.mk
	...
	"$(ORLIXOS_CC)" --target=aarch64-linux-gnu --sysroot="$$sysroot" -isystem "$$headers" -D_GNU_SOURCE -DORLIXOS_COREUTILS_TEST_TIMEOUT_SECONDS=$(ORLIXOS_COREUTILS_TEST_TIMEOUT_SECONDS) -std=c17 -O2 -fhosted -fno-builtin -ffixed-x18 -fPIE -static-pie -fuse-ld=lld -nostdlib -Wl,--gc-sections -Wl,-z,max-page-size=0x4000 "$$sysroot/usr/lib/crt1.o" "$$sysroot/usr/lib/crti.o" "$(ORLIXOS_COREUTILS_TEST_INIT_SOURCE)" -Wl,--start-group "$$sysroot/usr/lib/libc.a" "$$sysroot/usr/lib/libm.a" "$$sysroot/usr/lib/libpthread.a" "$$sysroot/usr/lib/libssp_nonshared.a" "$$sysroot/usr/lib/libssp.a" "$$rtlib" -Wl,--end-group "$$sysroot/usr/lib/crtn.o" -o "$(ORLIXOS_COREUTILS_TEST_INIT_BINARY)"; \
	"$(ORLIXOS_STRIP)" "$(ORLIXOS_COREUTILS_TEST_INIT_BINARY)"; \
	file "$(ORLIXOS_COREUTILS_TEST_INIT_BINARY)" | grep -F -q 'ELF 64-bit LSB pie executable, ARM aarch64' || { file "$(ORLIXOS_COREUTILS_TEST_INIT_BINARY)" >&2; exit 1; }; \
	echo "built OrlixOS Coreutils test init: $(ORLIXOS_COREUTILS_TEST_INIT_BINARY)"

$(ORLIXOS_COREUTILS_TEST_DYNAMIC_STAMP): $(ORLIXOS_COREUTILS_STAMP)
	@$(COREUTILS_MAKE) __test-dynamic PROFILE="$(PROFILE)" ORLIX_BUILD_ROOT="$(ORLIX_BUILD_ROOT)"

$(ORLIXOS_COREUTILS_TEST_INITRAMFS): $(ORLIXOS_COREUTILS_TEST_INIT_BINARY) $(ORLIXOS_COREUTILS_TEST_RUNNER) $(ORLIXOS_COREUTILS_TEST_VALGRIND) $(ORLIXOS_COREUTILS_TEST_CC) $(ORLIXOS_TCC_BINARY) $(ORLIXOS_COREUTILS_TEST_DYNAMIC_STAMP) $(ORLIXOS_COREUTILS_TEST_ENV) $(ORLIXOS_COREUTILS_SOURCE_STAMP) $(ORLIXOS_COREUTILS_TEST_LIST) $(ORLIXOS_COREUTILS_TEST_PASSWD) $(ORLIXOS_COREUTILS_TEST_GROUP) $(ORLIXOS_COREUTILS_TEST_SELINUX_CONFIG) $(ORLIXOS_COREUTILS_TEST_SELINUX_POLICY_BINARY) $(ORLIXOS_BASH_BINARY) $(ORLIXOS_COREUTILS_STAMP) $(ORLIXOS_GREP_BINARY) $(ORLIXOS_SED_BINARY) $(ORLIXOS_DIFF_BINARY) $(ORLIXOS_GAWK_BINARY) $(ORLIXOS_FINDUTILS_STAMP) $(ORLIXOS_SETSID_BINARY) $(ORLIXOS_MOUNT_BINARY) $(ORLIXOS_UMOUNT_BINARY) $(ORLIXOS_MKFS_BINARY) $(ORLIXOS_MKE2FS_BINARY) $(ORLIXOS_MKFS_EXT2_BINARY) $(ORLIXOS_MKFS_EXT4_BINARY) $(ORLIXOS_GETLIMITS_BINARY) $(ORLIXOS_GETCONF_BINARY) $(ORLIXOS_GETENT_BINARY) $(ORLIXOS_GETFATTR_BINARY) $(ORLIXOS_SETFATTR_BINARY) $(ORLIXOS_GETFACL_BINARY) $(ORLIXOS_SETFACL_BINARY) $(ORLIXOS_SETCAP_BINARY) $(ORLIXOS_GETCAP_BINARY) $(ORLIXOS_GETENFORCE_BINARY) $(ORLIXOS_SETENFORCE_BINARY) $(ORLIXOS_SELINUXENABLED_BINARY) $(ORLIXOS_POLICYVERS_BINARY) $(ORLIXOS_GETPOLICYLOAD_BINARY) $(ORLIXOS_SESTATUS_BINARY) $(ORLIXOS_PERL_BINARY) $(ORLIXOS_PERL_STAMP)
	@set -euo pipefail; \
	...
	{ \
		...
		printf 'file /bin/valgrind %s 0755 0 0\n' "$(ORLIXOS_COREUTILS_TEST_VALGRIND)"; \
		printf 'file /bin/cc %s 0755 0 0\n' "$(ORLIXOS_COREUTILS_TEST_CC)"; \
		printf 'file /bin/tcc %s 0755 0 0\n' "$(ORLIXOS_TCC_BINARY)"; \
		printf 'slink /bin/gcc cc 0777 0 0\n'; \
		printf 'slink /usr/bin/valgrind /bin/valgrind 0777 0 0\n'; \
		printf 'slink /usr/bin/cc /bin/cc 0777 0 0\n'; \
		printf 'slink /usr/bin/gcc /bin/gcc 0777 0 0\n'; \
		printf 'slink /usr/bin/tcc /bin/tcc 0777 0 0\n'; \
		for program in $(ORLIXOS_COREUTILS_PROGRAMS); do \
			src="$(ORLIXOS_COREUTILS_INSTALL_DIR)/usr/bin/$$program"; \
			case " $(ORLIXOS_COREUTILS_TEST_DYNAMIC_PROGRAMS) " in *" $$program "*) src="$(ORLIXOS_COREUTILS_TEST_DYNAMIC_DIR)/usr/bin/$$program" ;; esac; \
			printf 'file /bin/%s %s 0755 0 0\n' "$$program" "$$src"; \
		done; \
		...
		printf 'file /usr/lib/ld.so %s 0755 0 0\n' "$(ORLIXOS_MLIBC_LDSO)"; \
		printf 'file /usr/lib/libc.so %s 0755 0 0\n' "$(ORLIXOS_MLIBC_LIBC_SO)"; \
		printf 'file /usr/lib/libm.so %s 0755 0 0\n' "$(ORLIXOS_MLIBC_LIBM_SO)"; \
		printf 'file /usr/lib/libpthread.so %s 0755 0 0\n' "$(ORLIXOS_MLIBC_LIBPTHREAD_SO)"; \
		printf 'file /usr/lib/libdl.so %s 0755 0 0\n' "$(ORLIXOS_MLIBC_LIBDL_SO)"; \
		printf 'dir /lib 0755 0 0\n'; \
		printf 'slink /lib/ld-linux-aarch64.so.1 /usr/lib/ld.so 0777 0 0\n'; \
		printf 'dir /usr/lib/tcc 0755 0 0\n'; \
		printf 'file /usr/lib/tcc/libtcc1.a %s 0644 0 0\n' "$(ORLIXOS_TCC_LIBTCC1)"; \
