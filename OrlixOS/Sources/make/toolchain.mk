...
$(ORLIXOS_GAWK_TOOLCHAIN_STAMP): $(PROJECT_DIR)/Sources/make/toolchain.mk $(PROJECT_DIR)/Sources/make/config.mk | $(ORLIXOS_GAWK_TOOLCHAIN_DIR)
	...
	{ \
		...
		printf '%s\n' 'for arg in "$$@"; do' \
			...
			'esac' \
			'done' ''; \
		printf '%s\n' 'common=(--target=aarch64-linux-gnu -B "$$ORLIXOS_PACKAGE_TOOLCHAIN_DIR" "--sysroot=$$ORLIXOS_MLIBC_SYSROOT" -isystem "$$ORLIXOS_MLIBC_HEADERS" -D_GNU_SOURCE -fhosted -fno-builtin -ffixed-x18 "$$ORLIXOS_PACKAGE_CODE_MODEL_FLAG")' ''; \
