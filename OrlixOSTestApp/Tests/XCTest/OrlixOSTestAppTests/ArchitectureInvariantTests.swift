...
final class ArchitectureInvariantTests: XCTestCase {
    ...
    func testOrlixTCTIProductProfilesDisableDebugOracle() throws {
        let productProfiles = [
            "OrlixKernel/Sources/ports/orlix/configs/release_defconfig",
            "OrlixKernel/Sources/ports/orlix/configs/development_defconfig"
        ]
        ...
        for relativePath in productProfiles {
            ...
            XCTAssertTrue(
                config.contains("CONFIG_ORLIX_TCTI_HOSTED_EXEC=y"),
    ...
    func testOrlixTCTIProductionSourcesDoNotRequestJITRWXOrHostX18() throws {
        let files = try sourceFiles(under: [
            "OrlixKernel/Sources/ports/orlix/overlay/arch/orlix/hosted_exec/orlix_tcti",
            "OrlixKernel/Sources/ports/orlix/overlay/arch/orlix/include/asm/orlix_tcti.h",
            "OrlixKernel/Sources/ports/orlix/overlay/arch/orlix/mm/orlix_tcti_user_page.c",
            "OrlixKernel/Sources/ports/orlix/overlay/arch/orlix/mm/orlix_tcti_invalidate.c",
            "OrlixKernel/Sources/ports/orlix/overlay/arch/orlix/kernel/hosted_exec.c",
            "OrlixHostAdapter/Sources/OrlixHostAdapter/memory",
            "OrlixHostAdapter/Sources/OrlixHostAdapter/execution"
    ...
    func testKernelConformanceXCTestDoesNotDuplicateNativeTAPAssertions() throws {
        let relativePath = "OrlixOSTestApp/Tests/XCTest/OrlixKernelConformanceTests/OrlixKernelConformanceTests.swift"
        let source = try String(contentsOf: root.appendingPathComponent(relativePath))
        let supportPath = "OrlixOSTestApp/Tests/XCTest/Support/OrlixUpstreamXCTest.swift"
        ...
        for relativePath in relativePaths {
            ...
            guard manager.fileExists(atPath: url.path, isDirectory: &isDirectory) else {
                throw NSError(
                    ...
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: "missing expected source root: \(relativePath)"]
                )
            }
