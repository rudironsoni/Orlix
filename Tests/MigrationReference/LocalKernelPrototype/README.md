...
- `kunit/**`: keep temporarily as migration reference; migrate useful kernel-internal assertions to Linux KUnit under `Linux/ports/orlix/overlay/arch/orlix/**/<owner>_test.c` or `Linux/ports/orlix/overlay/drivers/orlix/**/<owner>_test.c`.
...
- `*Contract.c`, `*Contract.h`, and `*Tests.m`: keep temporarily as migration reference; migrate Linux-visible behavior to Linux kselftest, delete upstream-owned duplicate contracts, and move only narrow iOS host mechanics into `Tests/XCTest/OrlixHostAdapterTests/`.
