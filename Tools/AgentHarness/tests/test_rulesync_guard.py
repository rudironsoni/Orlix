...
class RuleSyncGuardTests(unittest.TestCase):
    ...
    def test_workflow_contracts(self):
        ...
        guard = generate.index("Wait for pull-request guard")
        merge = generate.index('gh pr merge "$PR_NUMBER"')
        self.assertLess(verify, status)
        self.assertLess(status, guard)
        self.assertLess(guard, merge)
        self.assertIn('.workflowName == "RuleSync generated output guard"', generate)
        self.assertIn('.__typename == "CheckRun"', generate)
