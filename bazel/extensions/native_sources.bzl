...
def _kernel_toolchain_repository_impl(ctx):
    ...
    for path in watched["trees"]:
        ctx.watch_tree(path)
    ctx.file("BUILD.bazel", 'exports_files(["identity.json", "compiler-identity.json", "compiler-runtime-identity.json", "mlibc-identity.json", "guest-compiler-identity.json", "coreutils-identity.json", "bash-identity.json"], visibility = ["//visibility:public"])\n')
