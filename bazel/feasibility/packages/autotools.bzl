...
def _pinned_env(ctx):
    ...
    if not developer_dir:
        fail("autotools package requires action_env DEVELOPER_DIR")
    env = {
...
def _autotools_package_impl(ctx):
    ...
    ctx.actions.write(
        ...
        "\n".join([
            ...
            "LIBS_EXTRA=%s" % _shell_squote(ctx.attr.libs_extra),
            "EXTRA_ARCHIVES=%s" % _shell_squote(",".join(ctx.attr.extra_archives)),
            "VERIFY_FILES=%s" % _shell_squote(",".join(ctx.attr.verify_files)),
            "",
        ]),
    )
    extra_path = extra.install_tree.path if extra else ""
    ctx.actions.run_shell(
        mnemonic = "OrlixGuestPackage",
        progress_message = "Building %s with configure/Make" % ctx.attr.package_name,
        command = r"""
set -euo pipefail
exec_root="$PWD"
...
test -s "$runtime"
work="$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/orlix-$PACKAGE_NAME.XXXXXX")"
trap '/bin/rm -rf "$work"' EXIT
src="$(/usr/bin/dirname "$configure")"
/bin/mkdir -p "$work/src" "$work/build" "$work/dest" "$work/toolchain"
/bin/cp -pR "$src/." "$work/src"
/usr/bin/find "$work/src" -type d -exec /bin/chmod u+w {} +
/bin/chmod +x "$work/src/configure" 2>/dev/null || true
/usr/bin/find "$work/src" -name Makefile.in -exec /usr/bin/touch {} +
...
code_model="-fPIE"
ld_common="--target=aarch64-linux-gnu -fuse-ld=lld -nostdlib -Wl,--gc-sections -L$libraries"
...
if [ "$IN_TREE" = 1 ]; then
  ...
  /bin/cp -pR "$work/src" "$work/build"
  /usr/bin/find "$work/build" -name Makefile.in -exec /usr/bin/touch {} +
  cd "$work/build"
  conf="./configure"
else
  cd "$work/build"
  conf="$work/src/configure"
fi
...
while [ -n "$remaining" ]; do
  ...
  case "$step" in
    *@USELIBS@*)
      ...
      # shellcheck disable=SC2086
      "$gmake_bin" $step LIBS="$LIBS" || { echo "orlix-autotools: make failed $?" >&2; /bin/ls -la . .libs 2>/dev/null || true; exit 1; }
      ;;
    *)
      echo "orlix-autotools: $gmake_bin $step" >&2
      # shellcheck disable=SC2086
      "$gmake_bin" $step || { echo "orlix-autotools: make failed $?" >&2; /bin/ls -la . .libs 2>/dev/null || true; exit 1; }
      ;;
  esac
done
...
""",
        arguments = [
            ...
            digest.path,
            recipe.path,
            extra.install_tree.path if extra else "",
        ],
        inputs = depset(
            direct = [
                ctx.file.configure,
            ...
            ] + ctx.files.sources + ([extra.install_tree, extra.source_input_digest] if extra else []),
        ),
...
orlix_autotools_package = rule(
    ...
    attrs = {
        ...
        "libs_extra": attr.string(default = ""),
