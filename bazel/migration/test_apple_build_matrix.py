...
class AppleBuildMatrixTests(unittest.TestCase):
    ...
    def test_product_graph_names_kernel_and_public_sdk(self) -> None:
        ...
        packages = (root / "bazel/feasibility/packages/BUILD.bazel").read_text(encoding="utf-8")
        self.assertIn("_UAPI = \"//bazel/feasibility/kernel:selected_uapi\"", packages)
        self.assertIn("_SYSROOT = \"//bazel/feasibility/mlibc:selected_sysroot\"", packages)
    ...
    def test_promoted_app_selects_locked_buildset(self) -> None:
        ...
        self.assertIn("composition.json", mk)
        self.assertIn("locked-buildset.json", mk)
        self.assertNotIn("promoted kernel composition must record the locked buildset", mk)
        self.assertIn("imported/rootfs/product/initramfs.cpio.gz", mk)
