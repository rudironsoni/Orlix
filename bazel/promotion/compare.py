...
def compare_trees(
    ...
    if artifact_format == ARTIFACT_IDENTITY_V2_FORMAT:
        left_identity = {
            "format": ARTIFACT_IDENTITY_V2_FORMAT,
            "version": ARTIFACT_IDENTITY_V2_VERSION,
            "digest": read_digest(left / V2_DIGEST_FILENAME),
        }
        right_identity = {
            "format": ARTIFACT_IDENTITY_V2_FORMAT,
            "version": ARTIFACT_IDENTITY_V2_VERSION,
            "digest": read_digest(right / V2_DIGEST_FILENAME),
        }
...
def write_proposal(
    ...
    if artifact_identity is None:
        ...
        if identity["digest"] != require_sha256(digest):
            raise ValueError("artifact identity digest differs from proposal digest")
        payload = {
            "schema": 2,
            "component": component,
            "unsigned_digest": identity["digest"],
    ...
    if any((args.proposal, args.sbom, args.in_toto, args.lock_proposal)) or args.first_tree or args.second_tree:
        ...
        if args.artifact_identity_format == ARTIFACT_IDENTITY_V2_FORMAT:
            ...
            if digest != declared_digest:
                raise ValueError("artifact identity digest differs from component product")
            artifact_identity = {
                "format": ARTIFACT_IDENTITY_V2_FORMAT,
                "version": ARTIFACT_IDENTITY_V2_VERSION,
                "digest": digest,
            }
        elif args.artifact_identity_format == LEGACY_IDENTITY_FORMAT:
            if not args.artifact_identity_marker:
                raise ValueError("legacy artifact identity marker is required")
            marker = args.artifact_identity_marker
            for tree in (Path(args.first_tree), Path(args.second_tree)):
                marker_path = tree / marker
                if not marker_path.is_file() or marker_path.is_symlink():
                    raise ValueError("legacy artifact identity marker is missing")
                if marker_path.read_text(encoding="utf-8").strip() != digest:
                    raise ValueError("legacy artifact identity marker differs from digest")
        ...
        elif args.artifact_identity_format is not None:
            raise ValueError(f"unknown artifact identity format: {args.artifact_identity_format!r}")
        else:
            content_digest = compare_trees(args.first_tree, args.second_tree)
    print(digest)
    if args.proposal:
