...
Create the iOS host app as `OrlixTerminal`. It should follow the MobileGhosttyApp terminal-first shape rather than a blank proof-only host.
...
XCTest should target `OrlixTerminal` for iOS-hosted Orlix launch, Linux test-output collection, and terminal/host integration proof.
