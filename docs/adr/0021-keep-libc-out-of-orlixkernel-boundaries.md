...
`OrlixHostAdapter` may include and link Apple SDK, Darwin, POSIX, and host libc APIs in its private implementation files. That is an implementation detail of the iOS host side. The kernel-visible contract between `OrlixKernel` and `OrlixHostAdapter` must remain freestanding and Linux-shaped.
...
- HostAdapter implementation files may use host libc and platform APIs privately.
