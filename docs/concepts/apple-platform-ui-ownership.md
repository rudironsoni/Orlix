...
Orlix ships one Apple app target across iOS and macOS. Shared feature code is valuable, but large SwiftUI files have historically accumulated many inline `#if os(iOS)` and `#if os(macOS)` branches, plus product UI type names such as `iOSServerRow`, `MacOSZenModePanel`, and legacy Stats macOS helper names.
...
- `Orlix/Features/Stats/UI/ServerStatsView.swift`: 28 platform gates, about 5036 lines
- `Orlix/Features/RemoteFiles/UI/RemoteFileBrowserScreen.swift`: 25 platform gates, about 2121 lines
- `Orlix/Features/TerminalSessions/UI/Terminal/TerminalContainerView.swift`: 22 platform gates, about 965 lines
- `Orlix/Features/Settings/UI/TerminalSettingsView.swift`: 22 platform gates, about 1940 lines
- `Orlix/Features/Servers/UI/ServerDetail/ServerFormSheet.swift`: 21 platform gates, about 1574 lines
- `Orlix/Features/Settings/UI/KeychainSettingsView.swift`: 18 platform gates, about 716 lines
- `Orlix/Features/Store/UI/ProUpgradeSheet.swift`: 16 platform gates, about 1329 lines
- `Orlix/Features/TerminalSessions/UI/Tabs/ConnectionTabsView.swift`: 12 platform gates, about 1335 lines
...
The `Orlix` target supports `iphoneos`, `iphonesimulator`, and `macosx` in one target. The project also uses synchronized filesystem groups, so adding Swift files under `Orlix/` is normally enough for inclusion.
...
```text
Orlix/Features/Example/UI/iOS/ExampleView.swift
```
...
```text
Orlix/Features/TerminalSessions/UI/Terminal/
...
Orlix/Features/TerminalSessions/UI/Tabs/
...
Orlix/Features/Servers/UI/ServerDetail/
...
- Done: split the custom theme manager's iOS navigation/list body and macOS dialog/list body into `TerminalSettingsView+iOS.swift` and `TerminalSettingsView+macOS.swift`, leaving import, edit, delete, and theme-creation state in the shared settings file.
- Done: split the custom theme save sheet's iOS navigation shell and macOS dialog/action row into `TerminalSettingsView+iOS.swift` and `TerminalSettingsView+macOS.swift`, leaving name entry, target selection, validation, and save behavior shared.
- Done: split the theme builder's iOS navigation shell and macOS dialog/action row into `TerminalSettingsView+iOS.swift` and `TerminalSettingsView+macOS.swift`, leaving theme parsing, validation, fields, preview, and save behavior shared.
...
- Done: split Pro upgrade presentation modifier behavior into `ProUpgradeSheet+iOS.swift` and `ProUpgradeSheet+macOS.swift`, leaving the shared modifier as a platform handoff and preserving the iOS sheet path and macOS window path.
...
1. Done: move app delegates into `AppDelegate+iOS.swift` and `AppDelegate+macOS.swift`.
...
```sh
DEVELOPER_DIR=/Applications/Xcode-beta.app/Contents/Developer xcodebuild -project Orlix.xcodeproj -scheme Orlix -destination 'generic/platform=iOS' CODE_SIGNING_ALLOWED=NO build
DEVELOPER_DIR=/Applications/Xcode-beta.app/Contents/Developer xcodebuild -project Orlix.xcodeproj -scheme Orlix -destination 'platform=macOS' build
```
