...
Herdr owns Session, Workspace, Tab, and Pane names and topology. `OrlixTerminalTestApp` is the private terminal application test host. `OrlixOSTestApp` is the private OrlixOS Linux userspace test host. Retired module, target, lifecycle, and payload names have no compatibility aliases.
