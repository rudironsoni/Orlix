...
Recorded that the 266 unique `SCALAR_FP` leaves have production decode and EL0 classification. The 140 always-on EL0 FEAT_FP S/D leaves have production observations for destination `FP_SIMD` or `REGISTERS` or `FLAGS`, plus `PC`. `MEMORY`, `ATOMICITY`, and `ORDERING` are not applicable. Production EL0 proof compares destination SIMD, GPR, NZCV, and FPSR against a source-mnemonic host-asm oracle before ingest. Conversion leaves seed GPR `-8` and FP `-1.5`, then compare extra `+1.5`, `+2.5`, `+inf`, and `INT32_MIN` vectors before ingest. FCVT extra vectors seed the source width, not `is_double()`. Unscaled and fixed-point SCVTF and UCVTF also compare inexact GPR operands (`2^24+1` for dest S, `2^53+1` for dest D from X, plus the signed negatives for SCVTF) under each FPCR rounding mode before ingest. W-to-D unscaled forms stay exact, so they skip that rounding walk. FMADD and FNMADD compare fused cancellation with `Rn=Rm=1+2^-12` (`0x3f800800`, S) or `1+2^-27` (D) and `Ra=-1` under each FPCR rounding mode. FMSUB and FNMSUB use `Ra=+1` so the product cancels. The S-form product has a discarded `2^-24` bit, so a separate multiply-then-add does not match the fused result. FADD, FSUB, FMUL, FDIV, and FNMUL compare inexact operands under each FPCR rounding mode before ingest. GPR-backed conversions and FMOV also compare `Rn=31` (WZR/XZR reads as 0) and `Rd=31` (writes discarded). Seed and compare use `regs->sp`, not `regs[31]`. float2fix also compares fbits `1` and `16` on W forms and `1` and `32` on X forms, besides the maximum legal fbits. Unscaled SCVTF and UCVTF copy host FPSR, including IXC. FCSEL, FCCMP, and FCCMPE compare EQ, NE, CS, MI, VS, HI, GE, GT, and AL, including the false-condition path. FCCMP/FCCMPE also compare fallback NZCV values that independently set N, Z, C, and V. FRINT leaves compare `+1.5`, `-1.5`, and `+2.5`; FRINTI also walks FPCR rounding modes. FMOV immediate compares imm8 `0x80`, `0x40`, `0x01`, and `0xff` besides zero. FABS compares a negative source. FSQRT compares a non-unit exact source (`4.0`) and inexact `2.0` under each FPCR rounding mode. Narrowing FCVT (`_SD`, `_HS`, `_HD`) also compares a halfway source under each FPCR rounding mode. FMAX, FMAXNM, FMIN, and FMINNM compare one quiet NaN against a finite value. FCMP, FCMPE, FCCMP, and FCCMPE compare a quiet NaN, including the IOC difference between the E and non-E forms, and a flush-to-zero min-subnormal versus zero under `FPCR.FZ`, including FPSR.IDC. Double forms seed true `1.0`/`2.0` bit patterns, not the 32-bit `1.0` pattern. FCVT seeds the source width, not the destination width. FCVT among H/S/D stays FEAT_FP. 32-bit float2fix encodings use `scale<5>=1`. 32-bit float2fix encodings with `scale<5>=0` stay `DECODE_UNSUPPORTED`. Ordinary leaves compare one V16-V31 encoding for each free Rd, Rn, Rm, and Ra field, plus `Rd==Rn`, `Rd==Rm`, and `Rd==Ra` when those fields are free. Unused SIMD lanes get a per-register poison, not `1.0`. Seed does not clear Rd when it aliases Rn, Rm, or Ra. `FMOV_64VX` writes a GPR from `Vn.D[1]`. `FMOV_V64I` writes `Vd.D[1]` from a GPR. The 126 optional FP16, FPRCVT, FRINTTS, JSCVT, and BF16 leaves remain unclassified blockers in the EL0 complete target. Runtime rejection does not complete them. Those features are not advertised. `linux_proof_executed` stays 0. The 4,350-leaf coverage task stays doing.
