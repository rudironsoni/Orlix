...
The canonical OrlixKernel proof artifact is the iOS app-hosted integration that actually runs inside the OrlixOS test host: private static `OrlixKernel.xcframework` plus `OrlixHostAdapter/Sources`, the public `OrlixOS` OrlixMachine and distribution-resource surface where relevant, `OrlixOSTestApp`, and simulator/device execution.
