...
Each obligation must bind its pinned AARCHMRS identity and applicability condition, its exact external DDI0602 document or ASL locator and digest, its independently authored production owner, explicit expected observations derived from that official specification, production-path KUnit owner and executed result, applicable Linux kselftest owner and executed result or typed `not_applicable` reason, and runtime `HWCAP` or `HWCAP2` dependency. When the matching official DDI0602 release provides no semantic entry and AARCHMRS explicitly records `// Not specified`, the row instead retains the exact AARCHMRS operation locator and digest with a typed `official_semantics_not_specified` disposition. That disposition closes source accounting only and remains a semantic-completion blocker. Orlix-authored implementation and tests cannot be their own semantic authority. A source locator or digest proves provenance only; the cited official specification defines the expected behavior, while execution evidence proves the Orlix implementation produced it.
...
1. The external Arm archive identity and every retained locator or digest match the pinned `2026-06` provenance contract; any absent DDI0602 semantic entry is accepted only with the exact AARCHMRS `// Not specified` source locator and digest and remains blocking.
