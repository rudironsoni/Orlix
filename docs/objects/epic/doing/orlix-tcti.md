...
The durable completion boundary is complete AArch64 ISA-on-ISA execution for the guest-exposed EL0 profile. Every legal instruction family requires architectural decode, production lowering or gadget execution, exact state-transition semantics, and deterministic exception behavior. Workload traces may order the work, but passing a workload cannot retire missing instruction families or justify reduced semantics.
