...
Rework the existing macOS terminal and file tab strips so they feel like native Mac/Safari-style tabs while preserving Orlix's current session architecture.
...
`Orlix/Features/TerminalSessions/UI/Tabs/ConnectionTabsView.swift` owns the macOS terminal tab strip:
...
`Orlix/Features/RemoteFiles/UI/Components/RemoteFileTabChrome.swift` owns the macOS file tab strip:
...
`Orlix/Core/UI/ServerTabChrome.swift` already owns shared controls and metrics:
...
- `Orlix/Features/TerminalSessions/UI/Splits/TerminalSplitContainerView.swift`
...
- `xcodebuild` macOS Debug build for Orlix.
