...
updated: 2026-07-15
status: implemented
summary: "Remote Shell Profiles and Multi-Shell SSH Support (Draft Spec)."
part_of:
...
## Summary
Refactor Orlix's remote SSH bootstrap logic around explicit remote shell profiles so shell-specific behavior is modeled in one place instead of being spread across generic SSH, tmux, mosh, and terminal restore paths.
...
## Problem
Orlix's current SSH flow is mostly POSIX-shaped.
...
Relevant files:
- `Orlix/Services/SSH/RemoteTerminalBootstrap.swift`
- `Orlix/Services/SSH/SSHClient.swift`
- `Orlix/Services/SSH/RemoteTmuxManager.swift`
- `Orlix/Services/SSH/RemoteMoshManager.swift`
- `Orlix/Services/Stats/ServerStatsCollector.swift`
- `Orlix/Services/Stats/Platforms/PlatformStatsCollector.swift`
- `Orlix/Services/Stats/Platforms/WindowsStatsCollector.swift`
- `Orlix/Views/Terminal/SSHTerminalWrapper.swift`
- `Orlix/Views/Splits/TerminalView.swift`
- `Orlix/Views/Stats/ServerStatsView.swift`
...
- `Fallback to plain SSH`: user settings remain available, but Orlix resolves capabilities at runtime and silently uses normal SSH shell behavior instead of tmux/mosh-specific behavior.
...
Create a shell strategy surface for operations Orlix performs after connect.
...
Orlix needs a best-effort way to resolve a remote shell profile early in connection startup.
...
Important UI rule:
- Orlix must not hide, disable, or block tmux/mosh UI based only on shell capability resolution.
- Existing settings and selections remain user-visible.
- At runtime, if the resolved remote environment does not support tmux or mosh, Orlix automatically falls back to normal SSH shell behavior.
...
- Orlix must not automatically rewrite the saved server connection mode or tmux/mosh preferences during fallback.
- When fallback repeats, Orlix may present a non-blocking recommendation to switch the server to `SSH`.
...
- if capability resolution is uncertain, Orlix should prefer safe fallback behavior over hard failure
...
Possible additions:
- `Orlix/Services/SSH/RemoteShellProfile.swift`
- `Orlix/Services/SSH/RemoteEnvironmentResolver.swift`
- `Orlix/Services/SSH/RemoteShellDetector.swift`
- `Orlix/Services/SSH/RemoteShellCommandBuilder.swift`
