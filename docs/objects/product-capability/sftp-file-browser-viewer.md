...
updated: 2026-07-15
status: implemented
summary: "SFTP Remote File Browser & Viewer (Spec)."
part_of:
...
## Problem
Orlix can connect and run shell commands, but users cannot browse remote files or quickly inspect file contents without typing shell commands (`ls`, `cat`, `less`) in Terminal. This adds friction for routine tasks like checking config files, logs, and deployment artifacts.
...
### macOS
- Update `ConnectionTerminalContainer` toolbar picker in `Orlix/Views/Tabs/ConnectionTabsView.swift`:
...
### iOS
- Update segmented control in `Orlix/Views/iOS/iOSContentView.swift`:
...
### New Models
Create `Orlix/Models/RemoteFile.swift`:
...
### SSH / SFTP Layer
Extend `SSHClient` + `SSHSession` in `Orlix/Services/SSH/SSHClient.swift` with SFTP operations backed by `libssh2_sftp`:
- `listDirectory(path:)`
- `stat(path:)`
...
Create `Orlix/Managers/RemoteFileBrowserManager.swift` (`@MainActor`, `ObservableObject`):
- Per-server state:
  - current path
  - breadcrumbs
  - entries
...
### macOS Path
Update `Orlix/Views/Tabs/ConnectionTabsView.swift`:
...
### iOS Path
Update `Orlix/Views/iOS/iOSContentView.swift`:
...
### New Views
Create:
- `Orlix/Views/Files/RemoteFileBrowserView.swift`
- `Orlix/Views/Files/RemoteFileViewerView.swift`
- `Orlix/Views/Files/RemoteFileRow.swift`
...
### Unit Tests
- Add `OrlixTests/RemoteFileBrowserManagerTests.swift`:
  - path navigation
  - sort and hidden-file filtering
  - state restoration
- Add `OrlixTests/SFTPPathNormalizationTests.swift`:
  - `.` / `..` handling
  - root handling
  - symlink-safe display path logic
- Add `OrlixTests/SFTPPreviewTests.swift`:
