...
- Match terminal tab behavior closely enough that the feature feels native to Orlix.
...
- When `Files` becomes active for a server with no file tabs yet, Orlix should auto-create one file tab so the current one-tab behavior remains unchanged.
...
### Domain
Add:
- `Orlix/Features/RemoteFiles/Domain/RemoteFileTab.swift`
...
Add:
- `Orlix/Features/RemoteFiles/Domain/RemoteFileTabSnapshot.swift`
...
### Application
Add:
- `Orlix/Features/RemoteFiles/Application/RemoteFileTabManager.swift`
