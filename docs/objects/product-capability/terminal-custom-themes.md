...
## Problem
Orlix ships many built-in Ghostty themes, but users cannot create or import their own themes. Theme choice is currently name-based and built-in-oriented, which blocks:
...
### Data Model
Create `Orlix/Models/TerminalTheme.swift`:
...
### Managers
Create `Orlix/Managers/TerminalThemeManager.swift` (`@MainActor`, `ObservableObject`):
...
### Ghostty Integration
Update `Orlix/GhosttyTerminal/Ghostty.App.swift`:
...
Update `Orlix/Utilities/ThemeColorParser.swift`:
...
- If selected ref is invalid/missing:
  - dark fallback: `Orlix Dark`
  - light fallback: `Orlix Light`
