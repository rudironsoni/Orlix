...
- Phase 1 stays entirely in Orlix app code and finishes the post-dismiss software-keyboard behavior.
...
- `selectionRects(for:)` still returns `[]`, so UIKit does not yet have real selection geometry from Orlix.
...
Outcome:
- touch selection ownership moves toward Orlix instead of Ghostty mouse-selection semantics
...
Rules:
1. When a terminal first opens or becomes active, Orlix may still auto-focus the keyboard.
...
- Orlix owns touch selection interaction
- Orlix owns the visible selection state and action menu behavior
...
### Phase 2
- add an iPhone touch-selection controller under `Orlix/GhosttyTerminal/`
...
- likely one or more new iPhone selection files under `Orlix/GhosttyTerminal/`
