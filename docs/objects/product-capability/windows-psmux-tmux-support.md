...
updated: 2026-07-15
status: proposed
summary: "Windows psmux tmux Support (Draft Spec)."
part_of:
...
Add Windows tmux-compatible session persistence in Orlix by supporting [psmux](https://github.com/psmux/psmux) as the Windows tmux runtime.

Orlix already has shell-aware SSH startup for Windows, but it still treats tmux as a Unix-only capability. psmux changes that assumption: it exposes a tmux-compatible CLI on native Windows and ships `psmux.exe`, `pmux.exe`, and `tmux.exe` aliases. The implementation should keep the existing Unix tmux behavior intact while adding a separate Windows psmux backend with Windows-safe probing, config writing, quoting, attach/create, install, cleanup, and current-path commands.

Research date: 2026-06-03.

## Goals

- Support tmux-style Orlix-managed sessions on Windows OpenSSH hosts when psmux is installed.
...
- Support Windows default shells used by Orlix today:
...
- Full control-mode integration in Orlix.
...
### psmux tmux compatibility relevant to Orlix

psmux claims and implements the tmux CLI surface Orlix currently depends on:
...
The documented command reference includes the key flags Orlix uses:
...
- `psmux -V`, `pmux -V`, and `tmux -V` intentionally print `tmux <version>` for compatibility. Orlix should not rely on `tmux -V` output to distinguish real tmux from psmux.
- psmux reads config from `~/.psmux.conf`, `~/.psmuxrc`, `~/.tmux.conf`, and `~/.config/psmux/psmux.conf`, but Orlix should pass its own generated config with `-f`.
- psmux supports `terminal-overrides` as a compatibility no-op. `terminal-features` is not a first-class option in the current source, but psmux stores unknown hyphenated tmux options in `user_options` instead of failing or injecting them as environment variables. Orlix may include it for config parity, but must not depend on it for behavior.
- psmux documents that `alternate_on` is always false because ConPTY consumes alternate-screen transitions. Orlix's current Unix tmux mouse-scroll binding uses `#{alternate_on}`, so Windows psmux must not reuse that binding unchanged.
...
- Issue `#73` remains open for mangled output in at least one Windows Server SSH setup. Orlix should validate Windows Server explicitly before release and keep plain SSH fallback reliable when psmux cannot be used.

## Current Orlix State

Relevant files:

- `Orlix/Core/SSH/RemoteEnvironmentResolver.swift`
- `Orlix/Core/SSH/RemoteTerminalBootstrap.swift`
- `Orlix/Core/SSH/RemoteTmuxManager.swift`
- `Orlix/Core/SSH/SSHClient.swift`
- `Orlix/Features/TerminalSessions/Application/ConnectionSessionManager.swift`
- `Orlix/Features/TerminalSessions/Application/TerminalTabManager.swift`
- `Orlix/Features/TerminalSessions/Application/TmuxAttachResolver.swift`
- `OrlixTests/RemoteEnvironmentTests.swift`
- `OrlixTests/RemoteTmuxManagerParserTests.swift`
...
- The generated config path is Unix-shaped: `~/.orlix/tmux.conf`.
...
- Unknown Windows shell: do not probe unless PowerShell is available and Orlix can run a PowerShell exec command safely.
...
if ($cmd) {
  ...
  if ($LASTEXITCODE -eq 0) { Write-Output "__ORLIX_TMUX_OK__" }
}
```
...
```cmd
where psmux >NUL 2>NUL && psmux -V >NUL 2>NUL && echo __ORLIX_TMUX_OK__
where pmux >NUL 2>NUL && pmux -V >NUL 2>NUL && echo __ORLIX_TMUX_OK__
```

`tmux` fallback should be implemented as a separate confirmed-alias probe, not as part of the first-pass availability check.

### 4) Use a Windows Orlix config path

Use an Orlix-owned generated config file:

- PowerShell path: `$HOME\.orlix\psmux.conf`
- cmd path: `%USERPROFILE%\.orlix\psmux.conf`

Always pass it through psmux's `-f` flag:

```powershell
& $psmux -f "$HOME\.orlix\psmux.conf" attach-session -t "=orlix_..."
```
...
```powershell
New-Item -ItemType Directory -Force -Path "$HOME\.orlix" | Out-Null
@'
...tmux config...
'@ | Set-Content -Encoding UTF8 -NoNewline "$HOME\.orlix\psmux.conf"
```
...
- `terminal-features`: psmux stores it as an unknown hyphenated option and does not fail, so it can stay for config parity, but Orlix must not rely on it for hyperlink or terminal capability behavior.
...
- `allow-set-title on`: keep it in the Orlix-managed psmux config to preserve the existing Orlix tmux behavior where the active pane title can propagate to the outer terminal title. Accept that PowerShell 7 will usually make that title the current working directory. This is less harmful in Orlix because the psmux status bar is hidden; if the resulting outer title is noisy, handle that as a targeted follow-up rather than disabling title propagation in V1.
...
- skip tmux
- cleanup detached Orlix-managed sessions
...
& $psmux list-sessions -F '#{session_name} #{session_attached}' |
  ForEach-Object { ... } # filter orlix prefix and attached count
```
...
When psmux is missing on Windows and the user taps the existing install action, Orlix should run a Windows psmux install script instead of the Unix tmux package-manager script.
...
V1 should keep the normal tmux UX. psmux intentionally markets itself as a tmux-compatible Windows runtime, ships a `tmux.exe` alias, and supports the tmux CLI surface Orlix needs. Existing Orlix UI copy, settings, prompts, and localized strings should continue saying "tmux" wherever they describe the feature generically.
...
- psmux config path is Windows-shaped and Orlix-owned.
...
- Keep most Orlix-generated tmux config parity. `terminal-features` is accepted/stored by psmux as an unknown hyphenated option, but should be treated as best-effort/no-op under psmux.
- Keep `allow-set-title on` and `set-titles on` for Orlix-managed psmux sessions. PowerShell 7 CWD titles are acceptable for V1 because Orlix hides the psmux status bar and uses title propagation for outer terminal metadata.
...
- Whether Windows Server issue `#73` reproduces in Orlix/Ghostty specifically.
- Whether psmux should grow first-class `terminal-features` handling upstream; Orlix does not need to block on this.
...
- Existing Orlix shell profile spec: `docs/specs/remote-shell-profiles.md`
