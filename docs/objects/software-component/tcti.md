...
Production TCTI behavior, the ISA inventory, diagnostics, and correctness proof are C-native under `arch/orlix`. External-language code may not generate TCTI behavior, define the inventory, model architectural semantics, act as an instruction oracle, or replace the owning KUnit and kselftest proof.
