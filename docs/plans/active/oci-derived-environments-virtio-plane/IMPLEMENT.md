...
## 2026-06-20 pipe-poll Linux oracle case expansion

Goal:
- Extend Linux oracle coverage toward plan's pipe readiness substrate gap
  before OCI Runtime lifecycle, feature reporting, and product `orlix run`
  claims.
- Keep project-side oracle tooling Swift/C/JSON/log only; do not add Python to
  `tools/` or any iOS/app runtime path.
- Reuse existing Orlix-owned `pipe_poll_probe` semantics instead changing
  kernel behavior in this checkpoint.

Changes:
- `tools/orlix-linux-oracle/cases/pipe-poll.json`
  - Added third oracle case for nonblocking pipe read `EAGAIN`,
    zero-timeout `poll`, writable/readable readiness, payload read, and hangup
    behavior.
- `tools/orlix-linux-oracle/fixtures/pipe_poll_probe.c`
  - Added Linux-runner fixture emits structured observation JSON for pipe
    creation, empty-read `EAGAIN`, empty-poll timeout, write-end readiness,
    read-end readiness after write, payload read, and read-end hangup after
    writer close.
- `tools/orlix-linux-oracle/orlix-linux-oracle.swift`
  - Added `pipe-poll` Linux fixture result construction.
  - Added `pipe-poll` Orlix kselftest log conversion based on existing
    `pipe_poll_probe` output markers.
  - Extended Swift-native `self-test` coverage for case validation, matching
    comparison, drift detection, and Orlix log conversion.
- `tools/orlix-linux-oracle/samples/`
  - Added matching Linux and Orlix `pipe-poll` result samples.
  - Added deliberate `pipe-poll` drift sample.
  - Added representative Orlix `pipe_poll_probe` kselftest log sample.
- `docs/plans/active/oci-derived-environments-virtio-plane/PLAN.md`
  - Updated current proved state to include the third local oracle case.
...
- `rtk swiftc -parse tools/orlix-linux-oracle/orlix-linux-oracle.swift`
  - Result: exited 0.
- `rtk clang -fsyntax-only -Wall -Wextra tools/orlix-linux-oracle/fixtures/pipe_poll_probe.c`
- Result: exited 0.
- `rtk swift tools/orlix-linux-oracle/orlix-linux-oracle.swift validate-case tools/orlix-linux-oracle/cases/pipe-poll.json`
- Result: exited 0, output `case pipe-poll is valid`.
- `rtk swift tools/orlix-linux-oracle/orlix-linux-oracle.swift compare --case tools/orlix-linux-oracle/cases/pipe-poll.json --linux-result tools/orlix-linux-oracle/samples/pipe-poll.linux.json --orlix-result tools/orlix-linux-oracle/samples/pipe-poll.orlix.json`
- Result: exited 0, output `case pipe-poll matches`.
- `rtk swift tools/orlix-linux-oracle/orlix-linux-oracle.swift orlix-result-from-log --case tools/orlix-linux-oracle/cases/pipe-poll.json --log tools/orlix-linux-oracle/samples/pipe-poll.orlix-kselftest.log --output /private/tmp/pipe-poll.orlix.generated.json`
- Result: exited 0, output `wrote Orlix result for case pipe-poll: /private/tmp/pipe-poll.orlix.generated.json`.
- `rtk swift tools/orlix-linux-oracle/orlix-linux-oracle.swift compare --case tools/orlix-linux-oracle/cases/pipe-poll.json --linux-result tools/orlix-linux-oracle/samples/pipe-poll.linux.json --orlix-result /private/tmp/pipe-poll.orlix.generated.json`
- Result: exited 0, output `case pipe-poll matches`.
- `rtk swift tools/orlix-linux-oracle/orlix-linux-oracle.swift compare --case tools/orlix-linux-oracle/cases/pipe-poll.json --linux-result tools/orlix-linux-oracle/samples/pipe-poll.linux.json --orlix-result tools/orlix-linux-oracle/samples/pipe-poll.orlix-drift.json`
...
- This does not prove fresh real-Linux oracle execution, fresh app-hosted Orlix
  runtime execution, imported-environment pipe readiness, `select`, `epoll`,
  PTY readiness, socket readiness, signal-interrupted `poll`, nonzero timeout
...
Current status:
- Active plan remains in progress.
- Linux oracle tooling now covers three local cases: `path-errno`, `fd-exec`,
  and `pipe-poll`.
- Next substrate work should continue converting existing Orlix kselftest
  probes such as `pipe_select_probe`, `pipe_epoll_probe`, `signal_wait_probe`,
...
## 2026-06-21 - OCI Runtime state report baseline

Changed:

- Added `OrlixOCIRuntimeStateStatus` and `OrlixOCIRuntimeStateReport` to `OrlixOS/Sources/Session/OrlixOCIImageLayout.swift`.
- Extended `OrlixOCIRuntimeLifecycleController` with `stateReport()`.
- Added lifecycle state-report tests to `OrlixOS/Tests/XCTest/OrlixOSTests/OrlixTerminalSessionTests.swift`.

Scope:

- The state report is deterministic OrlixOS data output for lifecycle records in `created`, `running`, and `stopped` states.
- The report includes OCI version, id, status, pid, bundle path, annotations, and optional exit status.
- `configured` and `deleted` states reject state reporting deterministically with `stateUnavailable`.
- Parsed `annotations` now flow through `OrlixOCIRuntimeConfigDescriptor` into the state report after validation for empty/NUL keys or NUL values.
- `jsonData()` emits stable sorted JSON for the report.
...
- Targeted `xcodebuild` for the state-report tests still failed before XCTest execution because CoreSimulator returned service errors including `CoreSimulator.SimError` and Code 409.

Non-claims:

- This is not Linux process execution or a full OCI Runtime `state` command implementation.
- This is not app-hosted product runtime proof.
- This does not prove Linux `execve`, wait/reap, signal delivery, or actual exit-status runtime behavior.

## 2026-06-21 - Current status after state report baseline

Current status:

- OrlixOS now has deterministic OCI Runtime feature reporting, expanded minimal Linux OCI Runtime `config.json` parsing, lifecycle state ordering, and OCI-shaped state report data for created/running/stopped records.
- Lifecycle work remains data/state semantics only; actual Linux execution and app-hosted runtime proof remain unproven.
- OrlixKernel substrate proof has recent coverage for virtio-fs path nodes, cgroup v2 child lifecycle, `/proc/net`/rtnetlink/loopback TCP/UDP, fd aliases, and pseudo-fs/device nodes.
- Targeted simulator XCTest execution remains blocked by CoreSimulator service errors, so state-report tests are not claimed green through XCTest execution.
...
## 2026-06-21 - Init Parser Key Contract Guard

Status: compile/build checkpoint accepted; runtime execution still unproven.

Changes:

- Added `testEnvironmentRootImageCommandLineKeysMatchInitParserContract` in `OrlixOS/Tests/XCTest/OrlixOSTests/OrlixTerminalSessionTests.swift`.
- The test ties the Swift command-line key constants generated by `OrlixEnvironmentRootImage` to the literal keys consumed by `OrlixOS/Sources/init/init.c`:
  - `orlix.exec=`
  - `orlix.argv%d=`
  - `orlix.env%d=`
  - `orlix.cwd=`
  - `orlix.uid=`
  - `orlix.gid=`
- This guards the existing OrlixOS-to-Linux `/init` handoff against silent drift while preserving the existing Linux-facing command-line contract.

Validation:

- `rtk xcrun swiftc -parse OrlixOS/Tests/XCTest/OrlixOSTests/OrlixTerminalSessionTests.swift` passed.
- `rtk xcrun swiftc -typecheck OrlixOS/Sources/Session/OrlixRootfsImport.swift OrlixOS/Sources/Session/OrlixEnvironment.swift OrlixOS/Sources/Session/OrlixEnvironmentImageMaterialization.swift OrlixOS/Sources/Session/OrlixOS.swift OrlixOS/Sources/Session/OrlixOCIImageLayout.swift OrlixOS/Sources/Session/OrlixStoragePolicy.swift OrlixOS/Sources/Session/OrlixHostDirectoryMetadata.swift /private/tmp/OrlixOCIRuntimeConfigParserCheck.swift` passed.
- `rtk xcodebuild -project OrlixSystem.xcodeproj -scheme OrlixOSTests -destination 'platform=iOS Simulator,name=iPhone 17,OS=26.5' build-for-testing` passed with exit 0.
- `rtk git diff --check` passed.
- `rtk python3 -m unittest discover .codex/hooks/tests` passed: 32 tests.
- `rtk python3 -m unittest discover .codex/rules/tests` passed: 5 tests.
- `rtk python3 .codex/hooks/compact_plan_check.py` exited 0 with the pre-existing warning: `IMPLEMENT.md has stale pending/blocked status that appears contradicted by later green status`.
...
- This does not prove app-hosted runtime execution.
- This does not prove `/init` successfully executes the OCI-derived command at runtime.
- This does not prove Linux `execve`, stdio, exit status, namespaces, cgroups, networking, or OCI Runtime lifecycle compliance.
- This does not add a HostAdapter surface, custom ABI, Docker/runc path, or Apple container runtime.
...
- Added `OrlixKernel/Sources/ports/orlix/overlay/tools/testing/selftests/orlix/cgroup_namespace_probe.c`.
- Registered `cgroup_namespace_probe` in `OrlixKernel/Sources/ports/orlix/overlay/tools/testing/selftests/orlix/Makefile`.
- Inspected the current Linux 6.12 port tree and did not add a defconfig symbol for `CONFIG_CGROUP_NS`; this tree exposes cgroup namespace behavior through the existing Linux cgroup/namespace code and `CLONE_NEWCGROUP`, not a separate durable Orlix config knob.

Linux surface covered:

- `unshare(CLONE_NEWCGROUP)` changes the Linux-visible `/proc/self/ns/cgroup` namespace inode.
- `/proc/self/cgroup` remains readable inside the new cgroup namespace and exposes the cgroup v2 root view.
...
- `rtk make -f OrlixKernel/Makefile __kselftest-initramfs PROFILE=release` passed and packaged `Build/OrlixMLibC/test-initramfs/release/OrlixTestInitramfs.bundle`.
- `rtk grep -n "cgroup_namespace_probe" Build/OrlixMLibC/kselftest/release/kselftest-list.txt` found `orlix:cgroup_namespace_probe`.
- `rtk grep -n "cgroup_namespace_probe" Build/OrlixMLibC/test-initramfs/release/OrlixTestInitramfs.bundle/initramfs.list` found the `/orlix/cgroup_namespace_probe` entry.
- `rtk test test -x Build/OrlixMLibC/kselftest/release/orlix/cgroup_namespace_probe` passed.
- `rtk python3 .codex/hooks/compact_plan_check.py` exited 0 with the known stale pending/blocked warning in this long `IMPLEMENT.md`.
...
## 2026-06-21 - OverlayFS Copy-Up And Unlink Substrate Probe

Status: build/install/package proof added for the OCI-relevant Linux OverlayFS writable-layer surface.

Changes:

- Added `OrlixKernel/Sources/ports/orlix/overlay/tools/testing/selftests/orlix/overlayfs_probe.c`.
- Registered `overlayfs_probe` in `OrlixKernel/Sources/ports/orlix/overlay/tools/testing/selftests/orlix/Makefile`.

Linux surface covered:

- A private mount namespace can mount a tmpfs-backed overlay workspace using Linux `mount("overlay", ...)`.
- OverlayFS reads through files from the lower directory.
- Writing an existing lower file through the merged view copies it up to the upper directory without modifying the lower file.
- Unlinking a lower file through the merged view hides it from the merged view while preserving the lower file.
...
- `rtk err make -f OrlixKernel/Makefile kselftest-install PROFILE=release` passed; output retained existing generated-kernel warnings.
- `rtk make -f OrlixKernel/Makefile __kselftest-initramfs PROFILE=release` passed and packaged `Build/OrlixMLibC/test-initramfs/release/OrlixTestInitramfs.bundle`.
- `rtk grep -n "overlayfs_probe" Build/OrlixMLibC/kselftest/release/kselftest-list.txt` found `orlix:overlayfs_probe`.
- `rtk grep -n "overlayfs_probe" Build/OrlixMLibC/test-initramfs/release/OrlixTestInitramfs.bundle/initramfs.list` found the `/orlix/overlayfs_probe` entry.
- `rtk test test -x Build/OrlixMLibC/kselftest/release/orlix/overlayfs_probe` passed.
- `rtk python3 .codex/hooks/compact_plan_check.py` exited 0 with the known stale pending/blocked warning in this long `IMPLEMENT.md`.

Non-claims:

- This is source/build/install/package evidence only; it is not a runtime-passed kselftest result.
- This does not claim OCI whiteout translation from image tar entries, OCI Runtime lifecycle compliance, container runtime readiness, registry pull support, external networking, or app-hosted execution proof.
- No custom ABI, Darwin host leakage, Docker daemon, `runc`, Apple Containerization, Virtualization.framework, or host-side Linux facade was introduced.
...
### 2026-06-22 07:56 +0200 - OCI runtime bundle rootfs import materialization plan

Status: green targeted checkpoint.

Implemented:

- Added SPI `OrlixOCIRuntimeBundleImportPlan` in OrlixOS.
- Added SPI `OrlixOCIRuntimeBundle.importPlan(id:rootMount:storagePolicy:fileManager:)`.
- The import plan composes the existing loaded local OCI Runtime bundle and lifecycle/session projection with the existing Orlix environment storage and image materialization model:
  - validates the loaded bundle `rootfs/` still exists and is a directory;
  - derives the OCI runtime session descriptor and environment descriptor;
  - resolves normal Orlix environment storage for the target environment id;
  - creates an `OrlixEnvironmentImageMaterializationPlan` whose `stagingRootDirectory` is the bundle `rootfs/`;
  - targets the standard environment `base.ext4` and `state.ext4` paths through `OrlixEnvironmentStorageLayout`.
- Added `testOCIRuntimeBundleImportPlanUsesBundleRootfsForMaterialization` to prove the bundle rootfs directory is the source for the standard materialization plan and the target environment id flows into normal Orlix storage.

Verification:

```text
/Volumes/1TB/Xcode/DerivedData/Logs/Test/Test-OrlixOSTests-2026.06.22_07-56-07-+0200.xcresult
result=Passed
passedTests=6
failedTests=0
skippedTests=0
...
- A delegated reviewer confirmed the next aligned direction is OrlixOS-owned root materialization for bundle `rootfs/`, not HostAdapter policy, direct host-directory root mounting, generated-tree edits, or a custom Linux ABI.

Non-claims:

- No `base.ext4` or `state.ext4` image was generated by this checkpoint.
