...
- The first manual-signing build exposed global profile propagation into framework targets. `project.yml` now binds `ORLIX_PROVISIONING_PROFILE_SPECIFIER` only to the Orlix application target; OrlixOS and OrlixKernel remain unprovisioned frameworks.
- Because the app-target binding changes `project.yml`, the old path-only freshness policy initially invalidated every simulator gate. Freshness now parses the actual `project.yml` diff: signing-only keys do not invalidate simulator Linux behavior, while version, dependency, source, linker, or other runtime-affecting changes still do.
...
### 2026-07-14 Checkpoint: Beta Simulator Gate Fails Closed

- Two initial `make beta-simulator-gate` attempts executed zero tests because `beta-prerequisites` stopped when Homebrew's API request failed with curl error 35, `Recv failure: Connection reset by peer`. `HOMEBREW_NO_AUTO_UPDATE=1` and `HOMEBREW_NO_INSTALL_FROM_API=1` allowed the already satisfied local Brewfile dependencies to be used without skipping Orlix build or test work.
- The local-metadata run executed the two OrlixOS target-derived payload tests with zero failures. Its original PTY selector omitted the XCTest class, executed zero tests, and produced a false-green Xcode result. `Makefile` now selects `OrlixRuntimeTests/OrlixRuntimeTests/testLinuxPTYCarriesInteractiveShellInputAndOutput`.
- The corrected PTY selector executed one test and failed. The OCI-derived materialized-root selector also executed one test and failed. Both exposed `Kernel panic - not syncing: Orlix: failed to synchronize hosted user mappings` after hosted user-window and page refresh failed. No matching host DiagnosticReports or simulator CrashReporter artifact was found for those runs.
- The TCTI status helper deadlocked while waiting for a verbose `git diff` child before draining stdout. The helper now discards stderr through `FileHandle.nullDevice` and drains stdout before `waitUntilExit()`. `make agent-harness-check` completed after the correction, and independent review confirmed both tracked fixes are minimal.
- `tcti-kernel-syscall-dispatch-smoke` was refreshed and passed for build 30. The next selected `tcti-kernel-execve-binfmt-elf-smoke` gate remained red because current pinned-simulator exec evidence was missing.
