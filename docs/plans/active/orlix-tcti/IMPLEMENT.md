...
- The unlocked physical retry reached Xcode signing and exposed that runtime validation did not forward the repository's existing `ORLIX_ALLOW_PROVISIONING_UPDATES` control. Physical builds now pass `-allowProvisioningUpdates` only when that explicit environment setting is `YES`.
...
- Runtime validation now reuses an app already installed on the simulator when its marketing version and build match `project.yml`. The first full build-21 stability run took 2097.72 seconds; the identical current-product/current-runtime rerun took 146.17 seconds, recorded `install-reused.txt`, and passed with all forbidden behavior false. Homebrew auto-update is disabled inside runtime validation while the existing dependency check remains.
- Final validation passed: payload, installed-app, simulator-runtime identity, and runtime-report selection fixtures; Swift typechecking; shell syntax; report schema; plan consistency; product build-version check; full agent harness check; and `git diff --check`.
...
## 2026-07-14 Build-30 No-Phone Oracle Refresh

- The mandated `tcti-plan-consistency` and full `agent-harness-check` commands passed before refreshing canonical status and the task envelope.
- `tcti-report-schema-check` passed. The build-30 `init_001_exit` structural golden ELF and switch-debug execution both passed at commit `09ba30fe`. Switch-debug entered the ELF entrypoint, decoded MOVZ, MOVZ, and SVC, captured guest `exit(42)`, and kept every forbidden-behavior field false.
- The build-30 `init_011_static_pie_got_byte_load` structural and switch-debug executions passed. The oracle entered the static-PIE entrypoint, decoded ADRP, GOT-backed 64-bit LDR, byte LDRB, MOVZ, and SVC, captured guest `exit(42)`, and kept every forbidden-behavior field false.
