...
| [#126](https://github.com/rudironsoni/Orlix/issues/126) AdvSIMD load/store | `memory.S`: single and multiple structures, replicate, interleaved, scalar/Q loads and stores. | Use fixed gadgets around `orlix_tcti_execute_simd_single_structure()` and `orlix_tcti_execute_simd_multiple_structure()`. Test V31-to-V0 register-list wrap, lane boundaries, replicate widths, post-index register/immediate forms, cross-page second-element faults, and no premature writeback. |
