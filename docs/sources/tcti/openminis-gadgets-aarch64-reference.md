...
| --- | --- | --- |
| [#123](https://github.com/rudironsoni/Orlix/issues/123) AdvSIMD crypto | `crypto.S`: AES, SHA-1/2/512, EOR3, SM3, SM4, PMULL. `math.S`: PMUL vector shape. | Orlix owns production decode and typed `FP_SIMD` and `PC` observations for all 32 EL0 `ADVSIMD_CRYPTO` leaves AESE/AESD/AESMC/AESIMC, SHA-1/SHA-256/SHA-512, SHA-3 (EOR3/BCAX/RAX1/XAR), SM3, SM4, and PMULL. `MEMORY`, `ATOMICITY`, `ORDERING`, and `FLAGS` are not applicable. Destructive destinations, source/destination aliasing, PMULL versus PMULL2 source halves, all-zero/all-one polynomial operands, AES round ordering, and Q=0 upper-bit preservation are proved. PMUL stays in AdvSIMD integer. TBL/TBX stay in [#127](https://github.com/rudironsoni/Orlix/issues/127). SVE/SME AES stay in later families. Crypto `HWCAP` bits are not advertised. |
| [#124](https://github.com/rudironsoni/Orlix/issues/124) AdvSIMD FP | `math.S`: FP unary, three-same, pairwise/reduction, compare-zero, rounding, conversion templates. | Orlix owns production decode and typed `FP_SIMD` and `PC` observations for the 113 always-on EL0 `ADVSIMD_FP` leaves. `MEMORY`, `ATOMICITY`, `ORDERING`, and `FLAGS` are not applicable. NaN, infinity, signed-zero, subnormal, lane widths, Q=0/Q=1, FPCR rounding, FPSR IOC, Q=0 upper-half clearing, reserved Q=0 2D, and overlapping source/destination registers are proved. The 155 optional FP16, FHM, BF16, FCMA, FRINTTS, FP8, FAMINMAX, and FSCALE leaves are classified `NON_EL0` and prove EL0 rejection. Integer 16-bit lanes stay in [#125](https://github.com/rudironsoni/Orlix/issues/125). Scalar FP stays in [#154](https://github.com/rudironsoni/Orlix/issues/154). `HWCAP_FPHP`, `HWCAP_ASIMDHP`, `HWCAP_ASIMDFHM`, `HWCAP_BF16`, FCMA, FRINTTS, FAMINMAX, FP8, and FSCALE are not advertised. |
| [#125](https://github.com/rudironsoni/Orlix/issues/125) AdvSIMD integer | `math.S`: widening/narrowing, saturating, halving/rounding, shifts, min/max, pairwise, reductions. | Orlix owns production decode and typed `FP_SIMD` and `PC` observations for the 222 always-on EL0 `ADVSIMD_INTEGER` leaves. `MEMORY`, `ATOMICITY`, `ORDERING`, and `FLAGS` are not applicable. Lane endpoints, signed versus unsigned saturation, QC updates, high-half forms, narrowing destination preservation, variable shift counts outside element width, and overlapping source/destination registers are proved. The 18 optional RDM/DotProd/I8MM leaves are classified `NON_EL0` and prove EL0 rejection. XTN stays here. PMULL stays in [#123](https://github.com/rudironsoni/Orlix/issues/123). TBL/TBX stay in [#127](https://github.com/rudironsoni/Orlix/issues/127). `HWCAP_ASIMDRDM`, `HWCAP_ASIMDDP`, and `HWCAP_I8MM` are not advertised. |
...
| [#127](https://github.com/rudironsoni/Orlix/issues/127) AdvSIMD permute/move | `math.S`: DUP, INS, UMOV/SMOV, XTN, UZP, TRN, ZIP, EXT, REV. `crypto.S`: TBL/TBX. | Orlix owns production decode and typed `FP_SIMD` and `PC` observations for the 33 always-on EL0 `ADVSIMD_PERMUTE_MOVE` leaves DUP/INS/UMOV/SMOV, ZIP/UZP/TRN/EXT/REV, TBL/TBX, and MOVI, plus `REGISTERS` for UMOV/SMOV. `MEMORY`, `ATOMICITY`, `ORDERING`, and `FLAGS` are not applicable. Lane-index endpoints, Q=0 upper-half rules, table lists wrapping V31, out-of-range TBL zeroing versus TBX preservation, signed SMOV extraction, and overlapping source/destination registers are proved. LUTI2/LUTI4 (`FEAT_LUT`) are classified `NON_EL0` and prove EL0 rejection. XTN stays in [#125](https://github.com/rudironsoni/Orlix/issues/125). SVE/SME LUTI stay in later families. `FEAT_LUT` is not advertised. |
...
| [#129](https://github.com/rudironsoni/Orlix/issues/129) bitfield/extract/unary | `math.S`: UBFM/SBFM/BFM, EXTR, CLZ/CLS/RBIT/REV variants. `bits.S` contributes only width test shapes; its bitfield bodies are stubs. | Orlix owns production decode and typed `REGISTERS` and `PC` observations for the 19 always-on EL0 `BASE_BITFIELD_UNARY` leaves `EXTR`, `SBFM`/`BFM`/`UBFM`, and `RBIT`/`REV*`/`CLZ`/`CLS`. `MEMORY`, `ATOMICITY`, `ORDERING`, and `FLAGS` are not applicable. Alias encodings cover LSL/LSR/ASR/UBFX/SBFX/BFI/UXTB and W upper-zeroing. `CTZ`/`CNT`/`ABS` (`FEAT_CSSC`) are classified `NON_EL0` and prove EL0 rejection. `FEAT_CSSC` is not advertised. UDIV/SDIV stay in [#136](https://github.com/rudironsoni/Orlix/issues/136). LSLV/LSRV/ASRV/RORV stay in [#183](https://github.com/rudironsoni/Orlix/issues/183). |
...
| [#131](https://github.com/rudironsoni/Orlix/issues/131) conditional compare/select/branch | `control.S`: condition tables, CSEL/CSINC/CSINV/CSNEG, fused compare/branch. `math.S`: CCMP/CCMN immediate/register. | Orlix owns production decode and typed `REGISTERS` and `PC` observations for the 23 always-on EL0 `BASE_CONDITIONAL` leaves `B.cond`, `CBZ`/`CBNZ`, `TBZ`/`TBNZ`, `CSEL`/`CSINC`/`CSINV`/`CSNEG`, plus `FLAGS` for `CCMP`/`CCMN`. `MEMORY`, `ATOMICITY`, and `ORDERING` are not applicable. All 16 `B.cond` and `CSEL` condition encodings are proved across representative NZCV. False `CCMP`/`CCMN` installs immediate NZCV. `CSINC`/`CSINV`/`CSNEG` keep W upper-zeroing. `BC.cond` (`FEAT_HBC`) and the 36 `FEAT_CMPBR` compare-and-branch leaves are classified `NON_EL0` and prove EL0 rejection. `FEAT_CMPBR` and `FEAT_HBC` are not advertised. Fusion remains a later equivalence optimization. |
| [#132](https://github.com/rudironsoni/Orlix/issues/132) exceptions/undefined | `entry.S`: typed interrupt/undefined exits. `control.S`: SVC. | Orlix owns production decode and structured exits for the 10 `BASE_EXCEPTIONS` leaves through `engine.c`. SVC has typed `REGISTERS` and `PC` observations on `EXIT_SYSCALL` without userspace syscall emulation. BRK exits as breakpoint. HLT and UDF exit as undefined at EL0. HVC, SMC, DCPS1, DCPS2, DCPS3, and TENTER prove EL0 rejection. TENTER official DDI0602 semantics are unspecified. `FEAT_TEV` is not advertised. Linux-visible kselftest stays with later ladder issues. |
...
| [#136](https://github.com/rudironsoni/Orlix/issues/136) multiply/divide/ternary | `math.S`: MADD/MSUB, signed/unsigned long forms, high multiply, UDIV/SDIV. | Orlix owns production decode and typed `REGISTERS` and `PC` observations for all 16 EL0 `BASE_MULTIPLY_DIVIDE` leaves `UDIV`/`SDIV`, `MADD`/`MSUB`, `SMADDL`/`SMSUBL`/`SMULH`, `UMADDL`/`UMSUBL`/`UMULH`, and `MADDPT`/`MSUBPT`. `MEMORY`, `ATOMICITY`, `ORDERING`, and `FLAGS` are not applicable. Zero divisor, signed minimum divided by `-1`, W upper-zeroing, long-form source extension, high-half products, accumulator aliasing, and XZR operands are proved. MADDPT and MSUBPT keep PAC tag bits [63:56] and do 56-bit pointer arithmetic. LSLV stays in [#183](https://github.com/rudironsoni/Orlix/issues/183). `FEAT_CPA` is not advertised. |
