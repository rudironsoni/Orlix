...
| [#126](https://github.com/rudironsoni/Orlix/issues/126) AdvSIMD load/store | `memory.S`: single and multiple structures, replicate, interleaved, scalar/Q loads and stores. The selected AARCHMRS generation owns exactly 152 direct source leaves, ordinals 2353 through 2504, and no semantic-variant rows. | Use fixed gadgets around `orlix_tcti_execute_simd_single_structure()` and `orlix_tcti_execute_simd_multiple_structure()`. Exhaust the complete Rd/Rn/Rm and free encoding-field products, V31-to-V0 register-list wrap, lane and replicate widths, post-index register/immediate forms, overlap using the original base, element-granular faults, unchanged rejected state, and STL1/LDAP1 competing-observer outcomes. Native proof remains uncredited until corrected #120 supplies `target_native_proof_contract.{c,h}`, `target_proof_ingestion.{c,h}`, `orlix_tcti_native_observation.{c,h}`, and their owning Make lists, and an app-hosted observation is ingested. |
...
| [#128](https://github.com/rudironsoni/Orlix/issues/128) atomics/exclusives/ordered memory | `memory.S`: `atomic_rmw`, `atomic_cas`, `ldxr`, `stxr`, pair exclusives, `atomic_casp*`, `dmb`. | Route exclusive and LSE gadgets using Linux-owned pages. Test monitor arm/clear/invalidation, address and size mismatch, failure without store, returned old value, CASP whole-pair atomicity, 32-bit pair packing, alignment faults, cross-page rejection, acquire/release ordering, and generation changes between lookup and commit. |
...
| [#130](https://github.com/rudironsoni/Orlix/issues/130) branch/control | `control.S`: direct/register branches, BL/BLR/RET, CBZ/CBNZ, TBZ/TBNZ. | Route branch gadgets without writable code chaining. Test PC-relative target sign extension, X30=`PC+4`, register target alignment, XZR compare semantics, bit 31/32/63 boundaries, target/fallthrough selection, and stale code-generation rejection. |
...
| [#133](https://github.com/rudironsoni/Orlix/issues/133) add/subtract | `math.S`: immediate, shifted, extended, carry, flag-setting and SP-specialized variants. | Route add/sub gadgets. Cross product 32/64-bit, S flag, SP/ZR context, immediate shift, all extend modes, shift limits, carry-in, signed overflow, unsigned carry/borrow, destination X31, and W upper-zeroing. |
| [#134](https://github.com/rudironsoni/Orlix/issues/134) load/store/prefetch | `memory.S`: literal, immediate, register-offset, signed load, scalar/pair, pre/post-index, fast and cross-page paths. | Route load/store gadgets through Orlix TLB/uaccess. Test 1/2/4/8/16-byte accesses, sign extension, SP bases, XZR destinations/sources, extend/shift addressing, unaligned and page-crossing faults, pair partial-fault behavior, writeback ordering, permissions, COW, and self-modifying-code invalidation. Prefetch must follow Arm fault/no-fault rules rather than perform a host load. |
