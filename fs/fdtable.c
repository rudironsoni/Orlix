...
void free_files(struct files_struct *files) {
    ...
    kmutex_lock_impl(&files->lock);
    for (size_t i = 0; i < files->max_fds; i++) {
        if (files->fd[i]) {
            free_file(files->fd[i]);
        }
    }
    kmutex_unlock_impl(&files->lock);

    free(files->fd);
    kmutex_destroy_impl(&files->lock);
    free(files);
}
...
static kmutex_t fd_table_lock = PTHREAD_MUTEX_INITIALIZER;
...
void file_init_impl(void) {
    ...
    kmutex_lock_impl(&fd_table_lock);
    memset(fd_table, 0, sizeof(fd_table));
    for (int i = 0; i < NR_OPEN_DEFAULT; i++) {
        kmutex_init_impl(&fd_table[i].lock);
    }
    ...
    kmutex_unlock_impl(&fd_table_lock);
}
...
fd_entry_t *get_fd_entry_impl(int fd) {
    ...
    int ret = kmutex_lock_impl(&fd_table_lock);
    if (ret != 0) {
        errno = ret;
        return NULL;
    }
    ...
    ret = kmutex_lock_impl(&entry->lock);
    if (ret != 0) {
        kmutex_unlock_impl(&fd_table_lock);
        errno = ret;
        return NULL;
...
void put_fd_entry_impl(void *entry) {
    if (entry) {
        ...
        kmutex_unlock_impl(&fd_entry->lock);
        release_fd_description(desc);
    }
}
