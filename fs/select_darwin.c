/*
 * IXLandSystem Darwin readiness bridge
 *
 * This file includes Darwin headers and provides the translation
 * between Linux-shaped internal readiness types and Darwin kqueue.
 * It is the ONLY file in the readiness subsystem that includes
...
static int poll_kqueue_impl(struct pollfd *fds, unsigned int nfds, int timeout_ms) {
    ...
    for (int i = 0; i < nevents; i++) {
        ...
        if (idx < nfds) {
            fds[idx].revents |= kfilter_to_poll_revents(eventlist[i].filter, eventlist[i].flags);
            ready_count++;
        }
    }
...
/* ============================================================================
 * SELECT - Darwin kqueue implementation
 * ============================================================================ */

static int select_kqueue_impl(int nfds, fd_set *readfds, fd_set *writefds,
    ...
    for (int fd = 0; fd < nfds; fd++) {
        int requested = 0;
        if (readfds && FD_ISSET(fd, readfds)) requested = 1;
        if (writefds && FD_ISSET(fd, writefds)) requested = 1;
        if (exceptfds && FD_ISSET(fd, exceptfds)) requested = 1;
...
int select_impl(int nfds, fd_set *readfds, fd_set *writefds,
                fd_set *exceptfds, struct timeval *timeout) {
    return select_kqueue_impl(nfds, readfds, writefds, exceptfds, timeout);
}
