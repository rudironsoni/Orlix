...
__bazel-apple-ci: __bazel-matrix-check
	...
	$(MAKE) __bazel-xctest-runtime-proof ORLIX_XCTEST_SIMULATOR_VERSION="$(ORLIX_CURRENT_SIMULATOR_VERSION)" ORLIX_XCTEST_SIMULATOR_ID="$(ORLIX_CURRENT_SIMULATOR_ID)" ORLIX_SIMULATOR_LABEL=current ORLIX_CANONICAL_IPA_REL="$$ipa_rel"; \
	$(MAKE) __bazel-xctest-runtime-proof ORLIX_XCTEST_SIMULATOR_VERSION=15.5 ORLIX_XCTEST_SIMULATOR_ID="$(ORLIX_IOS15_SIMULATOR_ID)" ORLIX_SIMULATOR_LABEL=ios15 ORLIX_CANONICAL_IPA_REL="$$ipa_rel"; \
...
__bazel-xctest-runtime-proof:
	...
	xcrun simctl bootstatus "$(ORLIX_XCTEST_SIMULATOR_ID)" -b || { echo "XCTest simulator $(ORLIX_XCTEST_SIMULATOR_ID) is not booted" >&2; exit 1; }; \
	test -n "$(ORLIX_SIMULATOR_LABEL)" || { echo "ORLIX_SIMULATOR_LABEL is required" >&2; exit 1; }; \
	test -n "$(ORLIX_CANONICAL_IPA_REL)" || { echo "ORLIX_CANONICAL_IPA_REL is required" >&2; exit 1; }; \
	set -euo pipefail; \
	evidence_dir="$(ORLIX_BUILD_ROOT)/AgentHarness/bazel-ci/$(ORLIX_SIMULATOR_LABEL)"; \
	mkdir -p "$$evidence_dir"; \
	echo "xctest-proof: label=$(ORLIX_SIMULATOR_LABEL) version=$(ORLIX_XCTEST_SIMULATOR_VERSION) simulator=$(ORLIX_XCTEST_SIMULATOR_ID)" | tee "$$evidence_dir/xctest-input.log"; \
	app_test_rel="$$(DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" "$(ORLIX_BAZEL)" --output_base="$(ORLIX_BAZEL_OUTPUT_BASE)" cquery //Orlix:Orlix --compilation_mode=dbg --config=$(PROFILE) --config=$(ORLIX_BAZEL_COMPONENT_MODE) --apple_platform_type=ios --ios_multi_cpus=sim_arm64 --ios_simulator_version=$(ORLIX_XCTEST_SIMULATOR_VERSION) --ios_simulator_device="$(ORLIX_XCTEST_SIMULATOR_ID)" --xcode_version=$(ORLIX_XCODE_VERSION) --repo_env=DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --host_action_env=ORLIX_PINNED_DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --action_env=ORLIX_PINNED_DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --output=files | awk '/Orlix[.]ipa$$/ {path=$$0; count++} END {if (count != 1) exit 1; print path}')"; \
	...
	DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" "$(ORLIX_BAZEL)" --output_base="$(ORLIX_BAZEL_OUTPUT_BASE)" test //Orlix:OrlixUITests --test_filter=AppLaunchSmokeUITests/testLaunchCapturesScreenshot --compilation_mode=dbg --config=$(PROFILE) --config=$(ORLIX_BAZEL_COMPONENT_MODE) --apple_platform_type=ios --ios_multi_cpus=sim_arm64 --ios_simulator_version=$(ORLIX_XCTEST_SIMULATOR_VERSION) --ios_simulator_device="$(ORLIX_XCTEST_SIMULATOR_ID)" --xcode_version=$(ORLIX_XCODE_VERSION) --repo_env=DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --host_action_env=ORLIX_PINNED_DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --action_env=ORLIX_PINNED_DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --test_output=errors --build_event_json_file="$$evidence_dir/test-events.json" 2>&1 | tee "$$evidence_dir/test.log"; test "$${PIPESTATUS[0]}" -eq 0; \
