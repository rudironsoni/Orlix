...
ORLIX_PINNED_DEVELOPER_DIR := $(ORLIX_PINNED_DEVELOPER_DIR)
ORLIX_BAZEL_STAGED_APP = $(ORLIX_BUILD_ROOT)/Bazel/product/Orlix-$(ORLIX_BAZEL_DESTINATION)/Payload/Orlix.app
ORLIX_SCENARIO ?=
ORLIX_SCENARIO_RUN ?= 0
ORLIX_SCENARIO_PREVIOUS ?=
ORLIX_SCENARIOS = clean-clone swift-app-shell terminal-ui orlix-engine orlix-bootloader orlix-host-adapter kernel-tcti installed-uapi mlibc guest-package rootfs-policy proof-noise second-worktree switch-back empty-promoted-store warm-promoted-local
...
.PHONY: __bazel-scenario-harness __bazel-routing-contract
...
__bazel-kernel-boot: __bazel-feasibility-bootstrap
	...
	@set -euo pipefail; \
	macho_rel="$$(DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" "$(ORLIX_BAZEL)" --output_base="$(ORLIX_BAZEL_OUTPUT_BASE)" cquery //bazel/feasibility/kernel:macho_archive $(ORLIX_BAZEL_KERNEL_FLAGS) --xcode_version=$(ORLIX_XCODE_VERSION) --repo_env=DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --host_action_env=ORLIX_PINNED_DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --action_env=ORLIX_PINNED_DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --action_env=CCACHE_DIR="$(CCACHE_DIR)" --output=files | awk '/OrlixKernel[.]a$$/ {path=$$0; count++} END {if (count != 1) exit 1; print path}')"; \
...
__bazel-matrix-check: __bazel-version-check __bazel-apple-routing-check
	...
	@DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" "$(ORLIX_BAZEL)" --output_base="$(ORLIX_BAZEL_OUTPUT_BASE)" test //bazel/feasibility/analysis:all --config=release --config=source --xcode_version=$(ORLIX_XCODE_VERSION) --repo_env=DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --host_action_env=ORLIX_PINNED_DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --action_env=ORLIX_PINNED_DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --action_env=CCACHE_DIR="$(CCACHE_DIR)" --disk_cache="$(ORLIX_BAZEL_DISK_CACHE)" --repository_cache="$(ORLIX_BAZEL_REPOSITORY_CACHE)" --test_output=errors
...
__bazel-orlix-app: __bazel-feasibility-bootstrap $(if $(filter promoted,$(ORLIX_BAZEL_COMPONENT_MODE)),__bazel-substitute-promoted)
	@DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" "$(ORLIX_BAZEL)" --output_base="$(ORLIX_BAZEL_OUTPUT_BASE)" build $(ORLIX_BAZEL_APP_TARGETS) --compilation_mode=$(ORLIX_BAZEL_COMPILATION_MODE) --config=$(PROFILE) --config=$(ORLIX_BAZEL_COMPONENT_MODE) --apple_platform_type=ios --ios_multi_cpus=$(ORLIX_BAZEL_IOS_CPU) --xcode_version=$(ORLIX_XCODE_VERSION) --repo_env=DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --host_action_env=ORLIX_PINNED_DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --action_env=ORLIX_PINNED_DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --action_env=CCACHE_DIR="$(CCACHE_DIR)" --disk_cache="$(ORLIX_BAZEL_DISK_CACHE)" --repository_cache="$(ORLIX_BAZEL_REPOSITORY_CACHE)" $(ORLIX_BAZEL_BUILD_FLAGS)
	@set -euo pipefail; \
...
__bazel-product-app: __bazel-feasibility-bootstrap $(if $(filter promoted,$(ORLIX_BAZEL_COMPONENT_MODE)),__bazel-substitute-promoted)
	@case "$(ORLIX_BAZEL_DESTINATION)" in iphoneos|iphonesimulator) ;; *) echo "unsupported Bazel destination: $(ORLIX_BAZEL_DESTINATION)" >&2; exit 1 ;; esac
	@case "$(ORLIX_BAZEL_COMPONENT_MODE)" in source|promoted) ;; *) echo "unsupported Bazel component mode: $(ORLIX_BAZEL_COMPONENT_MODE)" >&2; exit 1 ;; esac
	@DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" "$(ORLIX_BAZEL)" --output_base="$(ORLIX_BAZEL_OUTPUT_BASE)" build //Orlix:Orlix --compilation_mode=$(ORLIX_BAZEL_COMPILATION_MODE) --config=$(PROFILE) --config=$(ORLIX_BAZEL_COMPONENT_MODE) --apple_platform_type=ios --ios_multi_cpus=$(ORLIX_BAZEL_IOS_CPU) --xcode_version=$(ORLIX_XCODE_VERSION) --repo_env=DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --host_action_env=ORLIX_PINNED_DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --action_env=ORLIX_PINNED_DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --action_env=CCACHE_DIR="$(CCACHE_DIR)" --disk_cache="$(ORLIX_BAZEL_DISK_CACHE)" --repository_cache="$(ORLIX_BAZEL_REPOSITORY_CACHE)"
	@set -euo pipefail; \
	ipa_rel="$$(DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" "$(ORLIX_BAZEL)" --output_base="$(ORLIX_BAZEL_OUTPUT_BASE)" cquery //Orlix:Orlix --compilation_mode=$(ORLIX_BAZEL_COMPILATION_MODE) --config=$(PROFILE) --config=$(ORLIX_BAZEL_COMPONENT_MODE) --apple_platform_type=ios --ios_multi_cpus=$(ORLIX_BAZEL_IOS_CPU) --xcode_version=$(ORLIX_XCODE_VERSION) --repo_env=DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --host_action_env=ORLIX_PINNED_DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --action_env=ORLIX_PINNED_DEVELOPER_DIR="$(ORLIX_PINNED_DEVELOPER_DIR)" --action_env=CCACHE_DIR="$(CCACHE_DIR)" --output=files | awk '/Orlix[.]ipa$$/ {path=$$0; count++} END {if(count != 1) exit 1; print path}')"; \
	ipa="$(ORLIX_BAZEL_OUTPUT_BASE)/execroot/_main/$$ipa_rel"; \
	test -s "$$ipa" || { echo "missing //Orlix:Orlix ipa" >&2; exit 1; }; \
	app="$(ORLIX_BAZEL_STAGED_APP)"; \
	product_dir="$${app%/Payload/Orlix.app}"; \
	rm -rf "$$product_dir"; mkdir -p "$$product_dir"; \
	/usr/bin/ditto -x -k "$$ipa" "$$product_dir"; \
	test "$$app" = "$$product_dir/Payload/Orlix.app"; \
...
__bazel-scenario-harness:
	@set -euo pipefail; \
	scenarios="$(ORLIX_SCENARIOS)"; \
	set -- $$scenarios; \
	test "$$#" -eq 16 || { echo "scenario harness must name sixteen scenarios" >&2; exit 1; }; \
	printf '%s\n' "$$@"; \
	if [ "$(ORLIX_SCENARIO_RUN)" != 1 ]; then \
		echo "scenario harness listed; set ORLIX_SCENARIO and ORLIX_SCENARIO_RUN=1 to measure one scenario on the retained output base"; \
		exit 0; \
	fi; \
	test -n "$(ORLIX_SCENARIO)" || { echo "ORLIX_SCENARIO is required when ORLIX_SCENARIO_RUN=1" >&2; exit 1; }; \
	case " $(ORLIX_SCENARIOS) " in \
	*" $(ORLIX_SCENARIO) "*) ;; \
	*) echo "unknown scenario: $(ORLIX_SCENARIO)" >&2; exit 1 ;; \
	esac; \
	...
	obs_args=( \
		...
		--bep "$$evidence_dir/build-events.json" \
		--profile "$$evidence_dir/profile.json.gz" \
		--remote-grpc "$$evidence_dir/remote-grpc.log" \
		--access "$(or $(ORLIX_BUILDBUDDY_ACCESS),off)" \
		--orlixcc "$$evidence_dir/build.log" \
		--acquisition "$(ORLIX_BUILD_ROOT)/AgentHarness/bazel-promotion/acquisition.json" \
		--output-base "$(ORLIX_BAZEL_OUTPUT_BASE)" \
		--disk-cache "$(ORLIX_BAZEL_DISK_CACHE)" \
		--repository-cache "$(ORLIX_BAZEL_REPOSITORY_CACHE)" \
		--ccache-dir "$(CCACHE_DIR)" \
		--product "$(ORLIX_BAZEL_STAGED_APP)" \
		--out "$$evidence_dir/scenario-row.json" \
	); \
	if [ -n "$$ninja_log" ]; then obs_args+=(--ninja-log "$$ninja_log"); fi; \
	if [ -n "$(ORLIX_SCENARIO_PREVIOUS)" ]; then obs_args+=(--previous "$(ORLIX_SCENARIO_PREVIOUS)"); fi; \
	PYTHONPATH="$(CURDIR)/bazel/config" python3 -m cache_observation "$${obs_args[@]}"

__bazel-routing-contract:
	@set -euo pipefail; \
	rg -q '^ORLIX_BAZEL_AUTHORITY \?= 0$$' Makefile; \
	rg -q '^product-build-prepare:' Makefile; \
	rg -q '^product-build-version-check:$$' Makefile; \
	rg -q '^__build-product:$$' Makefile; \
	rg -F -q 'ORLIX_PINNED_DEVELOPER_DIR ?= $$(shell xcode-select -p 2>/dev/null)' make/bazel-migration.mk; \
	rg -F -q 'ORLIX_PINNED_DEVELOPER_DIR := $$(ORLIX_PINNED_DEVELOPER_DIR)' make/bazel-migration.mk; \
	rg -F -q -- '--action_env=CCACHE_DIR="$$(CCACHE_DIR)"' make/bazel-migration.mk; \
	developer_action=--action_env=DEVELOPER_DIR=; \
	developer_action=$${developer_action%=}; \
	host_action=--host_action_env=DEVELOPER_DIR=; \
	host_action=$${host_action%=}; \
	ccache_valued=--action_env=CCACHE_DIR=; \
	ccache_name=$${ccache_valued%=}; \
	if rg -q -- "$${developer_action}"'($$|[^A-Za-z0-9_=])' make/bazel-migration.mk; then echo "DEVELOPER_DIR must not be a global action env" >&2; exit 1; fi; \
	if rg -q -- "$${host_action}"'($$|[^A-Za-z0-9_=])' make/bazel-migration.mk; then echo "DEVELOPER_DIR must not be a global host action env" >&2; exit 1; fi; \
	if rg -q -- "$${ccache_name}"'($$|[^="])' make/bazel-migration.mk; then echo "CCACHE_DIR action env must be valued" >&2; exit 1; fi; \
	rg -F -q 'ORLIX_BAZEL_STAGED_APP = $$(ORLIX_BUILD_ROOT)/Bazel/product/Orlix-$$(ORLIX_BAZEL_DESTINATION)/Payload/Orlix.app' make/bazel-migration.mk; \
	test "$$(rg -F -c 'app="$$(ORLIX_BAZEL_STAGED_APP)"' make/bazel-migration.mk)" -ge 2; \
	if rg -F -q 'Orlix-$$(ORLIX_BAZEL_DESTINATION).app' make/bazel-migration.mk; then echo "IPA staging must be one Payload path" >&2; exit 1; fi; \
	rg -F -q '"ORLIX_PINNED_DEVELOPER_DIR": None' xcode/BUILD.bazel; \
	PYTHONPATH="$(CURDIR)/bazel/migration" python3 -m unittest test_workflow_policy; \
	$(MAKE) __bazel-scenario-harness ORLIX_SCENARIO_RUN=0
