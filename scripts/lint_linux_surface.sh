...
OBJC_FILES=$(find $OWNER_PATHS -type f \( -name '*.m' -o -name '*.mm' \) 2>/dev/null || true)
...
PUBLIC_IOS=$(rg -n '^\s*#\s*include\s*"internal/ios/.+"' "$KERNEL_ROOT/include" 2>/dev/null || true)
...
CROSS_SUBSYSTEM=$(rg -n '^\s*#\s*include\s*"internal/ios/fs/.*"' "$KERNEL_ROOT/kernel" "$KERNEL_ROOT/runtime" 2>/dev/null || true)
...
FORBIDDEN_LOGGING=$(rg -n -e '\b(printf|fprintf|vprintf|vfprintf|puts|fputs|putc|putchar|perror|NSLog)\s*\(' -e '\bos_log\s*\(' $OWNER_PATHS "$HOST_ADAPTER_ROOT" 2>/dev/null || true)
...
HANDDEFINED_ABI=$(rg -n '^\s*#define\s+(FUTEX_|AT_|SA_|SIG[A-Z0-9_]+|O_[A-Z0-9_]+|F_[A-Z0-9_]+|RENAME_[A-Z0-9_]+)' $OWNER_PATHS 2>/dev/null | rg -v -e 'I[X]_' -e 'TES[T]_' -e '_IMPL' || true)
if [ -n "$HANDDEFINED_ABI" ]; then
    echo "FAIL: Hand-defined Linux ABI constants found in Linux-owner paths:"
    echo "$HANDDEFINED_ABI"
    exit 1
fi
BRANDED_UAPI=$(rg -n -e '__attribute__\(\(visibility\("default"\)\)\)\s+.*\b(ixland_|ios_|darwin_)[A-Za-z0-9_]*\s*\(' $OWNER_PATHS 2>/dev/null || true)
...
KERNEL_TESTS=$(grep -E '^\s+IXLandKernelTests:' project.yml || true)
if [ -z "$KERNEL_TESTS" ]; then
    echo "FAIL: IXLandKernelTests target missing from project.yml"
    exit 1
fi
BRIDGE_TESTS=$(grep -E '^\s+IXLandHostAdapterTests:' project.yml || true)
if [ -z "$BRIDGE_TESTS" ]; then
    echo "FAIL: IXLandHostAdapterTests target missing from project.yml"
    exit 1
fi
...
BRANDED_ALIASES=$(rg -n -e '\bI[X]_AT_[A-Z0-9_]+\b' -e '\bI[X]_F_[A-Z0-9_]+\b' "$KERNEL_TEST_ROOT" "$HOST_TEST_ROOT" 2>/dev/null || true)
...
TEST_OBJC_LINUX_HEADERS=$(rg -n '^[[:space:]]*#(include|import)[[:space:]]*<(linux|asm)/' "$KERNEL_TEST_ROOT" "$HOST_TEST_ROOT" --glob '*.m' 2>/dev/null || true)
...
TEST_LINUX_ACCESSORS=$(rg -n -e '\blinux_(s_ifmt|s_is[a-z0-9_]+|at_[a-z0-9_]+|f_[a-z0-9_]+)\b' "$HOST_TEST_ROOT" 2>/dev/null || true)
...
TEST_RAW_LINUX_CONSTANTS=$(rg -n -e '0x[0-9A-Fa-f]+' "$KERNEL_TEST_ROOT" "$HOST_TEST_ROOT" --glob '*.m' 2>/dev/null | rg -v 'INVALID_FLAG_TEST_VALUE|RENAME_NOREPLACE|RENAME_EXCHANGE|RENAME_WHITEOUT|memset\(|/\*|//' || true)
...
TEST_LINUXKERNEL_HOST_INCLUDES=$(rg -n -e '^[[:space:]]*#(include|import)[[:space:]]*"internal/ios/' -e '^[[:space:]]*#(include|import)[[:space:]]*".*backing_io(_decls)?\.h"' "$KERNEL_TEST_ROOT" --glob '*.[mhc]' 2>/dev/null || true)
...
TEST_CONSTANTS=$(rg -n '^\s*#define\s+TEST_(AT_|RENAME_|F_|FD_)' "$KERNEL_TEST_ROOT"/*.m "$KERNEL_TEST_ROOT"/*.c "$HOST_TEST_ROOT"/*.m "$HOST_TEST_ROOT"/*.c 2>/dev/null || true)
...
UAPI_AT_HELPERS=$(rg -n '\bi[x]land_test_uapi[_]at_' "$KERNEL_TEST_ROOT"/*.m "$KERNEL_TEST_ROOT"/*.c "$HOST_TEST_ROOT"/*.m "$HOST_TEST_ROOT"/*.c 2>/dev/null || true)
...
UAPI_F_HELPERS=$(rg -n '\bi[x]land_test_uapi[_]f_' "$KERNEL_TEST_ROOT"/*.m "$KERNEL_TEST_ROOT"/*.c "$HOST_TEST_ROOT"/*.m "$HOST_TEST_ROOT"/*.c 2>/dev/null || true)
...
BRIDGE_BAG=$(rg -n 'internal/ios/fs/backing_io\.h|internal/ios/fs/backing_io_decls\.h' "$KERNEL_TEST_ROOT"/*.m 2>/dev/null || true)
...
text = Path('IXLandKernel/fs/exec.c').read_text()
...
TEST_UAPI_CONTAMINATION=$(rg -n 'include/ixland/linux_uapi_constants[.]h|\bI[X]_(AT_|F_)|\bTES[T]_(AT_|F_)|\bi[x]land_test_uapi[_](at_|f_)' "$KERNEL_TEST_ROOT" "$HOST_TEST_ROOT" 2>/dev/null || true)
...
DARWIN_STAT=$(rg -n '\bS_ISDIR\s*\(|\bS_ISLNK\s*\(|\bS_ISREG\s*\(|\bS_ISCHR\s*\(' "$KERNEL_TEST_ROOT"/*.m "$HOST_TEST_ROOT"/*.m 2>/dev/null | rg -v 'LinuxUAPITestSupport' || true)
...
# Verify IXLandKernelTests does not have global Linux UAPI include paths
KERNEL_GLOBAL_UAPI=$(grep -A10 'IXLandKernelTests:' project.yml | grep -E 'LINUX_UAPI_INCLUDE_ROOT\s*}' || true)
if [ -n "$KERNEL_GLOBAL_UAPI" ]; then
    echo "FAIL: IXLandKernelTests has global Linux UAPI include paths:"
    echo "$KERNEL_GLOBAL_UAPI"
    exit 1
fi
# Verify IXLandHostAdapterTests does not have global Linux UAPI include paths
BRIDGE_GLOBAL_UAPI=$(grep -A10 'IXLandHostAdapterTests:' project.yml | grep -E 'LINUX_UAPI_INCLUDE_ROOT\s*}' || true)
if [ -n "$BRIDGE_GLOBAL_UAPI" ]; then
    echo "FAIL: IXLandHostAdapterTests has global Linux UAPI include paths:"
    echo "$BRIDGE_GLOBAL_UAPI"
    exit 1
fi
# Verify LinuxUAPITestSupport.c has per-source compilerFlags
LINUX_UAPI_FLAGS=$(grep -A5 'path: IXLandKernelTests/LinuxUAPITestSupport.c' project.yml | grep 'LINUX_UAPI_INCLUDE_ROOT' || true)
...
UAPI_IN_M=$(grep -l '#include\s*<linux/' "$KERNEL_TEST_ROOT"/*.m "$HOST_TEST_ROOT"/*.m 2>/dev/null || true)
UAPI_IN_M_ASM=$(grep -l '#include\s*<asm/' "$KERNEL_TEST_ROOT"/*.m "$HOST_TEST_ROOT"/*.m 2>/dev/null || true)
...
HOST_SYSCALL_DECLS=$(rg -n 'extern\s+int\s+(ioctl|open|close|snprintf)\s*\(' "$KERNEL_TEST_ROOT"/*.c "$KERNEL_TEST_ROOT"/*.m "$HOST_TEST_ROOT"/*.c "$HOST_TEST_ROOT"/*.m 2>/dev/null || true)
...
SNPRINTF_USAGE=$(rg -n 'snprintf' "$KERNEL_TEST_ROOT"/*.c "$KERNEL_TEST_ROOT"/*.m "$HOST_TEST_ROOT"/*.c "$HOST_TEST_ROOT"/*.m 2>/dev/null || true)
...
LINUX_TEST_IOS=$(rg -n 'internal/ios' "$KERNEL_TEST_ROOT"/*.m "$KERNEL_TEST_ROOT"/*.c 2>/dev/null || true)
...
BRANDED_WRAPPERS=$(rg -n '\bi[x]land_test_[A-Za-z0-9_]*\b' "$KERNEL_TEST_ROOT" "$HOST_TEST_ROOT" 2>/dev/null || true)
...
LINUX_ACCESSORS=$(rg -n '^\s*int\s+linux_\w+\s*\(' "$KERNEL_TEST_ROOT"/*.m "$KERNEL_TEST_ROOT"/*.c "$HOST_TEST_ROOT"/*.m "$HOST_TEST_ROOT"/*.c 2>/dev/null || true)
...
TEST_CONSTANTS=$(rg -n '^\s*#define\s+TES[T]_' "$KERNEL_TEST_ROOT"/*.m "$KERNEL_TEST_ROOT"/*.c "$HOST_TEST_ROOT"/*.m "$HOST_TEST_ROOT"/*.c 2>/dev/null || true)
if [ -n "$TEST_CONSTANTS" ]; then
...
BRIDGE_BAG_LINUX=$(rg -n 'backing_io\.h|backing_io_decls\.h' "$KERNEL_TEST_ROOT"/*.m "$KERNEL_TEST_ROOT"/*.c 2>/dev/null || true)
...
GUTTED_TESTS=$(rg -n 'omitted for brevity\|Additional owner-only tests continue here' "$KERNEL_TEST_ROOT"/*.m "$KERNEL_TEST_ROOT"/*.c "$HOST_TEST_ROOT"/*.m "$HOST_TEST_ROOT"/*.c 2>/dev/null || true)
...
SIG_WAIT_ALIAS_DRIFT=$(rg -n '\b(I[X]_SIG|I[X]_W|TES[T]_SIG|TES[T]_W|linux[_]sig|linux[_]wait)[A-Za-z0-9_]*\b' "$KERNEL_ROOT/fs" "$KERNEL_ROOT/kernel" "$KERNEL_TEST_ROOT" "$HOST_TEST_ROOT" 2>/dev/null || true)
if [ -n "$SIG_WAIT_ALIAS_DRIFT" ]; then
    echo "FAIL: Signal/wait alias drift found:"
    echo "$SIG_WAIT_ALIAS_DRIFT"
    exit 1
fi
ALL_CAPS_SIGNAL_ALIAS=$(rg -n '\bLINU[X]_SIG[A-Za-z0-9_]*\b' "$KERNEL_ROOT/kernel/wait.c" "$KERNEL_ROOT/kernel/signal.c" "$KERNEL_ROOT/kernel/signal.h" 2>/dev/null || true)
...
TERMINAL_SIGNAL_ALIAS_DRIFT=$(rg -n '\bPT[Y]_SIG[A-Za-z0-9_]*\b' "$KERNEL_ROOT/fs/pty.c" 2>/dev/null || true)
...
TEST_LINUX_CONSTANT_ALIAS=$(rg -n '\b(I[X]_F|I[X]_AT|I[X]_TC|I[X]_TIOC|I[X]_SIG|I[X]_W|TES[T]_SIG|TES[T]_W)[A-Za-z0-9_]*\b|include/ixland/linux_(uapi|abi)_constants[.]h' "$KERNEL_TEST_ROOT" "$HOST_TEST_ROOT" 2>/dev/null || true)
...
LINUX_TEST_HOST_SUPPORT=$(rg -n 'HostTestSupport|IXLandHostAdapterTests' "$KERNEL_TEST_ROOT" 2>/dev/null || true)
...
for target in ("IXLandKernelTests", "IXLandHostAdapterTests"):
...
host_block = "\n".join(targets.get("IXLandHostAdapterTests", []))
if "LINUX_UAPI_INCLUDE_ROOT" in host_block:
    errors.append("IXLandHostAdapterTests references Linux UAPI include root")
...
for path, entry in source_entries.items():
    ...
    if path.startswith("IXLandKernelTests/") and "internal/ios" in path:
        errors.append(f"LinuxKernel test source references internal/ios: {path}")

for source in list(Path("IXLandKernelTests").glob("*.c")) + list(Path("IXLandHostAdapterTests").glob("*.c")):
...
HOST_PIPE_OWNER=$(rg -n '\bhost_pipe\b|\bdarwin_pipe\b|Darwin pipe' "$KERNEL_ROOT/fs" "$HOST_ADAPTER_ROOT" 2>/dev/null || true)
...
WAIT_QUEUE_IOS_INCLUDE=$(rg -n 'internal/ios' "$KERNEL_ROOT/kernel/wait_queue.h" 2>/dev/null || true)
...
HOST_EVENT_OWNER=$(rg -n 'host_epoll|kqueue|dispatch_source|host_pipe|Darwin.*pipe' "$KERNEL_ROOT/fs" "$KERNEL_ROOT/kernel" "$HOST_ADAPTER_ROOT" 2>/dev/null || true)
...
echo "rationale: Linux-facing headers must be vendored in a single tuple root (uapi/include plus kheaders/source and kheaders/generated) with no legacy category roots or ABI side-trees."
