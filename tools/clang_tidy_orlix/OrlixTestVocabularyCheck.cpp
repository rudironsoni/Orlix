...
const std::vector<RegexRule> KernelTestRules = {
    ...
    {R"(include/orlix/linux_(uapi|abi)_constants\.h)",
...
OrlixTestVocabularyCheck::OrlixTestVocabularyCheck(llvm::StringRef Name,
                                                     ClangTidyContext *Context)
    : ClangTidyCheck(Name, Context) {}

bool OrlixTestVocabularyCheck::isKernelTestPath(llvm::StringRef Path) const {
  return Path.contains("OrlixKernelTests/");
}

bool OrlixTestVocabularyCheck::isHostTestPath(llvm::StringRef Path) const {
  return Path.contains("OrlixHostAdapterTests/");
}

bool OrlixTestVocabularyCheck::isAnyTestPath(llvm::StringRef Path) const {
  return isKernelTestPath(Path) || isHostTestPath(Path);
}

void OrlixTestVocabularyCheck::registerPPCallbacks(const SourceManager &SM,
                                                    Preprocessor *,
                                                    Preprocessor *) {
...
void OrlixTestVocabularyCheck::onEndOfTranslationUnit() { scanMainFile(); }

void OrlixTestVocabularyCheck::scanMainFile() {
  if (!CurrentSM)
    return;
