...
- The `start_kernel()` link probe attempts a non-product Mach-O dynamic-library link of `init/main.o` plus the current Mach-O Linux archive. If it fails, `link-blockers.txt` records unresolved Linux dependencies and linker-defined section symbols.
- `start_kernel_section_bounds.c` is a probe-only empty boundary object for Linux linker-defined section symbols. It is not a product linker script replacement and must not be linked into `OrlixKernel.framework`.
