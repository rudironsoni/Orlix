	...
	const struct orlix_tcti_target_operation *operation)
{
	...
	if (!operation->canonical_operation_id)
		return NULL;
	return orlix_tcti_target_inventory_operation(inventory,
				  ...
				  FILE *output)
{
	...
	if (orlix_tcti_target_inventory_import(source, length, &inventory, &import_error))
		return ORLIX_TCTI_TARGET_ASL_AVAILABILITY_SOURCE;
	/* The source is authoritative but supplies no shared-ASL corpus or helpers. */
	if (fputs("ORLIX_TCTI_A64_ASL_AVAILABILITY_SOURCE(\"inline_aarchmrs_operations_v3\", \"a1ad2c6538a47cd97d8762791ac5af88bce1d5f6aff096c9b77aef853e76acfe\", ORLIX_TCTI_A64_ASL_CORPUS_ABSENT, ORLIX_TCTI_A64_ASL_HELPERS_UNAVAILABLE)\n", output) == EOF)
		goto out;
	for (index = 0; index < inventory.leaf_count; index++) {
		const struct orlix_tcti_target_leaf *leaf = &inventory.leaves[index];
		const struct orlix_tcti_target_operation *operation =
			orlix_tcti_target_inventory_operation(&inventory, leaf->operation_id);
		...
		if (fputs("ORLIX_TCTI_A64_ASL_AVAILABILITY_ROW(", output) == EOF ||
		    ...
		    fprintf(output, ", %s, ORLIX_TCTI_A64_ASL_CORPUS_ABSENT, "
