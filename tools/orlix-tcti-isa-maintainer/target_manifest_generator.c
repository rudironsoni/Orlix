...
{
	switch (error) {
	case ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_OK:
		return "success";
	case ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_PARSE:
		return "parse failure";
	case ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_METADATA:
		return "metadata failure";
	case ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_COUNT:
		return "count failure";
	case ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_DIGEST:
		return "digest failure";
	case ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_IO:
		return "I/O failure";
	case ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_MANIFEST_MISMATCH:
...
{
	switch (error->code) {
	case ORLIX_TCTI_TARGET_IMPORT_COUNT_MISMATCH:
		return ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_COUNT;
	case ORLIX_TCTI_TARGET_IMPORT_HASH_MISMATCH:
		return ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_DIGEST;
	case ORLIX_TCTI_TARGET_IMPORT_INVALID_SOURCE:
		if (!strcmp(error->message,
			    "Instructions.json metadata does not match the pinned Arm AARCHMRS 2026-06 source"))
			return ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_METADATA;
		return ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_PARSE;
	case ORLIX_TCTI_TARGET_IMPORT_OK:
		return ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_OK;
	default:
		return ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_PARSE;
	}
}
		     ...
		     uint32_t condition)
{
	...
	if (orlix_tcti_target_condition_serialize(inventory, condition, &bytes,
					      &error)) {
			 ...
			 const struct orlix_tcti_target_inventory *inventory)
{
	...
	fputs("/*\n"
	      ...
	      "ORLIX_TCTI_A64_SOURCE_MANIFEST_SOURCE(", output);
	...
	fprintf(output, ", %u, ", ORLIX_TCTI_A64_TARGET_LEAF_COUNT);
	...
	for (index = 0; index < inventory->leaf_count; index++) {
		...
		fputs("ORLIX_TCTI_A64_SOURCE_MANIFEST_ROW(", output);
...
target_manifest_generator_emit(const char *source, size_t length, FILE *output)
{
	...
	enum target_manifest_generator_error status =
		ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_PARSE;

	if (!source || !output)
		return status;
	/* The importer validates parsed metadata and the parsed A64 leaf count. */
	if (orlix_tcti_target_inventory_import(source, length, &inventory, &error)) {
		status = import_error_category(&error);
		goto out;
	}
	if (inventory.leaf_count != ORLIX_TCTI_A64_TARGET_LEAF_COUNT) {
		status = ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_COUNT;
		goto out;
	}
	if (length != SOURCE_BYTE_LENGTH) {
		status = ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_DIGEST;
		goto out;
	}
	...
	if (emit_manifest(output, &inventory))
		status = ferror(output) ? ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_IO :
			ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_PARSE;
	else
		status = ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_OK;
out:
...
int main(int argc, char **argv)
{
	...
	enum target_manifest_generator_error error =
		ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_OK;
	...
	if (read_source(source_path, &source)) {
		error = ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_IO;
		goto out;
	}
	error = target_manifest_generator_emit(source.data, source.length, output);
	if (error != ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_OK)
		goto out;
	if (expected && (fflush(output) || fseek(output, 0, SEEK_SET) ||
			 files_match(expected, output))) {
		error = ferror(expected) || ferror(output) ?
			ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_IO :
			ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_MANIFEST_MISMATCH;
		goto out;
	}
	status = EXIT_SUCCESS;
out:
