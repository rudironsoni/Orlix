...
static int valid_pinned_source_emits(const char *source, size_t length)
{
	...
	CHECK(strstr(header, "ORLIX_TCTI_A64_SOURCE_MANIFEST_SOURCE") != NULL);
					 ...
					 size_t source_length)
{
	...
	for (index = 0; index < sizeof(mutations) / sizeof(mutations[0]); index++) {
		...
		CHECK(output_is_empty_after_failure(
			...
			ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_METADATA) == 0);
		free(mutation);
	}
	return 0;
}
	...
	const char *source, size_t source_length)
{
	...
	CHECK(output_is_empty_after_failure(
		...
		ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_PARSE) == 0);
	...
	CHECK(output_is_empty_after_failure(
		mutation, mutation_length,
		ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_DIGEST) == 0);
	free(mutation);
	...
	CHECK(output_is_empty_after_failure(
		mutation, mutation_length,
		ORLIX_TCTI_TARGET_MANIFEST_GENERATOR_COUNT) == 0);
	free(mutation);
