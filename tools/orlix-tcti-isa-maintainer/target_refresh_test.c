...
static int digest_file(const char *path, char digest[65])
{
	...
	orlix_tcti_target_artifact_sha256(data, (size_t)length, digest);
	free(data);
	return 0;
}
...
static int published_manifest_has_exact_eight_artifacts(const char *root)
{
	...
	if (!current || strncmp(current, "ORLIX_TCTI_TARGET_ARTIFACT_CURRENT_V1\n"
				    "generation=aarchmrs-2026-06-v2-",
				    strlen("ORLIX_TCTI_TARGET_ARTIFACT_CURRENT_V1\n"
					   "generation=aarchmrs-2026-06-v2-")))
		goto out;
	generation = current + strlen("ORLIX_TCTI_TARGET_ARTIFACT_CURRENT_V1\n"
				      "generation=");
	...
	*newline = '\0';
	...
	if (!manifest || !strstr(manifest, "schema=orlix-tcti-aarchmrs-source-v2\n") ||
	    count_text(manifest, "artifact=") != ARRAY_SIZE(expected))
		goto out;
	for (index = 0; index < ARRAY_SIZE(expected); index++) {
		char line[ORLIX_TCTI_TARGET_ARTIFACT_MAX_NAME + sizeof("artifact= \n")];
	...
	const char *features, const char *registers)
{
	...
	if (root_fd < 0 || canonical_fd < 0)
		goto out;
	if (orlix_tcti_target_refresh(root_fd, canonical_fd, instructions, features,
				registers, &result) >= 0 ||
	    result.error != ORLIX_TCTI_TARGET_REFRESH_CANONICAL_MISSING ||
	...
	canonical_fd = open(canonical, O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW);
	if (canonical_fd < 0 ||
	    orlix_tcti_target_refresh(root_fd, canonical_fd, instructions, features,
				registers, &result) >= 0 ||
	    result.error != ORLIX_TCTI_TARGET_REFRESH_CANONICAL_MISMATCH ||
	    !result.canonical_artifact || strcmp(result.canonical_artifact, eighth))
		goto out;
	if (snprintf(path, sizeof(path), "%s/aarchmrs-2026-06-v2", root) >=
	    (int)sizeof(path) || access(path, F_OK) == 0)
		...
		const char *features, const char *registers)
{
	...
	if (orlix_tcti_target_refresh(root_fd, canonical_fd, instructions, features,
		...
		fprintf(stderr, "initial refresh failed: %s%s%s; publish=%s errno=%d stage=%u\n",
			...
			orlix_tcti_target_artifact_publish_error_name(result.publish.error),
			result.publish.system_error, (unsigned int)result.publish.stage);
		return -1;
	}
	EXPECT(result.error == ORLIX_TCTI_TARGET_REFRESH_OK);
	EXPECT(!orlix_tcti_target_refresh(root_fd, canonical_fd, instructions, features, registers,
				    NULL));
	EXPECT(!orlix_tcti_target_artifact_verify(root_fd, "aarchmrs-2026-06-v2",
					     &provenance, &verify));
	...
	EXPECT(orlix_tcti_target_refresh(root_fd, canonical_fd, invalid_path, features, registers,
				   &result) < 0);
	EXPECT(result.error == ORLIX_TCTI_TARGET_REFRESH_MANIFEST);
	EXPECT(!orlix_tcti_target_artifact_verify(root_fd, "aarchmrs-2026-06-v2",
					     &provenance, &verify));
		...
		const char *features, const char *registers)
{
	...
	EXPECT(orlix_tcti_target_refresh(root_fd, canonical_fd, instructions, features,
				   registers, &result) < 0);
	EXPECT(result.error == ORLIX_TCTI_TARGET_REFRESH_CANONICAL_MISSING);
	...
	EXPECT(orlix_tcti_target_refresh(root_fd, canonical_fd, instructions, features,
				   registers, &result) < 0);
	EXPECT(result.error == ORLIX_TCTI_TARGET_REFRESH_CANONICAL_MISMATCH);
	EXPECT(!strcmp(result.canonical_artifact, "source_manifest.def"));
