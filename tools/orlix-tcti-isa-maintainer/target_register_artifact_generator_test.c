...
static int validation_failure_emits_nothing(void)
{
	...
	CHECK(output != NULL);
	CHECK(orlix_tcti_target_register_artifact_emit_model(
		      &model, 1U, output) ==
	...
	output = tmpfile();
	CHECK(output != NULL);
	CHECK(orlix_tcti_target_register_artifact_emit(
		      "[]", ORLIX_TCTI_REGISTER_ARTIFACT_MAX_INPUT + 1U, output) ==
...
{
	struct orlix_tcti_register_model model = { 0 };
	struct orlix_tcti_register_model_error model_error = { 0 };
	char *source;
	...
	CHECK(second != NULL);
	CHECK(orlix_tcti_target_register_artifact_emit_model(
		      &model, source_length, first) == ORLIX_TCTI_REGISTER_ARTIFACT_OK);
	CHECK(orlix_tcti_target_register_artifact_emit_model(
		      &model, source_length, second) == ORLIX_TCTI_REGISTER_ARTIFACT_OK);
