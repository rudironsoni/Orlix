...
#define orlix_tcti_register_model_destroy orlix_tcti_register_model_destroy_base
...
struct parser { const char *s; size_t n, p, depth, count, cap, *allocated; struct tok *t; struct orlix_tcti_register_model_error *e; };
struct import {
	const char *s;
	struct tok *t;
	size_t n;
	...
	struct orlix_tcti_register_model *m;
	struct orlix_tcti_register_model_error *e;
};

static void fail(struct orlix_tcti_register_model_error *e, enum orlix_tcti_register_model_error_code c, size_t p, const char *f, ...)
{
	va_list ap;
	if (!e || e->code)
		return;
...
static int addtok(struct parser *p, enum kind k, size_t start)
{
	if (p->count >= MAX_TOKENS || grow((void **)&p->t, &p->cap, p->count + 1, sizeof(*p->t), p->allocated)) {
		fail(p->e, p->count >= MAX_TOKENS ? ORLIX_TCTI_REGISTER_MODEL_INPUT_LIMIT : ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, p->p, "JSON token storage limit exceeded"); return -1;
	}
...
bad: fail(p->e, ORLIX_TCTI_REGISTER_MODEL_INVALID_JSON, quote, "invalid JSON string"); return -1;
}
...
bad: fail(p->e, ORLIX_TCTI_REGISTER_MODEL_INVALID_JSON, start, "invalid JSON primitive"); return -1;
}
...
deep: fail(p->e, ORLIX_TCTI_REGISTER_MODEL_DEPTH_LIMIT, p->p - 1, "JSON depth limit exceeded"); return -1;
out: p->depth--; if (!p->e->code) fail(p->e, ORLIX_TCTI_REGISTER_MODEL_INVALID_JSON, p->p, "invalid JSON array"); return -1;
}
static int object(struct parser *p)
{
	...
	if (++p->depth > MAX_DEPTH) goto deep; ws(p);
	if (p->p < p->n && p->s[p->p] == '}') goto done;
	for (;;) { int key; size_t prior,c; if (p->p == p->n || p->s[p->p] != '"' || (key=string(p)) < 0) goto out; c=(size_t)i+1; for(prior=0;prior<p->t[i].size;prior++){int same=token_same(p,key,(int)c);if(same){fail(p->e,ORLIX_TCTI_REGISTER_MODEL_INVALID_JSON,p->t[key].start,"duplicate JSON object key");goto out;}if(same<0){fail(p->e,ORLIX_TCTI_REGISTER_MODEL_INVALID_JSON,p->t[key].start,"invalid JSON string");goto out;}c=p->t[p->t[c].next].next;} ws(p); if (p->p == p->n || p->s[p->p++] != ':' || value(p) < 0) goto out; p->t[i].size++; ws(p); if (p->p < p->n && p->s[p->p] == '}') goto done; if (p->p == p->n || p->s[p->p++] != ',') goto out; ws(p); }
done: p->p++; p->depth--; p->t[i].end = p->p; p->t[i].next = p->count; return i;
deep: fail(p->e, ORLIX_TCTI_REGISTER_MODEL_DEPTH_LIMIT, p->p - 1, "JSON depth limit exceeded"); return -1;
out: p->depth--; if (!p->e->code) fail(p->e, ORLIX_TCTI_REGISTER_MODEL_INVALID_JSON, p->p, "invalid JSON object"); return -1;
}
...
{
	struct orlix_tcti_register_expression *p;
	...
	if (parent >= x->m->expression_count ||
		...
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, 0,
...
static int addreg(struct import *x, int o, int type, uint32_t parent, uint32_t *index) { int meta=member(x,o,"_meta"); struct orlix_tcti_register_identity r = { .name=dup(x, member(x,o,"name")), .type=dup(x,type), .state=dup(x,member(x,o,"state")), .index_variable=dup(x,member(x,o,"index_variable")), .parent_register=parent, .metadata_owner=meta>=0?(uint32_t)x->m->register_count:(parent==UINT32_MAX?UINT32_MAX:x->m->registers[parent].metadata_owner), .metadata_index=parent==UINT32_MAX?UINT32_MAX:x->m->registers[parent].metadata_index, .default_access_expression=UINT32_MAX, .first_fieldset=UINT32_MAX, .fieldset_count=0, .metadata_offset=meta>=0?x->t[meta].start:0, .metadata_length=meta>=0?x->t[meta].end-x->t[meta].start:0, .source_offset=x->t[o].start, .source_length=x->t[o].end-x->t[o].start }; size_t i; if (meta >= 0) { for (i = 0; i < x->m->metadata_count; i++) if (x->m->metadata[i].source_offset == x->t[meta].start && x->m->metadata[i].source_length == x->t[meta].end - x->t[meta].start) { r.metadata_index = (uint32_t)i; break; } } if(!r.name||!r.type||r.metadata_owner==UINT32_MAX||r.metadata_index==UINT32_MAX){free(r.name);free(r.type);free(r.state);free(r.index_variable);fail(x->e,ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE,x->t[o].start,"register lacks identity or inherited typed metadata");return -1;} for(i=0;i<x->m->register_count;i++)if(x->m->registers[i].parent_register==parent&&!strcmp(x->m->registers[i].name,r.name)&&!strcmp(x->m->registers[i].type,r.type)&&((!x->m->registers[i].state&&!r.state)||(x->m->registers[i].state&&r.state&&!strcmp(x->m->registers[i].state,r.state)))){free(r.name);free(r.type);free(r.state);free(r.index_variable);fail(x->e,ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE,x->t[o].start,"duplicate register identity");return -1;} if (grow((void **)&x->m->registers, &x->m->register_capacity, x->m->register_count + 1, sizeof(r),x->allocated)) { free(r.name); free(r.type); free(r.state); free(r.index_variable); fail(x->e,ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY,x->t[o].start,"cannot store register identity"); return -1; } *index=(uint32_t)x->m->register_count; x->m->registers[x->m->register_count++] = r; return 0; }
...
static int addfieldset(struct import *x, int o, uint32_t reg, uint32_t width)
{
	...
	struct orlix_tcti_register_fieldset f = {
	...
	if (grow((void **)&x->m->fieldsets, &x->m->fieldset_capacity,
		...
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, x->t[o].start, "cannot store Fieldset");
		return -1;
	}
	...
	if (condition >= 0) {
		...
		if (x->t[condition].kind != OBJ ||
			...
			fail(x->e, ORLIX_TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE,
...
static int memory_read_access(const struct import *x, int token,
			      enum orlix_tcti_register_memory_read_access *out)
{
	if (eq(x, token, "R")) {
		*out = ORLIX_TCTI_REGISTER_MEMORY_READ_R;
		return 0;
	}
	if (eq(x, token, "RAZ")) {
		*out = ORLIX_TCTI_REGISTER_MEMORY_READ_RAZ;
		return 0;
	}
	if (eq(x, token, "RES0")) {
		*out = ORLIX_TCTI_REGISTER_MEMORY_READ_RES0;
		return 0;
	}
	if (eq(x, token, "RESERVED")) {
		*out = ORLIX_TCTI_REGISTER_MEMORY_READ_RESERVED;
		return 0;
	}
	if (eq(x, token, "UNKNOWN")) {
		*out = ORLIX_TCTI_REGISTER_MEMORY_READ_UNKNOWN;
		return 0;
	}
	if (eq(x, token, "ERROR")) {
		*out = ORLIX_TCTI_REGISTER_MEMORY_READ_ERROR;
		return 0;
	}
	return -1;
}
...
static int memory_write_access(const struct import *x, int token,
			       enum orlix_tcti_register_memory_write_access *out)
{
	if (eq(x, token, "W")) {
		*out = ORLIX_TCTI_REGISTER_MEMORY_WRITE_W;
		return 0;
	}
	if (eq(x, token, "RESERVED")) {
		*out = ORLIX_TCTI_REGISTER_MEMORY_WRITE_RESERVED;
		return 0;
	}
	if (eq(x, token, "WI")) {
		*out = ORLIX_TCTI_REGISTER_MEMORY_WRITE_WI;
		return 0;
	}
	if (eq(x, token, "RES0")) {
		*out = ORLIX_TCTI_REGISTER_MEMORY_WRITE_RES0;
		return 0;
	}
	if (eq(x, token, "ERROR")) {
		*out = ORLIX_TCTI_REGISTER_MEMORY_WRITE_ERROR;
		return 0;
	}
	return -1;
}
			     ...
			     uint32_t expression_index)
{
	...
	struct orlix_tcti_register_memory_access access = {
		.form = ORLIX_TCTI_REGISTER_MEMORY_ACCESS_EXPLICIT_OBJECT,
		.read_origin = ORLIX_TCTI_REGISTER_MEMORY_ACCESS_EXPLICIT,
		.write_origin = ORLIX_TCTI_REGISTER_MEMORY_ACCESS_EXPLICIT,
		.legacy_sentinel = ORLIX_TCTI_REGISTER_MEMORY_ACCESS_LEGACY_NONE,
		.owner_kind = ORLIX_TCTI_REGISTER_MEMORY_ACCESS_OWNER_EXPRESSION,
	...
	if (x->t[object].kind != OBJ || x->t[object].size != 3 || read < 0 ||
		...
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE, x->t[object].start,
	...
	if (grow((void **)&x->m->memory_accesses,
		...
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, x->t[object].start,
					 ...
					 uint32_t expression_index)
{
	...
	struct orlix_tcti_register_implementation_defined_permission permission = {
	...
	if (null_value(x, constraints)) {
		permission.kind = ORLIX_TCTI_REGISTER_IMPLEMENTATION_DEFINED_CONSTRAINT_NULL;
	} else {
		permission.kind = ORLIX_TCTI_REGISTER_IMPLEMENTATION_DEFINED_CONSTRAINT_ARRAY;
		...
		if (permission.memory_access_count != x->t[constraints].size) {
			fail(x->e, ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE,
			  ...
			  int role, uint32_t *index)
{
	...
	if (object < 0 || (size_t)object >= x->n) {
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE, 0,
	...
	struct orlix_tcti_register_expression expression = {
	...
	if (!tree_object(x, object) || !tree_operator_allowed(x, object)) {
		...
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE, x->t[object].start, "unsupported condition/action tree node");
		return -1;
	}
	if (!expression.type || grow((void **)&x->m->expressions, &x->m->expression_capacity,
		x->m->expression_count + 1, sizeof(expression), x->allocated)) {
		free(expression.type); free(expression.name); free(expression.op); free(expression.value); free(expression.role);
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, x->t[object].start, "cannot store condition/action tree");
		return -1;
	}
	...
	for (z = 0; z < x->t[object].size; z++) {
		if (!expression_key_allowed(x, type, (int)child)) {
			free(expression.type); free(expression.name); free(expression.op); free(expression.value); free(expression.role);
			fail(x->e, ORLIX_TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE, x->t[child].start, "unknown condition/action tree key");
			return -1;
		}
	...
	*index = (uint32_t)x->m->expression_count;
	...
	for (z = 0; z < x->t[object].size; z++) {
		...
		if (x->t[value].kind == OBJ && tree_candidate(x, (int)value) && !tree_object(x, (int)value)) {
			fail(x->e, ORLIX_TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE, x->t[value].start, "unsupported condition/action tree child");
			return -1;
			...
			for (item = 0; item < x->t[value].size; item++) {
				...
				if (nested >= 0 && x->t[nested].kind == OBJ && tree_candidate(x, nested) && !tree_object(x, nested)) {
					fail(x->e, ORLIX_TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE, x->t[nested].start, "unsupported condition/action tree array child");
					return -1;
...
{
	struct orlix_tcti_register_selector_group_fragment fragment = {
	...
	if (grow((void **)&x->m->selector_group_fragments,
		...
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, offset,
...
static int add_selector_payload(struct import *x, int object, uint32_t selector_index)
{
	...
	if (eq(x, member(x, object, "_type"), "Values.Value")) {
		...
		if (grow((void **)&x->m->selector_literals,
			...
			goto nomem;
		selector->value_kind = ORLIX_TCTI_REGISTER_SELECTOR_VALUE_LITERAL;
	...
	if (eq(x, member(x, object, "_type"), "Values.EquationValue")) {
		...
		struct orlix_tcti_register_selector_equation equation = {
		...
		selector->value_kind = ORLIX_TCTI_REGISTER_SELECTOR_VALUE_EQUATION;
		...
		for (index = 0; index < x->t[slices].size; index++) {
			...
			struct orlix_tcti_register_selector_slice slice;
			...
			slice = (struct orlix_tcti_register_selector_slice){
	...
	if (eq(x, member(x, object, "_type"), "Values.Group")) {
		struct orlix_tcti_register_selector_group group = {
			.selector_index = selector_index,
		...
		selector->value_kind = ORLIX_TCTI_REGISTER_SELECTOR_VALUE_GROUP;
		...
		x->m->selector_groups[x->m->selector_group_count++] = group;
		{
			...
			while (position < length) {
				...
				if (text[position] == '\'') {
					...
					literal = (struct orlix_tcti_register_selector_literal){
						.selector_index = selector_index,
					...
					equation = (struct orlix_tcti_register_selector_equation){ .selector_index=selector_index, .identifier=malloc(name_end - name_start + 1), .first_slice=(uint32_t)x->m->selector_slice_count, .slice_count=1, .source_offset=x->t[value_token].start+1+start, .source_length=position-start };
					...
					slice=(struct orlix_tcti_register_selector_slice){ .equation_index=(uint32_t)x->m->selector_equation_count,.start=low,.width=high-low+1,.source_offset=equation.source_offset,.source_length=equation.source_length };
					if (add_group_fragment(x,selector->payload_index,ORLIX_TCTI_REGISTER_SELECTOR_VALUE_EQUATION,(uint32_t)x->m->selector_equation_count,equation.source_offset,equation.source_length)) { free(equation.identifier); return -1; }
	...
	unsupported:
	fail(x->e, ORLIX_TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE, x->t[object].start,
...
static int addselector(struct import *x, int o, uint32_t encoding_index, int name)
{
	...
	struct orlix_tcti_register_system_selector selector = {
		...
		.value_kind = ORLIX_TCTI_REGISTER_SELECTOR_VALUE_NONE,
	...
	if (!selector.name || !selector.type || !allowed_selector_name(x,name) ||
		...
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, x->t[o].start, "cannot store system selector");
		return -1;
	}
	{
...
static int addsystem_encoding(struct import *x, int o, uint32_t accessor_index)
{
	...
	struct orlix_tcti_register_system_encoding encoding = {
	...
	if (asmvalue < 0 || !eq(x, member(x, o, "_type"), "Encoding") ||
		...
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE, x->t[o].start,
	...
	if (encoding.asmvalue_is_null) {
		if (*x->allocated >= MAX_ALLOC || !(encoding.asmvalue = malloc(1))) {
			free(encoding.type);
			fail(x->e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, x->t[o].start,
	...
	if (fields < 0 || x->t[fields].kind != OBJ ||
		...
		free(encoding.type); free(encoding.asmvalue);
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE, x->t[o].start, "invalid system encoding");
		return -1;
	}
...
static int accessor_range(struct import *x, int token,
			  struct orlix_tcti_register_accessor *accessor)
{
	uint32_t start, width;
...
{
	struct orlix_tcti_register_accessor_offset_expression item;
	uint32_t expression;

	if (token < 0 || x->t[token].kind != OBJ || !tree_object(x, token) ||
	    add_expression(x, token, UINT32_MAX, -1, &expression))
		return -1;
	item = (struct orlix_tcti_register_accessor_offset_expression){
...
static int addaccessor(struct import *x, int o, int type, uint32_t reg)
{
	...
	struct orlix_tcti_register_accessor a = {
	...
	if (!allowed_accessor(x, type) || !a.type) {
		...
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE, x->t[o].start, "unsupported accessor");
		return -1;
	}
	...
	if (grow((void **)&x->m->accessors, &x->m->accessor_capacity,
		...
		free(a.type); free(a.name); free(a.index_variable);
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, x->t[o].start, "cannot store accessor");
		return -1;
	}
	{
		...
		if (eq(x, type, "Accessors.MemoryMapped") ||
		    eq(x, type, "Accessors.ExternalDebug")) {
			struct orlix_tcti_register_accessor *stored = &x->m->accessors[index];
		...
		if (condition >= 0 && ( !tree_object(x, condition) ||
		    ...
		    &x->m->accessors[index].condition_expression))) { fail(x->e,ORLIX_TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE,x->t[condition].start,"unsupported accessor condition"); return -1; }
		if (!null_value(x, access) && access >= 0 && ( !tree_object(x, access) ||
		    add_expression(x, access, UINT32_MAX, -1,
		    &x->m->accessors[index].access_expression))) { fail(x->e,ORLIX_TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE,x->t[access].start,"unsupported accessor action"); return -1; }
...
static int addfield(struct import *x, int o, int type, uint32_t width, uint32_t reg, uint32_t parent, int wrapper_condition, int wrapper_object, uint32_t *index) { int condition=member(x,o,"condition"), branches=member(x,o,"fields"); struct orlix_tcti_field_identity f = { .name=dup(x,member(x,o,"name")), .type=dup(x,type), .width=width, .register_index=reg, .parent_field=parent, .fieldset_index=(uint32_t)(x->m->fieldset_count-1), .first_range=(uint32_t)x->m->range_count, .condition_expression=UINT32_MAX, .wrapper_condition_expression=UINT32_MAX, .condition_offset=condition>=0?x->t[condition].start:0, .condition_length=condition>=0?x->t[condition].end-x->t[condition].start:0, .wrapper_condition_offset=wrapper_condition>=0?x->t[wrapper_condition].start:0, .wrapper_condition_length=wrapper_condition>=0?x->t[wrapper_condition].end-x->t[wrapper_condition].start:0, .branch_offset=branches>=0?x->t[branches].start:0, .branch_length=branches>=0?x->t[branches].end-x->t[branches].start:0, .source_offset=x->t[o].start, .source_length=x->t[o].end-x->t[o].start }; struct orlix_tcti_register_field_value_relation relation; if (!f.type || grow((void **)&x->m->fields, &x->m->field_capacity, x->m->field_count + 1, sizeof(f),x->allocated)) { free(f.name); free(f.type); fail(x->e,ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY,x->t[o].start,"cannot store field identity"); return -1; } *index=(uint32_t)x->m->field_count; x->m->fields[x->m->field_count++] = f; relation = (struct orlix_tcti_register_field_value_relation){ .field_index = *index, .first_value_candidate = UINT32_MAX, .first_constraint_candidate = UINT32_MAX, .field_condition_expression = UINT32_MAX, .wrapper_condition_expression = UINT32_MAX, .source_offset = f.source_offset, .source_length = f.source_length }; if (grow((void **)&x->m->field_value_relations, &x->m->field_value_relation_capacity, x->m->field_value_relation_count + 1, sizeof(relation), x->allocated)) { fail(x->e,ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY,x->t[o].start,"cannot store field value relation"); return -1; } if (x->m->field_value_relation_count != *index) { fail(x->e,ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE,x->t[o].start,"field value relation ordering is ambiguous"); return -1; } x->m->field_value_relations[x->m->field_value_relation_count++] = relation; if (wrapper_object >= 0) { struct orlix_tcti_conditional_field_branch b = { .conditional_field_index=parent, .field_index=*index, .condition_offset=x->t[wrapper_condition].start, .condition_length=x->t[wrapper_condition].end-x->t[wrapper_condition].start, .source_offset=x->t[wrapper_object].start, .source_length=x->t[wrapper_object].end-x->t[wrapper_object].start }; if (parent==UINT32_MAX || grow((void **)&x->m->field_branches,&x->m->field_branch_capacity,x->m->field_branch_count+1,sizeof(b),x->allocated)) { fail(x->e,ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY,x->t[o].start,"cannot store conditional-field branch"); return -1; } x->m->field_branches[x->m->field_branch_count++]=b; } return 0; }
static int field_condition_roots(struct import *x, int object, uint32_t field,
				 int wrapper_condition)
{
...
static int addlink(struct import *x, int o, uint32_t domain_index)
{
	...
	for (z = 0; z < x->t[o].size; z++) {
		size_t value = x->t[child].next;
		struct orlix_tcti_register_link link = {
		...
		if (!link.key || !link.value || grow((void **)&x->m->links, &x->m->link_capacity,
			...
			fail(x->e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, x->t[child].start, "cannot store Values.Link key");
			return -1;
		}
...
{
	int type = member(x, o, "_type");
	struct orlix_tcti_register_valueset v = {
	...
	if (!v.type || (!eq(x, type, "Valuesets.Values") &&
		...
		free(v.type);
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE, x->t[o].start, "unsupported Valueset");
		return -1;
	}
	if (grow((void **)&x->m->valuesets, &x->m->valueset_capacity,
		 x->m->valueset_count + 1, sizeof(v), x->allocated)) {
		free(v.type);
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, x->t[o].start, "cannot store Valueset");
		return -1;
	}
...
static int record_field_values(struct import *x, uint32_t field,
			       ...
			       int values_token)
{
	struct orlix_tcti_register_field_value_relation *relation;
	struct orlix_tcti_register_field_value_candidate candidate;
	int type;
	...
	candidate = (struct orlix_tcti_register_field_value_candidate){
		.field_index = field,
	...
	if (eq(x, type, "Valuesets.Values"))
		candidate.kind = ORLIX_TCTI_REGISTER_FIELD_VALUE_SOURCE_VALUES;
	...
	if (grow((void **)&x->m->field_value_candidates,
		 ...
		 x->allocated)) {
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, x->t[values_token].start,
...
static int add_constraint_item(struct import *x, uint32_t constraint_index,
			       ...
			       int token)
{
	struct orlix_tcti_register_constraint_item item = {
	...
	if (grow((void **)&x->m->constraint_items,
		...
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, x->t[token].start,
...
static int addconstraint(struct import *x, int token, uint32_t field)
{
	...
	struct orlix_tcti_register_constraint c = {
		.kind = ORLIX_TCTI_REGISTER_CONSTRAINT_NULL, .field_index = field,
		.valueset_index = UINT32_MAX, .first_item = UINT32_MAX,
		.source_offset = x->t[token].start,
		.source_length = x->t[token].end - x->t[token].start,
	};
	if (null_value(x, token)) c.kind = ORLIX_TCTI_REGISTER_CONSTRAINT_NULL;
	else if (x->t[token].kind == ARR) c.kind = ORLIX_TCTI_REGISTER_CONSTRAINT_ARRAY;
	else if (x->t[token].kind == OBJ && eq(x, member(x, token, "_type"), "Valuesets.Values")) {
		int values;
		size_t z;
		c.kind = ORLIX_TCTI_REGISTER_CONSTRAINT_VALUESET;
		if (addvalueset(x, token, field, &c.valueset_index, 1)) return -1;
		values = member(x, token, "values");
		if (values < 0 || x->t[values].kind != ARR) {
			fail(x->e, ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE, x->t[token].start, "constraint Valueset lacks array");
			return -1;
		}
		for (z = 0; z < x->t[values].size; z++)
			if (domain(x, elem(x, values, z), field, c.valueset_index, UINT32_MAX, 1)) return -1;
	} else {
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE, x->t[token].start, "unsupported constraint container");
		return -1;
	}
	if (c.kind == ORLIX_TCTI_REGISTER_CONSTRAINT_VALUESET)
		c.item_count = 1;
	else if (c.kind == ORLIX_TCTI_REGISTER_CONSTRAINT_ARRAY)
	...
	if (grow((void **)&x->m->constraints, &x->m->constraint_capacity,
		...
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, x->t[token].start, "cannot store constraint container");
		return -1;
	}
	{
		...
		x->m->constraints[constraint_index].first_item =
			c.kind == ORLIX_TCTI_REGISTER_CONSTRAINT_NULL ? UINT32_MAX :
			(uint32_t)(x->m->constraint_item_count -
			 x->m->constraints[constraint_index].item_count);
		x->m->constraints[constraint_index].item_count =
			(uint32_t)(x->m->constraint_item_count -
			 (c.kind == ORLIX_TCTI_REGISTER_CONSTRAINT_NULL ?
			  x->m->constraint_item_count :
			  x->m->constraints[constraint_index].first_item));
	}
	if (field != UINT32_MAX) {
		struct orlix_tcti_register_field_value_relation *relation;
		struct orlix_tcti_register_field_constraint_candidate candidate;
		...
		relation = &x->m->field_value_relations[field];
		candidate = (struct orlix_tcti_register_field_constraint_candidate){
			.field_index = field,
...
static int domain(struct import *x, int o, uint32_t field, uint32_t valueset,
		  uint32_t parent, int constraint)
{
	int t = member(x,o,"_type"), nested, condition=member(x,o,"condition"), links=member(x,o,"links"); struct orlix_tcti_value_domain d = { .type=dup(x,t), .value=domain_text(x,member(x,o,"value")), .end=domain_text(x,member(x,o,"end")), .link=links>=0?dup(x,member(x,links,"ISS")):NULL, .meaning=domain_text(x,member(x,o,"meaning")), .field_index=field, .parent_domain=parent, .condition_expression=UINT32_MAX, .first_child_domain=UINT32_MAX, .nested_valueset_index=UINT32_MAX, .condition_offset=condition>=0?x->t[condition].start:0, .condition_length=condition>=0?x->t[condition].end-x->t[condition].start:0, .source_offset=x->t[o].start, .source_length=x->t[o].end-x->t[o].start, .start=domain_text(x,member(x,o,"start")), .valueset_index=valueset };
	if (!allowed_domain(x,t)) { free(d.type); free(d.value); free(d.end); free(d.link); free(d.start); fail(x->e,ORLIX_TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE,x->t[o].start,"unsupported Valuesets.Values domain"); return -1; }
	if (eq(x,t,"Values.ConditionalValue")) {
		...
		if (nested < 0 || x->t[nested].kind != OBJ ||
			...
			fail(x->e,ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE,x->t[o].start,"ConditionalValue lacks Valuesets.Values");
			return -1;
		}
		d.branch_offset = x->t[nested].start;
		d.branch_length = x->t[nested].end - x->t[nested].start;
	}
	if (!d.type || grow((void **)&x->m->domains,&x->m->domain_capacity,x->m->domain_count+1,sizeof(d),x->allocated)) { free(d.type); free(d.value); free(d.end); free(d.link); free(d.start); fail(x->e,ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY,x->t[o].start,"cannot store value domain"); return -1; }
	{ uint32_t index=(uint32_t)x->m->domain_count; x->m->domains[x->m->domain_count++] = d;
		if (parent != UINT32_MAX) {
			struct orlix_tcti_value_domain *owner = &x->m->domains[parent];
		...
		if (condition >= 0) { uint32_t root; if (x->t[condition].kind != OBJ || add_expression(x,condition,UINT32_MAX,-1,&root)) { fail(x->e,ORLIX_TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE,x->t[condition].start,"unsupported value condition"); return -1; } x->m->domains[index].condition_expression=root; }
...
{
	size_t z; if (a < 0 || x->t[a].kind != ARR) { fail(x->e,ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE,0,"field lacks rangeset"); return -1; }
	for(z=0;z<x->t[a].size;z++) { int r=elem(x,a,z); uint32_t start, bits; struct orlix_tcti_register_range item; if(r<0 || !eq(x,member(x,r,"_type"),"Range") || number(x,member(x,r,"start"),&start) || number(x,member(x,r,"width"),&bits) || !bits || start > width || bits > width-start) { fail(x->e,ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE,r<0?0:x->t[r].start,"invalid Range for fieldset width"); return -1; } item=(struct orlix_tcti_register_range){start,bits,field,x->t[r].start,x->t[r].end-x->t[r].start}; if(grow((void **)&x->m->ranges,&x->m->range_capacity,x->m->range_count+1,sizeof(item),x->allocated)){fail(x->e,ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY,x->t[r].start,"cannot store Range");return -1;} x->m->ranges[x->m->range_count++]=item; x->m->fields[field].range_count++; }
	return 0;
}
...
static int add_metadata(struct import *x, int object)
{
	...
	if (!metadata_key_count(x, license, "copyright", "info") ||
		...
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE, x->t[meta].start,
		     "register metadata has missing, extra, or incorrectly typed fields");
		return -1;
	}
	metadata = (struct orlix_tcti_register_metadata){
	...
	if (!metadata.copyright || !metadata.license_info || !metadata.architecture ||
		...
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, x->t[meta].start,
...
static int metadata(struct import *x, int o)
{
	...
	if(meta<0){fail(x->e,ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE,x->t[o].start,"register lacks pinned metadata");return -1;}
	if(version<0||!eq(x,member(x,version,"architecture"),"vFATAp1-A")||!eq(x,member(x,version,"build"),"818")||!eq(x,member(x,version,"ref"),"2026-06_rel")||!eq(x,member(x,version,"schema"),"2.9.5")||!eq(x,member(x,version,"timestamp"),"2026-06-24 17:12:14")){fail(x->e,ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE,x->t[o].start,"register metadata does not match pin");return -1;}return 0;
}
		...
		uint32_t parent_field, int require_metadata, int wrapper_condition, int wrapper_object)
{
	...
	if (wrapper_object >= 0 && wrapper_condition < 0) {
		fail(x->e, ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE, x->t[i].start,
		     "conditional field branch lacks condition");
		return -1;
	}
	if (++x->depth > MAX_DEPTH) { fail(x->e,ORLIX_TCTI_REGISTER_MODEL_DEPTH_LIMIT,x->t[i].start,"source tree depth limit exceeded"); x->depth--; return -1; }
	if (x->t[i].kind == OBJ) { t=member(x,i,"_type");
		...
		if(eq(x,t,"Fieldset")) { if(number(x,member(x,i,"width"),&width) || !width || width > 128 || reg == UINT32_MAX || addfieldset(x, i, reg, width)) { fail(x->e,ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE,x->t[i].start,"invalid Fieldset"); goto bad; } if(reg!=UINT32_MAX){struct orlix_tcti_register_identity *r=&x->m->registers[reg];if(r->first_fieldset==UINT32_MAX)r->first_fieldset=(uint32_t)(x->m->fieldset_count-1);r->fieldset_count++;} }
		if (t >= 0 && x->t[t].kind == STR && x->t[t].end-x->t[t].start > 7 && !memcmp(x->s+x->t[t].start,"Fields.",7)) { int vals, constraints; uint32_t field; if(!allowed_field(x,t)) { fail(x->e,ORLIX_TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE,x->t[i].start,"unsupported field type"); goto bad; } if(!width || reg==UINT32_MAX || addfield(x,i,t,width,reg,parent_field,wrapper_condition,wrapper_object,&field) || ranges(x,member(x,i,"rangeset"),width,field)) goto bad; vals=member(x,i,"values"); if(vals>=0) { size_t q, first_domain = x->m->domain_count; int a; uint32_t valueset; if (addvalueset(x, vals, field, &valueset, 0)) goto bad; a=member(x,vals,"values"); if(a<0||x->t[a].kind!=ARR) { fail(x->e,ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE,x->t[vals].start,"Valueset lacks array"); goto bad; } for(q=0;q<x->t[a].size;q++) if(domain(x,elem(x,a,q),field,valueset,UINT32_MAX,0)) goto bad; if (record_field_values(x, field, valueset, first_domain, vals)) goto bad; } constraints=member(x,i,"constraints"); if(constraints>=0 && addconstraint(x,constraints,field)) goto bad; parent_field=field; }
...
static int populate_field_condition_roots(struct import *x)
{
	...
	for (index = 0; index < x->m->field_count; index++) {
		struct orlix_tcti_field_identity *field = &x->m->fields[index];
...
static int rebuild_expression_children(struct import *x)
{
	...
	total = 0;
	for (index = 0; index < x->m->expression_count; index++) {
		struct orlix_tcti_register_expression *expression = &x->m->expressions[index];
...
void orlix_tcti_register_model_destroy(struct orlix_tcti_register_model *m)
{
	...
	orlix_tcti_register_model_destroy_base(m);
}
int orlix_tcti_register_model_import(const char *json,size_t length,struct orlix_tcti_register_model *m,struct orlix_tcti_register_model_error *e)
{
	struct orlix_tcti_register_model_error local={0};
	struct parser p={0};
	struct import x;
	size_t allocated=0;
	...
	if(!e)e=&local;
	memset(e,0,sizeof(*e));
	if(!json||!length||!m){fail(e,ORLIX_TCTI_REGISTER_MODEL_INVALID_ARGUMENT,0,"JSON and model are required");return -1;}
	if (m->source || m->registers || m->metadata || m->memory_accesses ||
		...
		fail(e, ORLIX_TCTI_REGISTER_MODEL_INVALID_ARGUMENT, 0,
		     "model must be destroyed before reuse");
		return -1;
	}
	if(length>MAX_INPUT){fail(e,ORLIX_TCTI_REGISTER_MODEL_INPUT_LIMIT,0,"Registers.json exceeds input limit");return -1;}
	memset(m,0,sizeof(*m));
	...
	root=value(&p);
	ws(&p);
	if(root<0||p.p!=length||p.t[root].kind!=ARR){if(!e->code)fail(e,ORLIX_TCTI_REGISTER_MODEL_INVALID_JSON,p.p,"Registers.json must be one JSON array");goto bad;}
	if (p.count > SIZE_MAX / sizeof(*expression_by_token) ||
		...
		fail(e, ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY, 0,
	...
	for (token_index = 0; token_index < p.count; token_index++) {
		...
		if (eq(&x, type,
		       "Accessors.Permission.AccessTypes.Memory.ReadWriteAccess") &&
		    expression_by_token[token_index] == UINT32_MAX) {
			fail(e, ORLIX_TCTI_REGISTER_MODEL_INVALID_SOURCE,
			     p.t[token_index].start,
			     "typed ReadWriteAccess was not reached by the source walk");
			goto bad;
		}
	}
	if(p.t[root].size!=ORLIX_TCTI_REGISTER_METADATA_COUNT||m->register_count!=ORLIX_TCTI_REGISTER_RECORD_COUNT||m->metadata_count!=ORLIX_TCTI_REGISTER_METADATA_COUNT||m->memory_access_count!=ORLIX_TCTI_REGISTER_MEMORY_ACCESS_COUNT||m->implementation_defined_permission_count!=17U||m->fieldset_count!=ORLIX_TCTI_REGISTER_FIELDSET_COUNT||m->field_count!=ORLIX_TCTI_REGISTER_FIELD_COUNT||m->range_count!=ORLIX_TCTI_REGISTER_RANGE_COUNT||m->top_level_valueset_count!=ORLIX_TCTI_REGISTER_VALUESET_COUNT||m->top_level_domain_count!=ORLIX_TCTI_REGISTER_DOMAIN_COUNT||m->accessor_count!=ORLIX_TCTI_REGISTER_ACCESSOR_COUNT||m->constraint_count!=1656U||m->constraint_domain_count!=2958U){
	...
	free(p.t);
	p.t = NULL;
	if (allocated > MAX_ALLOC - length || !(m->source = malloc(length))) { fail(e,ORLIX_TCTI_REGISTER_MODEL_NO_MEMORY,0,"cannot retain Registers.json source"); goto bad; }
...
bad:
	free(expression_by_token);
	orlix_tcti_register_model_destroy(m);
	free(p.t);
	return -1;
}
