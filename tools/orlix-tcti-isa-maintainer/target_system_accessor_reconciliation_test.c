...
{
	static const struct orlix_tcti_register_accessor accessors[] = {
	...
	const struct orlix_tcti_register_model model = {
		.accessors = (struct orlix_tcti_register_accessor *)accessors,
		.accessor_count = sizeof(accessors) / sizeof(accessors[0]),
		.system_encodings = (struct orlix_tcti_register_system_encoding *)encodings,
		.system_encoding_count = sizeof(encodings) / sizeof(encodings[0]),
		.system_selectors = (struct orlix_tcti_register_system_selector *)selectors,
	...
	artifact = tmpfile();
	CHECK(artifact != NULL);
	CHECK(orlix_tcti_system_accessor_reconciliation_emit(&model, artifact) ==
	      ORLIX_TCTI_SYSTEM_ACCESSOR_RECONCILIATION_OK);
	...
	CHECK(strstr(artifact_text,
	      "ORLIX_TCTI_A64_SYSTEM_ACCESSOR_COUNTS(4U, 1U, 0U, 0U, 1U, 1U, "
	      "0U, 1U)") != NULL);
	CHECK(strstr(artifact_text,
...
{
	struct orlix_tcti_system_accessor_reconciliation_entry entry = { 0 };
...
static int pinned_aarch64_census(const char *path)
{
	...
	CHECK(source != NULL);
	CHECK(!orlix_tcti_register_model_import(source, length, &model, &import_error));
	CHECK(orlix_tcti_system_accessor_reconcile(&model, &result) ==
	      ORLIX_TCTI_SYSTEM_ACCESSOR_RECONCILIATION_OK);
