...
#define TCTI_AUDIT_INSTRUCTIONS_MAX_BYTES (128U * 1024U * 1024U)
#define TCTI_AUDIT_FEATURES_MAX_BYTES (8U * 1024U * 1024U)
#define TCTI_AUDIT_REGISTERS_MAX_BYTES (128U * 1024U * 1024U)
...
int main(int argc, char **argv)
{
	...
	struct tcti_target_inventory target = { 0 };
	struct tcti_target_import_error import_error = { 0 };
	struct tcti_feature_model feature_model = { 0 };
	struct tcti_feature_error feature_error = { 0 };
	struct tcti_typed_expression typed_expression = { 0 };
	struct tcti_feature_typed_result typed_result = { 0 };
	struct tcti_register_model register_model = { 0 };
	struct tcti_register_model_error register_error = { 0 };
	struct tcti_target_feature_sat_audit feature_audit = { 0 };
	struct tcti_target_feature_sat_error feature_sat_error = { 0 };
	struct tcti_target_ledger_result ledger = { 0 };
	...
	instructions_json = read_file(argv[1], "Instructions.json",
				      TCTI_AUDIT_INSTRUCTIONS_MAX_BYTES,
				      &instructions_length);
	if (!instructions_json)
		goto out;
	features_json = read_file(argv[2], "Features.json",
				  TCTI_AUDIT_FEATURES_MAX_BYTES,
				  &features_length);
	if (!features_json)
		goto out;
	registers_json = read_file(argv[3], "Registers.json",
				   TCTI_AUDIT_REGISTERS_MAX_BYTES,
				   &registers_length);
	if (!registers_json)
		goto out;
	if (tcti_target_inventory_import(instructions_json, instructions_length,
					 &target, &import_error)) {
		fprintf(stderr, "Instructions.json:%zu: %s\n", import_error.offset,
			import_error.message[0] ? import_error.message : "target import failed");
		goto out;
	}
	if (tcti_target_feature_model_import(features_json, features_length,
	...
	if (tcti_register_model_import(registers_json, registers_length,
	...
	if (tcti_feature_typed_lower(&feature_model, &typed_expression,
				     &typed_result) ||
		...
		if (typed_result.diagnostic_count) {
			const struct tcti_feature_typed_diagnostic *diagnostic =
	...
	if (tcti_target_feature_sat_audit(
		    &feature_model, &target,
		    TCTI_TARGET_FEATURE_SAT_DEFAULT_BRANCH_LIMIT,
		    TCTI_TARGET_FEATURE_SAT_DEFAULT_ALLOCATION_LIMIT,
		    &feature_audit, &feature_sat_error) ||
	    feature_audit.leaf_count != TCTI_A64_TARGET_LEAF_COUNT) {
		fprintf(stderr,
	...
	for (index = 0; index < feature_audit.leaf_count; index++) {
		...
		case TCTI_TARGET_FEATURE_SAT_APPLICABLE:
			applicable_count++;
			break;
		case TCTI_TARGET_FEATURE_SAT_IMPOSSIBLE:
			impossible_count++;
			break;
		default:
			unresolved_count++;
	...
	source = tcti_target_ledger_source(&source_count);
	classification = tcti_target_ledger_classification(&classification_count);
	proof_registry = tcti_target_proof_registry_entries(&proof_registry_count);
	if (source_count != TCTI_A64_TARGET_LEAF_COUNT || classification_count != source_count ||
	    tcti_target_ledger_validate(source, source_count, classification,
					classification_count,
					TCTI_A64_TARGET_LEAF_COUNT,
					proof_registry, proof_registry_count,
					&ledger) ||
	    tcti_target_ledger_verify_import(source, source_count, &target, &ledger)) {
	...
	status = EXIT_SUCCESS;
out:
	tcti_target_feature_sat_audit_destroy(&feature_audit);
	tcti_register_model_destroy(&register_model);
	tcti_feature_typed_result_destroy(&typed_result);
	tcti_typed_destroy(&typed_expression);
	tcti_target_feature_model_destroy(&feature_model);
	tcti_target_inventory_destroy(&target);
	free(registers_json);
	free(features_json);
