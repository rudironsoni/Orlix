...
static int golden_expression_bytes(void)
{
	...
	static struct tcti_target_expr expressions[] = {
		{ .kind = TCTI_TARGET_EXPR_IN, .left = 1, .right = 2 },
		{ .kind = TCTI_TARGET_EXPR_FEATURE, .text = feature },
		{ .kind = TCTI_TARGET_EXPR_SET, .first_item = 0, .item_count = 2 },
	...
	EXPECT_TRUE(!tcti_target_condition_serialize(&inventory, 0, &bytes, NULL));
...
static int collisions_remain_distinct(void)
{
	...
	static char b[] = "B";
	static struct tcti_target_expr expressions[] = {
		{ .kind = TCTI_TARGET_EXPR_AND, .left = 1, .right = 2 },
		{ .kind = TCTI_TARGET_EXPR_FEATURE, .text = a },
		{ .kind = TCTI_TARGET_EXPR_FEATURE, .text = b },
		{ .kind = TCTI_TARGET_EXPR_OR, .left = 1, .right = 2 },
	};
	struct tcti_target_inventory inventory = {
		.expressions = expressions,
		.expression_count = sizeof(expressions) / sizeof(expressions[0]),
	};
	struct tcti_target_condition_bytes left;
	struct tcti_target_condition_bytes right;

	EXPECT_TRUE(!tcti_target_condition_serialize(&inventory, 0, &left, NULL));
	EXPECT_TRUE(!tcti_target_condition_serialize(&inventory, 3, &right, NULL));
	EXPECT_TRUE(left.length == right.length);
	EXPECT_TRUE(memcmp(left.data, right.data, left.length));
	tcti_target_condition_bytes_destroy(&left);
	tcti_target_condition_bytes_destroy(&right);
	return 0;
}
...
static int inherited_asts_remain_complete(void)
{
	...
	static struct tcti_target_expr expressions[] = {
		{ .kind = TCTI_TARGET_EXPR_FEATURE, .text = parent },
		{ .kind = TCTI_TARGET_EXPR_FEATURE, .text = child },
	...
	struct tcti_target_condition_bytes inherited;
	struct tcti_target_condition_bytes local;

	EXPECT_TRUE(!tcti_target_condition_serialize(&inventory, 2, &inherited, NULL));
	EXPECT_TRUE(!tcti_target_condition_serialize(&inventory, 1, &local, NULL));
	...
	tcti_target_condition_bytes_destroy(&inherited);
	tcti_target_condition_bytes_destroy(&local);
	return 0;
}
