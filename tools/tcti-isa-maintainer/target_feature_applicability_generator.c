...
#define TCTI_AARCHMRS_INSTRUCTIONS_SHA256 \
	"a1ad2c6538a47cd97d8762791ac5af88bce1d5f6aff096c9b77aef853e76acfe"
#define TCTI_AARCHMRS_FEATURES_SHA256 \
...
static enum tcti_target_feature_applicability_error validate(
	const struct tcti_feature_model *model,
	const struct tcti_target_inventory *inventory,
	const struct tcti_target_feature_sat_audit *audit)
{
	size_t index;

	if (!model || !inventory || !audit)
		return TCTI_TARGET_FEATURE_APPLICABILITY_INVALID_ARGUMENT;
	if (inventory->leaf_count != TCTI_A64_TARGET_LEAF_COUNT)
		return TCTI_TARGET_FEATURE_APPLICABILITY_PIN_MISMATCH;
	if (audit->leaf_count != inventory->leaf_count ||
	    audit->parameter_count != model->parameter_count || !audit->leaves)
		return TCTI_TARGET_FEATURE_APPLICABILITY_AUDIT_MISMATCH;
	if (model->parameter_count && !audit->witnesses)
		return TCTI_TARGET_FEATURE_APPLICABILITY_WITNESS_UNAVAILABLE;
	for (index = 0; index < inventory->leaf_count; index++) {
		const signed char *witness;
		size_t parameter;

		if (audit->leaves[index] == TCTI_TARGET_FEATURE_SAT_IMPOSSIBLE)
			continue;
		if (audit->leaves[index] != TCTI_TARGET_FEATURE_SAT_APPLICABLE)
			return TCTI_TARGET_FEATURE_APPLICABILITY_AUDIT_MISMATCH;
		if (!model->parameter_count)
			continue;
		witness = tcti_target_feature_sat_witness(audit, index);
		if (!witness)
			return TCTI_TARGET_FEATURE_APPLICABILITY_WITNESS_UNAVAILABLE;
		for (parameter = 0; parameter < model->parameter_count; parameter++)
			if (witness[parameter] != -1 && witness[parameter] != 1)
				return TCTI_TARGET_FEATURE_APPLICABILITY_WITNESS_INVALID;
	}
	return TCTI_TARGET_FEATURE_APPLICABILITY_OK;
}
...
{
	switch (error) {
	case TCTI_TARGET_FEATURE_APPLICABILITY_OK: return "success";
	case TCTI_TARGET_FEATURE_APPLICABILITY_INVALID_ARGUMENT: return "invalid argument";
	case TCTI_TARGET_FEATURE_APPLICABILITY_PIN_MISMATCH: return "pinned inventory mismatch";
	case TCTI_TARGET_FEATURE_APPLICABILITY_AUDIT_MISMATCH: return "SAT audit mismatch";
	case TCTI_TARGET_FEATURE_APPLICABILITY_WITNESS_UNAVAILABLE: return "SAT witness unavailable";
	case TCTI_TARGET_FEATURE_APPLICABILITY_WITNESS_INVALID: return "SAT witness invalid";
	case TCTI_TARGET_FEATURE_APPLICABILITY_IO: return "I/O failure";
	}
	...
	const struct tcti_target_feature_sat_audit *audit, FILE *output)
{
	...
	if (error != TCTI_TARGET_FEATURE_APPLICABILITY_OK)
		return error;
	stage = tmpfile();
	if (!stage)
		return TCTI_TARGET_FEATURE_APPLICABILITY_IO;
	if (fprintf(stage,
		"/* SPDX-License-Identifier: BSD-3-Clause */\n"
		"/* Generated by target_feature_applicability_generator.c. Do not edit. */\n"
		"TCTI_A64_FEATURE_APPLICABILITY_SOURCE(\"vFAPA1-A\", \"2026-06_rel\", \"%s\", \"%s\", %uU, %zuU)\n",
		TCTI_AARCHMRS_INSTRUCTIONS_SHA256, TCTI_AARCHMRS_FEATURES_SHA256,
		TCTI_A64_TARGET_LEAF_COUNT, model->parameter_count) < 0)
		goto io;
	for (index = 0; index < inventory->leaf_count; index++) {
		...
		unsigned int status = audit->leaves[index] ==
			TCTI_TARGET_FEATURE_SAT_APPLICABLE ? 1U : 0U;

		if (status && model->parameter_count)
			witness = tcti_target_feature_sat_witness(audit, index);
		if (fprintf(stage, "TCTI_A64_FEATURE_APPLICABILITY_ROW(%zuU, ", index) < 0 ||
