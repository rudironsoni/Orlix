...
static char *read_file(const char *path, size_t *length)
{
	...
	if (!file || fseek(file, 0, SEEK_END) || (size = ftell(file)) < 0 ||
	    (uintmax_t)size > TCTI_FEATURE_ARTIFACT_MAX_INPUT ||
	...
	enum tcti_feature_artifact_error expected)
{
	...
	CHECK(output != NULL);
	CHECK(tcti_target_feature_artifact_emit(source, length, output) == expected);
	...
	size_t source_length)
{
	...
	CHECK(tcti_target_feature_artifact_emit(source, source_length, second) ==
	      TCTI_FEATURE_ARTIFACT_OK);
	...
	CHECK(strstr(first_text, "TCTI_A64_FEATURE_ARTIFACT_COUNTS(409U, 1626U,") != NULL);
	CHECK(strstr(first_text,
		...
		"1243621U)") != NULL);
	CHECK(source_length == TCTI_FEATURE_ARTIFACT_PINNED_SOURCE_LENGTH);
	CHECK(strstr(first_text, "TCTI_A64_FEATURE_PARAMETER(0U,") != NULL);
	CHECK(strstr(first_text, "TCTI_A64_FEATURE_CONSTRAINT(1625U,") != NULL);
	CHECK(strstr(first_text, "TCTI_A64_FEATURE_NODE(") != NULL);
	CHECK(strstr(first_text, "TCTI_A64_FEATURE_CHILD(") != NULL);
	CHECK(strstr(first_text, "TCTI_A64_FEATURE_NODE(0U,") != NULL);
	CHECK(tcti_target_feature_model_import(source, source_length, &model,
		&error) == 0);
	for (index = 0; index < model.parameter_count; index++) {
		char marker[64];

		CHECK(snprintf(marker, sizeof(marker),
			"TCTI_A64_FEATURE_PARAMETER(%zuU,", index) > 0);
	...
	for (index = 0; index < model.constraint_count; index++) {
		...
		CHECK(snprintf(marker, sizeof(marker),
			"TCTI_A64_FEATURE_CONSTRAINT(%zuU, %" PRIu32 "U,",
	...
	for (index = 0; index < model.child_count; index++) {
		...
		CHECK(snprintf(marker, sizeof(marker),
			"TCTI_A64_FEATURE_CHILD(%zuU, %" PRIu32 "U)", index,
	...
	size_t source_length)
{
	...
	CHECK(require_empty_failure("{\"parameters\":[", strlen("{\"parameters\":["),
		TCTI_FEATURE_ARTIFACT_INVALID_SOURCE) == 0);
	...
	CHECK(require_empty_failure(mutation, mutation_length,
		TCTI_FEATURE_ARTIFACT_UNSUPPORTED_GRAMMAR) == 0);
	free(mutation);
	mutation = replace_once(source, source_length, "\"Features\"", "\"Features\" ",
		&mutation_length);
	CHECK(mutation != NULL);
	CHECK(require_empty_failure(mutation, mutation_length,
		TCTI_FEATURE_ARTIFACT_PIN_MISMATCH) == 0);
	free(mutation);
	CHECK(require_empty_failure(source, TCTI_FEATURE_ARTIFACT_MAX_INPUT + 1U,
		TCTI_FEATURE_ARTIFACT_LIMIT) == 0);
	return 0;
}
...
static int oversized_cli_is_rejected_before_read(const char *generator)
{
	...
	if (ftruncate(descriptor,
		      (off_t)TCTI_FEATURE_ARTIFACT_MAX_INPUT + 1) ||
	    close(descriptor))
		goto out;
	descriptor = -1;
	output = tmpfile();
