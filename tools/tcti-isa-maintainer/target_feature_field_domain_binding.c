...
static void binding_fail(struct tcti_feature_field_domain_binding_error *error,
	enum tcti_feature_field_domain_binding_error_code code, size_t offset,
	const char *format, ...)
{
	va_list ap;
	if (!error)
...
static int same_field_identity(const struct tcti_feature_node *left,
	const struct tcti_feature_node *right)
{
	return left->kind == TCTI_FEATURE_FIELD &&
		right->kind == TCTI_FEATURE_FIELD &&
...
static uint32_t field_identity_group(const struct tcti_feature_model *features,
	size_t node_index, uint32_t *occurrence_count)
{
	const struct tcti_feature_node *node = &features->nodes[node_index];
	size_t prior, scan;
	uint32_t groups = 0;
	for (prior = 0; prior < node_index; prior++) {
		int first = 1;
		if (features->nodes[prior].kind != TCTI_FEATURE_FIELD)
			continue;
...
static int model_is_valid(const struct tcti_feature_model *features,
	...
	struct tcti_feature_field_domain_binding_error *error)
{
	if ((features->node_count && !features->nodes) ||
		...
		binding_fail(error, TCTI_FEATURE_FIELD_DOMAIN_BINDING_INVALID_MODEL, 0,
	...
	struct tcti_feature_field_domain_binding_error *error)
{
	...
	if (!features || !registers || !bindings) {
		binding_fail(error, TCTI_FEATURE_FIELD_DOMAIN_BINDING_INVALID_ARGUMENT,
	...
	for (node_index = 0; node_index < features->node_count; node_index++)
		if (features->nodes[node_index].kind == TCTI_FEATURE_FIELD)
			field_count++;
	if (!field_count)
		return 0;
	if (field_count > SIZE_MAX / sizeof(*bindings->items)) {
		binding_fail(error, TCTI_FEATURE_FIELD_DOMAIN_BINDING_NO_MEMORY, 0,
	...
	for (node_index = 0; node_index < features->node_count; node_index++) {
		const struct tcti_feature_node *node = &features->nodes[node_index];
		struct tcti_feature_field_domain_binding *binding;
		...
		if (!node->field.state || !node->field.register_name ||
			!node->field.selector) {
			binding->disposition = TCTI_FEATURE_FIELD_DOMAIN_INVALID_MODEL;
			continue;
		}
		for (register_index = 0; register_index < registers->register_count;
		     register_index++) {
			const struct tcti_register_identity *identity =
		...
		if (!register_matches) {
			binding->disposition = TCTI_FEATURE_FIELD_DOMAIN_MISSING_REGISTER;
			continue;
		}
		if (register_matches != 1) {
			binding->disposition = TCTI_FEATURE_FIELD_DOMAIN_AMBIGUOUS_REGISTER;
			continue;
		}
		...
		if (!field_matches) {
			binding->disposition = TCTI_FEATURE_FIELD_DOMAIN_MISSING_FIELD;
			continue;
		}
		if (field_matches != 1) {
		...
		binding->disposition = TCTI_FEATURE_FIELD_DOMAIN_MAPPED;
	}
	return 0;
}
