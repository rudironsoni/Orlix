...
static int count_bindings(const struct tcti_feature_field_domain_bindings *bindings,
...
tcti_target_feature_field_domain_binding_artifact_validate(
	const struct tcti_feature_model *features, size_t feature_source_length,
	const struct tcti_register_model *registers, size_t register_source_length,
	const struct tcti_feature_field_domain_bindings *bindings
)
{
	size_t index;
	...
	if (!features || !feature_source_length || !registers ||
		...
		return TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_INVALID_ARGUMENT;
	for (index = 0; index < bindings->count; index++) {
		const struct tcti_feature_field_domain_binding *binding =
			&bindings->items[index];
		const struct tcti_feature_node *node;
		...
		if (binding->feature_node_index >= features->node_count)
			return TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_INVALID_BINDING;
		node = &features->nodes[binding->feature_node_index];
		if (node->kind != TCTI_FEATURE_FIELD ||
		    ...
		    node->field.instance.kind > TCTI_FEATURE_FIELD_QUALIFIER_NULL ||
		    node->field.slices.kind > TCTI_FEATURE_FIELD_QUALIFIER_NULL)
			return TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_INVALID_BINDING;
		if (!span_valid(binding->feature_provenance.offset,
		    ...
		    !span_valid(node->field.slices.provenance.offset,
				...
				feature_source_length))
			return TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_SOURCE_SPAN;
		if (binding->disposition == TCTI_FEATURE_FIELD_DOMAIN_MAPPED) {
			const struct tcti_register_identity *reg;
			const struct tcti_field_identity *field;
	...
	return TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_OK;
}

static enum tcti_feature_field_domain_binding_artifact_error emit_bindings(
	const struct tcti_feature_model *features,
	size_t feature_source_length, const struct tcti_register_model *registers,
	size_t register_source_length, FILE *output)
{
	struct tcti_feature_field_domain_bindings bindings = { 0 };
	struct tcti_feature_field_domain_binding_error binding_error = { 0 };
	size_t groups, mapped, ambiguous_field, index;
	uint64_t identity;
	enum tcti_feature_field_domain_binding_artifact_error validation;

	if (tcti_target_feature_field_domain_bindings_build(features, registers,
							      &bindings, &binding_error))
		return TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_BINDING;
	if (count_bindings(&bindings, &groups, &mapped, &ambiguous_field) ||
	    bindings.count != TCTI_FEATURE_FIELD_DOMAIN_BINDING_EXPECTED_OCCURRENCES ||
	    groups != TCTI_FEATURE_FIELD_DOMAIN_BINDING_EXPECTED_IDENTITY_GROUPS ||
	    mapped != TCTI_FEATURE_FIELD_DOMAIN_BINDING_EXPECTED_MAPPED ||
	    ambiguous_field !=
		TCTI_FEATURE_FIELD_DOMAIN_BINDING_EXPECTED_AMBIGUOUS_FIELD) {
		tcti_target_feature_field_domain_bindings_destroy(&bindings);
		return TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_COUNT_MISMATCH;
	}
	validation = tcti_target_feature_field_domain_binding_artifact_validate(
		features, feature_source_length, registers, register_source_length,
		&bindings);
	if (validation != TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_OK) {
		tcti_target_feature_field_domain_bindings_destroy(&bindings);
		return validation;
	}
	...
	if (fputs("/* SPDX-License-Identifier: BSD-3-Clause */\n"
		  ...
		  "TCTI_A64_FEATURE_FIELD_DOMAIN_SOURCE(\"vFATAp1-A\", \"818\", "
		  "\"2026-06_rel\", \"2.9.5\", "
	    ...
	    fprintf(output, "TCTI_A64_FEATURE_FIELD_DOMAIN_COUNTS(%zuU, %zuU, %zuU, %zuU)\n",
		    bindings.count, groups, mapped, ambiguous_field) < 0 ||
	    fprintf(output, "TCTI_A64_FEATURE_FIELD_DOMAIN_IDENTITY(UINT64_C(0x%016" PRIx64 "))\n",
		    identity) < 0)
		goto io;
	for (index = 0; index < bindings.count; index++) {
		const struct tcti_feature_field_domain_binding *binding =
		...
		if (fprintf(output, "TCTI_A64_FEATURE_FIELD_DOMAIN_OCCURRENCE("
...
io:
	tcti_target_feature_field_domain_bindings_destroy(&bindings);
	return TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_IO;
}
	...
	const char *register_source, size_t register_length, FILE *output)
{
	...
	enum tcti_feature_field_domain_binding_artifact_error result;
	...
	if (tcti_target_feature_model_import(feature_source, feature_length,
		...
		return TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_FEATURE_IMPORT;
	if (tcti_register_model_import(register_source, register_length,
				       &registers, &register_error)) {
		tcti_target_feature_model_destroy(&features);
		return TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_REGISTER_IMPORT;
	}
...
{
	switch (error) {
	case TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_OK: return "ok";
	case TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_INVALID_ARGUMENT: return "invalid argument";
	case TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_FEATURE_IMPORT: return "feature import failed";
	case TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_REGISTER_IMPORT: return "register import failed";
	case TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_BINDING: return "field-domain binding failed";
	case TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_INVALID_BINDING: return "field-domain binding violates artifact invariants";
	case TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_SOURCE_SPAN: return "source span is outside supplied source length";
	case TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_COUNT_MISMATCH: return "pinned field-domain census mismatch";
	case TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_IO: return "I/O failure";
	}
