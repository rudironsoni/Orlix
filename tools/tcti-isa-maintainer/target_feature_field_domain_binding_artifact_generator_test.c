	...
	const char *registers_path)
{
	...
	struct tcti_feature_field_domain_binding *mapped = NULL;
	struct tcti_feature_field_domain_binding *ambiguous = NULL;
	size_t index;
	...
	CHECK(!tcti_target_feature_model_import(features, feature_length,
		&feature_model, &feature_error));
	CHECK(!tcti_register_model_import(registers, register_length,
		&register_model, &register_error));
	CHECK(!tcti_target_feature_field_domain_bindings_build(&feature_model,
		&register_model, &bindings, &binding_error));
	CHECK(tcti_target_feature_field_domain_binding_artifact_validate(
		&feature_model, feature_length, &register_model, register_length,
		&bindings) == TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_OK);
	...
	CHECK(tcti_target_feature_field_domain_binding_artifact_validate(
		&feature_model, feature_length, &register_model, register_length,
		&bindings) == TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_OK);
	CHECK(tcti_target_feature_field_domain_binding_artifact_emit_model(
		&feature_model, feature_length / 2U, &register_model, register_length,
		first) == TCTI_FEATURE_FIELD_DOMAIN_BINDING_ARTIFACT_SOURCE_SPAN);
	...
	CHECK(tcti_target_feature_field_domain_binding_artifact_emit(features,
	...
	CHECK(strstr(first_text,
		"TCTI_A64_FEATURE_FIELD_DOMAIN_COUNTS(605U, 362U, 604U, 1U)") != NULL);
	CHECK(strstr(first_text, "TCTI_A64_FEATURE_FIELD_DOMAIN_IDENTITY(UINT64_C(") != NULL);
	CHECK(strstr(first_text, "TCTI_A64_FEATURE_FIELD_DOMAIN_OCCURRENCE(0U,") != NULL);
	CHECK(occurrences(first_text,
		"TCTI_A64_FEATURE_FIELD_DOMAIN_OCCURRENCE(") == 605U);
...
int main(int argc, char **argv)
{
	...
	if (tcti_target_feature_field_domain_binding_artifact_emit(NULL, 0,
		"x", 1U, stdout) !=
