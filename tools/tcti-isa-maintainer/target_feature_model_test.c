...
static int expect(const char *source, size_t length, enum tcti_feature_error_code code, const char *what)
{
	struct tcti_feature_model model;
	struct tcti_feature_error error = { 0 };
	int result = tcti_target_feature_model_import(source, length, &model, &error);
	if (!result)
...
{
	const char *path = argc > 1 ? argv[1] : getenv("TCTI_FEATURES_JSON");
	struct tcti_feature_model model;
	struct tcti_feature_error error = { 0 };
	...
	failed |= expect(decoded_duplicate, sizeof(decoded_duplicate) - 1, TCTI_FEATURE_DUPLICATE_KEY, "decoded duplicate key");
	failed |= expect(unpaired_surrogate, sizeof(unpaired_surrogate) - 1, TCTI_FEATURE_INVALID_JSON, "unpaired JSON surrogate");
	failed |= expect(short_source, sizeof(short_source) - 1, TCTI_FEATURE_COUNT_MISMATCH, "count mutation");
	...
	if (bounded) {
		...
		bounded[514] = '\0';
		failed |= expect(bounded, 514, TCTI_FEATURE_LIMIT, "JSON depth bound");
		free(bounded);
	}
	if (!path) {
		fprintf(stderr, "TCTI_FEATURES_JSON or a Features.json path is required\n");
		return 2;
	}
	...
	failed |= check(model.parameter_count == TCTI_FEATURE_PARAMETER_COUNT, "parameter count");
	failed |= check(model.constraint_count == TCTI_FEATURE_CONSTRAINT_COUNT, "constraint count");
	...
	failed |= check(mutation != NULL, "find AST.Bool"); if (mutation) { failed |= expect(mutation, mutated_length, TCTI_FEATURE_UNSUPPORTED_GRAMMAR, "unknown AST type"); free(mutation); }
	...
	failed |= check(mutation != NULL, "find malformed string"); if (mutation) { failed |= expect(mutation, mutated_length, TCTI_FEATURE_INVALID_JSON, "malformed JSON"); free(mutation); }
	mutation = replace_once(source, length, "\"vFATAp1-A\"", "\"vFATAp1-B\"", &mutated_length);
	failed |= check(mutation != NULL, "find metadata pin"); if (mutation) { failed |= expect(mutation, mutated_length, TCTI_FEATURE_PIN_MISMATCH, "metadata pin"); free(mutation); }
	mutation = replace_once(source, length, "\"Features\"", "\"Features\" ", &mutated_length);
	failed |= check(mutation != NULL, "construct content pin mutation"); if (mutation) { failed |= expect(mutation, mutated_length, TCTI_FEATURE_PIN_MISMATCH, "SHA-256 pin"); free(mutation); }
	free(source);
	return failed;
}
