...
static int synthetic(void) {
  struct tcti_feature_parameter p = {.name = "A"};
  struct tcti_feature_node nodes[] = {
      {.kind = TCTI_FEATURE_IDENTIFIER, .text = "A", .provenance = {2, 3}},
      {.kind = TCTI_FEATURE_BOOL, .integer = 1, .provenance = {7, 4}},
      {.kind = TCTI_FEATURE_IMPLIES,
       .left = 0,
       .right = 1,
       .provenance = {1, 10}}};
  uint32_t roots[] = {2};
  struct tcti_feature_model m = {.parameters = &p,
                                 .parameter_count = 1,
                                 .nodes = nodes,
                                 .node_count = 3,
                                 .constraints = roots,
  ...
  int failed = tcti_feature_typed_lower(&m, &x, &r) || r.parameter_count != 1 ||
               r.constraint_count != 1 || r.diagnostic_count ||
               x.nodes[r.constraints[0]].op != TCTI_TYPED_IMPLIES ||
...
int main(int argc, char **argv) {
  ...
  struct tcti_target_import_error unused;
  struct tcti_feature_error error;
  struct tcti_feature_model model;
  struct tcti_typed_expression typed = {0};
