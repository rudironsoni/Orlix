...
static int tag_for_expression(enum tcti_target_expr_kind kind, uint8_t *tag)
{
	switch (kind) {
...
static int expression_has_feature(const struct tcti_target_inventory *inventory,
				  uint32_t index, size_t depth)
{
	...
	if (depth >= TCTI_TARGET_CONDITION_MAX_DEPTH ||
	    index == TCTI_TARGET_EXPR_NONE || index >= inventory->expression_count)
		return -1;
	expression = &inventory->expressions[index];
	if (expression->kind == TCTI_TARGET_EXPR_FEATURE)
		return 1;
...
static int compare_record(const struct tcti_target_inventory *inventory,
			  ...
			  size_t length, size_t *offset, size_t depth)
{
	...
	if (depth >= TCTI_TARGET_CONDITION_MAX_DEPTH ||
	    expression_index == TCTI_TARGET_EXPR_NONE ||
...
static int compare_condition(const struct tcti_target_inventory *inventory,
...
static int all_pinned_conditions_roundtrip(const char *source, size_t source_length)
{
	...
	const struct tcti_target_operand **source_ordered = NULL;
	...
	for (leaf_index = 0; leaf_index < inventory.leaf_count; leaf_index++) {
		const struct tcti_target_leaf *source_leaf = &inventory.leaves[leaf_index];
		...
		for (operand_index = leaf->operand_first;
			...
			const struct tcti_target_operand *source_operand =
	...
	CHECK(checked_leaf_conditions == TCTI_A64_TARGET_LEAF_COUNT);
