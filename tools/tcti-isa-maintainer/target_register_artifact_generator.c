...
static enum tcti_register_artifact_error model_error(
	const struct tcti_register_model_error *error)
{
	switch (error->code) {
	case TCTI_REGISTER_MODEL_OK:
		return TCTI_REGISTER_ARTIFACT_OK;
	case TCTI_REGISTER_MODEL_INVALID_ARGUMENT:
		return TCTI_REGISTER_ARTIFACT_INVALID_ARGUMENT;
	case TCTI_REGISTER_MODEL_INVALID_JSON:
	case TCTI_REGISTER_MODEL_INVALID_SOURCE:
		return TCTI_REGISTER_ARTIFACT_INVALID_SOURCE;
	case TCTI_REGISTER_MODEL_UNSUPPORTED_TYPE:
		return TCTI_REGISTER_ARTIFACT_UNSUPPORTED_GRAMMAR;
	case TCTI_REGISTER_MODEL_INPUT_LIMIT:
	case TCTI_REGISTER_MODEL_DEPTH_LIMIT:
		return TCTI_REGISTER_ARTIFACT_LIMIT;
	case TCTI_REGISTER_MODEL_NO_MEMORY:
		return TCTI_REGISTER_ARTIFACT_NO_MEMORY;
	case TCTI_REGISTER_MODEL_HASH_MISMATCH:
		return TCTI_REGISTER_ARTIFACT_PIN_MISMATCH;
	case TCTI_REGISTER_MODEL_COUNT_MISMATCH:
		return TCTI_REGISTER_ARTIFACT_COUNT_MISMATCH;
	}
	return TCTI_REGISTER_ARTIFACT_INVALID_SOURCE;
}
...
{
	switch (error) {
	case TCTI_REGISTER_ARTIFACT_OK:
		return "success";
	case TCTI_REGISTER_ARTIFACT_INVALID_ARGUMENT:
		return "invalid argument";
	case TCTI_REGISTER_ARTIFACT_INVALID_SOURCE:
		return "invalid source";
	case TCTI_REGISTER_ARTIFACT_UNSUPPORTED_GRAMMAR:
		return "unsupported grammar";
	case TCTI_REGISTER_ARTIFACT_LIMIT:
		return "resource limit";
	case TCTI_REGISTER_ARTIFACT_NO_MEMORY:
		return "out of memory";
	case TCTI_REGISTER_ARTIFACT_PIN_MISMATCH:
		return "pin mismatch";
	case TCTI_REGISTER_ARTIFACT_COUNT_MISMATCH:
		return "count mismatch";
	case TCTI_REGISTER_ARTIFACT_UNRESOLVED_MODEL:
		return "unresolved typed model";
	case TCTI_REGISTER_ARTIFACT_IO:
		return "I/O failure";
	case TCTI_REGISTER_ARTIFACT_MISMATCH:
...
static int all_counts_fit(const struct tcti_register_model *model,
...
static int required_tables_present(const struct tcti_register_model *model)
{
 ...
 * fabricated default.
 */
static int model_resolved(const struct tcti_register_model *model,
			  size_t source_length)
{
	size_t index;

	if (!model->source || model->source_length != source_length ||
	    model->register_count != TCTI_REGISTER_RECORD_COUNT ||
	    model->metadata_count != TCTI_REGISTER_METADATA_COUNT ||
	    model->memory_access_count != TCTI_REGISTER_MEMORY_ACCESS_COUNT ||
	    model->implementation_defined_permission_count != 17U ||
	    model->fieldset_count != TCTI_REGISTER_FIELDSET_COUNT ||
	    model->field_count != TCTI_REGISTER_FIELD_COUNT ||
	    model->range_count != TCTI_REGISTER_RANGE_COUNT ||
	    model->top_level_valueset_count != TCTI_REGISTER_VALUESET_COUNT ||
	    model->top_level_domain_count != TCTI_REGISTER_DOMAIN_COUNT ||
	    model->accessor_count != TCTI_REGISTER_ACCESSOR_COUNT ||
	    !all_counts_fit(model, source_length) ||
	    !required_tables_present(model))
		return 0;

	for (index = 0; index < model->metadata_count; index++) {
		const struct tcti_register_metadata *item = &model->metadata[index];
	...
	for (index = 0; index < model->register_count; index++) {
		const struct tcti_register_identity *item = &model->registers[index];
	...
	for (index = 0; index < model->memory_access_count; index++) {
		const struct tcti_register_memory_access *item =
			&model->memory_accesses[index];

		if ((unsigned int)item->form >
		    TCTI_REGISTER_MEMORY_ACCESS_LEGACY_SENTINEL ||
		    (unsigned int)item->read > TCTI_REGISTER_MEMORY_READ_ERROR ||
		    (unsigned int)item->write > TCTI_REGISTER_MEMORY_WRITE_ERROR ||
	...
	for (index = 0;
		...
		const struct tcti_register_implementation_defined_permission *item =
		...
		for (child = 0; child < item->memory_access_count; child++) {
			...
			const struct tcti_register_expression *owner;
	...
	for (index = 0; index < model->fieldset_count; index++) {
		const struct tcti_register_fieldset *item = &model->fieldsets[index];
	...
	for (index = 0; index < model->field_count; index++) {
		const struct tcti_field_identity *item = &model->fields[index];
	...
	for (index = 0; index < model->range_count; index++) {
		const struct tcti_register_range *item = &model->ranges[index];
	...
	for (index = 0; index < model->field_branch_count; index++) {
		const struct tcti_conditional_field_branch *item =
	...
	for (index = 0; index < model->valueset_count; index++) {
		const struct tcti_register_valueset *item = &model->valuesets[index];
	...
	for (index = 0; index < model->domain_count; index++) {
		const struct tcti_value_domain *item = &model->domains[index];
	...
	for (index = 0; index < model->constraint_count; index++) {
		const struct tcti_register_constraint *item =
			&model->constraints[index];

		if ((unsigned int)item->kind > TCTI_REGISTER_CONSTRAINT_ARRAY ||
		    !index_valid(item->field_index, model->field_count) ||
	...
	for (index = 0; index < model->constraint_item_count; index++) {
		const struct tcti_register_constraint_item *item =
	...
	for (index = 0; index < model->link_count; index++) {
		const struct tcti_register_link *item = &model->links[index];
	...
	for (index = 0; index < model->accessor_count; index++) {
		const struct tcti_register_accessor *item = &model->accessors[index];
	...
	for (index = 0; index < model->accessor_offset_expression_count; index++) {
		const struct tcti_register_accessor_offset_expression *item =
	...
	for (index = 0; index < model->system_encoding_count; index++) {
		const struct tcti_register_system_encoding *item =
	...
	for (index = 0; index < model->system_selector_count; index++) {
		const struct tcti_register_system_selector *item =
			&model->system_selectors[index];

		if (!item->name ||
		    (!strcmp(item->type ? item->type : "", "Values.Value") &&
		     item->value_kind != TCTI_REGISTER_SELECTOR_VALUE_LITERAL) ||
		    (!strcmp(item->type ? item->type : "",
			     "Values.EquationValue") &&
		     item->value_kind != TCTI_REGISTER_SELECTOR_VALUE_EQUATION) ||
		    (!strcmp(item->type ? item->type : "", "Values.Group") &&
		     item->value_kind != TCTI_REGISTER_SELECTOR_VALUE_GROUP) ||
		    (!item->type ||
		...
		case TCTI_REGISTER_SELECTOR_VALUE_NONE:
			if (item->payload_index != INVALID_INDEX)
				return 0;
			break;
		case TCTI_REGISTER_SELECTOR_VALUE_LITERAL:
			if (item->payload_index >= model->selector_literal_count)
				return 0;
			break;
		case TCTI_REGISTER_SELECTOR_VALUE_EQUATION:
			if (item->payload_index >= model->selector_equation_count)
				return 0;
			break;
		case TCTI_REGISTER_SELECTOR_VALUE_GROUP:
	...
	for (index = 0; index < model->selector_literal_count; index++) {
		const struct tcti_register_selector_literal *item =
	...
	for (index = 0; index < model->selector_equation_count; index++) {
		const struct tcti_register_selector_equation *item =
	...
	for (index = 0; index < model->selector_slice_count; index++) {
		const struct tcti_register_selector_slice *item =
	...
	for (index = 0; index < model->selector_group_count; index++) {
		const struct tcti_register_selector_group *item =
	...
	for (index = 0; index < model->selector_group_fragment_count; index++) {
		const struct tcti_register_selector_group_fragment *item =
			&model->selector_group_fragments[index];

		if (item->kind < TCTI_REGISTER_SELECTOR_VALUE_LITERAL ||
		    item->kind > TCTI_REGISTER_SELECTOR_VALUE_GROUP ||
		    !span_valid(item->source_offset, item->source_length,
				source_length))
			return 0;
		if ((item->kind == TCTI_REGISTER_SELECTOR_VALUE_LITERAL &&
		     item->payload_index >= model->selector_literal_count) ||
		    (item->kind == TCTI_REGISTER_SELECTOR_VALUE_EQUATION &&
	...
	for (index = 0; index < model->expression_count; index++) {
		const struct tcti_register_expression *item =
	...
	const struct tcti_register_model *model, size_t source_length, FILE *output)
{
	...
	if (!model_resolved(model, source_length))
		return TCTI_REGISTER_ARTIFACT_UNRESOLVED_MODEL;
	return emit_artifact(output, model, source_length) ?
		TCTI_REGISTER_ARTIFACT_IO : TCTI_REGISTER_ARTIFACT_OK;
}
	...
	const char *source, size_t length, FILE *output)
{
	...
	enum tcti_register_artifact_error result;

	if (!source || !length || !output)
		return TCTI_REGISTER_ARTIFACT_INVALID_ARGUMENT;
	if (length > TCTI_REGISTER_ARTIFACT_MAX_INPUT)
		return TCTI_REGISTER_ARTIFACT_LIMIT;
	if (tcti_register_model_import(source, length, &model, &error))
		return model_error(&error);
	result = tcti_target_register_artifact_emit_model(&model, length, output);
...
static int read_source(const char *path, struct source_bytes *source)
{
	...
	if ((uintmax_t)length > TCTI_REGISTER_ARTIFACT_MAX_INPUT) {
		fclose(file);
		errno = EFBIG;
		return -1;
	}
...
int main(int argc, char **argv)
{
	...
	error = tcti_target_register_artifact_emit(source.data, source.length,
						  output);
	if (error != TCTI_REGISTER_ARTIFACT_OK) {
		fprintf(stderr, "target register artifact generator: %s\n",
			tcti_target_register_artifact_error_name(error));
		goto out;
	}
