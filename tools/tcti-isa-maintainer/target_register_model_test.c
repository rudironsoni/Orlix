...
static int typed_memory_permission_schema_rejected(void)
{
	...
	CHECK(!expect(invalid_read, TCTI_REGISTER_MODEL_INVALID_SOURCE));
...
static int non_system_accessor_rejection_and_rollback(void)
{
	...
	CHECK(!expect_rollback(NON_SYSTEM_REGISTER_PREFIX
		...
		NON_SYSTEM_REGISTER_SUFFIX, TCTI_REGISTER_MODEL_INVALID_SOURCE));
	CHECK(!expect_rollback(NON_SYSTEM_REGISTER_PREFIX
...
static int non_system_accessor_census(const struct tcti_register_model *model)
{
	size_t index;
	...
	for (index = 0; index < model->accessor_count; index++) {
		...
		for (offset = 0; offset < accessor->offset_expression_count; offset++) {
			const struct tcti_register_accessor_offset_expression *entry =
...
static int memory_permission_census(const struct tcti_register_model *model)
{
	size_t index;
	...
	CHECK(model->memory_access_count == TCTI_REGISTER_MEMORY_ACCESS_COUNT);
	...
	for (index = 0; index < model->memory_access_count; index++) {
		...
		CHECK(access->form == TCTI_REGISTER_MEMORY_ACCESS_EXPLICIT_OBJECT);
		...
		CHECK(!strcmp(model->expressions[access->owner_index].type,
		      "Accessors.Permission.AccessTypes.Memory.ReadWriteAccess"));
		switch (access->read) {
		case TCTI_REGISTER_MEMORY_READ_R: read_r++; break;
		case TCTI_REGISTER_MEMORY_READ_RAZ: read_raz++; break;
		case TCTI_REGISTER_MEMORY_READ_RES0: read_res0++; break;
		case TCTI_REGISTER_MEMORY_READ_RESERVED: read_reserved++; break;
		case TCTI_REGISTER_MEMORY_READ_UNKNOWN: read_unknown++; break;
		case TCTI_REGISTER_MEMORY_READ_ERROR: read_error++; break;
		default: CHECK(0);
		}
		switch (access->write) {
		case TCTI_REGISTER_MEMORY_WRITE_W: write_w++; break;
		case TCTI_REGISTER_MEMORY_WRITE_WI: write_wi++; break;
		case TCTI_REGISTER_MEMORY_WRITE_RES0: write_res0++; break;
		case TCTI_REGISTER_MEMORY_WRITE_RESERVED: write_reserved++; break;
		case TCTI_REGISTER_MEMORY_WRITE_ERROR: write_error++; break;
		default: CHECK(0);
		}
		CHECK(access->read_origin == TCTI_REGISTER_MEMORY_ACCESS_EXPLICIT);
		CHECK(access->write_origin == TCTI_REGISTER_MEMORY_ACCESS_EXPLICIT);
		CHECK(access->legacy_sentinel == TCTI_REGISTER_MEMORY_ACCESS_LEGACY_NONE);
	...
	for (index = 0; index < model->implementation_defined_permission_count;
	     index++) {
		const struct tcti_register_implementation_defined_permission *permission =
...
static int pinned_model(const char *path)
{
	...
	CHECK(model.metadata_count == TCTI_REGISTER_METADATA_COUNT);
	CHECK(model.memory_access_count == TCTI_REGISTER_MEMORY_ACCESS_COUNT);
	CHECK(model.implementation_defined_permission_count == 17U);
