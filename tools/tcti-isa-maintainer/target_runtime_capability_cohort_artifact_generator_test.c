	...
	const char *features, size_t features_length)
{
	...
	if (!file || tcti_runtime_capability_cohort_artifact_emit(instructions,
...
static int model_helpers_reject_cycle_and_deduplicate(void)
{
	...
	struct tcti_target_expr expressions[2] = {
		{ .kind = TCTI_TARGET_EXPR_AND, .left = 1U },
		{ .kind = TCTI_TARGET_EXPR_AND, .left = 0U },
	};
	struct tcti_target_inventory inventory = { .expressions = expressions,
		.expression_count = 2U };
	struct tcti_feature_parameter parameters[] = { { .name = "FEAT_X" } };
	struct tcti_feature_model features = { .parameters = parameters,
		.parameter_count = 1U };
	uint8_t visited[2] = { 0 };
...
static int emitted_memberships_are_sorted_and_provenanced(const char *text)
{
	...
	while ((cursor = strstr(cursor,
		"TCTI_A64_RUNTIME_CAPABILITY_COHORT_MEMBERSHIP(")) != NULL) {
		const char *tail = strchr(cursor, ')');
		unsigned long offset, length;

		EXPECT(tail);
		EXPECT(sscanf(cursor,
			"TCTI_A64_RUNTIME_CAPABILITY_COHORT_MEMBERSHIP(%uU, %uU, %uU,",
			&order, &leaf, &parameter) == 3);
		EXPECT(order == rows && leaf < TCTI_A64_TARGET_LEAF_COUNT);
		...
		/* The final two unsigned fields are the exact AST.FEAT source span. */
		{
			...
			EXPECT(sscanf(span,
				"TCTI_RUNTIME_CAPABILITY_COHORT_UNRESOLVED, %luU, %luU",
...
{
	struct cohort_model model = { 0 };
	struct tcti_target_expr expression = {
		.kind = TCTI_TARGET_EXPR_FEATURE,
		.text = "FEAT_UNKNOWN",
	};
	...
	struct tcti_feature_parameter parameter = { .name = "FEAT_KNOWN" };
	struct tcti_feature_model features = {
...
int main(int argc, char **argv)
{
	...
	if (!first || !second || strcmp(first, second) ||
	    !strstr(first, "TCTI_A64_RUNTIME_CAPABILITY_COHORT_COUNTS(4350U, 5592U, 5592U, 5592U)") ||
	    !strstr(first, "TCTI_RUNTIME_CAPABILITY_COHORT_UNRESOLVED") ||
