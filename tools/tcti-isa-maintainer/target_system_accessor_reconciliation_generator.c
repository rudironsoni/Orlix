...
#define TCTI_SYSTEM_ACCESSOR_GENERATOR_MAX_INPUT (128U * 1024U * 1024U)
...
static char *read_source(const char *path, size_t *length)
{
	...
	if (!file || fseek(file, 0, SEEK_END) ||
	    ...
	    (uintmax_t)file_length > TCTI_SYSTEM_ACCESSOR_GENERATOR_MAX_INPUT ||
...
int main(int argc, char **argv)
{
	...
	error = tcti_system_accessor_reconciliation_emit(&model, output);
	if (output != stdout) {
		if (fflush(output) || fclose(output))
			error = TCTI_SYSTEM_ACCESSOR_RECONCILIATION_INVALID;
		output = NULL;
	}
	tcti_register_model_destroy(&model);
	free(source);
	if (error != TCTI_SYSTEM_ACCESSOR_RECONCILIATION_OK) {
		fprintf(stderr, "system accessor reconciliation: %s\n",
			tcti_system_accessor_reconciliation_error_name(error));
